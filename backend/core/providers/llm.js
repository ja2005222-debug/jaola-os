...
/**
 * 🌊 تصريفُ تدفّقٍ يُحسب **نداءً واحداً** — لا قطعةً قطعة.
 *
 * قِيس من **سجلٍّ حيّ** لصاحب المشروع: `💰 [AI Usage]: … في ٧٩٩٥٤ نداء … ٧٩٩١٣ نداءً بلا
 * أرقام`. والحسابُ وحدَه يفضح الرقم: الصامتُ لا يتجاوز النداءات أبداً، فالمحسوبُ **٤١** —
 * أي أنّ «٧٩٩٥٤ نداءً» كانت قِطَعَ بثٍّ من نحو أربعين نداءً حقيقيّ. كان `noteUsage` يُنادى
 * داخل `for await` لكلِّ قطعة، فيرفع `calls` بكلِّ قطعةٍ ويعدّها «صامتة».
 *
 * والأثرُ أبعدُ من سطرٍ مغلوط: `byLabel` يرث العلّةَ نفسَها، فيظهر **المولّدُ المتدفّق** —
 * وهو الأثقل — بعشرات آلاف «النداءات» وبرموزٍ شبهِ صفر، بينما يظهر غيرُ المتدفّقين بأرقامٍ
 * صحيحة. فقرارُ توزيعِ الموديلات كان سيُبنى على مقامٍ مكسور، ويُرخَّص أغلى وكيلٍ لأنّه بدا صامتاً.
 *
 * والحسابُ في `finally`: تدفّقٌ ينقطع في منتصفه **نداءٌ وقع فعلاً**، فيُعدّ صامتاً ولا يُبتلَع.
 */
export async function drainStream(stream, onChunk) {
    let carrier = null;
    try {
        for await (const chunk of stream) {
            if (chunk?.usage) carrier = chunk;   // المزوّدُ يتطوّع بها في قطعةٍ واحدة لا في كلِّها
            onChunk(chunk);
        }
    } finally {
        noteUsage(stream?.__aiProvider || 'تدفّق', carrier);
    }
}
