...
| الملف | سطور | المسؤولية | القرار | العقد الجديد | الموقع النهائي |
|---|---|---|---|---|---|
| `jcr.js` | 1010 | وقت التشغيل المعرفي: `handleUserMessage` → 7 معالجات نية (كلُّها مفوِّضاتٌ إلى `stages/*` بعد JCR/28) → `executeMission` → `_runMissionNow` → 15 مرحلة؛ **ونمَت ٣٦ سطراً في «حُرّاس الصمت»** (لا انزلاقاً): `surgicalEdit` صار يُقرّ بالاستلام ويُقيّد نصَّ الطلب في السجلّ الدائم، و`_reportIfCrashed` تُسمِع انهيارَ المهمّة في الشات ثمّ تُعيد الرميَ | MODIFY (تدريجي، بلا إعادة كتابة) | Mission + Agent + Task (المراحل → TaskGraph) + Event | يبقى؛ يتقلّص مع كل Sprint لصالح `core/runtime/*` |
