...
| الملف | سطور | المسؤولية | القرار | العقد الجديد | الموقع النهائي |
|---|---|---|---|---|---|
| `jcr.js` | 1103 | وقت التشغيل المعرفي: `handleUserMessage` → 7 معالجات نية (كلُّها مفوِّضاتٌ إلى `stages/*` بعد JCR/28) → `executeMission` → `_runMissionNow` → 15 مرحلة | MODIFY (تدريجي، بلا إعادة كتابة) | Mission + Agent + Task (المراحل → TaskGraph) + Event | يبقى؛ يتقلّص مع كل Sprint لصالح `core/runtime/*` |
...
| ✅ `stages/reportMissionSuccess.js` (جديد، JCR/11) | 99 | `reportMissionSuccess(goal, ctx, reporter)` — تقريرُ التسليم بلغة المستخدم + الاقتراحات + قائمةُ الملفّات + الدفعُ التلقائيّ (`reporter.io`، موضعٌ واحد معلَن) + اللقطة + المقاييس + hook afterBuild؛ متزامنةٌ كما كانت | ADDED | Mission + Event | مفوِّض `jcr._reportMissionSuccess` من `_runMissionNow` |
