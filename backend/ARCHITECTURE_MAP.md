...
| ✅ `stages/understand.js` (جديد، JCR/5؛ PM/1 يبذر الفهمَ من المراجع؛ PM/22 يقول «فهمي لا يمسّ طلبَك» بدل ابتلاعه) | 91 | `understandGoal(goal, ctx, reporter)` — الفهم: ذاكرة/ملفّ ← مخطّط ← نموذج المجال؛ نقلٌ حرفيّ، المُبلِّغُ وسيط | ADDED | Mission | `jcr._understandGoal` مفوِّضٌ من سطر؛ ٧ استيراداتٍ رحلت معها |
...
| `chatCommands.js`, `textNormalizer.js` (+`isFullSpecification`: وثيقةٌ لا جملة؛ PM/9: `specSections`؛ PM/11: `specHead`/`clipWords`), `languageDetector.js`, `logLocalizer.js`, `failureMessages.js` | 120/347/200/204/31 | أوامر حتمية، تطبيع نص، لغة، ترجمة السجل، رسائل الفشل (تحمل تشخيصَ المزوّدين) | KEEP | — | `agents/` (أدوات النواة اللغوية) |
...
| `backendNeed.js` | 95 | **مصدرُ الحقيقة الواحد** لـ«أيحتاج خلفيةً؟» (Sprint 7/1) + العلاقية مجموعةً جزئيّة منه (2r) + 🚫 `stripNegated`: الجملةُ التي تنفي لا تُوجب («بلا حساب» كانت تُوجب حساباً) | KEEP | — | `plugins/coding/` |
