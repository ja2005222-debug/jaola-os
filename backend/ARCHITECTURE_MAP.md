...
## C) `backend/agents/*` — 135 وحدة (منها 42 قالب كلون)

### C1. النواة (Kernel candidates)
| الملف | سطور | المسؤولية | القرار | العقد الجديد | الموقع النهائي |
|---|---|---|---|---|---|
| `jcr.js` | 1492 | وقت التشغيل المعرفي: `handleUserMessage` → 7 معالجات نية → `executeMission` → `_runMissionNow` → 15 مرحلة | MODIFY (تدريجي، بلا إعادة كتابة) | Mission + Agent + Task (المراحل → TaskGraph) + Event | يبقى؛ يتقلّص مع كل Sprint لصالح `core/runtime/*` |
...
| ✅ `stages/intentHandlers.js` (جديد، JCR/25) | 128 | `handlePlanningStage(req, agents, reporter, {executeMission}) → boolean` — مرحلةُ الخطّة في حوار التوضيح: تأكيدٌ → تهيئةُ ذاكرة المشروع من المُوضِّح وتسجيلُه ثمّ التنفيذ؛ سؤالٌ → ملخّصُ الخطّة؛ إيقاف/لون/تعديل → في إجابات المُوضِّح. و`handleModifyPattern(req, agents, reporter, {surgicalEdit}) → boolean` — كشفُ التعديل بالنمط (احتياطُ الموجّه): في مرحلة الخطّة تعديلٌ عليها، وإلّا تعديلٌ جراحيّ بسياق الطلب؛ لا `io` | ADDED | Permission + Mission | مفوِّضان `jcr._handlePlanningStage`/`_handleModifyPattern` من `handleUserMessage` |
