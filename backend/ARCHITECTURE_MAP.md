...
> خطة تنفيذ ذكاء البناء، الفصل بين Site/System، بوابة الاستعلام المباشر من المستخدم، وتطوير الراوتر والوكلاء والـPlugins موثّقة في [`docs/BUILD_INTELLIGENCE_ROADMAP.md`](docs/BUILD_INTELLIGENCE_ROADMAP.md).
...
`core/contracts/projectTypes.js`: extensible project-type decision contract; blocks ambiguous builds before queueing.

`agents/arabicRequestContext.js`: original Arabic request evidence, constraints, and semantic router guidance. Normalization never replaces source text.

`agents/arabicReferenceDialog.js`: persisted user/project-scoped reference clarification, cancellation, expiry, and same-process answer consumption before generic intent handling.

`agents/protectedEditFiles.js`: explicit protected-file extraction and pre-write proposal checks; surgical editing additionally checks protected content after autofix.
...
| ✅ `core/runtime/TaskGraph.js` (جديد، Sprint 2a) | 51 | `orderTasks(items, {key})` — ترتيب طوبولوجي مستقرّ من `dependsOn` + كشف الدورات (خوارزمية `planExecution` حرفياً معمَّمة) | ADDED | Task | مستهلكاه: `runDynamicMultiAgentRuntime` (DELIVERY_STAGES) و`planExecution` (الفرق) |
...
| ✅ `core/runtime/AgentRuntime.js` (جديد، Sprint 2d؛ +٣ #١٩١: `withUsageLabel(\`agent:${agent.id}\`)` — نقطةُ نسبِ الكلفة لوكلاء العقود الواحدة والعشرين، وُسِمت هنا لأنّ `agent.id` معروفٌ سلفاً فلا تُغيَّر تواقيعُ المنادين لأجل عدّاد) | 105 | `runAgent` + `gatherCooperationInputs` — منفّذ الوكيل الواحد: عقدٌ → نداء نموذج → ملفات مُطهَّرة | MOVED (Sprint 2d ✅، حرفياً عدا إسقاط افتراض `TEAM_BY_ID` الميت) | **Agent** | `runBackendTeam` (فريقا الخلفية **والواجهة** معاً) |
...
| ✅ `core/providers/llm.js` (Sprint 4g، مَنقول من `agents/baseAgent.js`) | 639 (25 مستورداً) | بوّابةُ الـLLM الوحيدة: `smartChat`/`ai`/`groq`/`deepseek` + سلسلة failover Groq → DeepSeek → Gemini → OpenAI + تصنيف الأعطال (`MODEL_PROBLEM` مصدرٌ واحد: **٤٠٣ رمزٌ ملتبس فتحسمه الرسالة** — كان يُقرأ `auth` فيُقال «مفتاحُه غير صالح» عن مفتاحٍ سليم وعلّتُه اسمُ موديل، و٤٠١ تبقى `auth` بلا شرط) + `aggregateFailure` (قرارُ الإيقاف ونصُّ صاحب المشروع في موضعٍ واحد) + أسماءُ الموديلات من البيئة `GROQ_MODEL`/`GEMINI_MODEL`/`OPENAI_MODEL`/`DEEPSEEK_MODEL` + `AI_PROVIDERS` (`PROVIDER_NAMES`/`resolveEnabledProviders`/`isProviderEnabled`): استبعادُ حلقةٍ بلا حذفِ مفتاحها + `modelReportLines` (اسمُ موديل كلِّ مزوّدٍ عاملٍ ومصدرُه) و`discoverModels` (سؤالُ المزوّد عمّا يقبله عند عطبِ إعداد) + عدّادُ الرموز (`noteUsage`/`readAIUsage`/`usageLine`) **موسوماً بالوكيل** (#١٩١؛ +٢٦ من سجلٍّ حيّ: `drainStream` — التدفّقُ يُحسب **نداءً واحداً** لا قطعةً قطعة، فسطرُ سجلّ المالك قال «٧٩٩٥٤ نداء» وحقيقتُها ~٤١، و`byLabel` كان يرث العلّةَ فيُظهر المولّدَ المتدفّقَ صامتاً: `withUsageLabel`/`currentUsageLabel` بـ`AsyncLocalStorage` + `byLabel` + `usageByLabelLine` — العدّادُ كان يجيب «كم» ولا يجيب «أين»، فأيُّ توزيعِ موديلاتٍ على الوكلاء يُبنى على حدس؛ وفي الطريق سُدّ ثقبٌ مقيس: حقولُ Gemini `usageMetadata` بأسماءٍ camelCase لم يكن `noteUsage` يقرؤها فكان **كلُّ** نداءِ Gemini يُعدّ «صامتاً» بصفرِ رموز) + **#١٩٣** (+٥٦): صنفٌ ثالثٌ `prompt` بين الدائم والعابر — `PROMPT_REJECTED` تقرأ لفظَ المزوّد نفسِه («Failed to generate/validate JSON. **Please adjust your prompt**») مقروناً بـ`400`: دائمٌ **لهذا النداء** فلا تُعاد المحاولةُ على مُوجَّهٍ لا يتغيّر (قِيس على سجلٍّ حيّ: ٩ نوبات، و٢٠ ثانيةً محروقةً في نوبةٍ واحدة)، ولا يُدين المزوّدَ فمفتاحُه سليمٌ ورصيدُه قائم؛ ورسالتُه `AI_PROMPT_MSG` لا `AI_UNAVAILABLE_MSG` لأنّ «رصيدُك منتهٍ» جملةٌ **كاذبة** هنا تُطيّر صاحبَ المشروع خلف فاتورة. وشاهدٌ ثانٍ من صاحب المنصّة بلفظٍ مغاير (`property 'at' is unsupported`) حسم **شكلَ** القاعدة: رمزُ الحالة لا قائمةُ ألفاظ — فقائمةُ الألفاظ أمُّ الأعطاب، وقد وُسّعت `MODEL_PROBLEM` مرّتين لهذا. و`sanitizeMessages` تعالج ذلك الشاهدَ في أصله: `conversationStore` يخزّن كلَّ دورٍ بختمٍ زمنيّ `{role, content, at}` و`chatResponse` ينشره في الرسائل، فحقلُ **تخزينٍ** عندنا صار حقلَ **بروتوكولٍ** عندهم — والمزوّدان العاملان اليومَ كلاهما على واجهة OpenAI فيرفضانه، وGemini كان يستره بتسطيح الرسائل نصّاً. و`ensureJsonWordInPrompt` تعالج **العلّة** لا تصنيفَها: الواجهةُ المتوافقة مع OpenAI تشترط ذكرَ كلمة JSON في الرسائل في وضع `json_object`، فتُحقن **في البوّابة** حين تغيب لا في أربعةَ عشرَ موضعَ نداء + إعادةُ محاولةٍ محدودةٍ بتراجعٍ تصاعديّ على `transient` وحدَه (`attemptChain` هي السلسلةُ كما كانت، و`createWithFailover` غلافُها) | MOVED (نقلٌ حرفيّ: لا سطرَ منطقٍ تغيّر) | Provider | `core/providers/llm.js` — لا Registry قبل Model Router (لا تجريد بلا مستهلك) |
...
## C) `backend/agents/*` — 144 وحدة (منها 42 قالب كلون)

### C1. النواة (Kernel candidates)
| الملف | سطور | المسؤولية | القرار | العقد الجديد | الموقع النهائي |
|---|---|---|---|---|---|
| `jcr.js` | 1070 | وقت التشغيل المعرفي: `handleUserMessage` → 7 معالجات نية (كلُّها مفوِّضاتٌ إلى `stages/*` بعد JCR/28) → `executeMission` → `_runMissionNow` → 15 مرحلة؛ **ونمَت ٣٦ سطراً في «حُرّاس الصمت»** (لا انزلاقاً): `surgicalEdit` صار يُقرّ بالاستلام ويُقيّد نصَّ الطلب في السجلّ الدائم، و`_reportIfCrashed` تُسمِع انهيارَ المهمّة في الشات ثمّ تُعيد الرميَ؛ و`classifyIntent` تقصّ صدرَ الرسالة (١٥٠٠ حرفاً) وتقبل `llm` وسيطاً بافتراضيّ — آخرُ مستهلكٍ للمزوّد في الصنف صار مقيساً. **النقطة ١ (StageResult) لم تلمس هذا الملفّ**: حارسُ الحدود (`deliveryVerdict.test.mjs`) يثبّت أنّ `recordGateOutcome(` تظهر مرّةً واحدة هنا (`guard-and-write` فقط) — بوّاباتُ المراحل الأخرى تُسجَّل من *داخل* وحدة كلِّ مرحلة (نمط `requirementsVerify.js`/`verify.js`)، فمراحلُ الجودة الستّ اتّبعت النمطَ نفسَه في `quality.js` لا في حلقة هذا الملفّ | MODIFY (تدريجي، بلا إعادة كتابة) | Mission + Agent + Task (المراحل → TaskGraph) + Event | يبقى؛ يتقلّص مع كل Sprint لصالح `core/runtime/*` |
...
| ✅ `stages/understand.js` (جديد، JCR/5؛ PM/1 يبذر الفهمَ من المراجع؛ PM/22 يقول «فهمي لا يمسّ طلبَك» بدل ابتلاعه؛ PM/23 يطرحه ويبني على عناوين صاحب المشروع؛ PM/24 وبأداة التعريف حين لا عناوين) | 152 | `understandGoal(goal, ctx, reporter)` — الفهم: ذاكرة/ملفّ ← مخطّط ← نموذج المجال؛ نقلٌ حرفيّ، المُبلِّغُ وسيط؛ **#١٩٤** (+٢٣): بذرةُ مكتبة الفئة تُغربَل بـ`groundedSubset` ثمّ تُدمج **بعد** المشتقّ لا قبلَه — الأولويّةُ المكتوبة في التعليق («المشروع نفسه > اشتقاق هذه الجولة > مكتبة الفئة») لم تكن منفَّذة، فكانت البذرةُ تملأ سقفَي التطبيع (٦ كيانات/٤ أدوار) ويُقصّ فهمُ المشروع ذيلاً؛ مقيسٌ على سجلّ إنتاجٍ حيّ: جمعيّةٌ خيريّة ورثت `SalesRep/Customer/Company/Interaction/Workspace/MeetingRoom` و**طُردت** منها `Donor`/`Donation`/`Volunteer`. وما يُسقَط **يُسمّى** في السجلّ، فهو أوّلُ دليلٍ على تلوّثِ فئةٍ في المكتبة الدائمة | ADDED | Mission | `jcr._understandGoal` مفوِّضٌ من سطر؛ ٧ استيراداتٍ رحلت معها |
| ✅ `stages/enrich.js` (جديد، JCR/6) | 74 | `enrichBuildContext(goal, blueprint, ctx, reporter)` + `resolveProjectType` — متطلّباتٌ ضمنيّة + صور + توجيهاتُ الإضافات؛ نقلٌ حرفيّ، المُبلِّغُ وسيط | ADDED | Mission | `jcr._enrichBuildContext` مفوِّضٌ من سطر؛ `resolveProjectType` يعيد `jcr` تصديرَها لمستورِديها |
| ✅ `stages/requirementsVerify.js` (جديد، JCR/8؛ PM/12: بنودُ الوثيقة في بوّابة الحلقة) | 108 | `runRequirementsVerify(context, roomName, agents, reporter, { verify })` — التحقّقُ من المتطلبات وإكمالُ الناقص بجولاتٍ محدودة؛ نقلٌ حرفيّ إلّا حقنَ `verify` اختياريّاً | ADDED | Evidence + Mission | تُنادى بالاسم من `DELIVERY_STAGES` عبر مفوِّض `jcr._stageRequirementsVerify` |
...
| ✅ `stages/buildFromClone.js` (جديد، JCR/12؛ PM/7: يمرّر متطلّباتِ الفهم المدمَج على ما وصل القرص؛ PM/8: جولةُ إكمالٍ واحدة بـ`complete` محقونٍ افتراضُه `patchEditPlan`؛ PM/9: بنودُ الوثيقة للحكم والإكمال) | 310 | `buildFromClone(clone, goal, ctx, reporter)` — البناءُ من كلونٍ عامل: ملفّاتُ القالب بلغة المستخدم + بصمةٌ موضعيّة (عيّنة/صور/علامة) بتراجعٍ عند الكسر + هويّةٌ ونشرٌ ثابت + نهائيّاتُ النجاح؛ `reporter.io` موضعٌ واحد معلَن | ADDED | Mission + Tool | مفوِّض `jcr._buildFromClone` من `_selectBuildStrategy` |
...
| ✅ `stages/scaffold.js` (جديد، JCR/14) | 79 | `runAdvancedModules/runFullStackScaffold(context, roomName, reporter)` + `runProjectMemory(context)` — وحداتٌ متقدّمة (Stripe/Upload/OAuth/Travelpayouts بتنبيه env)، سكافولد Next.js+Prisma في `fullstack/`، وذاكرةُ المشروع (بلا بثّ فلا مُبلِّغ) | ADDED | Task (مراحل التسليم) | ثلاثُ مفوِّضاتٍ في `jcr` تُستدعى بالاسم من `DELIVERY_STAGES` |
| ✅ `stages/backend.js` (جديد، JCR/16؛ PM/5: `resolveType` بالدليل) | 211 | `runBackendStage(context, roomName, agents, reporter)` — فريقُ الخلفية (best-effort عبر LLM) ← المولّدُ التقليديّ `agents.generateBackend` + تكاملُ script.js عبر الحارس ← قاعدةُ البيانات ← Postgres/Prisma ← المصادقة؛ أكبرُ مرحلةِ تسليم (٢٩ سطرَ بثّ) | ADDED | Task (مراحل التسليم) + Agent | مفوِّض `jcr._stageBackend` يُستدعى بالاسم من `DELIVERY_STAGES` |
| ✅ `stages/verify.js` (**#١٩٨: «لم أتتبّعه بمفرداته» يُقال بعدده — `unverified` تمنع PASS ولا تتّهم**) (جديد، JCR/17–18؛ PM/7: `requirementsTraceOutcome` و`strategyVerdict({requirements, files})`؛ PM/9: `sections`؛ PM/10: `behaviorOutcome` بتفصيل الفحص؛ PM/12: عددا الوثيقة مع الحكم؛ PM/21: يقول لمَ نقص المقام؛ 🔤 لا نجاحَ بأثرٍ في نصٍّ لا يشغّله شيء؛ 📚 لا إيداعَ في مكتبة النماذج من بناءٍ حكمُه FAILED؛ PM/22: ولا من فهمٍ لا يمسّ الطلبَ أصلاً) | 192 | `verifyAndAutofix({projectPath, blueprint, username, activeProject, roomName, agents, lang, canFix}, reporter)` — التحقّقُ السلوكيّ (jsdom) + جولةُ إصلاحٍ واحدة عبر `agents.coreEditCodePlan` والحارس + تسجيلُ ما بقي درساً؛ و`runBehaviorVerifyStage(context, roomName, agents, reporter)` — مرحلةُ التسليم `behavior-verify`: الميزانيةُ تحسم `canFix`، والنجاحُ يُغني مكتبةَ النماذج | ADDED | Evidence + Tool | مفوِّضان في `jcr`: `_verifyAndAutofix` (ثلاثة مستدعين + استبدالُ `jcrSurgicalEdit`) و`_stageBehaviorVerify` (بالاسم من `DELIVERY_STAGES`) |
| ✅ `stages/buildReact.js` (جديد، JCR/19؛ PM/7: يمرّر فهمَ الذاكرة على المعاينة الثابتة؛ PM/9: بنودَ الوثيقة؛ PM/15: الحكمُ على ما بُني كلِّه؛ PM/16: النموذجُ يصل كاتبَ كلِّ صفحة + منفذُ حقن `llm`؛ PM/19: يسجّل ما بناه السكافولد حين لا يسمّي الطلبُ قسماً) | 152 | `buildReactProject(goal, ctx, {sections, llm}, reporter)` — سكافولد Next+Tailwind بمحتوىً مكتوبٍ بالذكاء (best-effort) وتخصيصِ كلِّ صفحةٍ بقيت افتراضيّة، معاينةٌ ثابتة لكلِّ مسار + `dashboard.html`، نهائيّاتُ النجاح، ثمّ تحقّقٌ سلوكيّ بلا إصلاح؛ `reporter.io` موضعٌ واحد معلَن | ADDED | Mission + Tool | مفوِّض `jcr._buildReactProject` من `_selectBuildStrategy` |
| ✅ `stages/selectBuildStrategy.js` (جديد، JCR/29؛ PM/1 يختار بالفهم ويقوله؛ **قياسُ POS**: وثيقةٌ/سيستم لا يخطفهما الاختصارُ التسويقيّ؛ + الإطارُ المنصوص) | 213 | `selectBuildStrategy(goal, blueprint, ctx, reporter, {buildFromRegistry, buildFromClone, buildReactProject, trackOf}) → result \| null` — اختيارُ الاستراتيجيّة قبل النواة: Registry (تسويقيّ) / Clone (تطبيقٌ عامل مطابق) / React-Next (كبيرٌ جديد) بحماياتها الثلاث (الاستئنافُ لا يُعاد بناؤه ولا يُطابَق كلوناً؛ تطبيقٌ يعمل لا يُستبدل دون «أعد البناء»)، وإلّا `null`. البناةُ الثلاثة عبر `ops` (تستبدلها الاختبارات)؛ تلميحُ المسار `trackByRoom` عبر `ops.trackOf` دالّةً مربوطة؛ **`isFreshBuild` يُشتقّ من `hasProjectSource` (سؤالُ وجودٍ للقرص) لا من قارئ المحتوى ذي القائمة المغلقة** — مستودعٌ عامرٌ بغير الأسماء الثلاثة كان يُقرأ صفراً فيُدهَس؛ لا `io` | ADDED | Mission + Task | مفوِّض `jcr._selectBuildStrategy` من `_runMissionNow` |
| ✅ `stages/missionMeta.js` (جديد، JCR/30) | 93 | `runMissionMeta(context, roomName, reporter, {loadExecutiveMemory}, client = groq)` + `noteUnknowns(context, roomName, reporter)` — المرحلةُ الثانية «2. MISSION & META»: مهمّةٌ ووعيٌ ذاتيّ من استدعاءِ JSON واحد، ميزانيةٌ من الأولويّة (complex ١٥ / medium ٧)، واحتياطٌ حتميّ بسببٍ صريح؛ صنفُ `CognitiveBudget` معها (مستهلكُه الوحيد)؛ العميلُ يُحقَن بافتراضيّ على سابقة `router`؛ لا `io` | ADDED | Mission + Task | مفوِّضا `jcr.buildMissionAndMeta`/`jcr._noteUnknowns` من `_runMissionNow` |
| ✅ `stages/chatResponse.js` (جديد، JCR/31؛ + لا انتظارَ على بابٍ مغلق؛ +١ #١٩١: وسمُ `chat` يلفّ **تصريفَ التدفّق** عبر `drainStream` — نداءٌ واحدٌ لا قطعةً قطعة) | 175 | `generateChatResponse(userMessage, username, roomName, userLang, reporter, ops, client = groq)` — ردُّ الشات: ذاكرةٌ تنفيذيّة + نافذةُ المحادثة + دماغُ المشروع مُؤرَّضاً على الكود الفعليّ + معرفةُ المنصّة، ثمّ بثٌّ حيٌّ بمحاولتَين — **والعطبُ الدائم (`aiUnavailable`) يُكسَر عليه فوراً**: قِيس نداءان و٢٥٠٥ms من انتظارٍ بلا احتمالِ نجاح، لأنّ الحلقةَ سبقت `createWithFailover` فلم تعرف عَلَمَه؛ `loadExecutiveMemory`/`summarizeConversation` في `ops`؛ لا `io` | ADDED | Event + Mission | مفوِّض `jcr.generateChatResponse`؛ مستهلكوه: معالجا النيّة في `stages/intentHandlers.js` |
...
| ✅ `stages/surgicalEdit.js` (جديد، JCR/22؛ PM/22) | 285 | `runSurgicalEdit(instruction, ctx, reporter, ops)` — التعديلُ الجراحيّ على مشروعٍ قائم: نسخةٌ احتياطيّة، كشفُ نوع المشروع من القرص وتوجيهُ عمليّات صفحات React («صفحة» صريحةً تحسم، وبغيابها لا تُفترَض العمليّةُ إلّا على صفحةٍ قائمة — PM/22)، قرارُ «تعديلٌ كبير → بناءٌ كامل»، تعديلٌ موضعيّ ثمّ كامل بعقد الحفظ، الحارسان، المعاينة، حارسُ الارتداد بمرحلتيه، التحقّقُ السلوكيّ، التقرير؛ `ops = {runMission, renamePage, deletePage, addPage, verify}` شقٌّ محقَن من المفوِّض حتّى تبقى استبدالاتُ الاختبارات على النسخة نافذة؛ `reporter.io` موضعٌ واحد | ADDED | Mission + Tool + Event | مفوِّض `jcr._runSurgicalEditNow` (مستدعيه `surgicalEdit` عبر الصفّ) |
| ✅ `stages/addPage.js` (جديد، JCR/23؛ PM/16) | 121 | `addPageNow(instruction, projectPath, username, activeProject, roomName, lang, reporter, ops)` — إضافةُ صفحةٍ لمشروع React قائم بلا إعادة بناء: قراءةُ `lib/content.js` (تعذّرُها → `ops.runMission` بسياقٍ من الوسائط)، اسمُ مكوّنٍ ومسارٌ فريدان، قسمٌ قالبيٌّ يخصّصه الذكاء best-effort، كتابةُ المحتوى والمكوّن وصفحة Next، إعادةُ توليد الموقع الثابت، الذاكرةُ والقياساتُ والمعاينةُ والتقرير؛ `ops = {runMission, llm}`؛ `reporter.io` موضعٌ واحد؛ PM/16: سياقُ الكاتب نموذجُ المجال لا عنوانُ البطل، و`ops.llm` منفذُ حقن | ADDED | Tool + Event | مفوِّض `jcr._addPageNow` (يصله `runSurgicalEdit` عبر `ops.addPage`، وتستبدله الاختبارات) |
...
| ✅ `stages/intentHandlers.js` (جديد، JCR/25–28؛ + «مواصفةٌ كاملة ⇒ بناءٌ لا ترقيع»؛ + سؤالُ الوجود للقرص) | 450 | `handlePlanningStage(req, agents, reporter, {executeMission}) → boolean` — مرحلةُ الخطّة في حوار التوضيح: تأكيدٌ → تهيئةُ ذاكرة المشروع من المُوضِّح وتسجيلُه ثمّ التنفيذ؛ سؤالٌ → ملخّصُ الخطّة؛ إيقاف/لون/تعديل → في إجابات المُوضِّح. `handleModifyPattern(req, agents, reporter, {surgicalEdit}) → boolean` — كشفُ التعديل بالنمط (احتياطُ الموجّه). `handleBareConfirmations(req, agents, reporter, gate, {executeMission, surgicalEdit}) → boolean` (JCR/26) — «نعم» مجرّدةً تنفّذ المحجوبَ (يُقرأ ويُمسح عبر `gate`) أو تستأنف من الذاكرة حين يسمح القرار؛ «نفّذ» مجرّدةً تنفّذ آخرَ ما وصفه المساعد. `handleUnifiedRoute(req, agents, reporter, gate, {surgicalEdit, generateChatResponse}, router = routeMessage) → boolean` (JCR/27) — الموجّهُ الموحَّد (`routeMessage`) بشبكة أمان الحجب/الإصرار: أمرٌ أو إصرارٌ → تعديل، جملةٌ إخباريّة → تُحجَب مرّةً (`gate.set` + `gate.confirmReply`)، سؤالٌ → محادثة، `edit`/`delete_project`/`stop`، و`build` أو فشلُ الموجّه → `false`. الموجّهُ `router` وسيطٌ أخير قابلٌ للحقن (سابقةُ `router.js#routeMessage(…, llm)`)؛ المفوِّضُ لا يمرّره. `handleClassifiedIntent(req, agents, reporter, gate, {classifyIntent, surgicalEdit, generateChatResponse}) → true` (JCR/28) — المصنِّفُ الأخير: `ops.classifyIntent` أو نيّةُ المعنى (ثقة ≥ 75) ثمّ — **قبل كلِّ شيء — `isFullSpecification`: وثيقةٌ (≥١٢٠٠ حرفاً و≥٦ بنودٍ مرقّمة) طلبُ بناءٍ مهما قال المصنِّف، تتخطّى حارسَ «فعلٍ في المستهلّ» وتذهب إلى فرع البناء الذي **يسأل** ويعلّق الهدفَ كاملاً** — ثمّ build (غيرُ صريح على مشروعٍ قائم → تعديل/محادثة؛ صريح → حوارُ توضيحٍ أو تأكيدٌ يعلّق الهدف) / modify (سؤالٌ → محادثة، جملةٌ → حجبٌ مرّةً، إصرارٌ/فعلٌ → تعديل) / stop / وإلّا (فعلٌ أو إصرارٌ → تعديل، جملةٌ → حجب، سؤالٌ أو كلامٌ قصير أو بلا مشروع → محادثة). لا `io`؛ حالةُ الحجب تبقى على الصنف وتصل بـ`jcr._gate()` دوالَّ مربوطة — لم يبقَ قارئٌ لها في الصنف غيرَ الشقّ. **مواضعُ «أثمّة مشروع؟» الثلاثةُ تسأل `hasProjectSource` (عتبةُ ١٠٠ كما كانت) لا `readCodeContext`** — فلم يبقَ لقارئ المحتوى مستهلكٌ في هذا الملفّ | ADDED | Permission + Mission | خمسةُ مفوِّضات في `jcr` من `handleUserMessage` |
| ✅ `projectReader.js` (جديد، JCR/15؛ PM/15، PM/22؛ + سؤالُ الوجود) | 176 | `readCodeContext(projectPath)` → نصُّ سياقٍ للـLLM (index/styles/script)؛ `readProjectFiles(projectPath)` → **قارئُ التعديل**: كلُّ CSS + الصفحةُ وما تُحمّله فعلاً + احتياطُ script.js؛ `readBuiltFiles(projectPath)` → **قارئُ الحكم** (PM/15): كلُّ مصدرٍ وصل القرصَ بمجلّداته، بلا `node_modules` ولا مخفيّ؛ `isReactProject(projectPath)` → **كشفُ النوع من القرص** (PM/22): `lib/content.js` أو `app/page.jsx` — لا اشتقاقاً من قائمة قارئ التعديل الضيّقة؛ `readReactSources(projectPath)` → **مصادرُ** المشروع (المحتوى + `components/*.jsx`) هدفاً للتعديل؛ `hasProjectSource(projectPath, {minBytes=80, budget=400})` → **سؤالُ الوجود** (سادسةُ حالات PM/15): «أعلى القرص مصدرٌ يزن شيئاً؟» بـ`stat` لا قراءة، **بلا قائمةِ أسماءٍ ولا امتدادات**، بلا `node_modules`/مخفيّ/ثنائيّات، ويخرج عند تجاوز العتبة — العطبُ المقيس: مستودعُ Laravel بسبعةِ ملفّات يُقرأ صفرَ حرفٍ عبر `readCodeContext` فيصير `isFreshBuild = true` وهو **شرطُ الاستبدال الكامل**؛ والعتبةُ تبقى فلا يصير ركامُ `<h1>x</h1>` مشروعاً — ستّةُ قرّاءٍ صامتين بلا `this` | ADDED | Tool | مفوِّضان على `jcr` (١٢ مستدعياً + استبدالُ الاختبارات) + `hasProjectSource` بأربعةِ مواضعِ نداء في مرحلتَين |
| `contracts.js` → ✅ `core/contracts/index.js` | 99/249 | typedefs الأحد عشر + `assertBuildAgents` + `DELIVERY_STAGES` + مدقّقا Capability؛ +١١ سطراً (النقطة ١): `StageResult` typedef بجانب `StageFn` | MOVED (Sprint 1 ✅) | Mission/Agent/Task/Capability/Provider/Transaction | `core/contracts/index.js` |
...
| `router.js` | 117 | الموجّه الموحّد للرسائل | MODIFY | Mission | مع `ceoBrain` |
| `chatCommands.js`, `textNormalizer.js` (+`isFullSpecification`: وثيقةٌ لا جملة؛ PM/9: `specSections`؛ PM/11: `specHead`/`clipWords`؛ 🚫 PM/23: `stripNegated` — **مصدرُ الحقيقة الواحد** لطيِّ النفي، لمّا تبيّن أنّ له مستهلكَين: سؤالُ الخادم وبوّابةُ الفهم؛ **#١٩٧**: `stripDiacritics` — **مصدرٌ واحدٌ لإسقاط التشكيل**، يقرؤه `normalizeArabic` ومطابقةُ الكلمات المفتاحيّة معاً), `languageDetector.js`, `logLocalizer.js`, `failureMessages.js` | 120/436/200/204/31 | أوامر حتمية، تطبيع نص، لغة، ترجمة السجل، رسائل الفشل (تحمل تشخيصَ المزوّدين) | KEEP | — | `agents/` (أدوات النواة اللغوية) |
...
| `knowledgeEngine.js` (PM/13: لا أفعالَ طلبٍ في كلمات الصناعات؛ **#١٩٧**: `keywordMatches` تُسقط التشكيلَ من الطرفَين عبر `stripDiacritics` — مقيسٌ حيّاً: «منصّة» بالشدّة لم تطابق «منصة» في المعجم، فخلا طلبُ منصّةٍ من كلِّ لفظِ تطبيقٍ وصُنّف «موقع تعريفي») | 348 | كشف نوع المشروع + سياق معرفي + `needsBackend` | KEEP | — | `plugins/coding/` |
| `backendNeed.js` | 80 | **مصدرُ الحقيقة الواحد** لـ«أيحتاج خلفيةً؟» (Sprint 7/1) + العلاقية مجموعةً جزئيّة منه (2r)؛ و`stripNegated` انتقلت إلى `textNormalizer` (PM/23) حين وُجد لها مستهلكٌ ثانٍ — تُعاد تصديراً | KEEP | — | `plugins/coding/` |
...
| الملف | سطور | المسؤولية | القرار | العقد الجديد | الموقع النهائي |
|---|---|---|---|---|---|
| `coderAgent.js` | 430 | `coreGenerateCodePlan` / `coreEditCodePlan` (إلزامي) + `selectModels` (يُرشِّح خطَّ النماذج بـ`AI_PROVIDERS` — العملاءُ يُنادَون هنا مباشرةً خارج السلسلة). **والحلقتان متساويتان الآن**: مسارُ التعديل الجراحيّ كان مصفوفةَ دوالٍّ عارية بلا مرشِّح ولا إشارةِ عطبٍ دائم، فقِيس أنّه ينادي **خاماً** مزوّدَين أعلن السجلُّ استبعادَهما. **و#١٩١**: وسمان (`coder:generate`/`coder:edit`) حول `model.call()` كلِّه لا حول إنشاء الطلب (مسارُ التدفّق يحسب في `for await`)، وسُدّ ثقبٌ مقيس: `callDeepSeek` غيرُ المتدفّق يخاطب عميلاً **خاماً** لا يمرّ بـ`tagged`، فقِيس بـ`AI_PROVIDERS=deepseek` نداءٌ واحدٌ للمزوّد و**صفرُ** رموزٍ محسوبة — فيُقرأ «المولّد رخيص» وهو لم يُقَس | MODIFY | Agent (spec تصريحي تحت Agent Runtime) | `plugins/coding/agents/` |
...
| `deployAgent.js` | 523 | نشر Vercel (يستقبل `io`) | MODIFY | Tool (`deploy`, requiresConfirmation, riskLevel: high) | `plugins/coding/tools/` |
...
| الملف | سطور | المسؤولية | القرار | العقد الجديد | الموقع النهائي |
|---|---|---|---|---|---|
| `designerAgent.js` (PM/5: يقبل تلميحَ النوع) | 302 | ملخّص تصميم + احتياط حتمي + تخصيصُ AI ناطقٌ بسبب تخلّفه (2p) | KEEP | Task | `plugins/coding/stages/` |
...
| `renderAgent.js` | 265 | نشر Render | MODIFY | Tool (deploy) | `plugins/coding/tools/` |
...
| `backendTeam/backendTeam.js` | 198 | منسّق عام `runBackendTeam/runAgent` + `safeRelPath/writeBackendTeamFiles`؛ ✅ `planExecution` صار غلافاً لـ`TaskGraph.orderTasks` | MOVE (المنسّق) / MODIFY (الكتابة → Tool) | Agent Runtime + Task + Tool | `core/runtime/AgentRuntime.js` (Sprint 2b) |
...
| الملف | سطور | المسؤولية | القرار | العقد الجديد | الموقع النهائي |
|---|---|---|---|---|---|
| `templateLibrary.js`, `templateLibraryExtended.js`, `templateLocalizer.js` | 1061/1382/1961 | مكتبة القوالب وترجمتها | KEEP | — | `plugins/coding/templates/` (Sprint 6) |
...
| `cloneAssets.js`, `seedStamp.js`, `fullstackTemplates.js`, `reactGenerator.js` (PM/17: اسمُ العرض غيرُ مُعرِّف الحزمة؛ PM/19: `meta.sections` تُعلن ما بُني؛ PM/20: تسميةُ القسم المحقون بلغة صاحب المشروع والمفتاحُ لاتينيّ), `blockRegistry.js` (الاختصارُ التسويقيّ يقرأ الفهمَ قبل الكلمات؛ لا ألفاظَ عميلٍ في `MARKETING_HINTS`), `starterRegistry.js` (**الإطارُ المنصوص**: `explicitStackRequest` — «React»/«Next.js» مكتوبةً في الطلب لا يُنقضها تصنيفٌ خمّنه نموذج؛ قِيس حيّاً أنّ فئةَ مخطّطٍ `business` أسقطت طلبَ React إلى Vanilla)، `starterFetch.js`, `libraryRegistry.js`, `referenceBlueprints.js` (PM/1: `referenceModel`؛ PM/4: `referenceEntities`/`referenceFlows`), `appBlueprint.js` (PM/11: `appType` من رأس الوثيقة على حدّ كلمة؛ `hardenBlueprint` موضعٌ واحد يحصّن مسارَي النموذج والاحتياط — لا حقلَ معروضاً بلا قيمة), `requirementAnalyzer.js` | 74/116/676/450/267/111/171/78/249/170/198 | استراتيجيات البناء ومخططاته | KEEP | Task (استراتيجية = Task Graph مختلف) | `plugins/coding/` |
| `projectModel.js` (PM/1: معجمُ المفاهيم + `modelAffinity`؛ PM/3: `conceptsInText` + `domainFidelity`؛ PM/4: `conceptKind`؛ PM/5: `modelProjectType`؛ PM/6: `conceptFrequencies` + `lexiconModel` والاحتياطُ يقرأهما؛ PM/7: `isGenericConcept`؛ PM/14: `productText` — التنسيقُ وأسماءُ الوسوم والسمات ليست لغةَ المنتج؛ PM/22: `goalWords` + `goalFidelity` — مقارنةٌ **مفتوحة** بكلمات الطلب لا بقائمةٍ مغلقة، لأنّ المعجمَ يرى صفراً في ٨/٨ ممّا ليس موقعاً تجاريّاً؛ PM/23: `headingEntityNames` — رأسُ كلِّ عنوانٍ مرقّم اسمُ كيان (٩٥٪ مقيسة)، والنفيُ يُطوى قبل الحكم؛ PM/24: `normalizeLetters` + `articleEntityNames` — «ال» علامةُ اسمٍ لا تدخل على فعل (٩٦٪ للطلب القصير)؛ **PM/25**: `labelledRoleNames` — الأدوارُ من حيث يسمّيها صاحبُها صراحةً: سطرٌ معنون («أدوار: …») **أو قوسٌ ملاصقٌ للفظ** («أدوار (owner, editor, viewer)») بشرط أن يكون قائمةً لا مفردة — قِيس أنّ ٣ أقواسٍ من ٢٩ في نصوصٍ حقيقيّة تلي لفظَ دور، وأنّ وحيدَها المفرد «(RBAC)» اختصارُ مفهومٍ لا اسمُ دور، ٥/٥ مقيسةً على مواصفةٍ حقيقيّة، وتسبق معجمَ `lexiconModel` في الاحتياط لأنّ المعجمَ كان يُخرج `staff/customer/admin/tenant` فتفحص `role-coverage` ألفاظاً لم يكتبها صاحبُ المشروع قطّ)؛ **#١٩٤**: `groundedSubset` — غربلةُ فهمٍ **موروث** بطلبِ صاحبِه: `goalFidelity` تحسب `groundless` لكلِّ اسمٍ على حدة أصلاً ثمّ كان المصفوفُ يُهمَل ولا يُقرأ إلّا نعم/لا، فتُقرأ الإشارةُ بدِقّتها. المبدأ: المكتبةُ **تُثري ما سمّاه صاحبُ الطلب ولا تُضيف ما لم يُسمِّه** — فما نجا يحمل حقولَه المتراكمة عبر المشاريع، وما لا أثرَ له لا يدخل. ؛ **#١٩٥** (+٢٢): مجالُ العمل الخيريّ كان **غائباً بتمامه** عن المعجم (`campaign`/`donor`/`donation`/`volunteer`/`charity`) فاشتُقّ لجمعيّةٍ خيريّة فهمٌ **صحيح** ثمّ طرحه الحارسُ وبنى على `تسجيل/ملف/لوحه` وانتهى FAILED — مقيسٌ على سجلّ ٢٠٢٦-٠٩-٠٩؛ و`pushRoleItems` تنزع القوسَ من طرفَي اسم الدور، إذ كان `(مدير الجمعية` و`متطوع)` يدخلان بقوسَيهما فيستحيل نجاحُ `role-coverage` لمن يكتب أدوارَه بين قوسَين. وقيدُها مقيسٌ ومثبَّتٌ باختبار: `Company` تسقط رغم «شركتِه» لأنّ الجسرَ المعجميّ لا يربطهما (#١٨٦)، واتّجاهُ الخطأ مقصود — إسقاطٌ كاذبٌ يكلّف حقولاً يعيد الاشتقاقُ أكثرَها، وإبقاءٌ كاذبٌ يُدخل منتجاً آخرَ ثمّ يطرد صاحبَ الطلب), `projectMemory.js`, `userProfile.js` | 918/297/248 | ذاكرة المشروع والمستخدم | KEEP | Memory | `core/memory/` (لاحقاً) |
...
| `metricsStore.js`, `usageMeter.js`, `errorLog.js`, `logger.js`, `adminAudit.js` | 155/78/45/36/41 | قياس واستهلاك وتدقيق | MODIFY | Audit (AuditLog يوحّدها) | `core/audit/` (Sprint 4) |
...
| الملف | سطور | المسؤولية | القرار | العقد الجديد | الموقع النهائي |
|---|---|---|---|---|---|
| `deploymentLayout.js` | 73 | اكتشاف Next ومجلد التشغيل وربط PostgreSQL | KEEP | Tool | `plugins/coding/tools/` |
| `generatedUpgrade.js` | 69 | تحديث الملفات المولدة مع كشف التخصيص والنسخ الاحتياطي | KEEP | Tool | `plugins/coding/tools/` |
...
| `bookingClient.js` | 69 | حجز الخادم وعزل العملاء | KEEP | Booking | `services/` |

| `bookingConfiguration.js` | 63 | جودة الحجز والمتجر: إعدادات ومعاملات وعميل الخادم | KEEP | Product runtime | `services/` |

| `commerceStore.js` | 122 | جودة الحجز والمتجر: إعدادات ومعاملات وعميل الخادم | KEEP | Product runtime | `services/` |
