...
| الملف | سطور | المسؤولية | القرار | العقد الجديد | الموقع النهائي |
|---|---|---|---|---|---|
| `jcr.js` | 1017 | وقت التشغيل المعرفي: `handleUserMessage` → 7 معالجات نية (كلُّها مفوِّضاتٌ إلى `stages/*` بعد JCR/28) → `executeMission` → `_runMissionNow` → 15 مرحلة؛ **ونمَت ٣٦ سطراً في «حُرّاس الصمت»** (لا انزلاقاً): `surgicalEdit` صار يُقرّ بالاستلام ويُقيّد نصَّ الطلب في السجلّ الدائم، و`_reportIfCrashed` تُسمِع انهيارَ المهمّة في الشات ثمّ تُعيد الرميَ؛ و`classifyIntent` تقصّ صدرَ الرسالة (١٥٠٠ حرفاً) وتقبل `llm` وسيطاً بافتراضيّ — آخرُ مستهلكٍ للمزوّد في الصنف صار مقيساً | MODIFY (تدريجي، بلا إعادة كتابة) | Mission + Agent + Task (المراحل → TaskGraph) + Event | يبقى؛ يتقلّص مع كل Sprint لصالح `core/runtime/*` |
...
| ✅ `stages/reportMissionSuccess.js` (جديد، JCR/11) | 113 | `reportMissionSuccess(goal, ctx, reporter)` — تقريرُ التسليم بلغة المستخدم + الاقتراحات + قائمةُ الملفّات + الدفعُ التلقائيّ (`reporter.io`، موضعٌ واحد معلَن) + اللقطة + المقاييس + hook afterBuild؛ متزامنةٌ كما كانت | ADDED | Mission + Event | مفوِّض `jcr._reportMissionSuccess` من `_runMissionNow` |
| ✅ `stages/buildFromClone.js` (جديد، JCR/12؛ PM/7: يمرّر متطلّباتِ الفهم المدمَج على ما وصل القرص؛ PM/8: جولةُ إكمالٍ واحدة بـ`complete` محقونٍ افتراضُه `patchEditPlan`) | 270 | `buildFromClone(clone, goal, ctx, reporter)` — البناءُ من كلونٍ عامل: ملفّاتُ القالب بلغة المستخدم + بصمةٌ موضعيّة (عيّنة/صور/علامة) بتراجعٍ عند الكسر + هويّةٌ ونشرٌ ثابت + نهائيّاتُ النجاح؛ `reporter.io` موضعٌ واحد معلَن | ADDED | Mission + Tool | مفوِّض `jcr._buildFromClone` من `_selectBuildStrategy` |
...
| `chatCommands.js`, `textNormalizer.js` (+`isFullSpecification`: وثيقةٌ لا جملة), `languageDetector.js`, `logLocalizer.js`, `failureMessages.js` | 120/308/200/204/25 | أوامر حتمية، تطبيع نص، لغة، ترجمة السجل، رسائل الفشل | KEEP | — | `agents/` (أدوات النواة اللغوية) |
...
| `requirementsVerifier.js` (PM/4: `composeRequirements`؛ PM/7: `_kind` + `traceRequirements` بمعجم projectModel) | 156 | هل نُفِّذت المتطلبات؟ | KEEP | Evidence | `core/verification/` (Sprint 4) |
