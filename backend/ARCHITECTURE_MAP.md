...
| ✅ `projectReader.js` (جديد، JCR/15؛ PM/15) | 89 | `readCodeContext(projectPath)` → نصُّ سياقٍ للـLLM (index/styles/script)؛ `readProjectFiles(projectPath)` → **قارئُ التعديل**: كلُّ CSS + الصفحةُ وما تُحمّله فعلاً + احتياطُ script.js؛ `readBuiltFiles(projectPath)` → **قارئُ الحكم** (PM/15): كلُّ مصدرٍ وصل القرصَ بمجلّداته، بلا `node_modules` ولا مخفيّ — ثلاثةُ قرّاءٍ صامتين بلا `this` | ADDED | Tool | مفوِّضان على `jcr` (١٢ مستدعياً + استبدالُ الاختبارات) |
