...
| ✅ `stages/buildFromClone.js` (جديد، JCR/12؛ PM/7: يمرّر متطلّباتِ الفهم المدمَج على ما وصل القرص) | 216 | `buildFromClone(clone, goal, ctx, reporter)` — البناءُ من كلونٍ عامل: ملفّاتُ القالب بلغة المستخدم + بصمةٌ موضعيّة (عيّنة/صور/علامة) بتراجعٍ عند الكسر + هويّةٌ ونشرٌ ثابت + نهائيّاتُ النجاح؛ `reporter.io` موضعٌ واحد معلَن | ADDED | Mission + Tool | مفوِّض `jcr._buildFromClone` من `_selectBuildStrategy` |
