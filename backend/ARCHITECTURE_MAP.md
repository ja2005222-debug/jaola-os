...
| الملف | سطور | المسؤولية | القرار | العقد الجديد | الموقع النهائي |
|---|---|---|---|---|---|
| `jcr.js` | 2191 | وقت التشغيل المعرفي: `handleUserMessage` → 7 معالجات نية → `executeMission` → `_runMissionNow` → 15 مرحلة | MODIFY (تدريجي، بلا إعادة كتابة) | Mission + Agent + Task (المراحل → TaskGraph) + Event | يبقى؛ يتقلّص مع كل Sprint لصالح `core/runtime/*` |
...
| ✅ `stages/verify.js` (جديد، JCR/17) | 55 | `verifyAndAutofix({projectPath, blueprint, username, activeProject, roomName, agents, lang, canFix}, reporter)` — التحقّقُ السلوكيّ (jsdom) + جولةُ إصلاحٍ واحدة عبر `agents.coreEditCodePlan` والحارس + تسجيلُ ما بقي درساً؛ مشتركٌ بين البناء والتعديل الجراحيّ وReact | ADDED | Evidence + Tool | مفوِّض `jcr._verifyAndAutofix` لثلاثة مستدعين (واستبدالُ `jcrSurgicalEdit`) |
