...
| `chatCommands.js`, `textNormalizer.js` (+`isFullSpecification`: وثيقةٌ لا جملة؛ PM/9: `specSections`؛ PM/11: `specHead`/`clipWords`؛ 🚫 PM/23: `stripNegated` — **مصدرُ الحقيقة الواحد** لطيِّ النفي، لمّا تبيّن أنّ له مستهلكَين: سؤالُ الخادم وبوّابةُ الفهم؛ **#١٩٧**: `stripDiacritics` — **مصدرٌ واحدٌ لإسقاط التشكيل**، يقرؤه `normalizeArabic` ومطابقةُ الكلمات المفتاحيّة معاً), `languageDetector.js`, `logLocalizer.js`, `failureMessages.js` | 120/433/200/204/31 | أوامر حتمية، تطبيع نص، لغة، ترجمة السجل، رسائل الفشل (تحمل تشخيصَ المزوّدين) | KEEP | — | `agents/` (أدوات النواة اللغوية) |
