...
const INTENT_RULES = [
    ...
    // لذلك نستخدم (?:\s|$|[!؟?.]) كحدود للكلمات العربية
    {
        intent: 'continue',
        patterns: [
            // 🛡️ لا هدف محدّد بعد فعل الاستئناف — "اكمل"/"اكمل من فضلك"/"كمل الآن"
            // فقط (مع لاحقة عربية اختيارية: ها/ه/وا/هم/ي). أي نصّ إضافي (اسم قسم/صفحة
            // محدّد كـ"اكمل صفحة الادمن") هو تعليمة تعديل فعلية لا استئناف عام —
            // كان يُطابَق سابقاً بلا هذا القيد (حدّ كلمة فقط، لا نهاية جملة) فتُسقَط
            // تعليمة المستخدم المحدّدة ويُستبدَل بهدف استئناف عام من الذاكرة يُعاد به
            // توليد الموقع كاملاً بذكاء حرّ (لا تعديل موضعي) فينحرف لموقع مختلف كلياً
            // (عطل إنتاجي حقيقي: "اكمل صفحة الادمن" حوّل مشروعاً قائماً لآخر).
            /^(كمل|أكمل|اكمل|كمّل|واصل|تابع|استمر|استانف|استأنف)(?:ها|ه|وا|هم|هما|ي)?\s*(?:من\s*فضلك|لو\s*سمحت|رجاء|الآن|الان)?\s*[!؟?.]*$/,
            // "نفذ" وحدها = أمر تنفيذ عام → استئناف من الذاكرة
            /^(نفذ|نفّذ)(?:\s*(الان|الآن|now))?\s*[!؟?.]*$/,
            /^(تمام\s*)?(نفذ|نفّذ)(?:\s|$|[!؟?.])/,
            /^(continue|resume|proceed|keep going|carry on|do it|execute|go ahead|run it)\s*(?:please|now)?\s*[!.?]*$/i,
            // "كمل/واصل + المشروع/الموقع..." يبقى استئنافاً عاماً (اسم عامّ لا قسم محدّد)
            /^(كمل|أكمل|اكمل|واصل)\s+(المشروع|الموقع|الشغل|البناء|مشروع)\s*[!؟?.]*$/,
            /^(continue|resume)\s+(the\s+)?(project|website|work|build)\s*[!.?]*$/i,
        ],
    },
    {
        intent: 'status',
...
export function buildContinuationGoal(username, project) {
    ...
    // 🛡️ وسم [استئناف] يحمي الهدف من أن يُصنَّف بناءً جديداً/إعادة بناء —
    // عبارة «لا تبدأ من الصفر» القديمة كانت تُطابق «من الصفر» فتُفهم عكسياً
    // وتدهس المشروع القائم وتعيد تسميته (عطل إنتاجي: «اكمل» → مشروع جديد).
    return `[استئناف] تابع تطوير المشروع القائم من حيث توقّف — طوّر الموجود وحافظ على هويته واسمه ومحتواه.\n${parts.join('\n')}`;
}
