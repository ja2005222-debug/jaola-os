...
function fallbackModel(goal, blueprint) {
    ...
    // 👥 PM/25 — وأدوارُ صاحب المشروع تسبق معجمَنا حين يسمّيها. قِيس على مواصفة نقاط البيع:
    //    المعجمُ يُخرج `staff، customer، admin، tenant` — أربعةً بالإنجليزيّة من قائمةٍ مغلقة —
    //    وصاحبُها كتب في سطرٍ واحد «أدوار: مالك النظام، مدير الفرع، الكاشير، أمين المخزن، المحاسب».
    //    وليست هذه تسميةً أجمل فحسب: `role-coverage` **تقع** بأربعةٍ (٤ > ١) فتفحص الصفحةَ بحثاً
    //    عن «tenant» و«staff» — ألفاظٍ لم يكتبها صاحبُ المشروع قطّ. فالحكمُ يقيس اسماً لم يُطلَب.
    //    (عائلةُ PM/22 وPM/23: قائمةٌ مغلقةٌ تحلّ محلَّ كلمات صاحب المشروع.)
    const lex = lexiconModel(stripNegated(goal));
    const named = labelledRoleNames(goal);
    if (named.length) lex.roles = named.map(name => ({ name }));
...
const ROLE_CONCEPTS = {
    ...
    // 🎗️ #١٩٥ — مجالُ العمل الخيريّ كان **غائباً بتمامه** عن المعجم. قِيس على سجلّ
    //    إنتاجٍ حيّ: طُلبت منصّةُ جمعيّةٍ خيريّة، فاشتُقّ لها `Campaign/Donor/Donation`
    //    — وهو الفهمُ **الصحيح** — ثمّ قال الحارسُ «لا أثرَ في وصفِك» فطرحه وبنى على
    //    `تسجيل، ملف، لوحه`. والعلّةُ أنّ `conceptsInText` لم ترَ في الطلب كلِّه إلّا
    //    `accountant` و`admin`: لا «حملة» ولا «متبرّع» ولا «تبرّع» ولا «متطوّع» في المعجم،
    //    فلا جسرَ بين اسمٍ إنجليزيٍّ وطلبٍ عربيّ.
    donor: ['donor', 'donors', 'contributor', 'متبرع', 'متبرّع', 'متبرعين', 'متبرّعين', 'مانح', 'مانحين'],
    volunteer: ['volunteer', 'volunteers', 'متطوع', 'متطوّع', 'متطوعين', 'متطوّعين'],
    beneficiary: ['beneficiary', 'beneficiaries', 'مستفيد', 'مستفيدين', 'محتاج', 'محتاجين'],
};
const ENTITY_CONCEPTS = {
    ...
    // 🎗️ #١٩٥ — تتمّةُ مجال العمل الخيريّ (انظر السببَ المقيسَ عند `donor` أعلاه).
    //    و«حملة» تلتبس بالحملة التسويقيّة، وهو التباسٌ مقبولٌ هنا: كلاهما **حملة**
    //    كياناً لها هدفٌ ومدّةٌ ومتحصّل، فالمفهومُ واحدٌ وإن اختلف المجال.
    campaign: ['campaign', 'campaigns', 'حملة', 'حملات', 'حمله'],
    donation: ['donation', 'donations', 'pledge', 'تبرع', 'تبرّع', 'تبرعات', 'تبرّعات', 'تبرعاته', 'هبة'],
    charity: ['charity', 'nonprofit', 'non-profit', 'ngo', 'جمعية خيرية', 'جمعية', 'جمعيه', 'خيرية', 'خيريه'],
};
...
/**
 * 👥 أسماءُ **أدوارٍ** من سطرٍ يسمّيها صاحبُ المشروع صراحةً (PM/25).
 *
 * **حدُّ PM/23 كان مكتوباً**: «العناوينُ تسمّي كياناتٍ لا أدواراً — العنوانُ لا يقول من يستعمل
 * ماذا، فالأدوارُ تسقط ولا تُختلق». وهو صوابٌ في موضعه؛ لكنّه تُرك حدّاً ولم يُسأل بعده
 * السؤالُ الذي فتح PM/23 نفسَها: **أين يكتب صاحبُ المشروع أدوارَه أصلاً؟**
 *
 * **العطبُ المقيس** على مواصفة نقاط البيع الحقيقيّة (`tests/fixtures/pos_spec.txt`): صاحبُها
 * يكتب سطراً واحداً لا لبسَ فيه —
 *     `- أدوار: مالك النظام، مدير الفرع، الكاشير، أمين المخزن، المحاسب.`
 * ولا يصل منها **ولا واحد**: `fallbackModel(...).roles` تعود `null`، و`headingEntityNames`
 * تُخرج «صلاحيات» (رأسَ قسم الأدوار) بوصفه **كياناً**.
 *
 * **وأثرُ ذلك صمتٌ لا فشل**: `role-coverage` في `behaviorVerifier` مشروطةٌ بـ`roles.length > 1`،
 * فبصفرِ أدوارٍ **لا تُبثّ أصلاً** — لا تنجح ولا تفشل؛ و`composeRequirements` لا تضيف «شاشة
 * <دور>». فمواصفةٌ تسمّي خمسةَ أدوارٍ تُبنى وتُحكَم كأنّها بلا أدوار. **والصمتُ يُقرأ لا مشكلة.**
 *
 * **القاعدة**: سطرٌ عنوانُه (ما قبل النقطتَين) لفظُ أدوار → قيمتُه قائمةٌ تُقسَّم على الفواصل.
 * قِيست على المواصفة نفسِها: **٥/٥**. بلا نموذجٍ لغويّ ولا قائمةِ مجال.
 *
 * ⚠️ `\b` **لا يعمل مع الحروف العربيّة** (تحذيرٌ مكتوبٌ في `textNormalizer.js`): `أدوار\b`
 *    لا يطابق أبداً لأنّ الراء ليست `\w`. وقد وقع الفخُّ هنا فعلاً قبل القياس.
 *
 * ⚠️ **حدٌّ مكتوب**: تسمّي من **يُسمّى صراحةً** في سطرٍ معنون. لا تستنبط دوراً من فعلٍ في المتن
 *    («يسجّل الحافظ…»)، ولا تختلق واحداً حين لا يُسمّى — تعود فارغةً، ويُقال ولا يُخترع.
 *    وقياسُها نقطةٌ واحدةٌ حقيقيّة لا ستّ كـPM/23: تُقال كما هي، لا تُضخَّم.
 * @param {string} goalText نصُّ الطلب كما كتبه صاحبُه
...
function pushRoleItems(out, value) {
    for (const item of value.split(ROLE_SPLIT)) {
        // 🧹 #١٩٥ — القوسُ ليس من الاسم. قِيس على سجلّ إنتاجٍ حيّ (٢٠٢٦-٠٩-٠٩): كتب صاحبُ
        //    المشروع `الأدوار: (مدير الجمعية، مسؤول الحملات، محاسب، متطوع)` فالتُقطت
        //    `(مدير الجمعية` و`متطوع)` **بقوسَيهما**، فصار المتحقّقُ السلوكيّ يفتّش الشفرةَ
        //    عن دورٍ اسمُه حرفيّاً `(مدير الجمعية` — ولن يجده أبداً في شفرةٍ مولَّدة.
        //    فبوّابةُ `role-coverage` كانت **يستحيل** أن تنجح لمن يكتب أدوارَه بين قوسَين.
        //    والنزعُ هنا لأنّه المصدرُ الواحد لمساري الالتقاط (السطرِ المعنون والقوسِ
        //    الملاصق)؛ وفي الثاني لا أثرَ له لأنّ ما يصله محتوى القوس أصلاً.
        const name = item.trim().replace(/^[(（[{]+/u, '').replace(/[)）\]}]+$/u, '').trim();
