 ...
 *
 * تنظيفٌ نصّيٌّ محافظ للكود المُنتج. لا يُحلّل الشفرة ولا يعيد بناءها:
 *
 * - JS: حذفُ `console.log/debug` الزائدة (يُبقي أوّل ثلاثة) بعدِّ الأقواس،
 *       وإضافةُ `'use strict'` إن غابت.
 * - CSS: توحيدُ التصريحات المكرّرة داخل الكتلة الواحدة (تفوز الأخيرة).
 * - HTML: ضمانُ `lang` على `<html>` و`<meta charset>`.
 *
 * 🔴 كانت الترويسةُ تَعِد بأربعةٍ لا يفعلها: «إزالة الكود المكرر» و«تنظيم
 *    CSS Variables» و«تحسين أسماء المتغيرات» و«تقسيم الدوال الطويلة».
 *    وتلك دعاوى تُقرأ عقداً: مَن يقرؤها يظنّ الوكيلَ يُعيد الهيكلة فلا
 *    يفحص ما بقي. القرارُ (بأمر المالك): يبقى محافظاً، وتُصحَّح الترويسة.
 *    وكلُّ ما زُعم ولم يُفعل فمكانُه CONTRACTS.md لا هذه الترويسة.
 */
