/**
 * 🧹 jaola-cleaning — موقع خدمات تنظيف منازل بحجز موعد (track: site — لزوّار).
 *
 * باقات تنظيف (شقة/فيلا/تنظيف عميق) بسعر ثابت، اختيار تاريخ ووقت
 * الزيارة (يمنع حجز وقت محجوز مسبقاً لنفس الفريق)، تأكيد حجز قابل
 * للطباعة، ولوحة إدارة (باقات/حجوزات). أدوار: عميل (يحجز) + إدارة.
 * بلا اعتماد خارجي. الحالة في localStorage (jclean_*).
 */
import { sharedSystemStyles } from './jaolaClinic.js';

export function jaolaCleaning() {
    const INDEX_HTML = `<!doctype html>
<html lang="ar" dir="rtl">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>خدمات jaola لتنظيف المنازل</title>
  <link rel="stylesheet" href="styles.css">
</head>
<body>
  <header class="topbar no-print">
    <div class="brand"><span class="mk">🧹</span> <span id="brandName">تنظيف jaola</span></div>
    <nav class="tabs" id="tabs"></nav>
    <button class="btn ghost no-print" id="authBtn" data-action="openAuth">دخول الإدارة</button>
  </header>
  <main class="no-print">
    <section id="view-home" class="view">
      <div class="hero"><div class="hero-in"><h1>منزلك نظيف بضغطة زر</h1><p>فرق محترفة · معدّات حديثة · حجز موعد فوري بتأكيد لحظي.</p></div></div>
      <h2>باقات التنظيف</h2>
    ...
    <section id="view-book" class="view hidden">
      ...
      <div class="login-card" style="margin:0 auto;max-width:440px">
        ...
        <label>تاريخ الزيارة</label><input id="bkDate" type="date">
        <label>وقت الزيارة</label><input id="bkTime" type="time">
        <div class="panel" id="bookSummary"></div>
        <label>اسم العميل</label><input id="bkName" placeholder="الاسم الكامل">
        <label>العنوان</label><input id="bkAddress" placeholder="الحي/الشارع">
    ...
    <section id="view-admin" class="view hidden">
      ...
      <div class="panel form-row">
        ...
        <input id="pkDuration" type="number" placeholder="المدة (دقيقة)" min="30">
...
`;
    ...
    const APP_JS = `/* 🧹 موقع خدمات jaola لتنظيف المنازل — jaola-cleaning */
const SEED_PACKAGES = [
  { id: 'pk1', name: 'تنظيف شقة', price: 150, duration: 120, desc: 'تنظيف شامل لشقة حتى ٣ غرف' },
  { id: 'pk2', name: 'تنظيف فيلا', price: 350, duration: 240, desc: 'تنظيف شامل لفيلا بجميع الطوابق' },
  { id: 'pk3', name: 'تنظيف عميق', price: 500, duration: 300, desc: 'تنظيف عميق يشمل السجاد والمطبخ والحمامات' }
];
function todayStr() { return new Date().toISOString().slice(0, 10); }
function toMinutes(hhmm) { var p = (hhmm || '00:00').split(':'); return (parseInt(p[0], 10) || 0) * 60 + (parseInt(p[1], 10) || 0); }

function load(k, fb) { try { var v = localStorage.getItem('jclean_' + k); return v ? JSON.parse(v) : fb; } catch (e) { return fb; } }
function save(k, val) { try { localStorage.setItem('jclean_' + k, JSON.stringify(val)); } catch (e) {} }
let packages = load('packages', SEED_PACKAGES);
let reservations = load('reservations', []); // { id, no, pkgId, customer, address, phone, date, time, price, createdAt }
let settings = load('settings', { name: 'تنظيف jaola', pass: 'admin', currency: 'ر.س', resSeq: 1 });
...
function overlapsSlot(dur, aStart, bStart, bDur) { var aEnd = aStart + dur; var bEnd = bStart + (bDur || 120); return aStart < bEnd && bStart < aEnd; }
function isSlotTaken(date, start, dur) {
  var n = 0;
  for (var i = 0; i < reservations.length; i++) {
    var r = reservations[i]; if (r.date !== date) continue;
    var rPkg = pkgById(r.pkgId); var rDur = rPkg ? rPkg.duration : 120;
    if (overlapsSlot(dur, start, toMinutes(r.time), rDur)) n++;
  }
  return n >= 2; // فريقان يعملان بالتوازي كحد أقصى
}
...
function updateBookSummary() {
  ...
  if (!date || !time) { byId('bookSummary').innerHTML = '<span class="hint">اختر تاريخ ووقت الزيارة.</span>'; return; }
  if (isSlotTaken(date, toMinutes(time), p.duration)) { errEl.textContent = 'كل الفرق مشغولة في هذا الوقت — اختر وقتاً آخر.'; show(errEl, true); byId('bookSummary').innerHTML = ''; return; }
...
function confirmBooking() {
  ...
  if (!date || !time) { errEl.textContent = 'اختر تاريخ ووقت الزيارة.'; show(errEl, true); return; }
  if (isSlotTaken(date, toMinutes(time), p.duration)) { errEl.textContent = 'كل الفرق مشغولة في هذا الوقت — اختر وقتاً آخر.'; show(errEl, true); return; }
  var name = byId('bkName').value.trim(); var address = byId('bkAddress').value.trim(); var phone = byId('bkPhone').value.trim();
  if (!name || !address || !phone) { errEl.textContent = 'اكتب الاسم والعنوان والهاتف.'; show(errEl, true); return; }
  show(errEl, false);
  var res = { id: uid('res'), no: settings.resSeq++, pkgId: p.id, customer: name, address: address, phone: phone, date: date, time: time, price: p.price, createdAt: new Date().toISOString() };
  reservations.push(res); save('reservations', reservations); save('settings', settings);
  byId('bkName').value = ''; byId('bkAddress').value = ''; byId('bkPhone').value = '';
...
function printConfirmation(id) {
  ...
  byId('printArea').innerHTML = '<div class="receipt"><h2>' + esc(settings.name) + '</h2>' +
    ...
    '<div class="r-row"><span>العنوان</span><span>' + esc(r.address) + '</span></div>' +
    ...
    '<p style="text-align:center">منزل نظيف ينتظرك 🧹</p></div>';
  window.print();
}
...
function renderReservations() {
  byId('reservationsList').innerHTML = reservations.length ? reservations.slice().reverse().map(function (r) {
    ...
    return '<div class="panel"><div class="tk-head" style="display:flex;justify-content:space-between"><b>🧹 ' + esc(p ? p.name : '؟') + '</b><span>#' + r.no + '</span></div>' +
      '<div class="hint" style="line-height:1.9">' + esc(r.customer) + ' · ' + r.date + ' ' + r.time + '<br>' + esc(r.address) + ' · الإجمالي: ' + money(r.price) + '</div>' +
...
function addPackage() {
  ...
  packages.push({ id: uid('pk'), name: name, price: Math.max(0, parseFloat(byId('pkPrice').value) || 0), duration: Math.max(30, parseInt(byId('pkDuration').value, 10) || 120), desc: '' });
...
`;
    ...
    const EXTRA_CSS = `
.hero{border-radius:16px;background:linear-gradient(120deg,#0f766e,#155e75);padding:44px 26px;margin-bottom:22px}
...
`;
    ...
    return {
        ...
        name: 'موقع خدمات تنظيف منازل',
        nameEn: 'Home Cleaning Service',
        description: 'موقع خدمات تنظيف منازل للزوّار: باقات تنظيف (شقة/فيلا/تنظيف عميق) بسعر ثابت، اختيار تاريخ ووقت الزيارة مع منع ازدحام الفرق، تأكيد حجز قابل للطباعة، ولوحة إدارة للباقات والحجوزات.',
        descriptionEn: 'Visitor-facing home cleaning service site: cleaning packages (apartment/villa/deep clean) with flat rates, date and time slot picking with team-overbooking prevention, a printable reservation confirmation, and an admin panel for packages and reservations.',
        keywords: ['تنظيف منازل', 'خدمات تنظيف', 'تنظيف شقق', 'تنظيف فلل', 'تنظيف عميق', 'شركة تنظيف', 'حجز موعد تنظيف', 'عاملات نظافة', 'home cleaning', 'cleaning service', 'house cleaning', 'maid service', 'deep cleaning booking'],
        model: {
            roles: [{ name: 'عميل' }, { name: 'إدارة' }],
            entities: [{ name: 'باقة تنظيف' }, { name: 'حجز' }],
            flows: [{ name: 'تصفّح الباقات واختيار تاريخ ووقت الزيارة' }, { name: 'منع ازدحام الفرق في نفس الوقت' }, { name: 'تأكيد الحجز وطباعته' }, { name: 'إدارة الباقات والحجوزات' }],
        },
        files: [
