...
// ألفاظُ المنتج وحدَها — لا ألفاظ العميل. كانت «شركة/company/corporate/وكالة/agency/startup/ستارت اب» هنا (نسخةٌ ثانية من
// قائمةٍ أصلحها `appBlueprint` بالمبدأ نفسِه: «شركة تصف الطالبَ لا المطلوب»)، فقِيس: «متجر إلكتروني لشركة ملابس» و«تطبيق حجز
// مواعيد لشركة صيانة» و«لوحة تحكم مبيعات لشركة عقارات» — سبعةٌ من تسعة أهدافٍ تطبيقيّة — كانت تخرج صفحةَ هبوطٍ بحكم PASS.
const MARKETING_HINTS = [
    'landing', 'هبوط', 'لاندنج', 'لاندنق', 'بروشور', 'brochure', 'تعريفي', 'تعريفية',
    'بورتفوليو', 'portfolio', 'معرض اعمال', 'معرض أعمال',
    'صفحة تسويق', 'marketing page', 'one page', 'ونبيج', 'onepage',
    'صفحة هبوط', 'موقع تعريفي', 'coming soon', 'قريبا',
];

/**
 * هل الهدف صفحة تسويقيّة/تعريفيّة ثابتة → إعادة تركيب من الـ Registry؟
 * الفهمُ قبل الكلمات: حكمُ المخطّط يُقرأ بوجهَيه — بروشور/هبوط → نعم، تطبيق/أداة → لا (ولو ذُكر لفظُ هبوطٍ عابر)؛
 * وبلا مخطّطٍ تحكم ألفاظُ المنتج وحدَها. كان يُقرأ بوجهٍ واحد (بروشور → نعم) فتُرجَّح الكلماتُ على الفهم.
 */
export function isMarketingPageGoal(goal = '', blueprint = null) {
    const k = blueprint?.kind;
    if (k === 'landing' || k === 'brochure') return true;
    if (k === 'webapp' || k === 'tool') return false;
