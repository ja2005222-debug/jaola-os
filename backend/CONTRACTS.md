...
## JCR/25 — «معالجا الخطّة والنمط يخرجان — وما بقي من المعالجات يحمل حالةً»

الاستخراجُ السادسَ عشر. **لا تغييرَ في السلوك** — نقلٌ حرفيّ لـ`_handlePlanningStage` (٧٥ سطراً) و`_handleModifyPattern` (٣٣) إلى
`agents/stages/intentHandlers.js#handlePlanningStage/handleModifyPattern(req, agents, reporter, ops) → boolean`.

### القياس والقرار

الاثنان بلا حالةٍ على النسخة: `reporter.send` ٥+٢، `emitLiveLog` ١، وطريقةُ صنفٍ واحدة لكلٍّ (`executeMission` / `surgicalEdit` —
تستبدلهما `scenario()` على النسخة) → شقٌّ من دالّةٍ واحدة لكلِّ معالج، كما في JCR/22–24. لا `io`. ثلاثُ يتيماتٍ من ثلاثة أسطرٍ مشتركة
(`initFromClarifier`، `recordProject`، `normalizeArabic`)؛ السكربتُ يؤكّد بقاءَ مستهلكٍ لثلاثةَ عشرَ اسماً مشتركاً آخر. جُمعا في ملفٍّ واحد
لأنّ عقدَهما واحد (`(req, agents, reporter, ops) → boolean`) ولأنّ الباقين من عائلتهم سيلحقون به حين يُكتب قرارُ الحالة.

### ما تغيّر

- `jcr.js` **١٥٨٧ ← ١٤٩٢**. أرضيّةُ `reporter.send` **٤٠ ← ٣٠** (قِيس ٣٦). `## C)` **١٣٤ ← ١٣٥**. `this.io` يبقى ١.

### الاختبار الجديد (`tests/intentHandlersStage.test.mjs`، ٧)

`jcrRuntimeFlows` يوصّف المسارَين عبر `handleUserMessage` ويبقى كما هو. الجديدُ يوصّف ما لم يكن موصَّفاً والشقَّ:
- الخطّة + تأكيد: `ops.executeMission(finalGoal, ctx)` بسياقِ الطلب المجمَّد ووكلائه بالهويّة؛ **الذاكرةُ تُهيَّأ** من المُوضِّح (الأقسامُ والميزاتُ
  ومزاجُ اللون) و**المشروعُ يُسجَّل** في ملفّ المستخدم بنوعه — لم يكن أيٌّ منهما مقيساً.
- تأكيدٌ بلا خطّةٍ في الحالة: التنفيذُ يمضي بلا تهيئةٍ ولا تسجيل؛ خارجَ مرحلة الخطّة → `false` بلا بثّ.
- سؤالٌ بالإنجليزيّة → ملخّصٌ إنجليزيٌّ بمحتوىً عربيّ (الأقسامُ كما أُدخلت)؛ ولا خطّةَ → «لم تُبنَ خطة بعد».
- النمط: «أضف …» **بالهمزة** يُلتقط عبر التطبيع (النمطُ مكتوبٌ بلا همزات) → سطرُ النيّة ثمّ `ops.surgicalEdit` بالرسالة الخام وسياقٍ مجمَّد؛ بلا نمط → `false`.
- النمطُ داخل مرحلة الخطّة: اللونُ سؤالٌ **بلا تسجيل** (بخلاف مرحلة الخطّة نفسِها التي تسجّله)، وغيرُه يُسجَّل — ولا تعديلَ جراحيّاً.
- التكافؤ مع المفوِّضَين بثّاً ونداءً، والاستبدالاتُ على النسخة تصل.
- الحدود: لا `this`، لا `io`، `ops` بنداءٍ واحدٍ لكلٍّ ولا ثالث، المفوِّضان بنصّهما، اليتيماتُ الثلاث غائبةٌ والسبعُ المفحوصةُ باقية.

### الطفرات (١٥، ضدّ `jcrRuntimeFlows` + `intentHandlersStage`)

**١٥/١٥ أُمسكت** من الجولة الأولى — بينها تهيئةُ الذاكرة وتسجيلُ المشروع (اللذان لم يكونا مقيسَين قبل هذا الاختبار)، و«الالوان» بلا همزة،
والتنفيذُ بسياقٍ عارٍ أو بالرسالة بدل الهدف النهائيّ، والنمطُ بلا تطبيع، والنمطُ في مرحلة الخطّة يُنفَّذ جراحيّاً.
درسُ أداة: موضعا `state.answers.push(\`edit: …\`)` متطابقان نصّاً وبالمسافة نفسِها في المعالجَين، فأُضيف للمُطفِّر استهدافٌ **بترتيب الظهور**
(وسيطٌ رابع) بدل شرط التفرّد — والطفرتان على الموضعَين أُمسكتا كلٌّ باختبارها.

### ما بقي من المعالجات — قرارٌ مكتوب

`_handleBareConfirmations` (٦٧)، `_handleUnifiedRoute` (٧٥)، `_handleClassifiedIntent` (١٢١) و`handleUserMessage` (١٥٦) تقرأ وتكتب
`this.gatedMessages` (خريطةُ الرسائل المحجوبة بانتظار الإصرار) و`gateConfirmReply`/`trackByRoom`. هذه **حالةٌ مشتركةٌ بين المعالجات**
عبر الرسائل، لا حالةَ معالجٍ واحد — فلا تُنقل بشقِّ دوالّ، بل بكائنِ حالةٍ محقَن (`gate = { messages: Map, confirmReply }`) يملكه الصنفُ
ويُمرَّر. القرارُ: **JCR/26 يقيس `gatedMessages`** (من يكتب، من يقرأ، متى تُمسح) ويكتب عقدَ الكائن قبل أيِّ نقل — وإلّا بقيت الأربعُ على الصنف.
...
## JCR/27 — «الموجّهُ الموحَّد يخرج — أوّلُ كاتبٍ للحاجز خارجَ الصنف، والموجّهُ يُحقَن على سابقة `router.js`»

الاستخراجُ الثامنَ عشر. **لا تغييرَ في السلوك** — نقلٌ حرفيّ لـ`_handleUnifiedRoute` (٧٤ سطراً) إلى
`agents/stages/intentHandlers.js#handleUnifiedRoute(req, agents, reporter, gate, ops, router = routeMessage) → boolean`.

### القياس والقرار

`this` ×١٢: `reporter.send` ٣، `emitLiveLog` ١، الحاجزُ `has`/`delete`/`set`/`gateConfirmReply` ×١ لكلٍّ (**أوّلُ معالجٍ يكتب الحاجز** — عبر
`gate.set` و`gate.confirmReply` من شقّ JCR/26 نفسِه)، `surgicalEdit` ٢ و`generateChatResponse` ١ (يستبدلهما `scenario()` على النسخة → `ops`)،
و`readCurrentCodeContextAsync` ١ — **لا اختبارَ يستبدل مفوِّضَه** فيُستورد `readCodeContext` مباشرةً (كما استُورد `readProjectFiles` في JCR/22):
الشقُّ لِما تستبدله الاختبارات، والاستيرادُ لِما لا تستبدله. يتيمتان أُزيلتا (`routeMessage` سطرٌ كامل، `recordEditAction` من سطر `metricsStore`).

**قرارُ حقن الموجّه (مكتوب)**: الجولةُ الأولى من الطفرات (١٠) أمسكت **٢ فقط** — فروعُ `route.action` كلُّها (chat/edit/delete_project/stop)
غيرُ قابلةٍ للوصول في بيئة الاختبار لأنّ `routeMessage` نداءُ LLM يعود `null` بلا مفتاح. البديلان: (أ) تجريدٌ جديد (`RouterPort`/حقنُ الوحدة) —
مرفوضٌ بلا مستهلكٍ حقيقيّ؛ (ب) **وسيطٌ أخير افتراضيُّه الموجّهُ الحقيقيّ** — السابقةُ قائمةٌ في المستودع نفسِه: `router.js#routeMessage(message, ctx, llm = smartChat)`
موصوفٌ هناك «قابلٌ للحقن للاختبار». اختير (ب): لا واجهةَ جديدة، المفوِّضُ في `jcr` **لا يمرّر شيئاً** (الافتراضيُّ يعمل)، والاختبارُ يمرّر
دالّةً تعيد قراراً جاهزاً وتسجّل السياقَ الذي تلقّته. فصار كلُّ فرعٍ موصَّفاً على المصدر لا على LLM.

### ما تغيّر

- `jcr.js` **١٤٤٥ ← ١٣٧٧**. أرضيّةُ `reporter.send` **٣٠** تبقى (قِيس ٣٠ بالضبط — الاستخراجُ التالي يعيدها). `this.gatedMessages` في `jcr`:
  ١٤ ← ١١ (البانيةُ + `_gate()` الخمس + `_handleClassifiedIntent` الستّ).
- حارسا الحدود السابقان أُعيد تثبيتُهما بالقياس: `intentHandlersStage` سجلُّ الملفّ (ops ٢/٥/١ = ٨، send ١٣، liveLog ٥)؛ و`bareConfirmationsStage`
  صار يعدّ **شريحةَ جسده** لا الملفَّ كلَّه — فلا يتذبذب مع كلِّ معالجٍ يلحق بالملفّ (المنهجُ الذي بدأه `unifiedRouteStage`).

### الاختبار الجديد (`tests/unifiedRouteStage.test.mjs`، ٨)

الموجّهُ يُحقَن بقرارٍ جاهز، والسياقُ الذي يتلقّاه يُسجَّل؛ الحاجزُ خريطةٌ مع سجلِّ نداءات، و`ops` مسجِّلان. ما يُوصَّف على المصدر:
- بابُ الدخول: داخلَ حوار المُوضِّح لا يُستشار الموجّهُ أصلاً (صفرُ نداءاتٍ عليه) — `false` بلا بثّ.
- سياقُ الموجّه: اسمُ المشروع، `hasProject` من القرص (مشروعٌ فارغ → `false`)، آخرُ ردٍّ للمساعد، اللغة. `null` أو رميٌ → `false` بلا أثر؛
  `build` → `false` **بعد سطر الموجّه** (`[ROUTER] ➔ [Unified]: 🧭 build (90%)`) — سلوكٌ قائم قبل النقل ثُبِّت كما هو.
- `chat` على مشروعٍ قائم: أمرٌ صريح → تعديلٌ فوريّ (`gate`: has, delete؛ سياقٌ مجمَّد؛ `totalEdits` في الملفّ الشخصيّ واللوحة)؛ جملةٌ إخباريّة
  → تُحجَب مرّةً (`gate`: has, set, confirmReply — بلغة المستخدم)؛ الإصرارُ بعدها → تعديل. السؤالُ لا يُحجَب → محادثة؛ بلا مشروعٍ كلُّ `chat` محادثة.
- `edit` → `ops.surgicalEdit` بتعليمة الموجّه أو بالرسالة حين لا تعليمة، ويُسجَّل. `delete_project` → طلبُ تأكيدٍ حرفيّ باسم المشروع؛ `sandbox_app`
  لا يُحذف. `stop` → مسحُ حالة المُوضِّح **والحوار** (`getPendingGoal` → `null`) وردٌّ بلغته (ar/en).
- التكافؤ مع المفوِّض: كلاهما `false` بلا أثر بلا مفتاح؛ والمفوِّضُ **لا يمرّر موجّهاً** (نصّه مثبَّت).
- الحدود **على شريحة الجسد**: `gate.has/delete/set/confirmReply` مرّةً لكلٍّ ولا خامسة، `ops.surgicalEdit` ×٢ و`generateChatResponse` ×١،
  `readCodeContext` يُستورد، `await router(` مرّةً و`routeMessage(` صفراً في الجسد، المفوِّضُ بنصّه، اليتيمتان غائبتان.

### الطفرات (١٦، ضدّ `jcrClassifiedIntent` + `jcrRuntimeFlows` + `unifiedRouteStage`)

**١٦/١٦ أُمسكت** (بعد الحقن؛ كانت ٢/١٠ قبله):
بابُ الدخول يُهمَل · الموجّهُ لا يُستشار · بلا سياق المشروع · سطرُ الموجّه لا يُبثّ · chat: الإصرارُ يُهمَل · الحاجزُ لا يُمسح عند التنفيذ ·
التعديلُ لا يُسجَّل في اللوحة · المحادثةُ بلا لغة · الجملةُ الإخباريّة لا تُحجَب · ردُّ الحجب ليس من الصنف · السؤالُ يُحجَب أيضاً ·
edit: التعليمةُ تُهمَل · delete_project: sandbox_app يُحذف · stop: حالةُ المُوضِّح لا تُمسح · stop: الحوارُ لا يُمسح · فشلُ الموجّه يرمي.

طفرتان كشفتا ثغرتَي توصيف أُغلقتا قبل الاعتماد: «الحوارُ لا يُمسح» نجت لأنّ اختبارَ `stop` ادّعى مسحَ الحوار في عنوانه ولم يفحصه
(أُضيف `setPendingGoal` → `getPendingGoal === null`)؛ و«حالةُ المُوضِّح» لم تُطبَّق لأنّ المرساةَ فقدت أربعَ مسافاتٍ بالنقل ثمّ تكرّرت في الملفّ
(استُهدفت الثانية). وافتراضٌ خاطئ في الاختبار («build بلا أثر») صحّحه المصدر: سطرُ الموجّه يُبثّ لكلِّ قرارٍ غير `null`.

### الأثر

- `jcr.js` **١٤٤٥ ← ١٣٧٧** (٣٢٥٢ عند JCR/4 — خرج ٥٨٪). الاختبارات **١٦٠٠ ← ١٦٠٨**.
- بقي من المعالجات `_handleClassifiedIntent` (١٢٠؛ آخرُ طريقةٍ في الصنف) — يحتاج `ops.classifyIntent` (تستبدله `jcrClassifiedIntent`) مع
  `gate` والشقّ نفسِه → JCR/28. بعدها `handleUserMessage` (١٥٦) والنواة.


---

## JCR/28 — «المصنِّفُ الأخير يخرج — لم يبقَ في الصنف معالجُ نيّةٍ ولا قارئٌ للحاجز»

الاستخراجُ التاسعَ عشر. **لا تغييرَ في السلوك** — نقلٌ حرفيّ لـ`_handleClassifiedIntent` (١٢٠ سطراً؛ آخرُ طريقةٍ في الصنف) إلى
`agents/stages/intentHandlers.js#handleClassifiedIntent(req, agents, reporter, gate, ops) → true`.

### القياس والقرار

`this` ×٢٧: `emitLiveLog` ٩، `gatedMessages` ٦ (`has`/`set`/`delete` ×٢ لكلٍّ)، `reporter.send` ٤، `surgicalEdit` ٣، `generateChatResponse` ٣،
`readCurrentCodeContextAsync` ٢، `gateConfirmReply` ٢، `classifyIntent` ١. الشقُّ على القاعدة نفسِها: ما تستبدله الاختباراتُ على النسخة
(`classifyIntent` في `jcrClassifiedIntent`، و`surgicalEdit`/`generateChatResponse` في `scenario()`) يُمرَّر في `ops`؛ وما لا تستبدله
(`readCurrentCodeContextAsync`) يُستورد مباشرةً (`readCodeContext`). الحاجزُ عبر `gate` من JCR/26 — **هذا آخرُ قارئٍ له في الصنف**:
`this.gatedMessages` في `jcr` **١١ ← ٥** (البانيةُ + أربعُ دوالّ `_gate()`). خمسُ يتائم أُزيلت من أربعة أسطرٍ مشتركة
(`contextFromRequest`, `hasActionIntent`, `isQuestionMessage`, `recordEdit`, `setPendingGoal`)؛ `intentHandlers.js` لم يحتج إلّا `setPendingGoal`.

### ما تغيّر

- `jcr.js` **١٣٧٧ ← ١٢٦٥**. `handleUserMessage` صار سلسلةَ **سبعة مفوِّضات** كلُّها إلى `stages/*` — لا معالجَ نيّةٍ بجسدٍ في الصنف.
- أرضيّةُ `reporter.send` **٣٠ ← ٢٦** (قِيست ٢٦ بالضبط).
- أربعةُ حرّاسِ حدودٍ أُعيد تثبيتُها بالقياس لأنّ قوائمَ «بقي له مستهلك» فيها سمّت ما صار يتيماً: `unifiedRouteStage` (`hasActionIntent`,
  `isQuestionMessage`)، `bareConfirmationsStage` (`recordEdit`)، `ceoIntentStage` (`contextFromRequest`)، `intentHandlersStage` (كلاهما + سجلُّ الملفّ:
  ops ٢/٨/٤/١ = ١٥، send ١٧، liveLog ١٤). سطرُ الاستيراد في حارسَين صار مطابقةً بالاسم لا بالنصّ الكامل — كي لا يتذبذب مع كلِّ معالجٍ يلحق.

### الاختبار الجديد (`tests/classifiedIntentStage.test.mjs`، ٨)

ما لا يُرى إلّا على الشقّ (السلوكُ عبر المفوِّض في `jcrClassifiedIntent` كما هو، ١٢):
- المصنِّف: يُستدعى **مرّةً دائماً** بالرسالة المطبَّعة واسم المستخدم؛ ثقةُ المعنى ≥ 75 تطغى على نتيجته، ودونها تُعتمد — وسطرُ «نية:» يقول أيَّهما.
- build غيرُ صريح: فعلٌ → `ops.surgicalEdit` بسياقٍ مجمَّد ويُسجَّل في الملفّ الشخصيّ؛ سؤالٌ بفعلٍ أو وصفٌ → محادثة **بوسائطها الأربعة**؛ الحاجزُ لا يُلمس؛
  وعلى مشروعٍ فارغ → سؤالُ التأكيد. build صريح: حوارُ التوضيح بلا هدفٍ معلّق؛ وإلّا سؤالُ التأكيد **بالإنجليزيّة** نصّاً وخياراتٍ يعلّق الهدف — ولا تعديلَ
  حتّى على مشروعٍ قائم.
- modify: سؤالٌ (`gate`: has)، جملةٌ (has, set, confirmReply + ردُّ الصنف)، إصرارٌ (has, delete + تعديلٌ يُسجَّل)، فعلٌ (has فقط).
- stop: مسحُ حالة المُوضِّح **والحوار** (`getPendingGoal` → `null`)، سطرُ سجلّ، لا بثَّ ولا `ops` ولا `gate`.
- وإلّا: فعلٌ (has, delete)، جملةٌ (has, set, confirmReply)، إصرارٌ (has, delete)، سؤالٌ و**كلامٌ قصير** (< ٤ أحرف) → محادثة بلا حجب، بلا مشروعٍ → محادثة.
- التكافؤ مع المفوِّض على `rt.gatedMessages` (حجبٌ ثمّ إصرارٌ يمسح)، والحدودُ على شريحة الجسد: `gate` ٢/٢/٢/٢ ولا تاسعة، `ops` ١/٣/٣ = ٧، `readCodeContext` ٢،
  send ٤، liveLog ٩، المفوِّضُ بنصّه، اليتائمُ الخمس غائبة، و`this.gatedMessages` في `jcr` = ٥.

### الطفرات (٢٤، ضدّ `jcrClassifiedIntent` + `jcrRuntimeFlows` + `classifiedIntentStage`)

**٢٤/٢٤ أُمسكت** من الجولة الأولى (المراسي المكرّرة — `recordEdit` ×٥، `generateChatResponse` ×٤، ردُّ الحجب ×٣، `clearState` ×٣، `clearDialog` ×٢ —
استُهدفت بالترتيب بعد عدِّها على الملفّ):
ثقةُ المعنى لا تطغى · المصنِّفُ لا يُستشار · سطرُ النيّة بلا نيّة · build: الصريحُ يُعامَل غيرَ صريح · المشروعُ الفارغ يُعامَل قائماً · السؤالُ بفعلٍ يُعدِّل ·
التعديلُ لا يُسجَّل · المحادثةُ بلا لغة · حوارُ التوضيح لا يُستشار · فعلُ البناء لا يُحذف من التلميح · الهدفُ لا يُعلَّق · modify: الإصرارُ يُهمَل ·
السؤالُ يُحجَب · ردُّ الحجب ليس من الصنف · الحاجزُ لا يُمسح عند الإصرار · التعديلُ لا يُسجَّل · stop: حالةُ المُوضِّح لا تُمسح · الحوارُ لا يُمسح ·
وإلّا: الكلامُ القصير يُحجَب · بلا مشروعٍ يُعامَل قائماً · الجملةُ لا تُحجَب · ردُّ الحجب ليس من الصنف · المحادثةُ بلا لغة · التعديلُ لا يُسجَّل.

ثلاثٌ منها ما كانت لتُمسَك بالاختبارات القديمة وحدها (كشفها القياسُ قبل الكتابة فأُغلقت في الاختبار الجديد): «الصريحُ يُعامَل غيرَ صريح» (اختبارُ
البناء الصريح القديم على مشروعٍ فارغ فلا يفرّق)، «المشروعُ الفارغ يُعامَل قائماً»، و«الكلامُ القصير يُحجَب».

### الأثر

- `jcr.js` **١٣٧٧ ← ١٢٦٥** (٣٢٥٢ عند JCR/4 — خرج ٦١٪). الاختبارات **١٦٠٨ ← ١٦١٦**.
- انتهت معالجاتُ النيّة. ما بقي في الصنف كبيراً: `handleUserMessage` (١٥٦؛ يبني `req` ويُسلسل المفوِّضات)، `generateChatResponse` (١٣٧)،
  `_runMissionNow` (١١٥)، `_selectBuildStrategy` (١٠٩)، `buildMissionAndMeta` (٥٠). التالي يُقاس قبل أن يُقرَّر.


---

## JCR/29 — «مختارُ الاستراتيجيّة يخرج — تلميحُ المسار دالّةً مربوطة على سابقة الحاجز»

الاستخراجُ العشرون. **لا تغييرَ في السلوك** — نقلٌ حرفيّ لـ`_selectBuildStrategy` (١٠٧ أسطر؛ مستدعٍ واحد `_runMissionNow`) إلى
`agents/stages/selectBuildStrategy.js#selectBuildStrategy(goal, blueprint, ctx, reporter, ops) → result | null`.

### القياس والقرار

`this` ×١٢: `emitLiveLog` ٥، `reporter.send` ٢، البناةُ الثلاثة `_buildFromRegistry`/`_buildFromClone`/`_buildReactProject` ×١ لكلٍّ
(تستبدلها `jcrBuildStrategy` و`jcrMissionStrategy` على النسخة → `ops`)، `readCurrentCodeContextAsync` ١ (لا اختبارَ يستبدل مفوِّضَه → `readCodeContext`
يُستورد)، و`trackByRoom?.get(roomName)` ١ — خريطةٌ على النسخة يكتبها `handleUserMessage` وتُقرأ هنا مرّةً.

**قرارُ تلميح المسار (مكتوب)**: البدائل: (أ) تمريرُ الخريطة نفسِها — يفتح الكتابةَ لمرحلةٍ لا تكتب؛ (ب) قيمةُ `track` داخل `ctx` — `ExecutionContext`
مجمَّدٌ وحقولُه الستّة مقيسة (Sprint 2b)، ولا نوسّعه لقارئٍ واحد؛ (ج) **دالّةٌ مربوطة `ops.trackOf(room)`** على سابقة شقّ `gate` في JCR/26:
قراءةٌ فقط، لا كائنَ جديد، والمفوِّضُ يربطها بـ`this.trackByRoom?.get(room)`. اختير (ج). `this.trackByRoom` في `jcr`: الكاتبُ + الدالّةُ المربوطة = ٢.
ستُّ يتائم أُزيلت (`resolveStack`, `matchCloneTemplate`, `isMarketingPageGoal` أسطرٌ كاملة؛ `isExplicitRebuild`, `isExplicitNewBuild`, `isContinuationGoal`
من سطر `textNormalizer`). `resolveProjectType` تُستورد في المرحلة من مصدرها `stages/enrich.js` لا من واجهة `jcr` (حارسُ الحدود يثبّت ذلك)،
وإعادةُ تصديرها من `jcr` تبقى لمستورِديها (JCR/6).

### ما تغيّر

- `jcr.js` **١٢٦٥ ← ١١٦٣**. ملفٌّ جديد `stages/selectBuildStrategy.js` (١٣٤) → وحداتُ `agents/*` **١٣٥ ← ١٣٦**.
- أرضيّةُ `reporter.send` **٢٦ ← ٢٤** (قِيست ٢٤ بالضبط).
- لا حارسَ حدودٍ قديم سمّى اليتائمَ الستّ في قوائم «بقي له مستهلك» — لم يتذبذب شيءٌ غيرُ الأرضيّة والخريطة.

### الاختبار الجديد (`tests/selectBuildStrategyStage.test.mjs`، ٦)

قرارُ الاختيار موصَّفٌ عبر المفوِّض (`jcrBuildStrategy` ١١ + `jcrMissionStrategy` ١٣). هنا ما لا يُرى إلّا على الشقّ:
- `trackOf`: يُقرأ **مرّةً باسم الغرفة في فرع الكلون فقط** — لا في المسار التسويقيّ (يعود قبله) ولا عند الاستئناف على مشروعٍ قائم (لا مطابقةَ أصلاً)؛
  والتلميحُ `system` يُغيّر المطابقةَ فعلاً (يُقصي كلونات المواقع فلا يُطابَق التوصيل).
- رسالتا «يعمل» **بالإنجليزيّة نصّاً** (التسويقيّ والكلون) بلا بناء.
- وسائطُ البناة الثلاثة كاملةً: السياقُ المجمَّد نفسُه، الكلونُ بمعرّفه، و`sections` الافتراضيّة `[]`.
- حدُّ الجدّة: صفحةٌ أقصر من ٨٠ حرفاً (٣١ مع ترويسة القارئ) بناءٌ جديد → React للكبير؛ ونطاقُ الخطّة «كامل» في ذاكرة المشروع يرفع النوعَ الصغير إلى React.
- التكافؤ مع المفوِّض: `rt.trackByRoom` يصل المطابقةَ عبر المفوِّض، وغيابُ الخريطة لا يرمي، والشكلُ نفسُه على الشقّ.
- الحدود **على شريحة الجسد**: `ops` أربعةٌ مرّةً لكلٍّ ولا خامس، `readCodeContext` ١، send ٢، liveLog ٥، `transitionState` ٢، `analyzeProjectStatic` ٢،
  المفوِّضُ بنصّه، اليتائمُ الستّ غائبة، و`this.trackByRoom` في `jcr` = ٢.

### الطفرات (٢٤، ضدّ `jcrBuildStrategy` + `jcrMissionStrategy` + `selectBuildStrategyStage`)

**٢٤/٢٤ أُمسكت** بعد جولتين (٢٢/٢٤ في الأولى):
«بناءٌ جديد» لا يُحسب · الاستئنافُ يُهمَل · إعادةُ البناء الصريحة تُهمَل · تسويقيّ: شرطُ الجدّة يسقط · حارسُ «يعمل» يُتجاوَز · «يعمل» لا يُحسب · الرسالةُ بلا إنجليزيّة ·
لا COMPLETED · السجلُّ لا يُبنى · كلون: التلميحُ يُهمَل · الاستئنافُ يُطابَق · يُطبَّق دائماً · المعطّلُ لا يُصلَح · لا يُبنى أبداً · حارسُ «يعمل» يُتجاوَز ·
الرسالةُ بلا إنجليزيّة · لا COMPLETED · البناءُ يُهمَل · هجين: النوعُ ثابت · النطاقُ يُهمَل · React على مشروعٍ قائم · الأقسامُ تُهمَل · سطرُ Vanilla لا يُبثّ · الاحتياطُ ليس null.

الناجيتان في الجولة الأولى كشفتا فجوتَي توصيف أُغلقتا: «بناءٌ جديد لا يُحسب» كانت مكافئةً على المجلّد الفارغ لأنّ القراءةَ الفارغة تسبق عتبةَ الطول
(أُضيفت صفحةٌ قصيرة غيرُ فارغة)، و«النطاقُ يُهمَل» لأنّ لا اختبارَ كان يضع `plan.scope` في ذاكرة المشروع (أُضيف عبر المرجع الحيّ للذاكرة).

### الأثر

- `jcr.js` **١٢٦٥ ← ١١٦٣** (٣٢٥٢ عند JCR/4 — خرج ٦٤٪). الاختبارات **١٦١٦ ← ١٦٢٢**.
- بقي في الصنف كبيراً: `handleUserMessage` (١٥٦)، `generateChatResponse` (١٣٧)، `_runMissionNow` (١١٢)، `buildMissionAndMeta` (٤٦). التالي يُقاس قبل أن يُقرَّر.

## JCR/30 — «مرحلةُ المهمّة والوعي الذاتيّ تخرج — وصنفُ الميزانية معها، والعميلُ يُحقَن على سابقة الموجّه»

الاستخراجُ الحادي والعشرون. **لا تغييرَ في السلوك** — نقلٌ حرفيّ لـ`buildMissionAndMeta` (٤٦ سطراً؛ مستدعٍ واحد `_runMissionNow` + اختباراتُ التوصيف)
و`_noteUnknowns` (١٠) وصنفِ `CognitiveBudget` (٢٠) إلى `agents/stages/missionMeta.js#runMissionMeta(context, roomName, reporter, ops, client = groq)`
و`#noteUnknowns(context, roomName, reporter)`.

### القياس والقرار

`this` ×٧: `emitLiveLog` ٣ + ٢ (→ `reporter.liveLog`)، `loadExecutiveMemory` ١ (تقرأ `executiveMemoryPath` على النسخة → `ops`)، `_noteUnknowns` ١ (تخرج معها
فتُستدعى مباشرةً). `CognitiveBudget`: ثلاثُ إنشاءاتٍ كلُّها داخل المرحلة، وبقيّةُ النظام (`debate`/`verify`/`jcrMissionStrategy`) تستهلك **النسخة** عبر
`context.budget` لا الصنف → يخرج مع مستهلكه الوحيد، بلا تصدير («لا تجريد بلا مستهلك»).

**قرارُ العميل (مكتوب)**: المرحلةُ تستدعي `groq.chat.completions.create` مباشرةً؛ في بيئة الاختبار `groq === null` فلا يُرى إلّا مسارُ الاحتياط.
على سابقة `handleUnifiedRoute(…, router = routeMessage)` في JCR/27: معاملٌ افتراضيّ `client = groq` — الإنتاجُ لا يمرّره، والاختبارُ يحقن عميلاً
مسجِّلاً بلا شبكة. الاسمُ الوحيد الذي تغيّر داخل الجسد هو `groq` → `client` (موضعان: الحارسُ والاستدعاء).

### ما تغيّر

- `jcr.js` **١١٦٣ ← ١٠٩٧**. ملفٌّ جديد `stages/missionMeta.js` (٩٣) → وحداتُ `agents/*` **١٣٦ ← ١٣٧**.
- لا يتيمَ في `jcr` غيرَ الصنف (خرج)؛ `groq` تبقى مستوردةً لمستهلكَيها `classifyIntent` و`generateChatResponse` (٣ مواضع مع الاستيراد — مقيسة).
- أرضيّةُ `reporter.send` ٢٤ كما هي (المرحلةُ تبثّ عبر `liveLog` فقط). لا حارسَ حدودٍ تذبذب غيرَ الخريطة.

### الاختبار الجديد (`tests/missionMetaStage.test.mjs`، ٧)

مسارُ الاحتياط والمجاهيلُ موصَّفان عبر المفوِّض في `jcrRuntime` (٣). هنا ما لم يكن يُرى قبل الشقّ — **مسارُ النجاح** بعميلٍ محقون:
- المهمّةُ تُنسخ إلى النموذج الذهنيّ (`uxGoal` → `visualIdentity`؛ غيرُ المصفوفة → `[]`)، الثقةُ `0.9` → `90`، Critical/High → ميزانية `complex` (١٥)
  باستدعاءٍ واحدٍ محسوب، والميزانيةُ الجديدة تُحصي وترفض عند الاستنفاد؛ التفضيلاتُ تُقرأ مرّةً عبر `ops`؛ الطلبُ بشكله (النموذج، `json_object`،
  رسالتان، التفضيلاتُ والهدفُ نصّاً).
- ثقةٌ منخفضة مع مجاهيل → توضيحٌ مطلوب والمجاهيلُ تُبثّ مرقَّمة؛ أولويّةٌ خارج القائمة → Medium (٧).
- **اكتشافاتٌ مثبَّتة كما هي**: ثقة `0` تسقط إلى ٧٠ (`|| 70` — لا «ثقة صفر»)؛ ثقة `1` تُقرأ نسبةً فتصير ١٠٠؛ عتبةُ التوضيح `< 45` حصراً
  (٤٤ نعم، ٤٥ لا)؛ مجاهيلُ بلا انخفاضٍ لا تُعرض؛ مجاهيلُ غيرُ مصفوفة → `[]`.
- الاحتياطُ الحتميّ على أربعة أسباب (رميُ العميل، JSON معطوب، ميزانيةٌ مستنفدة بلا استدعاء، عميلٌ غائب): هدفٌ عامّ، ثقة ٧٠، `medium` بلا استهلاك،
  والسببُ كما هو؛ والتفضيلاتُ تُقرأ قبل المحاولة حتى في الاحتياط.
- `noteUnknowns`: صامتةٌ بلا توضيحٍ أو بلا مجاهيل أو بمجاهيلَ بلا طلبِ توضيح أو بلا `metaReasoning` (لا ترمي)؛ سطران مرقَّمان على الغرفة عند وجودهما.
- التكافؤ مع المفوِّض: `rt.buildMissionAndMeta` يقرأ التفضيلات من النسخة ويسقط إلى الاحتياط بلا مزوّد؛ `rt._noteUnknowns` هي `noteUnknowns` على مُبلِّغ النسخة.
- الحدود على شريحتَي الجسد: liveLog ٣ + ٢، `ops` واحدٌ ولا ثانٍ، العميلُ مرّةً وحارسُه مرّةً، ثلاثُ ميزانيات، `noteUnknowns` مرّةً، `groq` في المرحلة
  استيرادٌ + افتراضيّ فقط، الصنفُ فيها بلا تصدير، لا `this` خارج الصنف ولا `io`، المفوِّضان بنصّهما، والصنفُ غادر `jcr` و`groq` بقيت لمستهلكَيها.

### الطفرات (٢٨، ضدّ `jcrRuntime` + `missionMetaStage`)

**٢٨/٢٨ أُمسكت** بعد جولتين (٢٤/٢٧ في الأولى + مرساةٌ معطوبة):
الميزانية: complex بلا زيادة · الاستهلاكُ لا يُحصى · المستنفدةُ تُقبل · التفضيلاتُ لا تُقرأ · الغائبةُ لا تُنشأ · حارسُ الميزانية يُتجاوز · حارسُ المزوّد يُتجاوز ·
نموذجٌ آخر · بلا JSON · الهدفُ لا يُرسل · uxGoal لا تُنسخ · المخاطرُ بلا حارس · الثقة ≤ ١ لا تُحوَّل · بلا افتراضيّ · المجاهيلُ بلا حارس · العتبة ≤ ٤٥ ·
المجاهيلُ لا تُعرض · الأولويّةُ بلا قائمة · High ليست complex · الاستدعاءُ غير محسوب · سطرُ الأولويّة لا يُبثّ · الاحتياط: الهدفُ لا يُعمَّم · ثقةٌ أخرى ·
ميزانية complex · السببُ لا يُقال · noteUnknowns: تعرض بلا توضيح · بلا ترقيم · تعيد false دائماً.

الناجياتُ الثلاث في الجولة الأولى كشفت فجوات توصيف أُغلقت: الصنفُ نفسُه لم يكن مرصوداً (الاستهلاكُ والاستنفادُ على الميزانية *الجديدة*)، و`noteUnknowns`
بمجاهيلَ بلا طلبِ توضيح لم تكن مغطّاة.

### الأثر

- `jcr.js` **١١٦٣ ← ١٠٩٧** (٣٢٥٢ عند JCR/4 — خرج ٦٦٪). الاختبارات **١٦٢٢ ← ١٦٢٩**.
- بقي في الصنف كبيراً: `handleUserMessage` (١٥٥)، `generateChatResponse` (١٣٧)، `_runMissionNow` (١١٢)، `_stageTemplate` (٣٩)، `runDynamicMultiAgentRuntime` (٣٨).
  التالي يُقاس قبل أن يُقرَّر.

## PM/1 — «الاختيارُ بالفهم لا بالكلمات — مطابقةُ الكلون نموذجاً بنموذج، والفهمُ يُبذَر من المراجع»

أوّلُ خطوةٍ في مسار «عقل المنتج» (`PRODUCT_MIND.md`) الموازي لتفكيك JCR. **تغييرُ سلوكٍ مقصودٌ ومكتوب**: مختارُ الكلون كان يقرّر
بكلمةٍ مفتاحيّة واحدة ويتجاهل نموذجَ الفهم ونماذجَ الكلونات، فصار طلبُ «نظام إدارة وتشغيل تاكسي» كلونَ ERP بعلامةِ تاكسي.

### القياس والجذر

- `matchCloneTemplate`: `bestKw ≥ 1 && score ≥ 2`؛ ERP يحمل «نظام إدارة» كلمةً → الطلبُ أعلاه → `jaola-erp` (تشغيلٌ مباشر). وبلا مسار
  سيستم: «تطبيق تاكسي متعدّد الواجهات» → `jaola-marketplace` (تعادلٌ ١٠ = ١٠ يحسمه ترتيبُ القائمة).
- كلُّ كلونٍ يحمل `model {roles, entities, flows}`، و`referenceBlueprints` تعرف `taxi_fleet` بأدواره — ولا أحد يقارن أو يبذر.
- نموذجُ الفهم بلا نموذجٍ لغويّ = `User/Item`: لا معنى يُقارَن به.

### ما تغيّر

- `projectModel.js` **٢٤٧ ← ٣٩١**: معجمُ مفاهيم ثنائيّ اللغة (~٧٠ مفهوماً: أدوارٌ وكيانات) + `normalizeConceptText` + `conceptOf(name)`
  («العميل (Passenger)» → `passenger`: ما بين القوسين هو المصطلحُ الأدقّ؛ «مدير أسطول» → `fleet_manager`: الأطولُ يغلب؛ المجهولُ يطابق نفسَه)
  + `modelAffinity(understood, candidate) → { roleCoverage, entityOverlap, missingRoles, sharedEntities, score, substantive }`.
  `User/Item/Employee` مفاهيمُ عامّة لا تُقيِّد.
- `cloneTemplates/index.js` **١٢٦ ← ١٧٦**: `matchCloneTemplateDetailed(…) → { clone, rejected, reason }` والغلافُ القديم `matchCloneTemplate` يعيد `.clone`
  (٣٩ موضعَ استدعاءٍ في الاختبارات لم تتغيّر). القواعد: **الفيتو** على كلونٍ لا يغطّي دوراً من أدوار الفهم إلّا إن سمّى المستخدمُ منتجَه صراحةً؛
  **عبارةُ المسار** (`isTrackPhrase` = `SYSTEM_INTENT_RE`) ليست تسميةَ منتج فلا ترفع الفيتو؛ **الترتيب** كما كان + قربُ النموذج (≤ ٨ فلا يغلب كلمةً = ١٠)؛
  **الفهمُ وحده** عند دورَين فأكثر كلُّها مغطّاة + كيانٍ مشترك + لا منافس؛ الكلونُ المختار يحمل `matchReason`.
- `referenceBlueprints.js` **١٩٥ ← ٢٠٩**: `referenceModel(bp)` — المرجعُ بصيغة نموذج المشروع (المصطلحُ الدقيق اسماً والعربيّةُ وصفاً، `_source: 'reference'`).
- `stages/understand.js` **٧٢ ← ٧٨**: `matchBlueprint(goal)` → `referenceModel` يُدمَج **أوّلاً** (كي لا تُسقطه سقوفُ التطبيع: ٤ أدوار/٦ كيانات)؛ السجلُّ يلحق «(مرجع: …)».
- `stages/selectBuildStrategy.js` **١٣٤ ← ١٤٨**: يستدعي المفصَّلة ويبثّ ثلاثةَ أسطر `ProductMind`: «🧠 استُبعد بالفهم: id (بلا passenger/driver)»،
  «🧠 اختيارٌ بالفهم: id — كلمات: …، الأدوار ١٠٠٪»، و«🧠 يعمل لكنّه لا يغطّي كلَّ الأدوار — …».
- **قرارٌ مكتوب — فجوةُ الأدوار ليست عطلاً**: على مشروعٍ قائمٍ *يعمل*، فشلُ `role-coverage` وحدَه لا يجعله «معطّلاً» يُدهَس بكلون؛ الفهمُ يُقال والقرارُ
  للمستخدم حتى يأتي الحكمُ (PM/2). كشفَه اختبارُ `jcrMissionStrategy` «لا نكلبره» حين صار للفهم أدوارٌ بلا نموذجٍ لغويّ.
- حارسُ حدودٍ أُعيد تثبيتُه: `selectBuildStrategyStage` liveLog ٥ ← ٨ (السبب مكتوب). الخريطةُ: أعدادُ الأسطر الأربعة.

### الاختبار الجديد (`tests/productMind.test.mjs`، ١٠)

المفاهيمُ (عربيّ/إنجليزيّ/جمع/قوسان/الأطول/المجهول) · القربُ (تاكسي ضدّ ERP يفقد راكباً وسائقاً، ضدّ كلون التاكسي مغطّى، العامُّ بلا معنى) ·
**العطبُ الأصل** (مع فهمٍ: لا ERP ولا أيُّ كلون سيستم والسببُ مُسمّى؛ بلا فهمٍ: الكلماتُ وحدَها — مثبَّتٌ حدّاً أدنى لا مرغوباً) · عبارةُ المسار
لا ترفع الفيتو والتعادلُ يحسمه الفهم (تاكسي لا سوق، وبلا فهمٍ كان السوق) · كلماتُ المستخدم الصريحة تغلب النموذجَ المهلوس (كما كان) ·
الفهمُ وحده (`model-only`؛ دورٌ واحد لا يكفي؛ بلا كيانٍ مشترك لا يكفي؛ كيانٌ واحد يكفي) · المرجعُ يبذر الفهم بلا نموذجٍ لغويّ (`Passenger/Driver/Admin`
بأوصافها العربيّة والسجلُّ يسمّي المرجع؛ هدفٌ بلا مرجع بلا لاحقة) · من طرفٍ إلى طرف على مرحلة الاختيار (سيستم: لا كلون + سطرُ الاستبعاد؛ موقع: التاكسي بدليله) ·
مشروعٌ يعمل بفجوة أدوار لا يُدهَس والمعطّلُ يُستبدَل · الحدود.

### الطفرات (٢٥، ضدّ productMind + cloneTemplates + logsAndMatch + jcrMissionStrategy + selectBuildStrategyStage + understandStage)

**٢٥/٢٥ أُمسكت** بعد جولتين (٢٢/٢٥ في الأولى: ناجيةٌ واحدة ومرساتان معطوبتا الاقتباس أُعيدتا يدويّاً):
المفاهيم: القوسان لا يُقدَّمان · المرادفُ المتعدّد يُهمَل · «ال» لا تُنزَع · العامُّ يُعدّ معنىً · القرب: التغطيةُ دائماً تامّة · الأدوارُ الناقصة معكوسة ·
لا كيانَ مشترك · كلُّ فهمٍ ذو معنى · الفيتو يُلغى · الفيتو يتجاهل كلمةَ المستخدم · عبارةُ المسار تُعدّ تسمية · الترتيبُ بلا قرب · الفئةُ وحدها تكفي ·
الفهمُ وحده بدورٍ واحد · بلا كيانٍ مشترك · سببُ الرفض يُخفى · الغلافُ يعيد الكائن · البذرُ يُلغى · ترتيبُ الدمج معكوس · لاحقةُ المرجع لا تُبثّ ·
الاستبعادُ لا يُقال · الاختيارُ بلا نسبة الأدوار · فجوةُ الأدوار تُعدّ عطلاً · الفجوةُ لا تُقال · المختارُ يُهمَل.

الناجيةُ في الجولة الأولى («المرادفُ المتعدّد يُهمَل») كانت مكافئةً لأنّ «مدير أسطول» يطابق مرادفَه تماماً — أُضيفت حالتا احتواءٍ («مدير أسطول الشركة»، «Senior Fleet Manager»).

### الأثر

- الاختبارات **١٦٢٩ ← ١٦٣٩**. لا تغييرَ في مسارٍ بلا فهم (الحالةُ المتدهورة موصوفة). ما بقي من الورقة وترتيبُه وحالتُه في `PRODUCT_MIND.md` §٥.

## PM/2 — «الحكم — لم يُتحقَّق لا يساوي نجح»

ثاني خطوةٍ في مسار «عقل المنتج» (`PRODUCT_MIND.md`). **تغييرُ سلوكٍ مقصودٌ ومكتوب**: كانت الحلقةُ تعيد `{ success: true }` بعد مراحل التسليم
دون أن تقرأ ما وجدته بوّاباتُ التحقّق، فيصل المستخدمَ «✅ اكتملت المهمة» ولو كان محقّقُ المتطلّبات صامتاً أو التحقّقُ السلوكيّ قد وجد ثغراتٍ لم تُصلَح.

### القياس والجذر

- `runRequirementsVerify` و`runBehaviorVerifyStage` تحسبان حكماً **محليّاً** (`verdict`) وتبثّانه ولا تسجّلانه على السياق؛ الحلقةُ في
  `runDynamicMultiAgentRuntime` لا تقرأ شيئاً وتعيد النجاحَ دائماً بعد قبول الخطّة. `DELIVERY_STAGES` كلُّها `optional` بلا تمييزٍ بين ما يحكم وما يُغني.
- بلا مزوّد: `verifyRequirements` تعيد `null` صامتةً → لا سطرَ ولا حكم → «اكتملت المهمة» (مثبَّتٌ في `jcrRuntimePipeline` قبل هذا).

### ما تغيّر

- `core/contracts/index.js` **١٩٧ ← ٢٣٦**: حقلُ `gate` على كلّ مرحلة (`gate` ×٣: `guard-and-write`, `requirements-verify`, `behavior-verify`؛ `advisory` ×٢:
  `executive-memory`, `project-memory`؛ `enhancement` ×١٠). `optional` كما هو (لا مرحلةَ تُسقط الحلقة). `VERDICT` و`GATE_KINDS`؛ `recordGateOutcome(context, name, status, detail)`
  (`pass | fail | unverified | skipped`، التفصيلُ ≤ ٣٠٠) و`deliveryVerdict(outcomes, stages) → { status, gates, summary }` نقيّة: ثغرةٌ → FAILED، وإلّا «لم يُتحقَّق»
  (أو غائب) → UNVERIFIED، وإلّا اجتيازٌ واحد → PASS، وكلُّها «لا ينطبق» → UNVERIFIED. التحسيناتُ خارج الحكم.
- `stages/requirementsVerify.js` **٨٧ ← ٩٥**: خمسُ حالات — صامت → `unverified`؛ ناقص → `fail` بأسمائه؛ مكتمل → `pass`؛ لا مكوّنات → `skipped`؛ رمي → `unverified`.
- `stages/verify.js` **٨١ ← ٨٩**: أربعُ حالات — لم يُشغَّل/تُخطّي → `unverified`؛ اجتاز → `pass`؛ ثغرات → `fail` بأسماء الفحوص؛ رمي → `unverified`.
- `jcr.js` **١٠٩٧ ← ١١٠٣**: الحارسُ يسجّل `pass` بعد الكتابة؛ الحلقةُ تحكم وتبثّ «⚖️ الحكم: … — …» (`7. VERDICT`/`Judge`) وتعيد `{ success: true, verdict }`؛
  المفوِّض `_reportMissionSuccess(goal, ctx, verdict = null)`.
- `stages/reportMissionSuccess.js` **٨٤ ← ٩٩**: العنوانُ من الحكم — PASS «✅ اكتملت المهمة — تقرير التسليم:» كما كان + سطر «⚖️ التحقّق: …»؛
  UNVERIFIED «☑️ اكتمل البناء — ولم يكتمل التحقّق: <البوّابات>»؛ FAILED «⚠️ اكتمل البناء لكنّ التحقّق وجد ثغرات — <البوّابات>». بلا حكمٍ (مستدعٍ قديم) كما كان.
- **قرارٌ مكتوب — حالةُ المهمّة ≠ حكمُ المنتج**: الحالةُ تبقى `COMPLETED` للثلاثة (المهمّةُ اكتملت والملفّاتُ على القرص والمعاينةُ تتحدّث)؛ الحكمُ يصل التقريرَ
  والمستدعي (`result.verdict`). نقلُ الحالة إلى FAILED بحكمٍ سلوكيّ كان سيُسقط الدروسَ والمقاييس في مسارٍ لم يُصمَّم له؛ يُعاد النظرُ حين يكون لـFAILED مستهلكٌ يقرّر.
- **حدٌّ مكتوب**: مساراتُ الاستراتيجيّة (Registry/Clone/React) تعود قبل الحلقة بنتائجها الخاصّة بلا `verdict` — PM/2b.
- إعادةُ تثبيت بسببٍ مكتوب: `jcrRuntimePipeline` المسارُ السعيد بلا مزوّد صار UNVERIFIED (`requirements-verify: المحقّقُ لم يُجب`) — الصدقُ لا «اكتملت المهمة»؛
  `reportStage` نصُّ المفوِّض؛ الخريطةُ خمسةُ أعداد.

### الاختبار الجديد (`tests/deliveryVerdict.test.mjs`، ٩)

العقدُ (٣/٢/١٠، optional، مجمَّد) · `recordGateOutcome` (استبدال، قصّ ٣٠٠، حالةٌ مجهولة ترمي، سياقٌ غائب) · `deliveryVerdict` نقيّة (مصفوفةُ الحالات، الغائب،
المجهول، التحسيناتُ خارج الحكم، الملخّص، `stages` مخصّصة) · بوّابةُ المتطلّبات بحالاتها الخمس (بمحقّقٍ محقون) · بوّابةُ السلوك بثلاث (يعمل/معطّل/بلا صفحة) ·
الحلقةُ كاملةً: PASS بعنوانه وسطر التحقّق وCOMPLETED؛ FAILED بعنوانه وتفصيله ولا «اكتملت المهمة» وCOMPLETED والمعاينةُ تتحدّث · التقريرُ مباشرةً
(قديم/UNVERIFIED إنجليزيّ/FAILED عربيّ يجمع السيّئات/PASS) · الحدود.

### الطفرات (٢٤، ضدّ deliveryVerdict + jcrRuntimePipeline + reportStage + contracts)

**٢٤/٢٤ أُمسكت** من الجولة الأولى:
العقد: المتطلّباتُ ليست بوّابة · الحارسُ استشاريّ · التسجيل: الحالةُ المجهولة تُقبل · التفصيلُ لا يُقصّ · الحكم: الغائبُ اجتياز · الثغرةُ لا تغلب · «لم يُتحقَّق» يُهمَل ·
كلُّها «لا ينطبق» اجتياز · التحسيناتُ بوّابات · الملخّصُ بلا علامات · المتطلّبات: الصامتُ اجتياز · الناقصُ اجتياز · لا ينطبق لا يُسجَّل · الرميُ لا يُسجَّل ·
السلوك: المتخطّى اجتياز · الثغراتُ اجتياز · الحلقة: الحكمُ لا يُحسب · الحارسُ لا يسجّل · الحكمُ لا يصل التقرير · سطرُ القاضي لا يُبثّ · التقرير: FAILED يُعنون اكتمالاً ·
UNVERIFIED يُعنون اكتمالاً · سطرُ التحقّق يُحذف · البوّاباتُ السيّئة لا تُذكر.

### الأثر

- الاختبارات **١٦٣٩ ← ١٦٤٨**. `PRODUCT_MIND.md` §٥: PM/2 ✅؛ التالي PM/3 (صدقُ المجال في المحقّق) وPM/2b (حكمُ مسارات الاستراتيجيّة).

## PM/2b — «حكمُ مسارات الاستراتيجيّة — لا تسليمَ بلا حكم»

تتمّةُ PM/2. **تغييرُ سلوكٍ مقصودٌ ومكتوب**: Registry وClone وReact تعود من `_runMissionNow` قبل حلقة التسليم بنتائجها الخاصّة، فكانت بلا حكمٍ:
«✅ اكتمل» يصل المستخدمَ ولو لم يُتحقَّق من الصفحة المركّبة قطّ (Registry لم يكن يشغّل تحقّقاً)، أو تحقّق الكلونُ من القالب النظيف لا ممّا وصل القرص.

### القياس

| المسار | كان يتحقّق | كان يعيد |
|---|---|---|
| Registry | لا شيء | `{ success, registry, blocks }` |
| Clone | `verifyBehavior` مرّتين داخل كتلة البصمة فقط (ارتدادٌ لا حكم)؛ بلا مزوّدٍ لا تحقّقَ أصلاً | `{ success, clone }` |
| React | `verifyAndAutofix` على المعاينة الثابتة ويُهمل الناتج | `{ success, stack, files }` |

### ما تغيّر

- `stages/verify.js` **٨٩ ← ١٠٩**: `behaviorOutcome(verdict)` (التصنيفُ الذي كان داخل مرحلة السلوك، صار مشتركاً وتستعمله المرحلةُ نفسُها) +
  `strategyVerdict({ filesCount, behavior, requirementsNote })` — الحارسُ اجتاز بالكتابة، المتطلّباتُ «لا ينطبق» بسببٍ مكتوب، السلوكُ يحكم → `deliveryVerdict`.
- `stages/reportMissionSuccess.js` **٩٩ ← ١١٣**: `withVerdict(message, verdict, lang)` — PASS يلحق «⚖️ التحقّق: …»؛ UNVERIFIED/FAILED يبدّل «✅» بـ«☑️/⚠️» ويقول ما لم يُتحقَّق أو ما وُجد.
- `buildFromRegistry` **٧٨ ← ٨٥**: يشغّل `verifyBehavior` على الصفحة المركّبة (`kind: 'landing'` — لا شرطَ تفاعل) → حكمٌ فعليّ.
- `buildFromClone` **٢٠٥ ← ٢١١**: تحقّقٌ نهائيّ على ما وصل القرص (بعد البصمة أو الاسترجاع والتلميع) لا على القالب النظيف.
- `buildReact` **١٣٦ ← ١٤٠**: يلتقط ناتجَ `verifyAndAutofix` على المعاينة الثابتة بدل إهماله.
- الثلاثةُ يعودون بـ`verdict` بالشكل نفسِه ورسالتُهم عبر `withVerdict`. `jcr.js` **١١٠٣ ← ١١٠٧**: القاضي يبثّ حكمَ الاستراتيجيّة كما يبثّ حكمَ الحلقة؛ «يعمل/تخطّي» بلا حكمٍ لأنّ شيئاً لم يُبنَ.
- إعادةُ تثبيت بسببٍ مكتوب: `cloneStage`/`registryStage` نتيجةُ البانى تحمل `verdict` ورسالتُه تلحق سطرَ التحقّق؛ `reactStage` التقريرُ الإنجليزيّ خمسةُ أسطر؛
  `deliveryVerdict` استيرادُ verify.js وعدُّ التصنيف بعد المشاركة؛ الخريطةُ ستّةُ أعداد.

### الاختبار الجديد (`tests/strategyVerdict.test.mjs`، ٧)

`behaviorOutcome` بحالاته · `strategyVerdict` بالشكل والأسباب · `withVerdict` (PASS/UNVERIFIED/FAILED × ar/en، بلا حكم، بلا أيقونة) · Registry حقيقيّ → PASS بثلاث بوّابات
والرسالةُ بسطر التحقّق · Clone سليم → PASS، وكلونٌ ملفّاتُه معطوبة → **FAILED** بأسماء الفحوص والرسالةُ «⚠️ اكتمل … التحقّق وجد ثغرات» و`success: true` (حالةُ المهمّة ≠ حكمُ المنتج) ·
القاضي يبثّ حكمَ الاستراتيجيّة ويصمت بلا حكم · الحدود.

### الطفرات (٢٠، ضدّ strategyVerdict + cloneStage + registryStage + deliveryVerdict + reactStage)

**٢٠/٢٠ أُمسكت** بعد جولتين (١٨/٢٠ في الأولى):
behaviorOutcome: المتخطّى اجتياز · الثغراتُ اجتياز · أسماءُ الفحوص تُخفى · strategyVerdict: الحارسُ لا يُسجَّل · المتطلّباتُ تُعدّ اجتيازاً · السلوكُ لا يُسجَّل ·
المرحلة: التصنيفُ المشترك يُتجاوَز · withVerdict: بلا حكمٍ تُلحق · PASS بلا سطر · الأيقونةُ لا تُبدَّل · FAILED كـUNVERIFIED · Registry: لا تحقّقَ فعليّ ·
الرسالةُ بلا حكم · الحكمُ لا يعود · Clone: التحقّقُ على القالب لا القرص · الرسالةُ بلا حكم · الحكمُ لا يعود · React: التحقّقُ لا يُلتقط · الحكمُ لا يعود ·
القاضي لا يبثّ حكمَ الاستراتيجيّة.

**الناجيتان في الجولة الأولى كشفتا ثغرتَي منهج**: (١) توصيفُ Registry كان يقبل حكماً مُرضياً مكتوباً يدويّاً — شُدّ بأن يكون تفصيلُ بوّابة السلوك
بصيغة المحقّق نفسِها (`N ✅ / N ⚠️ / N ❌ (N%)`)، فلا يُنتجه إلّا تحقّقٌ فعليّ؛ (٢) **مسارُ React كان خارج قائمة اختبارات الطفرات** —
أُضيف `reactStage` إلى القائمة ووُضع تأكيدُ الحكم في اختباره. خطأُ الأداة لا خطأُ الكود، ويُسجَّل هنا كي لا يتكرّر: قائمةُ الطفرات تشمل كلَّ مسارٍ لمسته الجراحة.

### الأثر

- الاختبارات **١٦٤٨ ← ١٦٥٥**. كلُّ تسليمٍ يحمل الآن حكماً بالشكل نفسِه: الحلقةُ (PM/2) والمساراتُ الثلاثة (PM/2b). `PRODUCT_MIND.md` §٥: PM/2b ✅.

## PM/3 — «صدقُ المجال بوّابةً — هل هذا المنتجُ الذي طُلب؟»

الخطوةُ الثالثة في ورقة «عقل المنتج». PM/1 جعل **الاختيار** بالفهم، وPM/2 جعل **التسليم** يحمل حكماً — وبقي السؤالُ الذي لم يُسأل قطُّ:
المحقّقُ السلوكيّ يفحص الاتّصالَ والأدوارَ والبياناتِ والتفاعل، فلو خرج تطبيقُ مطعمٍ على طلبِ تاكسي **لعبَر البوّابةَ سليماً** — يعمل، ولكنّه ليس المطلوب.
هذا هو عطبُ التاكسي نفسُه في آخر حلقةٍ من السلسلة: لا مكانَ في النظام كلِّه يقارن **المبنيَّ** بـ**المفهوم**.

### المبدأ

الفهمُ الذي بُني في PM/1 (`domainModel`: كياناتٌ وأدوار) صار الآن **مرجعاً يُحاكَم إليه المبنيّ**، بالمعجم نفسِه لا بقائمةِ كلماتٍ ثانية.
فحصٌ حتميّ بلا LLM: يُطبَّع نصُّ الواجهة والكود، ويُقرأ بفهرسٍ عكسيّ للمعجم، فتُقارن مفاهيمُه بمفاهيم النموذج.

### ما تغيّر

- `agents/projectModel.js` **٣٩١ ← ٤٣٠**:
  - `conceptsInText(text)` — فهرسٌ عكسيّ على `SYNONYMS` نفسِها: مطابقةُ كلمةٍ كاملة (`' ' + syn + ' '`) لا احتواءً، والمرادفُ أقصرُ من ثلاثة أحرف مُستبعَد
    («رد» تَرِد في كلِّ نصٍّ عربيّ فلا تجعل كلَّ مشروعٍ مشروعَ ردود)، والمفاهيمُ العامّة (`user/item/employee`) لا تُحتسب لأنّها لا تسمّي منتجاً.
  - `domainFidelity(understood, text)` → `{ applicable, expected, spoken, covered, foreign, missing, contaminated }`.
    **ينطبق** عند مفهومَين متوقَّعَين فأكثر (الواحدُ يصادف كثيراً). **تلوّثٌ** = صفرُ تقاطعٍ **و** ثلاثةُ مفاهيمَ أجنبيّةٍ فأكثر: التقاطعُ الواحد يكفي لنفي التهمة،
    فالنقصُ يُكمَل والمنتجُ المغاير يُردّ.
  - `normalizeConceptText`: نزعُ سوابق العطف/الجرّ الملتصقة بـ«ال» (`والسائق`، `بالمركبة`، `للراكب`) — بلا هذا تفلت نصفُ مفردات النصّ العربيّ.
    السابقةُ وحدها لا تُنزع: «وقت» ليست «قت».
- `agents/behaviorVerifier.js` **٥١١ ← ٥٢٨**: فحصُ `domain-fidelity` في `analyzeStatic` على **الواجهة والكود معاً** (مفاهيمُ المنتج تعيش في أسماء المتغيّرات
  لا في العناوين فقط)، بثلاث حالات: تلوّثٌ → `fail` بمفاهيم الطرفين؛ نقصٌ → `warn` بأسماء الغائب؛ تغطيةٌ → `pass`. وصامتٌ تماماً حين لا ينطبق.
- الأثرُ يسري تلقائيّاً: `fail` تُسقط `ok` في المحقّق → `behaviorOutcome` تعيد `fail` → حكمُ PM/2 يصير **FAILED**، في الحلقة وفي المسارات الثلاثة معاً. لا سلكَ جديد.

### القرار المكتوب

**التلوّثُ عطلٌ، والنقصُ تنبيه.** أن يتكلّم المبنيُّ لغةَ منتجٍ آخر بلا أثرٍ للمطلوب دليلٌ على أنّ الاختيار أخطأ — يُردّ. أمّا غيابُ مفهومٍ من الواجهة
فقد يكون شاشةً لم تُبنَ بعد أو تسميةً مختلفة، فيُقال ولا يُسقِط. والحكمُ لا يُبنى على فهمٍ بلا معنى: نموذجٌ كلُّه `User/Item` لا يُحاكَم إليه أحد.

### الاختبار الجديد (`tests/domainFidelity.test.mjs`، ٥)

الفهرسُ العكسيّ (كاملُ الكلمة، العربيّةُ والإنجليزيّة، العامُّ والقصيرُ مُستبعَدان، سوابقُ العطف) · حالاتُ `domainFidelity` الستّ (تلوّث/تغطية/لا ينطبق/صمت/عتبة/تقاطعٌ واحد) ·
الفحصُ بحالاته الثلاث ونصوصِ تفصيله والصمتِ حين لا ينطبق، ومن الكود وحدَه · **من طرفٍ إلى طرف**: مشروعُ مطعمٍ يعمل تقنيّاً على فهمِ تاكسي →
`verdict.ok === false` وبوّابةُ PM/2 `fail` تسمّي `domain-fidelity`، والمشروعُ نفسُه على فهمِ مطعمٍ صحيح لا يسقط · الحدود: المعجمُ واحد (استيرادٌ من `projectModel`، نداءٌ واحد، ثلاثُ حالات).

### الطفرات (١٦، ضدّ domainFidelity + productMind + behaviorVerifier + behaviorVerifyStage)

**١٦/١٦ أُمسكت** بعد جولتين (١١/١٦ في الأولى). الناجياتُ الخمسُ كنَّ **فجواتِ توصيفٍ حقيقيّة، وكلُّ واحدةٍ منها عطبٌ لو مرّ**:
حدُّ الثلاثة أحرف (بلا اختبارٍ يمنع، «رد» تصير مفهوماً) · عتبةُ الانطباق مفهومان (الواحدُ يُحاكِم على مصادفة) · شرطُ التقاطع في التلوّث (منتجٌ نصفُه صحيح يُردّ ظلماً) ·
تصفيةُ المتوقَّع من الأجنبيّ (المفهومُ الصحيح يُعدّ تهمة) · قراءةُ الكود مع الواجهة (مشروعٌ كلُّ مفاهيمه في `script.js` يُتّهم بالنقص).
أُغلقت الخمسُ بتأكيداتٍ سلوكيّة لا بنيويّة.

### الحدُّ المعروف (PM/3b)

`ROLE_SYNONYMS` في `behaviorVerifier.js` ما زالت قائمةً ثانيةً مستقلّةً لتغطية الأدوار في الواجهة. لم تُوحَّد الآن لأنّ توحيدَها يمسّ فحصاً قائماً بتغطيةٍ قائمة،
والقاعدةُ «تدريجيّ ومُتحقَّقٌ دوماً». اختبارُ الحدود **يثبّت وجودَها** كي لا يُنسى الدَّين.

### الأثر

- الاختبارات **١٦٥٥ ← ١٦٦٠**. صار للنظام سؤالٌ لم يكن يملكه: *هل هذا المنتجُ الذي طُلب؟* — يُسأل حتميّاً، بلا مزوّد، على كلِّ تسليم.
  `PRODUCT_MIND.md` §٥: PM/3 ✅.

## PM/4 — «مرجعٌ → نموذجٌ أغنى — الفهمُ يأخذ ما يعرفه المرجع، والمتطلّباتُ تُقاس على ما فُهم»

الخطوةُ الرابعة في «عقل المنتج». PM/1 بذر الفهمَ من المرجع، وPM/3 جعل الفهمَ مرجعاً يُحاكَم إليه المبنيّ —
فظهر أنّ **الفهمَ نفسَه أفقرُ ممّا يعرفه المرجع**، وأنّ **قائمةَ المتطلّبات أفقرُ من الفهم**. بابان مفتوحان تحت بوّابتَين مغلقتَين.

### القياس

| ما يعرفه المرجع (`taxi_fleet`) | ما كان يصل | |
|---|---|---|
| ٣ أدوار | ٣ أدوار | ✅ |
| ٥ صفحات | — | تُرمى |
| ٤ مكوّنات (فيها تدفّقُ حالة الرحلة) | — | تُرمى |

فيخرج فهمُ التاكسي `entities: []` و`flows: []`. والأثرُ ليس تجميليّاً: صفحةٌ تذكر «الراكب/السائق/الإدارة» ولا أثرَ
فيها لرحلةٍ ولا مركبةٍ كانت تعبر بوّابةَ صدق المجال (PM/3) بـ`missing: []` — **تغطيةٌ كاملة على نصف منتج**.

وفي الطرف الآخر: بلا مزوّدٍ يخرج المخطّطُ بمكوّنٍ وظيفيٍّ واحدٍ اسمُه «الميزة الأساسية التفاعلية»، وهي كلُّ مواصفةِ
النظام التي يراها محقّقُ المتطلّبات — وشرطُ المرحلة كان `blueprint.functionalComponents.length`، فتُتخطّى البوّابةُ كلّما رقّ المخطّطُ ولو كان الفهمُ غنيّاً.

### ما تغيّر

- `agents/projectModel.js` **٤٣٠ ← ٤٤٥**: المعجمُ صار مقسوماً `ROLE_CONCEPTS` / `ENTITY_CONCEPTS` (بالترتيب نفسِه، فبناءُ
  `SYNONYMS` لم يتغيّر) و`CONCEPTS` تركيبُهما، مع `conceptKind(concept)` → `'role'|'entity'|null`. **وإصلاحُ معجمٍ مقيس**:
  «عرض»/`show` مشتركتان لفظاً (عرضُ القيمة، عرضُ سعر، اعرض القائمة) فكانتا تجعلان كلَّ مشروعٍ مشروعَ أفلام — أُسقطتا وبقي ما لا يلتبس.
- `agents/referenceBlueprints.js` **٢٠٩ ← ٢٤٩**: `referenceEntities(bp)` تشتقّ الكياناتِ من نصّ الصفحات والمكوّنات
  بمعجم PM/1 نفسِه (لا قائمةَ ثالثة)، مرتّبةً بالأكثر ذكراً لأنّ سقفَ النموذج ستّة؛ و`referenceFlows(bp)` تقرأ ما كُتب
  بسهم («طلب → قبول → في الطريق → وصل») تدفّقاً بخطواته، وما سواه مكوّنٌ لا تدفّق. `referenceModel` تركّبهما.
- `agents/requirementsVerifier.js` **١٠١ ← ١٣٢**: `composeRequirements(blueprint, domainModel)` — كلُّ دورٍ مفهومٍ شاشتُه،
  وكلُّ كيانٍ تمثيلُ بياناته، وكلُّ تدفّقٍ انتقالُه؛ تُضاف لِما في المخطّط ولا تُزيحه، بلا تكرار. `verifyRequirements` تقبل النموذجَ وسيطاً رابعاً.
- `agents/stages/requirementsVerify.js` **٩٥ ← ٩٨**: شرطُ البوّابة صار «هل من متطلّبات؟» لا «هل في المخطّط مكوّنات؟»،
  والنموذجُ يُقرأ قبلها ويمرّ في نداءَي التحقّق (الأوّل وجولةُ الإكمال).

### الأثر المقيس

مكوّنٌ عامٌّ واحد صار **إحدى عشرة متطلّبةً مسمّاة** لنظام التاكسي؛ والصفحةُ التي تذكر الأدوارَ وحدَها صار نقصُها مسمّى
(`trip`، `vehicle`، `booking`، `order`، `payment`، `account`) بدل «تغطيةٌ كاملة».

### إعادةُ تثبيتٍ بسببٍ مكتوب

- `productMind` كان يثبّت `entities: []` في نموذج المرجع — وهو بعينه ما تُصلحه الخطوة، فصار يثبّت حضورَ «رحلة» و«مركبة» والتدفّقَ الوحيد.
- `deliveryVerdict` سببُ التخطّي صار يذكر الفهمَ أيضاً، لأنّ الشرطَ نفسَه تغيّر.

### الاختبار الجديد (`tests/referenceModelDepth.test.mjs`، ٧)

نوعُ المفهوم · اشتقاقُ الكيانات (كيانٌ لا دور، الترتيبُ بالتكرار، بلا صفحاتٍ لا اشتقاق، «عرضُ القيمة» ليس فيلماً) ·
اشتقاقُ التدفّقات (السهمُ شرطاً، الخطوةُ الواحدة ليست انتقالاً) · **الأثرُ على بوّابة PM/3** (النقصُ مسمّى، والاكتمالُ لا يُلام) ·
التركيبُ (المخطّطُ أوّلاً، بلا تكرار، وبلا فهمٍ كما كان تماماً) · **ما يصل المدقّق فعلاً** (القائمةُ المُرسَلة تحمل شاشاتِ الأدوار وبياناتِ الكيانات) · الحدود.

### الطفرات (١٨، ضدّ referenceModelDepth + productMind + requirementsVerifier + requirementsVerifyStage + deliveryVerdict + domainFidelity)

**١٨/١٨ أُمسكت** بعد جولتين (١٦/١٨ في الأولى). الناجيتان:
(١) حارسُ السهم في `referenceFlows` — تبيّن أنّه **زائدٌ منطقيّاً**: نصٌّ بلا سهمٍ لا ينقسم، فشرطُ «خطوتان فأكثر» يكفي وحدَه.
حُذف الحارسُ ولم يُكتب له اختبار: لا نُبقي كوداً لا يملك سلوكاً يُقاس.
(٢) وصولُ النموذج إلى التركيب داخل `verifyRequirements` — لم يكن أحدٌ يتحقّق ممّا يصل **المدقّقَ** فعلاً، فأُضيف تأكيدٌ على نصّ الطلب المُرسَل.

### الأثر

- الاختبارات **١٦٦٠ ← ١٦٦٧**. `PRODUCT_MIND.md` §٥: PM/4 ✅.

## PM/5 — «النموذجُ إلى المصمّم والخلفيّة — لا افتراضَ متجرٍ لنظامِ تاكسي»

الخطوةُ الخامسة. الفهمُ (PM/1، وأُغني في PM/4) كان يصل الكودرَ والمحقّقَ — ولا يصل **مَن يقرّر الشكلَ والبيانات**.

### القياس

| مَن | ما كان يقرأ | الأثر |
|---|---|---|
| `stages/designer.js` | لا شيءَ من الفهم — `generateDesignBrief(goal, user, project, lang)` | النوعُ من مطابقة كلمات الهدف وحدَها |
| `stages/backend.js` (قاعدة البيانات) | `designBrief.projectType \|\| 'business'` | `'business'` ليست مفتاحاً — تسقط للمولّد الديناميكيّ |
| `stages/backend.js` (Prisma) | `designBrief.projectType \|\| 'ecommerce'` | **`'ecommerce'` مفتاحٌ موجود** |

فنظامُ تاكسي بلا نوعٍ في البريف كان يأخذ `Product`/`OrderItem`/`Review` **حتميّاً وبلا مزوّد**. وأعمقُ من ذلك:
`generateDynamicSchema` كان يعيد `PRISMA_SCHEMAS.ecommerce` داخل `catch` صامت — فحتّى بعد إصلاح الاحتياط الظاهر
يبقى الخفيُّ يكتب مخطّطَ متجرٍ لمشروعٍ ليس متجراً. الاحتياطُ الذي يُخمّن أسوأُ من الاحتياط الذي يعترف.

### ما تغيّر

- `agents/projectModel.js` **٤٤٥ ← ٤٨٧**: `TYPE_SIGNATURES` + `modelProjectType(domainModel)` — نوعُ المشروع من المفاهيم
  المفهومة، بمطابقتَين فأكثر (الواحدةُ تصادف)، والتعادلُ يُكسر باسمِ النوع (ترتيبٌ معلَنٌ مستقرّ لا ترتيبُ مصفوفة)،
  و`null` إن لم يميّز الفهمُ شيئاً — «لا أعرف» جوابٌ صحيح يُمرَّر لمن يقرأ الهدفَ نفسَه.
- `agents/designerAgent.js` **٣٠٠ ← ٣٠٢**: `generateDesignBrief(..., typeHint)`. `getProjectContext` كانت تقبل تلميحَ نوعٍ
  **منذ البداية ولا يمرّره أحد** — الفهمُ يملؤه الآن، و`null` يعيد الكشفَ بالكلمات كما كان.
- `agents/stages/designer.js` **٣٦ ← ٤٤**: يقرأ الفهمَ المخزّن ويمرّره، والسطرُ يقول النوعَ **ومن أين جاء**.
- `agents/stages/backend.js` **١٨٨ ← ٢١١**: `resolveType(context)` بثلاث مراتب — البريف، فالفهم، فكشفُ النوع من الهدف.
  لا نوعَ مكتوباً احتياطاً. وفشلُ Prisma يُقال (٢٨ ← ٢٩ سطرَ بثّ) بدل أن يُبتلع.
- `agents/postgresAgent.js` **٣٦٢ ← ٣٦٩**: `catch` لا يعيد مخطّطَ المتجر بل `null`؛ و`generatePrismaSetup` تعود
  `{ success: false, reason }` بلا ملفّات. **لا مخطّطَ ⇒ لا تسليم**: مخطّطٌ من مجالٍ آخر أسوأُ من لا مخطّط، لأنّ المستخدم يبني عليه.

### إعادةُ تثبيتٍ بسببٍ مكتوب

- اختبارا `postgresAgent` كانا يُشغَّلان بـ`'business'` — نوعٌ بلا قالب — فيمرّان عبر احتياطِ الـ`catch`؛ أي أنّهما كانا
  **يثبّتان العطب**: «نظامُ محاسبةٍ يأخذ مخطّطاً» والمخطّطُ متجرٌ إلكترونيّ. نُقلا إلى نوعٍ له قالبٌ فعلاً، وأُضيف اختبارُ الاعتراف.
- `scaffoldStages` سطرُ المصمّم صار يذكر النوعَ ومصدرَه؛ السلوكُ القديمُ موصوفٌ لا مغيَّر (بلا نموذجٍ فالمصدرُ الكلمات).
- `backendStage` عددُ أسطر البثّ ٢٨ ← ٢٩: سطرُ الصدق الجديد، لا توسّعٌ في المرحلة.

### الطفرات (١٣، ضدّ modelToBuilders + postgresAgent + backendStage + scaffoldStages + productMind + referenceModelDepth)

**١٣/١٣ أُمسكت** بعد جولتين — والجولةُ الأولى كانت **٤/١٣ فقط**، أسوأَ جولةٍ في المسار كلِّه. الدرسُ مكتوبٌ لأنّه يتكرّر:
- **تسعُ ناجياتٍ، سببُها واحد**: اختبرتُ الدوالَّ النقيّة ولم أختبر **المرحلتَين** اللتين تستهلكانهما. لا اختبارَ كان يخزّن فهماً
  ثمّ يشغّل `runBackendStage`/`runDesigner`. أُضيف اختباران يفعلان ذلك، فسقطت سبعُ ناجيات دفعةً واحدة.
- **اثنتان كانتا كوداً زائداً لا فجوةَ توصيف**: حارسُ `have.size < 2` (لأنّ `score ≤ have.size` وشرطُ «مطابقتان» يكفي وحدَه)
  و`{ dropGeneric: true }` (لا توقيعَ يحوي مفهوماً عامّاً). حُذفا، وحلّ محلَّ الثاني تأكيدٌ بنيويّ يثبّت الضمانَ نفسَه.

### الأثر

- الاختبارات **١٦٦٧ ← ١٦٧٤**. لم يبقَ في مسار البناء نوعُ مشروعٍ مكتوبٌ احتياطاً. `PRODUCT_MIND.md` §٥: PM/5 ✅.

## JCR/31 — «ردُّ الشات يخرج — وآخرُ مستهلكٍ للمزوّد في الصنف إلّا المصنِّف»

آخرُ الطرائق الكبرى غير المستخرَجة. `generateChatResponse` **١٣٧ سطراً بلا تغطيةٍ إطلاقاً**، وهي الطريقةُ التي يراها
المستخدمُ في كلِّ رسالةٍ يكتبها — أكثرُ الشيفرة استعمالاً وأقلُّها اختباراً.

### القياس قبل النقل

تلمس من `this` ثلاثةَ أشياءَ فقط:

| ما تلمسه | العدد | إلى أين |
|---|---|---|
| `loadExecutiveMemory` / `summarizeConversation` | ١ + ١ | `ops` (الاختباراتُ تستبدلهما على النسخة) |
| `reporter.send` (بدء/قطعة/نهاية) + `emitChatReply` | ٣ + ١ | `reporter` |
| `emitLiveLog` | ٢ | `reporter.liveLog` |
| `groq.chat.completions.create` | ١ | عميلٌ يُحقَن بمعاملٍ افتراضيّ (سابقة JCR/30) |

### ما تغيّر

- `agents/stages/chatResponse.js` **جديد، ١٦١**: `generateChatResponse(userMessage, username, roomName, userLang, reporter, ops, client = groq)`.
- `agents/jcr.js` **١١٠٧ ← ٩٧٤**: مفوِّضٌ من سبعة أسطر. و`emitChatReply` غلافٌ من سطرٍ واحد حول `reporter.send(…, 'chat_reply')`،
  فكُتب النداءُ صريحاً في المرحلة والغلافُ باقٍ في الصنف لمستدعيه الآخرين — لا تغييرَ سلوك.
- **اثنا عشر استيراداً ماتت في `jcr`** لأنّ ردَّ الشات كان آخرَ مستهلكٍ لها: `scanProjectFiles`/`buildProjectBrain`/`summarizeBrain`/`summarizeFacts`،
  `getLangInfo`، `getProjectMemory`، `analyzeProjectStatic`، `projectPathOf`، `getPlatformKnowledge`، `loadConversation`/`recordTurn`، `WORKSPACE_ROOT` — أُزيلت.

### إعادةُ تثبيتٍ بسببٍ مكتوب

- `jcrReporterSeam`: أرضيّةُ البثّ ٢٤ ← ٢١ (ردُّ الشات أخذ ٣: بدءُ البثّ وقطعتُه ونهايتُه). تُعاد بالقياس لا بالتخمين.
- `missionMetaStage`: مستهلكو `groq` في الصنف ٣ ← ٢ — لم يبقَ إلّا `classifyIntent`.
- أربعةُ اختباراتِ حدودٍ كانت تثبّت أنّ لأسماءَ بعينها **مستهلكاً باقياً** في `jcr`؛ ردُّ الشات كان ذلك المستهلك.
  لم تُحذف الأسطرُ بصمت: نُقلت الأسماءُ إلى تأكيدِ **غيابٍ** صريح، فما غادر مُثبَّتٌ كما كان حضورُه مُثبَّتاً.

### الاختبار الجديد (`tests/chatResponseStage.test.mjs`، ٤)

البثُّ الحيّ (قطعٌ بين بدءٍ ونهاية، الردُّ هو المتراكم، الدورةُ تُسجَّل بطرفَيها) · فشلُ المزوّد (محاولتان بسطرَيهما وسببِهما،
رسالةٌ صادقة دفعةً واحدة، **ولا دورةَ تدخل الذاكرة**) · الإنجليزيّةُ والمتراكمُ الفارغ · الحدود.

### الطفرات (١٣، ضدّ chatResponseStage + classifiedIntentStage + unifiedRouteStage + jcrReporterSeam + deadImports + missionMetaStage)

**١٣/١٣ أُمسكت** بعد جولتين. والناجيةُ الوحيدة تستحقّ التسجيل لأنّها **عطبُ أداةٍ نادر**:
طفرةُ «الدورةُ تُسجَّل رغم الفشل» نجت لأنّ تأكيدي كان يراقب ردَّ الطيّ (`summarizeConversation`) لا التسجيلَ نفسَه.
ولمّا صوّبتُ التأكيدَ إلى المخزن مباشرةً، فشل — لأنّ **الطفرةَ نفسَها كانت قد كتبت دورةً فاشلة في المخزن الدائم أثناء تشغيلها**،
فأخفت أثرَها عن التأكيد التالي. الحلُّ: خَتْمُ الرسائل بطابعِ الجولة (`STAMP`) كما في `scaffoldStages`.
الدرس: **اختبارٌ يلمس مخزناً دائماً يجب أن يختم بياناته بطابعِ جولته، وإلّا لوّثت جولةُ الطفرات توصيفَها.**

### الأثر

- `jcr.js` **١١٠٧ ← ٩٧٤** (‑١٣٣). الاختبارات **١٦٧٤ ← ١٦٧٨**. بقي في المسار JCR/32 (`_runMissionNow`).

## صمتُ الشات — «الطلبُ يُقبَل ويبدأ العملُ بلا كلمة» (2026-09-06)

بلاغُ المالك: مواصفةُ نظامِ نقاطِ بيعٍ طويلة (٤٤ بنداً، ≈٢٦ ألف حرف) تُرسَل، **فيصمت جولا تماماً** — لا ردَّ ولا خطأ،
ومع تكرار المحاولة الصمتُ نفسُه. لم يكن هذا انطباعاً: أُعيد إنتاجُه بلا شبكة، وخرج الأثرُ مطابقاً لِما رآه في الإنتاج.

### القياس — إعادةُ الإنتاج

بلا مزوّدٍ مُهيَّأ يسقط `classifyIntent` إلى احتياطِه `{ intent: 'chat', confidence: 50 }` — **وهو بعينه السطرُ الذي رآه
المالك**: `INTENT Classifier نية: chat (ثقة: 50%)`. أي أنّ المصنِّفَ في الإنتاج **فشل** على النصّ الطويل ولم يُصنِّفه.
فيصل النصُّ فرعَ «مشروعٌ قائم + نيّةُ فعلٍ + ليس سؤالاً» في `handleClassifiedIntent`، وهذا يُطلق التعديلَ الجراحيَّ
بأسلوب **أطلِق-وانسَ** ويعود. الحصيلةُ المقيسة: **سطرا سجلٍّ وصفرُ `chat_reply`**.

### ثلاثةُ منابعَ للصمت، لا واحد

| # | الموضع | ما يحدث |
|---|---|---|
| ١ | `jcr.surgicalEdit` | **المُطلِقُ الوحيدُ الصامت**: كلُّ مسارٍ آخرَ يبدأ عملاً طويلاً يقول كلمةً أوّلاً («⚡ ممتاز! أبني الآن...»، رسالةُ الصفّ، نصُّ الحجب) — أمّا هذا فيُدرِج المهمّةَ ويعود. وثلاثةُ مواضعَ في `handleClassifiedIntent` وحدَها تناديه هكذا. |
| ٢ | `ExecutionQueue#pump` | `.catch(e => console.error(…))` — انهيارُ المهمّة سطرٌ في سجلّ الخادم لا يراه صاحبُ الطلب، فينتظر رداً لن يأتي. |
| ٣ | حارسُ `/api/chat` في `server.js` | يبثّ `log` لا `chat_reply`، والاستجابةُ `{ accepted: true }` رُدَّت قبله — فلا سبيلَ إلى المستخدم إلّا الشات، وهو الذي أُهمل. |

### ما تغيّر

- **`jcr.surgicalEdit`**: يقول إنّه تسلّم وبدأ حين يبدأ فوراً (`accepted && !waited` — فالمنتظِرُ سمع مركزَه، والمرفوضُ سمع الانشغال).
  وصار يُقيّد **نصَّ الطلب** في السجلّ الدائم (`goal`) بعد أن كان يُقيّده فارغاً، فإشعارُ «مهمّةٌ سقطت مع إعادة التشغيل»
  يخرج بتلميحٍ يعرّفه صاحبُه.
- **`jcr._reportIfCrashed(promise, roomName, lang)` جديدة**: تلفُّ `run` في المُطلِقَين كليهما (`executeMission` و`surgicalEdit`)،
  تقول العطبَ في الشات بسببه الحقيقيّ (١٦٠ حرفاً)، ثمّ **تُعيد رميَه** كي يبقى سطرُ الصفّ في السجلّ — لا نستبدل أثراً بأثر، نُضيف صوتاً.
- **حارسُ `/api/chat`**: يبقى سطرُ السجلّ ويُضاف `chat_reply` بلغة المستخدم.

### الاختبار الجديد (`tests/silenceGuards.test.mjs`، ٩)

العطبُ بعينه (المواصفةُ الطويلةُ تسلك مسارَ الإنتاج نفسَه فتُسمَع) · الإقرارُ بلغتَيه ولا يجتمع مع نصّ الانشغال ·
**المنتظِرُ في الصفّ يسمع مركزَه وحدَه** (ثلاثةُ مشاريعَ متزامنة — وهذا وحدَه ما يجعل شرطَ `!waited` مقيساً لا زخرفاً) ·
نصُّ الطلب في السجلّ الدائم · انهيارُ التعديل وانهيارُ البناء مسموعان بلغتَيهما · **إعادةُ الرمي ليست زخرفاً**
(`console.error` مُلتقَطٌ في الاختبار: الشاتُ سمع **وسجلُّ الخادم سمع**) · النجاحُ لا يستدعي رسالةَ انهيار · حارسُ `server.js`.

### الطفرات (١٢)

**١٢/١٢ أُمسكت** بعد جولتين. والناجيةُ الوحيدة كانت «الرسالةُ تُرسَل إلى غرفةٍ أخرى»: كنتُ أؤكّد على **نوع الحدث ونصِّه**
ولا أؤكّد على **عنوانه**. ورسالةٌ إلى غرفةٍ أخرى صمتٌ عند صاحبِ الطلب تماماً كغيابِها.
الدرس: **في نظامٍ يبثّ بالغرف، الغرفةُ جزءٌ من المُدَّعى — لا يكفي أن تُقال، بل لِمن تُقال.**

### عطبُ تلوّثٍ ثالث، ظهر أثناء الجولة وأُغلق

جولةُ التحقّق (ستُّ تشغيلاتٍ متتالية) أسقطت واحدةً في `chatResponseStage`. لم يكن ذلك «تذبذباً»:
مفتاحُ المحادثة في `scenario` **حتميّ** (`__chat1_u1__`)، فدوراتُ كلِّ جولةٍ تتراكم في
`memory/chat_memory.json` — قِيست **مئةُ رسالة** تحت ذلك المفتاح. وحين تجاوز التراكمُ
`CONTEXT_WINDOW = 40` انطلق الطيُّ، فسقط تأكيدُ «لا طيَّ على دورةٍ واحدة». JCR/31 كان قد ختم
**الرسائل** بطابع الجولة ولم يختم **المفتاح**، فبقي البابُ مفتوحاً. العلاج: `clearConversation`
قبل كلِّ سيناريو — الاختبارُ يبدأ من صفحةٍ بيضاء. ومثلُه لسجلّ المهامّ: `silenceGuards` أوّلُ اختبارٍ
يقود `surgicalEdit` الحقيقيَّ فيكتب في السجلّ الدائم المشترك، و`writeLedger` **يستبدل الملفَّ كاملاً**
بخريطةِ عمليّتِه — فيمحو صفَّ `missionLedger` الذي يجري بالتوازي. عُزل بسجلٍّ خاصٍّ عبر
`MISSION_LEDGER_PATH` واستيرادٍ ديناميكيّ (الوحدةُ تقرأ المسارَ مرّةً عند التحميل).

📌 **الدرس مُعمَّماً**: اختبارٌ يلمس مخزناً دائماً مشتركاً لا يكفيه ختمُ بياناته — يحتاج **مفتاحاً
خاصّاً بجولته أو ملفّاً خاصّاً بها**. وإلّا فالسقوطُ يأتي متأخّراً وبعيداً عن سببه.

### الأثر

- `jcr.js` **٩٧٤ ← ١٠١٠** (+٣٦، نموٌّ مقصودٌ مسجَّلٌ في الخريطة لا انزلاق). `server.js` **٣٨١٨ ← ٣٨٢٧**.
- الاختبارات **١٦٧٨ ← ١٦٨٧**، وأربعُ تشغيلاتٍ كاملةٍ متتالية خضراء بعد إغلاق التلوّثَين.

## «مواصفةٌ كاملة ⇒ بناءٌ لا ترقيع» (2026-09-06) — الوجهُ الثاني لبلاغ الصمت

بعد إغلاق الصمت قال المالك: «اصلح الصمت.. وكل عايق يعيق الانتاج بصورة مكتملة». والصمتُ كان **العَرَض**؛
هذا سببُه: مواصفةُ نظامٍ كاملةٍ لا تُعامَل بناءً أبداً ما دام في المشروع كود.

### مسارانِ يلتقيان عند الخطأ نفسِه (مقيسان)

١. المصنِّفُ يفشل على ٢٦ ألفَ حرف ويسقط إلى `{chat, 50%}` → فرعُ «مشروعٌ قائم + نيّةُ فعلٍ + ليس سؤالاً»
   → **تعديلٌ جراحيّ**: وثيقةُ نظامٍ تذهب إلى `patchEditPlan` لترقيعِ ملفّ.
٢. ولو صنّفها `build` لَما نجت: حارسُ (ب) يشترط فعلَ بناءٍ في **مستهلّ** الرسالة، و«أريد منك بناء…»
   ليس كذلك — فيردُّها إلى التعديل الجراحيّ عينِه. (طفرةٌ نجت حتّى كُتب له اختبارُه.)

### القاعدة — من قياسٍ لا من ذوق

| النصّ | حروف | بنودٌ مرقّمة |
|---|---|---|
| مواصفةُ نقاط البيع | ١٨٦٨٠ | ٣٦ |
| تعديلٌ قصير / متوسّط / أمرُ بناءٍ صريح / سؤال / جملةٌ إخباريّة / وصفُ مشروعٍ متوسّط / طلبٌ طويلٌ بلا ترقيم | ≤ ١٨٩ | **٠** |

`isFullSpecification(text)` في `textNormalizer.js`: `length ≥ 1200` **و** `numberedSections ≥ 6`.
والشرطان معاً لا أحدُهما — الطولُ وحدَه يلتقط طلبَ تعديلٍ مُسهَباً، والترقيمُ وحدَه يلتقط «١. أحمر ٢. أزرق».
ولا نُعمّم أكثر عمداً: خطأُ الإيجاب يحوّل تعديلاً مشروعاً إلى سؤالِ «أبني؟» على مشروعٍ قائم — وهو أضرُّ من خطأ السلب.

### ما تغيّر

- **`handleClassifiedIntent`**: `isSpec` يسبق التصنيفَ كلَّه، يتخطّى حارسَ (ب)، ويذهب إلى فرع البناء —
  **الذي يسأل ويعلّق الهدف**، فلا يُبنى فوق مشروعٍ قائمٍ بلا إذن. وسطرُ سجلٍّ يقول عددَ البنود والحروف
  ولماذا خالف المصنِّف.
- **سؤالُ التأكيد كان يُعيد الوثيقةَ كاملةً** على المستخدم (ظهر لحظةَ توجيهِ المواصفة إلى هذا الفرع):
  صار السطرَ الأوّلَ مقتضَباً (١٤٠ حرفاً) — **والهدفُ المحفوظ يبقى كاملاً**، فالقصُّ للعرض لا للتنفيذ.
- **`classifyIntent`**: صدرٌ محدود (١٥٠٠ حرفاً بنقاطٍ مُعلَنة) — إرسالُ ٢٦ ألفِ حرفٍ لجوابٍ من ثمانين رمزاً
  هو ما أسقطه أصلاً. وصار يقبل `llm` وسيطاً بافتراضيّ (سابقة JCR/30): آخرُ مستهلكٍ للمزوّد في الصنف
  كان بلا شقٍّ، فما كان القصُّ قابلاً للقياس.

### الطفرات (١٤)

**١٤/١٤ أُمسكت** بعد جولتين. والناجيتان كانتا فجوتَين حقيقيّتَين لا زخرفاً:
`!isSpec` على حارس (ب) — مسارٌ لم أطرقه لأنّ اختباراتي كلَّها تمرّ بمصنِّفٍ يقول `chat`؛ وقصُّ الصدر —
لا سبيلَ إلى قياسه بلا شقٍّ في `classifyIntent`. فالطفراتُ لم تكشف اختباراً ناقصاً فحسب، بل **تصميماً
غيرَ قابلٍ للقياس**، والعلاجُ كان في الكود لا في التأكيد.

### الأثر

- `jcr.js` **١٠١٠ ← ١٠١٧**، `intentHandlers.js` **٤١٦ ← ٤٣٨**، `textNormalizer.js` **٢٧٥ ← ٣٠٨`.
- الاختبارات **١٦٨٧ ← ١٦٩٢**، وثلاثُ تشغيلاتٍ كاملةٍ متتالية خضراء.

## قياسُ مواصفةِ نقاطِ البيع — «ماذا يخرج فعلاً من ٤٤ بنداً؟» (2026-09-06)

بأمر المالك «ابدا به» بعد إغلاق الصمت: تُشغَّل المواصفةُ نفسُها خلال `_runMissionNow` الحقيقيّة على مجلّدٍ فارغ
بلا مزوّد (المسارُ الحتميّ الذي يبني عليه النموذجُ حين يحضر)، وتُقاس ثلاثُ طبقات. النصُّ الحرفيّ لم ينجُ من
ضغط السجلّ فأُعيد تركيبُه من البنود المسجَّلة (٣٨٣٥ حرفاً، ٤٤ بنداً مرقّماً) — والشكلُ هو المقيس لا الحرف؛
المواصفةُ ملفُّ `tests/fixtures/pos_spec.txt` والأرقامُ كلُّها مثبَّتة في `tests/posSpecMeasure.test.mjs`.

### ما خرج فعلاً (قبل الإصلاح)

| الطبقة | المقيس |
|---|---|
| الفهم (المخطّط) | `_source: fallback`، `appType` = **أوّلُ ستّين حرفاً من الوثيقة**، مكوّنٌ وظيفيٌّ واحد: «الميزة الأساسية التفاعلية»، `keySections: []` |
| الفهم (النموذج) | `1 كيان (Item) • 1 دور (User) • 1 تدفّق` — بينما `conceptsInText` على النصّ نفسِه ترى **١٦ مفهوماً** (invoice, product, customer, payment, shift, tenant, accountant, storekeeper…) |
| المرجع | `saas_dashboard` (لِذكر «لوحة التحكم») و`referenceModel` له **`null`** — لا مرجعَ لنقاط البيع |
| التوجيه | **صفحةُ هبوطٍ تسويقيّة** من الـRegistry: nav · hero · logos · features · stats · testimonials · cta · footer — و`<h1>x</h1>` |
| الحكم | **PASS** — `requirements-verify: skipped` («صفحةٌ من بلوكات — لا مكوّناتٍ تُتحقَّق»)، `behavior-verify: 1/1` |
| الشات | «✅ اكتمل — ركّبنا صفحة احترافية **كاملة** لـ «أريد منك بناء» … ⚖️ التحقّق: ✓ – ✓» |
| البنود | **٣ من ٣٤** بندَ ميزةٍ لها أثرٌ — وهي ضجيج (`try/catch`، `lang`، `dir="rtl"`)؛ ١٠ بنودُ منهجيّةٍ لا تُقاس |

### لماذا؟ — أربعُ آليّاتٍ مقيسة، بترتيب السبب

١. **الاختصارُ التسويقيّ يسبق الفهمَ كلَّه ويُصيب بلفظٍ واحد.** `isMarketingPageGoal` تُطابق احتواءً على ٢٨ لفظاً
   قبل مطابقة الكلون وقبل أيّ شيء. اللفظُ المُصيبُ الوحيد: **«شركة»** — مرّتان: «كل شركة مستأجر مستقل» (بند ١٨)
   و«إعدادات الشركة» (بند ٣٢). وكان `appBlueprint.js` قد أزال «شركة» من قائمته لهذا السبب بعينه («كلماتُ العميل
   تنقض كلماتِ المنتج») — وبقيت في `blockRegistry.MARKETING_HINTS` نسخةٌ ثانية.
٢. **الفهمُ الحتميّ أعمى عن معجمه.** `fallbackModel` تعود بجدول الفئة أو بـ`User/Item`، ولا تستشير
   `conceptsInText` التي بُنيت في PM/3 على المعجم نفسِه ورأت ١٦ مفهوماً. الأداةُ موجودة والاحتياطُ لا يستعملها.
٣. **مساراتُ الاستراتيجيّة لا تتحقّق من المتطلّبات أبداً.** `strategyVerdict` تكتب `requirements-verify: skipped`
   حتميّاً، و`deliveryVerdict` تعدّ `skipped` «لا ينطبق» فلا يمنع PASS. بندٌ واحد أو أربعةٌ وأربعون سواء.
٤. **احتياطُ المخطّط يسمّي المشروعَ بمستهلّه.** `appType: goal.slice(0, 60)` — فيقول الشاتُ «صفحة لـ «أريد منك بناء»».

### المقياسُ المضادّ — لو صحّ التوجيه

كلونُ `jaola-pos` (يطابق بالكلمات: نقطة بيع/كاشير/وردية/pos) يمثّل **١١ من ٣٤** بنداً بأثرٍ لا اكتمال
(شاشة الكاشير، الوردية، الطابعة، المرتجعات، الإعدادات…) و**٢٣ بلا أثر** (RBAC الخماسيّ، الباركود، المخزون،
الموردون، الضريبة، الفروع، المستأجرون، التدقيق…) — **ويُعلن PASS أيضاً** بالسبب ٣ نفسِه. فالرقمُ ١١/٣٤ هو
نقطةُ الانطلاق الصادقة، لا خطَّ النهاية. وأثرُ حضور النموذج اللغويّ لا يُقاس في هذه البيئة — يُقال ولا يُدَّعى.

### ما أُصلح هنا (السبب ١ وحدَه — الخطفُ الإنتاجيّ بعينه)

`selectBuildStrategy`: **وثيقةٌ** (`isFullSpecification`) أو **مسارُ سيستم** (زرُّ الواجهة أو كلماتُ الطلب)
ليس بروشوراً مهما ورد فيه لفظُ عميل — إلّا أن يقول المخطّطُ نفسُه إنّ النوعَ بروشور/هبوط («موقع تعريفي لشركة
محاماة» تُصيب «محاماة» في لائحة السيستم، وتبقى بروشوراً). سطرُ سجلٍّ يقول لماذا لم يُطبَّق الاختصار.
النتيجةُ المقيسة بعده: الوثيقةُ → `clone: jaola-pos`، ولا ذكرَ للـRegistry، والبروشوراتُ الثلاثة كما كانت.

### الأسبابُ ٢ و٣ و٤ — ديونٌ مكتوبةٌ بترتيب الرافعة

- **PM/6 (المقترَح): الاحتياطُ يقرأ معجمَه** — `fallbackModel` تبذر الأدوارَ والكياناتِ من `conceptsInText` حين
  يغيب النموذج؛ فيصير الفهمُ ١٦ مفهوماً لا `User/Item`، وتصير `composeRequirements` عشراتِ متطلّباتٍ مسمّاة
  بدل أربعٍ عامّة، ويعمل `domain-fidelity` (اليومَ `applicable: false` لأنّ المتوقَّعَ فارغ).
- **PM/7 (المقترَح): لا PASS بمتطلّباتٍ متخطّاة** — حين تكون للوثيقة متطلّباتٌ (من الفهم أو المخطّط)
  فتخطّي `requirements-verify` على مسارات الاستراتيجيّة **UNVERIFIED** لا PASS؛ أو يُشغَّل المحقّقُ على
  ناتج الكلون فعلاً. الحكمُ الصادق: «بُني نظامُ نقاط بيعٍ يعمل لكنّه يغطّي ١١ من ٣٤ متطلّباً».
- **صغير:** `appType` الاحتياطيّ من السطر الأوّل لا أوّل ستّين حرفاً (كما فعلنا في سؤال التأكيد).
- **صغير:** توحيدُ `MARKETING_HINTS` مع قرار `appBlueprint` (إسقاطُ ألفاظ العميل) — نسختان لقائمةٍ واحدة.

## PM/6 — «الاحتياطُ يقرأ معجمَه — الفهمُ بلا نموذجٍ لغويّ يرى ما يراه المعجم» (2026-09-06)

السببُ الثاني من قياس مواصفة نقاط البيع. الأداةُ (`conceptsInText`، PM/3) كانت ترى في النصّ ١٦ مفهوماً،
والاحتياطُ (`fallbackModel`) يعود بـ`User/Item` لأنّه لا يستشيرها — فتخرج أربعُ متطلّباتٍ عامّة لأربعةٍ
وأربعين بنداً، و`domain-fidelity` معطَّلٌ لأنّ المتوقَّعَ فارغ.

### القياسُ قبل الكود (مسوّدةٌ خارج المستودع ثمّ `lexiconFallback.test.mjs`)

| | قبل | بعد |
|---|---|---|
| الفهم | `User / Item / الفعل الأساسي` | ٤ أدوار (staff, customer, admin, tenant) / ٦ كيانات (product, invoice, currency, payment, shift, account) |
| المتطلّباتُ المسمّاة (`composeRequirements`) | ٤ | **١٢** |
| `domain-fidelity` | `applicable: false` | يعمل — متوقَّع ≥ ٨، مفقود ٠ |
| اختيارُ الكلون | بالكلمات وحدَها | `jaola-pos` بالكلمات **والفهم**، و١٤ كلوناً خاطئاً يُرفض بالفيتو |

### سؤالُ التصميم الذي كشفه القياس — ولماذا التكرار

`conceptsInText` تجمع بترتيب **طول المرادف** (الفهرسُ مرتّبٌ الأطولُ أوّلاً)، وسقوفُ التطبيع (٤ أدوار، ٦ كيانات)
تقطع الزائد — فمن ١٦ مفهوماً وصل النموذجَ ١٠، وسقط `admin`/`accountant`/`payment` لا لأنّها أقلُّ شأناً بل لأنّ
مرادفاتها أقصر. الإشارةُ الصادقة بلا نموذجٍ لغويّ هي **التكرار**: في المواصفة «منتج» ١٢ مرّةً و«فاتورة» ١١
و«موظّف» ٧ و«عملة» ٢. `conceptFrequencies` تعدّ كلَّ مفهومٍ بمجموع مرادفاته ككلماتٍ كاملة، و`lexiconModel`
ترتّب بالتكرار ثمّ بالاسم (حتميّةٌ عند التساوي).

### ما تغيّر

- `conceptFrequencies(text)` + `lexiconModel(text)` في `projectModel.js` — نقيّتان، على الفهرس نفسِه، والعامُّ مستبعَد.
- `fallbackModel`: جدولُ الفئة أوّلاً كما كان (منسَّقٌ بيد إنسان)؛ ثمّ المعجمُ إن رأى شيئاً — والتدفّقُ الوحيدُ من مكوّنات
  المخطّط كما كان لكنّ فاعلَه أهمُّ الأدوار لا `User` المكتوب؛ ثمّ الحدُّ الأدنى القديم للنصّ الذي لا يسمّي شيئاً
  («أداة حاسبة زكاة» تبقى `User/Item` بعينها).
- `mergeProjectModel`: الاسمُ القائم يبقى عند التطابق (المطابقةُ لا تفرّق الحالة أصلاً) — مفتاحُ معجمٍ `admin` لا يُطفئ
  لقبَ المرجع `Admin`؛ والوصفُ الفارغ لا يمحو وصفاً قائماً. ظهر هذا فوراً في اختبار التاكسي (PM/4).

### الاختبار الجديد (`tests/lexiconFallback.test.mjs`، ٥)

التكرارُ إشارةُ الأهمّيّة · نموذجُ المعجم مرتّباً وفارغاً لنصٍّ بلا مفاهيم · الاحتياطُ بلا مزوّد (المواصفةُ ٤/٦،
القصيرُ `User/Item`، جدولُ الفئة أولى) · المستهلكون (١٢ متطلّباً، `domain-fidelity` قابلٌ للتطبيق) ·
**والمرحلةُ** (`understandGoal` تخزّن وتبثّ «6 كيان • 4 دور» لا «1 كيان (Item)») — درسُ PM/5 مطبَّقٌ من البداية.

### إعادةُ تثبيتٍ بسببٍ مكتوب

`GENERIC_CONCEPTS.has` ٢ ← ٣ (مجموعةٌ ثالثة على المعجم نفسِه)؛ «متجر عطور» بلا مزوّد يُشتقّ `store` لا `Item`؛
الحقيقةُ المقيسة ٢ في `posSpecMeasure` تُثبَّت بصورتها الجديدة (المخطّطُ ما زال يسمّي بأوّل ستّين حرفاً — ديْنٌ باقٍ).

### حدودٌ مكتوبة

- المرادفاتُ المتداخلة تُعدّ لكلا مفهومَيها («مدير أسطول» تعدّ `fleet_manager` و`admin`) — للترتيب يكفي.
- «موقع مطعم» يُقرأ `location` (المعجمُ لا يميّز الموقعَ الإلكترونيّ من المكان) — عيبُ معجمٍ قائمٌ صار الآن
  **مرئيّاً في النموذج** بعد أن كان مدفوناً؛ يُعالَج في المعجم لا هنا.
- `modelProjectType` على مواصفة نقاط البيع يقول `ecommerce` (product + customer + invoice) — قريبٌ لا دقيق؛
  توقيعُ `pos` ينتظر دليلاً من مشاريعَ حقيقيّة (حدُّ PM/5 نفسُه).

## PM/7 — «لا PASS بمتطلّباتٍ متخطّاة — الغيابُ قاطعٌ والحضورُ أثرٌ لا تنفيذ» (2026-09-06)

السببُ الثالث من قياس مواصفة نقاط البيع. مساراتُ الاستراتيجيّة (Registry/Clone/React) كانت تسجّل
`requirements-verify: skipped` **حتميّاً** بسببٍ مكتوب («لا محقّقَ متطلّبات على هذا المسار») — وكان ذلك صادقاً
حين كان الفهمُ `User/Item`. بعد PM/6 صار للوثيقة ١٢ متطلّباً مسمّى، فصار تخطّيها كذبةً مقيسة: كلونُ نقاط
البيع يمثّل سبعةً منها ولا يذكر `customer`/`tenant`/`account` بحرف — ويُعلَن PASS.

### القياسُ قبل الكود (مسوّدةٌ خارج المستودع ثمّ `requirementsTrace.test.mjs`)

| المسار | ما يصل البوّابة قبل | ما يُقاس | بعد |
|---|---|---|---|
| Clone (`jaola-pos` على الوثيقة) | لا شيء → `skipped` → **PASS** | ١٥ متطلّباً من الفهم المدمَج: ٧ لها أثر، ٣ بلا أثر (customer, tenant, account)، ٥ لا تُتتبَّع (٤ تدفّقات + «الفعل الأساسي») | **FAILED** بالأسماء |
| Clone (`jaola-store` على «متجر عطور») | `skipped` → PASS | Customer/Admin/Product/Order كلُّها لها أثر؛ تدفّقان لا يُتتبَّعان | PASS — `4/4 له أثر — أثرٌ لا تنفيذ` |
| Registry (`Visitor`) | `skipped` | `Visitor` → `user` عامٌّ فلا يُتتبَّع | `skipped` بعدده (`1 متطلّب بلا مفردةٍ تُتتبَّع`) |
| React (فهمُ تاكسي على مطعم) | `skipped` → PASS | Driver/trip بلا أثر في المعاينة الثابتة | FAILED بالأسماء (والسلوكُ يقول `domain-fidelity` من جهته) |

### سؤالُ التصميم — UNVERIFIED أم فحصٌ حتميّ؟ ولماذا `pass` مع «أثرٌ لا تنفيذ»

الخياران اللذان طُرحا: تسجيلُ `unverified` (صادقٌ لكنّه يخفي ما يمكن معرفتُه بلا مزوّد)، أو فحصٌ حتميّ بمعجم
PM/1 على الملفّات. اختير الثاني بحدوده مكتوبةً في الدالّة نفسِها (`traceRequirements`):
- **الغيابُ قاطع**: مفهومٌ لا تنطق به الملفّاتُ لم يُنفَّذ فيها — `fail` بأسمائه. هذا هو ما كان يُخفيه الـ`skipped`.
- **الحضورُ أثرٌ لا تنفيذ**: لفظُ «عميل» في الصفحة لا يُثبت شاشةَ عميلٍ تعمل. لذلك تفصيلُ `pass` يقولها بحرفها
  («N/N له أثر — أثرٌ لا تنفيذ»)، والحكمُ على التنفيذ يبقى للمحقّق السلوكيّ (`behavior-verify`، PM/2–3) ولمحقّق
  المتطلّبات بمزوّد على مسار الحلقة. لم يُختَر `unverified` للحاضر لأنّه كان سيجعل كلَّ كلونٍ سليم UNVERIFIED
  إلى الأبد — فيصير الحكمُ ضجيجاً لا يقرؤه أحد؛ والبوّابةُ السلوكيّة هي من تحكم على التنفيذ أصلاً.
- **ما لا يُتتبَّع لا يُحسب له ولا عليه**: العامُّ (`user`/`item` — `conceptsInText` تستبعده أصلاً فغيابُه ليس ثغرة)،
  والتدفّقُ (انتقالُ حالةٍ لا مفردة — ولو حمل اسمُه لفظاً؛ لذلك يسم `composeRequirements` كلَّ متطلّبٍ بـ`_kind`)،
  وما خارج المعجم. كلُّها لا يُتتبَّع → `skipped` بعددها، لا `pass` ولا `fail`.

### ما تغيّر

- `requirementsVerifier.js`: `composeRequirements` يسم متطلّباتِ الفهم بـ`_kind` (role/entity/flow)؛ `traceRequirements(requirements, files)` نقيّة على معجم `projectModel` (`conceptOf` + `conceptsInText` + `isGenericConcept` المصدَّرةُ لهذا).
- `stages/verify.js`: `requirementsTraceOutcome(requirements, files, note)` تُترجم الأثرَ حكماً؛ `strategyVerdict` يقبل `requirements` و`files` ويسجّل ما وجد — بلا متطلّباتٍ أو ملفّات يبقى `skipped` بالسبب كما كان.
- البناةُ الثلاثة يمرّرون: الكلونُ متطلّباتِ **الفهم المدمَج** (ما فهمه جولا + نموذجُ الكلون) على **ما وصل القرص** (`readProjectFiles`)؛ الـRegistry نموذجَ الزائر على الملفّات المركَّبة؛ React فهمَ الذاكرة على المعاينة الثابتة (ما يقرؤه المحقّق).

### الاختبار الجديد (`tests/requirementsTrace.test.mjs`، ٦)

المتتبِّعُ النقيّ (بما فيه: التدفّقُ لا يُتتبَّع ولو حمل اسمُه مفردة، وبلا `_kind` يقرّر المعجم) · حكمُ البوّابة بفروعه الأربعة وتأثيرُه في `strategyVerdict` · وسمُ `_kind` · **المسارُ كاملاً** (الوثيقة → كلون نقاط البيع → FAILED بالأسماء في السجلّ والشات) · React بفهمٍ مغاير وبلا فهم · الحدود. **الطفرات ١٦/١٦ أُمسكت.**

### إعادةُ تثبيتٍ بسببٍ مكتوب

- `strategyVerdict.test`: تفصيلُ الـRegistry يحمل عددَ ما لا يُتتبَّع؛ تفصيلُ الكلون السليم `4/4 له أثر…` وسطرُ التحقّق `requirements-verify ✓`؛ الكلونُ المعطوب يُسقطه المتطلّباتُ قبل السلوك (الإفراغُ أزال مفرداتِ المتجر أيضاً — Admin وحدَه بقي في `styles.css`)؛ توقيعُ `strategyVerdict`.
- `cloneStage.test`: البوّاباتُ `['pass','pass','pass']` لا `['pass','skipped','pass']`.
- `domainFidelity.test`: `GENERIC_CONCEPTS.has` ٣ ← ٤ (تصديرُ `isGenericConcept`).
- `posSpecMeasure.test` الحقيقةُ ٣: من «PASS مهما بلغت الوثيقة» إلى «FAILED بأسماء الثلاثة» — والصورةُ القديمة (بلا متطلّباتٍ تُمرَّر → skipped) تبقى مؤكَّدةً كما كانت.
- `renderConfigShape.test`: مرساتا السطر `buildFromClone.js:178→180` و`buildFromRegistry.js:62→65` — أسطرُ التمرير أُضيفت فوق نداء `prepareRenderDeploy`؛ الجوابُ (`false`) لم يتغيّر.

### حدودٌ مكتوبة

- التتبّعُ بالمفردات لا يرى التدفّقات (٥ من ١٥ في الوثيقة) ولا يميّز بين ذكرٍ عابر وشاشةٍ عاملة — ذلك للسلوك وللمحقّق بمزوّد.
- الكلونُ يمرّر `blueprint = null` إلى `composeRequirements` (المخطّطُ لا يصل `buildFromClone`)؛ مكوّنُه الوحيدُ بلا مزوّد «الميزة الأساسية التفاعلية» عامٌّ لا يُتتبَّع أصلاً — فلا فرقَ مقيس.
- النتيجةُ الصادقة على مواصفة نقاط البيع الآن: **FAILED — ٣ بلا أثر** + `role-coverage` من السلوك. هذا أوّلُ حكمٍ يقول للمستخدم ما لم يُبنَ له بالاسم؛ وسدُّ الفجوة (بناءُ الشاشات الناقصة على الكلون) خطوةٌ تاليةٌ مقيسةُ الحدود لا هنا.

## PM/8 — «الحكمُ يسمّي الفجوةَ ثمّ يسدّها — جولةُ إكمالٍ واحدة على الكلون بالرقعة لا بإعادة الكتابة» (2026-09-06)

بعد PM/7 صار الكلونُ يقول للمستخدم «٣ متطلّب بلا أثر: شاشة customer، شاشة tenant، بيانات account» — **ويقف**. قِيس قبل
الكتابة ما يفعله المسارُ بعد FAILED (لا شيء: يعلن ويعود)، وما أقربُ آليّةٍ قائمة تبني الناقص، وهل تصلح للكلون.

### القياسُ قبل الكود

| السؤال | المقيس |
|---|---|
| ماذا بعد FAILED على الكلون؟ | لا شيء — الحكمُ يُبثّ وتعود الدالّة |
| أقربُ آليّةِ إكمالٍ قائمة | `requirementsVerify` على مسار الحلقة: حتّى ٣ جولات بـ`agents.coreEditCodePlan` |
| هل تصلح للكلون؟ | **لا**: `coreEditCodePlan` يقصّ مدخلَ كلِّ ملفٍّ عند **٨٠٠٠ حرف** ويعيد الملفَّ **كاملاً**؛ `app.js` في `jaola-pos` ١٣٫٧ ألفاً وفي `jaola-store` ١٥٫٥ ألفاً — أي عينُ «الملف الكبير يُبتَر فتُفقد الدوال» الممنوعِ في `buildFromClone` منذ البصمة |
| الأداةُ الصالحة | رقعةُ الكلون نفسُها: `patchEditPlan` (SEARCH/REPLACE؛ «للإضافة: SEARCH سطرٌ مرجعيّ وREPLACE = نفسُه + الإضافة») — لا بترَ، وما لا يُذكر لا يُلمس |
| حارسُها | حارسُ البصمة بعينه: خطُّ أساسٍ للفشل القائم، `guardFiles` + `ensureEditIntegrity`، فقدُ دالّة، تحقّقٌ سلوكيّ، استرجاع |

### ما تغيّر — `buildFromClone(clone, goal, ctx, reporter, { complete = patchEditPlan } = {})`

بعد إعداد النشر وقبل الحكم: `traceRequirements` على لقطة القرص → إن كان ثمّة بلا أثر: `buildFixInstruction` بأسماء
المتطلّبات **وسلوكها** المطلوب (من `composeRequirements`) وإرشادِ النموذج المدمَج → `complete(instruction, snapshot, lang)`
**مرّةً واحدة** → الحارسُ ثمّ الكتابة → فقدُ دالّةٍ أو عطلٌ سلوكيّ **جديد** (لا القائم في خطّ الأساس) → **استرجاعُ لقطة القرص**
(لا القالبِ النظيف — فالبصمةُ والتلميعُ محفوظان) → وإلّا إعادةُ تتبّعٍ فـ«✅ أُكمل (N/M): … — وبقي بلا أثر: …». الحكمُ يُحسب بعد
ذلك على ما وصل القرص كما في PM/7. ما أُكمل يُقال في الشات بالاسم قبل سطر الحكم. بلا مزوّد: «ℹ️ لم تُطبَّق رقعةٌ» والحكمُ كما كان.

### الاختبار الجديد (`tests/cloneCompletion.test.mjs`، ٨)

القياسُ مثبَّتاً (القصُّ عند ٨٠٠٠ وأحجامُ الكلونات) · جولةٌ واحدة بالتعليمة الصحيحة على ملفّات القرص الثلاثة، «أُكمل (2/3)» والحكمُ يقول ما بقي · رقعةٌ تسدّ الثلاثة → `10/10 له أثر` · **حارسُ الارتداد** بحالتَيه (تبسيطٌ يحذف `csvDownload` ونداءَه — لا يراه السلوكُ ويراه عقدُ الحفظ؛ وزرٌّ إلى دالّةٍ غائبة) والاسترجاعُ حرفاً بحرف · الحارسُ قبل الكتابة (DOCTYPE يُستعاد) ورقعةٌ فارغة تُعدّ غيرَ مطبَّقة · بلا مزوّد، وبلا فجوةٍ لا نداء · المسارُ كاملاً · الحدود. **الطفرات ١٢/١٢ أُمسكت** (بعد إصلاح ثغرةٍ في الاختبار نفسِه جعلت الجولةَ الأولى تُمسك الطفراتِ على قاعدةٍ حمراء — لا يُحتسب إمساكٌ على قاعدةٍ لا تخضرّ).

### حدودٌ مكتوبة

- ~~**`ensureEditIntegrity` لا يستعيد رابطَ التنسيق المحلّيّ على كلونٍ ملمَّع**~~ — أُغلق في القسم التالي (حارسُ التعديل والصفحاتُ الملمَّعة).
- الإكمالُ بمزوّد لم يُقَس هنا (لا مفتاح) — المقيسُ هو العقد: ما يُطلب، وما يُرفض، وما يُقال. جودةُ الشاشة المضافة شأنُ النموذج والسلوك.
- جولةٌ واحدة عمداً: ثلاثُ جولاتٍ على قالبٍ يعمل مخاطرةٌ لا يعوّضها تحقّقٌ بالمفردات.
- «أُكمل» تعني **صار له أثر** (PM/7: أثرٌ لا تنفيذ) — الحكمُ على التنفيذ يبقى للسلوك.

## حارسُ التعديل والصفحاتُ الملمَّعة — «تنسيقُ التلميع ليس تنسيقَ الصفحة» (2026-09-06)

الديْنُ الذي كشفه قياسُ PM/8: على صفحةٍ ملمَّعة، تعديلٌ يُسقط `<link rel="stylesheet" href="styles.css">` لا يُستعاد —
والعطبُ الحرفيّ المسجَّل في رأس `editIntegrity.test.mjs` (test17-7-5: «أضف نظام جلسات فيديو» أعاد الصفحةَ بلا رابط
فظهر الموقعُ HTML خاماً) كان سيعود بعينه على كلِّ كلونٍ أو صفحة Registry تُعدَّل لاحقاً.

### القياسُ قبل الكود

| السؤال | المقيس |
|---|---|
| مواضعُ `ensureEditIntegrity` | ستّة: حلقةُ التسليم (`jcr` guard-and-write)، إكمالُ الحلقة (`requirementsVerify`)، التعديلُ الجراحيّ، إصلاحُ السلوك (`verify`)، بصمةُ الكلون، إكمالُ الكلون (PM/8) |
| من ينتج صفحاتٍ ملمَّعة | `buildFromClone` و`buildFromRegistry` (بعد البصمة — فالبصمةُ لا تراها) |
| المواضعُ التي تصلها صفحةٌ ملمَّعة | **ثلاثة**: التعديلُ الجراحيّ على مشروعٍ قائم، إصلاحُ السلوك، إكمالُ الكلون |
| ما الذي يخدع الشرط | لا الروابطُ البعيدة — `localHrefOf` يستبعدها أصلاً فـ`newLinks` محلّيّةٌ فقط — بل `<style data-jaola-polish>` وحدَه يُرضي `!/<style/` |

### الإصلاحُ الأضيق

`ownInlineStyle = /<style\b(?![^>]*data-jaola-polish)[^>]*>/` — تنسيقٌ مضمَّن **بلا علامة التلميع** هو ما يُعدّ تنسيقَ
الصفحة. لا تغييرَ في أيّ فرعٍ آخر من الحارس. الاختبارُ أُعاد العطبَ أوّلاً (سقط على القاعدة القديمة) ثمّ اخضرّ؛ وحالةُ
«صفحةٌ ملمَّعة بتنسيقها المضمَّن» تبقى بلا حقنٍ كما كان. حقيقةُ PM/8 المقيسة في `cloneCompletion.test` انقلبت من
«لا يُستعاد — ديْن» إلى «يُستعاد» بسببٍ مكتوب.

## الاختصارُ التسويقيّ يقرأ الفهمَ قبل الكلمات — «شركة تصف الطالبَ لا المطلوب» في الموضع الثاني (2026-09-06)

الديْنُ الرابع من قياس مواصفة نقاط البيع (#567): `blockRegistry.MARKETING_HINTS` نسخةٌ ثانية من قائمةٍ أصلحها `appBlueprint`
بمبدأٍ مكتوب هناك («كلماتُ العميل تنقض كلماتِ المنتج»)، و`isMarketingPageGoal` كانت تقرأ حكمَ المخطّط بوجهٍ واحد (بروشور → نعم)
فتُرجَّح الكلماتُ على الفهم كلَّما قال المخطّطُ «تطبيق».

### القياسُ قبل الكود (مسوّدةٌ خارج المستودع ثمّ `marketingShortcut.test.mjs`)

تسعةُ أهدافٍ قصيرة تذكر لفظَ عميل، بلا مزوّد، عبر `_runMissionNow` على مجلّدٍ فارغ:

| الهدف | `kind` | ما اختير |
|---|---|---|
| متجر إلكتروني لشركة ملابس — المثالُ الذي كتبه `appBlueprint` نفسُه | webapp | **Registry (صفحة هبوط)** |
| تطبيق حجز مواعيد لشركة صيانة | webapp | **Registry** |
| لوحة تحكم مبيعات لشركة عقارات | webapp | **Registry** |
| تطبيق توصيل طلبات مع تتبع للسائق لشركة لوجستية | webapp | **Registry** |
| منصة لستارت اب لتوصيل الطعام | webapp | **Registry** |
| company dashboard for inventory tracking | webapp | **Registry** |
| نظام إدارة طلبات لوكالة سفر | webapp | كلون ERP — أنقذه `inferTrack` (لفظُ سيستم) لا الفهمُ |
| موقع تعريفي لشركة محاماة · صفحة هبوط لشركة استشارات | brochure | Registry — صواب |

**سبعةٌ من تسعة**: المستخدمُ يطلب متجراً أو تطبيقاً فيستلم بروشوراً بحكم PASS. أمّا `appType` (أوّلُ ستّين حرفاً) فأثرُه سطرُ
سجلٍّ وسياقُ الكودر — أدنى رافعةً، بقي ديْناً مكتوباً.

### الإصلاحُ الأضيق

- `isMarketingPageGoal`: حكمُ المخطّط يُقرأ بوجهَيه — `webapp`/`tool` → `false` قبل أيّ كلمة (الفهمُ قبل الكلمات، المبدأُ نفسُه الذي
  طُبّق في PM/1 على الاختيار وفي #567 على الوثائق). بلا مخطّطٍ تحكم الكلمات.
- `MARKETING_HINTS`: خرجت ألفاظُ العميل السبعة (شركة، company، corporate، وكالة، agency، startup، ستارت اب). بقيت ألفاظُ المنتج.
- الاستثناءُ المكتوب في `selectBuildStrategy` (وثيقة/سيستم) باقٍ حارساً ثانياً — لا يُستدعى للوثيقة بعد اليوم لأنّ الاختصارَ لا يُصيبها أصلاً.

### الاختبار الجديد (`tests/marketingShortcut.test.mjs`، ٥)

الحقيقةُ المقيسة مغلقةً (الستّة webapp وليست تسويقيّة) · المسارُ كاملاً (لا Registry في السجلّ) · البروشوراتُ الأربعة كما كانت
(Registry) · الدالّةُ النقيّة بوجهَي المخطّط وبلا مخطّط · الحدود. **الطفرات ٧/٧ أُمسكت.** أُعيد تثبيتُ حقيقتَي `posSpecMeasure` ١ و٤
بسببٍ مكتوب (الإصابةُ الزائفة صارت غيابَ إصابة؛ وسطرُ الاستثناء لا يُبثّ).

### حدودٌ مكتوبة

- المطابقةُ ما زالت احتواءً بلا حدود كلمات (`hay.includes`) — درسُ `appBlueprint` الثاني (`app` داخل `happy`) لم يُطبَّق هنا؛ لم يُقَس له عطب، فلم يُمَسّ.
- «قريبا» و«coming soon» بقيا (صفحةُ «قريباً» منتجٌ) — لفظُ «قريباً» العابر في وثيقةٍ يحميه الآن حكمُ المخطّط لا القائمة.

## PM/9 — «الحكمُ بلغة الوثيقة — بنودُ المواصفة تُتتبَّع بمفرداتها، والإكمالُ يطلبها بنصّها» (2026-09-06)

بعد إغلاق أسباب #567 الأربعة وديْنَين، أُعيد القياسُ الشامل نفسُه (مواصفةُ نقاط البيع عبر المسار كاملاً بلا مزوّد؛ الجدولُ في
`PRODUCT_MIND.md` تحت «قياسٌ بعد»). الفهمُ ٦ كيانات/٤ أدوار، الاختيارُ `jaola-pos` بالفهم، الحكمُ FAILED — لكنّ الحكمَ يتكلّم
**لغةَ المعجم**: «٣ متطلّبات بلا أثر» — بينما المستخدمُ كتب **٤٤ بنداً مرقّماً**، وبمفردات عناوينها **١٣ فقط** لها أثرٌ في
ملفّات الكلون (الباركود، المخزون، المشتريات، الموردون، الخصومات، الضريبة، الفروع، المستأجرون، التدقيق… بلا أثر). المعجمُ يرى
ما يعرفه؛ الوثيقةُ تعرف ما طُلب.

### القياسُ قبل الكود

| | المقيس |
|---|---|
| بنودُ الوثيقة | ٤٤ سطراً مرقّماً (٣٤ ميزة + ١٠ منهجيّة/مراحل) — `specSections` تقرؤها برقمٍ وعنوانٍ ومتن |
| أثرُها في `jaola-pos` بمفردات العنوان (تطبيعُ المعجم نفسُه، بلا كلمات إطار، كلمةٌ كاملة) | **١٣/٤٤** — الأوّلُ بلا أثر: ١ الصلاحيات، ٣ الباركود، ٧ المرتجعات، ٨ دفتر المخزون، ٩ المشتريات، ١٠ الموردون… |
| ما يُقال للمستخدم قبل | «3 متطلّب بلا أثر: شاشة customer، شاشة tenant، بيانات account» |
| ما يُقال بعد | «31 بنداً من 44 في وثيقتك بلا أثر: 1 الصلاحيات والأدوار (RBAC)، 3 الباركود، 7 المرتجعات، 8 دفتر المخزون، 9 المشتريات، 10 الموردون +25 (13/44 له أثر — أثرٌ لا تنفيذ؛ مفاهيمُ الفهم 7/10)» |

### ما تغيّر

- `textNormalizer.js`: `specSections(text)` → `[{ n, title, body }]` (أرقامٌ عربيّة أو لاتينيّة؛ المتنُ الأسطرُ التالية حتّى البند التالي).
- `requirementsVerifier.js`: `traceSections(sections, files)` — مفرداتُ **العنوان** بعد التطبيع، ≥٣ أحرف، بلا كلمات إطار (`SECTION_STOPWORDS`)، كلمةً كاملةً في الملفّات: له أثر / بلا أثر / لا يُتتبَّع (عنوانٌ من كلمات إطارٍ فقط). `sectionLabel` («٣ الباركود»). `buildSectionFixInstruction(missing, sections, model, {limit: 8})` — البنودُ **بنصّها كما كُتبت** لا بصياغةٍ عامّة، ثمانيةٌ في الجولة و«وبقي N».
- `stages/verify.js`: `requirementsTraceOutcome(…, sections)` — إن كان للوثيقة بنودٌ تُتتبَّع فالحكمُ بلغتها (الغائبُ برقمه وعنوانه، ستّةٌ ثمّ +N، ضمن ٣٠٠ حرف)، ومفاهيمُ الفهم ذيلاً؛ وإلّا المعجمُ كما في PM/7. `strategyVerdict({ …, sections })`.
- `buildFromClone`: `sections = isFullSpecification(goal) ? specSections(goal) : []`؛ جولةُ الإكمال (PM/8) تطلب البنودَ الغائبة بنصّها وتُعيد تتبّعَها («✅ أُكمل من وثيقتك (3/31): … — وبقي بلا أثر 28 بنداً»)؛ الحكمُ بعدها بلغة الوثيقة. `buildReact` يمرّر البنودَ كذلك؛ الـRegistry لا (البروشورُ ليس وثيقة).

### الاختبار الجديد (`tests/specVerdict.test.mjs`، ٦)

قراءةُ البنود (٤٤، أرقامٌ عربيّة، جملةٌ → لا بنود) · التتبّعُ على الكلون ١٣/٤٤ بأسماء الغائب والعنوانُ من كلمات الإطار لا يُتتبَّع · التعليمةُ بنصّ الوثيقة بسقفٍ وبقيّة · حكمُ البوّابة (fail ضمن ٣٠٠ حرف / pass / عودةٌ إلى المعجم / skipped) · **الكلونُ بوثيقة**: التعليمةُ من النصّ، رقعةٌ تُعطي أثراً لثلاثة → «أُكمل من وثيقتك (3/31)» والحكمُ «28 بنداً من 44» والشات · الحدود. **الطفرات ١٦/١٦ أُمسكت.**

### إعادةُ تثبيتٍ بسببٍ مكتوب

- `cloneCompletion.test`: اختباراتُ PM/8 تعمل بهدفٍ قصير (لا وثيقة) فتبقى بلغة المعجم؛ المسارُ الكامل بالوثيقة انتقل إلى لغتها؛ مواضعُ `'CloneCompletion'` ٥ ← ٦.
- `requirementsTrace.test`: تفصيلُ المسار الكامل صار بلغة الوثيقة؛ سطرا الاستيراد. `strategyVerdict.test`: التوقيعُ بـ`sections`.

### حدودٌ مكتوبة

- الأثرُ بمفردات **العنوان** لا المتن — عنوانُ «الأمان:» يبحث عن «امان» فقط؛ متنُه (bcrypt، CSRF) لا يُقرأ. توسيعُه إلى المتن يرفع الإصاباتِ الزائفة («المرحلة 3: المالية (فواتير…)» تُحسب بأثر «فواتير») — بنودُ المراحل والمنهجيّة (٣٥–٤٤) تُعامَل كباقي البنود؛ تمييزُها قرارٌ مؤجَّل بلا دليل.
- التتبّعُ يقول «له أثر» بحرف — «7 المرتجعات» قبل الإكمال بلا أثر رغم أنّ جدولَ #567 اليدويّ عدّها بأثرٍ عبر `return` في JS؛ التطبيعُ الحاليّ أصدق.
- الإكمالُ بمزوّد لم يُقَس (لا مفتاح)؛ المقيسُ ما يُطلب (نصُّ ثمانية بنود ~١١٠٠ حرف) وما يُقال بعدُ. حلقةُ التسليم (`requirementsVerify` بمزوّد) لم تُلمَس — تتلقّى المعجمَ كما كانت.

## PM/10 — «حالةُ المهمّة ≠ حكمُ المنتج» لفظاً أيضاً (2026-09-06)

من جدول «قياسٌ بعد» (`PRODUCT_MIND.md`) بقيت ثلاثةُ بنود «يُقال زوراً أو لا يُقال». قِيس أثرُ كلٍّ على ما يراه المستخدم
وعددُ مواضعه: (٢) «✨ نجاح» سطرٌ مكتوب في **أربعة** مواضع — البناةُ الثلاثة يبثّونه **قبل** `⚖️ الحكم: FAILED` مباشرةً،
و`_runMissionNow` يبثّه **بعد** `⚖️ الحكم: UNVERIFIED` في كلِّ بناءٍ بلا مزوّد؛ (٣) `behaviorOutcome` يجمع **أسماءَ** الفحوص
الساقطة فيصل الشاتَ `role-coverage` بينما تفصيلُ الفحص يقول «أدوار بلا واجهة/تمثيل: staff، tenant» — موضعٌ واحد يمسّ كلَّ
حكم FAILED؛ (٤) `appType` أوّلُ ستّين حرفاً — سطرُ سجلٍّ واحد. اختير ٢ و٣ معاً: سطرٌ واحد كلٌّ، وكلاهما «حالةُ المهمّة ≠ حكمُ المنتج».

### ما تغيّر

- `stages/reportMissionSuccess.js`: `kernelOutcomeLine(verdict, note = '')` — بلا حكمٍ أو PASS → `✨ نجاح${note}`؛ FAILED →
  `⚠️ اكتملت المهمّة — ولم يجتز المنتجُ التحقّق${note}`؛ وما سواهما (UNVERIFIED أو مجهول) → `☑️ اكتملت المهمّة — ولم يكتمل التحقّق${note}`.
- `buildFromClone` / `buildFromRegistry` / `buildReact`: السطرُ الختاميّ عبر `kernelOutcomeLine(verdict, note)` بملحقاتها كما كانت
  (« (قالب jaola عامل)»، « (إعادة تركيب من Registry)»، بلا ملحق). `jcr.js#_runMissionNow`: `execResult.success ? kernelOutcomeLine(execResult.verdict) : '❌ فشل'`.
- `stages/verify.js#behaviorOutcome`: الفجوةُ بتفصيل فحصها حتّى « — » (الشرحُ للمُصلِح) وبلا نقطةٍ ختاميّة؛ الفحصُ بلا تفصيلٍ يبقى باسمه؛
  الفاصلُ بين الفحوص «؛» لأنّ التفاصيلَ تحوي «،». `selectBuildStrategy` يقرأ `checks[].name` مباشرةً — لم يُمسّ.

### الاختبار الجديد (`tests/verdictWords.test.mjs`، ٤)

`kernelOutcomeLine` بحالاته الخمس · `behaviorOutcome` بتفصيلٍ وبلا تفصيل وبلا «—» · المسارُ كاملاً (كلون نقاط البيع بوثيقة): لا «✨ نجاح»
في السجلّ، السطرُ السابقُ للحكم بعينه، والشاتُ يسمّي `staff، tenant` لا `role-coverage` · حلقةُ التسليم بلا مزوّد تختم بـ«☑️ … لم يكتمل التحقّق».
**الطفرات ١٤/١٤ أُمسكت** (الـ١٢ — النقطةُ الختاميّة — نجت أوّلاً حتّى أُضيف فحصٌ بلا «—»؛ أُعيد على أساسٍ أخضر).

### إعادةُ تثبيتٍ بسببٍ مكتوب

- `jcrRuntimePipeline.test` (٢): آخرُ سطرٍ في الحلقة بلا مزوّد كان `/✨ نجاح/` — والحكمُ هناك UNVERIFIED منذ PM/2؛ صار «☑️ اكتملت المهمّة — ولم يكتمل التحقّق».
- `strategyVerdict.test`: الفاصلُ «؛»؛ الكلونُ المعطوب بتفصيله الكامل (دوالُّ غائبة؛ أدوارٌ غائبة؛ خطأُ تشغيل)؛ حدودُ الاستيراد تطلب `kernelOutcomeLine` وتمنع «✨ نجاح» مكتوباً.
- `requirementsTrace.test` و`domainFidelity.test`: تفصيلُ بوّابة السلوك بأسماء الأدوار/المفردات الأجنبيّة لا بمعرّف الفحص. `architectureMap.test`: الأعداد.

### حدودٌ مكتوبة

- التكرارُ باقٍ حيث كان: على مسارات الاستراتيجيّة يُبثّ سطرُ الباني ثمّ الحكم؛ وفي الحلقة الحكمُ ثمّ التقريرُ ثمّ السطرُ الختاميّ — لم يُحذف سطرٌ، غُيِّر لفظُه فقط.
- الشرحُ بعد «—» يُسقَط بالاتّفاق لا بالعقد: فحصٌ جديد يضع الجوهرَ بعد الشرطة يخسر جوهرَه في الشات. `appType` المقطوع ديْنٌ.

## PM/11 — «الاسمُ الحرفيّ للمنتج: كلمةٌ كاملة، تسمّي منتجاً لا مساراً، من رأس الوثيقة» (2026-09-06)

بعد PM/10 أُعيد القياسُ الشامل ثمّ قِيست وثيقةٌ مرقّمة لمجالٍ **بلا كلون** (نظامُ إدارة مكتبة، ١٠ بنود). النتيجة: كلونُ
**نقاط البيع** («⚠️ اكتمل — بدأنا من قالب نظام نقطة بيع (كاشير)») بدليل «كلمات: إيصال، الأدوار 50٪» — كلمةٌ عابرة في البند ١٠
(«إيصال الإعارة») عُدّت **تسميةً حرفيّةً للمنتج** فرفعت فيتو الفهم (PM/1) عن كلونٍ لا يعرف «عضواً». منتجٌ خاطئ — الطبقةُ نفسُها التي
بدأ منها #567. ثلاثةُ أعطابٍ في قاعدةٍ واحدة (`matchCloneTemplateDetailed`):

| | المقيس |
|---|---|
| الكلمةُ نصٌّ فرعيّ (`includes`) | رأسُ وثيقة نقاط البيع وحدَه → **ERP** بكلمة «جرد» داخل «مجرد» |
| عبارةُ المسار = `SYSTEM_INTENT_RE` كلُّه | **١١٦ من ٦٤٦** كلمةَ كلون «عبارةُ مسار» لا ترفع الفيتو — منها `كاشير`/`نقطة بيع`/`pos`/`صيدلية`/`محاسبة`/`إدارة مصنع`: **كلُّ أسماء منتجات السيستم**. فكلونُ نقاط البيع لم ينجُ من الفيتو على وثيقته إلّا بكلمة «وردية» في البند ١٤ |
| الكلمةُ من أيّ موضع | «إيصال» في البند ١٠ من وثيقة مكتبة = تسميةٌ للمنتج |
| `appType` الاحتياطيّ | أوّلُ ستّين حرفاً: «…متكامل وقابل ل» مبتورةً؛ ولوثيقة المكتبة يحوي سطراً جديداً و«1.» |

### ما تغيّر

- `cloneTemplates/index.js`: `isTrackPhrase(kw)` = لا يبقى من الكلمة شيءٌ بعد كلمات المسار العامّة (`TRACK_WORDS`: سيستم/نظام/داخلي/إداري/إدارة/internal/system/management) —
  «نظام إدارة» و«سيستم داخلي» عبارتا مسار؛ «كاشير» و«إدارة مصنع» تسميتان. المبتلَع **١١٦ ← ٧** (كلُّها عامّة).
  المطابقةُ بمُطابِق `keywordMatch` (حدودُ كلمةٍ بسوابقَ ولواحقَ عربيّة مغلقة) بدل `includes`، مُطابِقٌ واحد لكلِّ كلمة.
  `namingHay`: نصٌّ فيه **٣ بنودٍ مرقّمة فأكثر** تُقرأ تسميةُ منتجه من رأسه (`specHead`) — كلماتُ المتن ترجّح (+١٠) ولا ترفع الفيتو.
- `textNormalizer.js`: `specHead(text)` (ما قبل أوّل بندٍ مرقّم) و`clipWords(text, max)` (قصٌّ على حدّ كلمة بـ«…»).
- `appBlueprint.js`: الاحتياطُ `appType = clipWords(specHead(goal) || goal, 60)`.

**ما يراه المستخدم**: وثيقةُ المكتبة → «🧠 استُبعد بالفهم: … jaola-pos (بلا member) +15» ثمّ حلقةُ التسليم تبنيها وتحكم
«FAILED — أدوار بلا واجهة/تمثيل: member، admin» — منتجُه هو، بحكمٍ صادق. وثيقةُ نقاط البيع كما كانت (jaola-pos بـ«نقطة بيع/كاشير/وردية/pos»).
السطرُ «🧭»: «أريد بناء نظام إدارة مكتبة عامة متكامل للاستخدام الفعلي. — تطبيق تفاعلي».

### الاختبار الجديد (`tests/productNaming.test.mjs`، ٦)

`isTrackPhrase` بوجهَيه وعدُّ المبتلَع ≤ ١٢ · رأسُ نقاط البيع → نقاط البيع باسمها ولا «جرد» · وثيقةُ المكتبة → لا كلون (`rejected-by-understanding`،
نقاطُ البيع بلا `member`) ووثيقةُ نقاط البيع تبقى · `specHead` بحدوده · `appType` برأس الوثيقة على حدّ كلمة · المسارُ كاملاً للمكتبة (لا كلون، حلقةُ التسليم، «🧭» برأسها).
**الطفرات ١٧/١٧ أُمسكت.** حرّاسُ الاختيار القائمون (٦٠ مرجعاً في `cloneTemplates.test`، تدقيقُ التوجيه الذاتيّ للقوالب الـ٤١، `productMind`) بقوا خضراً بلا تعديل — الحدودُ لم تُسقط مطابقةً صحيحة.

### إعادةُ تثبيتٍ بسببٍ مكتوب

- `posSpecMeasure.test`: ديْنُ `appType` أُغلق — الاسمُ «…متكامل وقابل…» على حدّ كلمة. `architectureMap.test`: الأعداد.

### حدودٌ مكتوبة

- **ديْنٌ لوحظ بالقياس ولم يُمَسّ**: `🧱 الأقسام` في تقرير التسليم تأتي من ذاكرة المشروع المحفوظة، فمشروعٌ أُعيد بناؤه بمنتجٍ آخر يرث أقسامَ سابقه (مِقياسي أظهر «واجهة مدير، واجهة كاشير» على وثيقة المكتبة تحت المستخدم نفسِه). اختباراتُ `scenario` تنجو منه بمستخدمٍ جديد لكلّ جولة — وهذا يعني أنّ الحارسَ لا يراه؛ مكتوبٌ هنا حتّى يُقاس.
- عتبةُ «وثيقة» هنا ٣ بنود (تسميةُ المنتج من الرأس)، بينما `isFullSpecification` ١٢٠٠ حرف/٦ بنود (لغةُ الحكم PM/9 والإعفاءُ التسويقيّ) — وثيقةُ المكتبة (٦٠٠ حرف) وثيقةٌ للأولى لا للثانية؛ توحيدُهما مؤجَّل حتّى يُقاس أثرُ خفض الثانية على التعديلات الطويلة.
- المطابقةُ الصرفيّة تقبل لاحقةً صحيحةً صدفةً (حدُّ `keywordMatch` المكتوب). موجّهُ التصميم يقول «النوع: ecommerce (من كلمات الهدف)» لنظام مكتبة — طبقةٌ أخرى (PM/5) لم تُمَسّ.
- رأسٌ لا يسمّي منتجاً ومتنٌ يسمّيه → لا رفعَ للفيتو؛ يبقى مسارُ «الفهم وحده» (model-only) إن غطّى كلونٌ كلَّ الأدوار.

## PM/12 — «حكمُ الحلقة بلغة الوثيقة أيضاً» (2026-09-07)

PM/9 جعل الحكمَ يتكلّم لغةَ وثيقة المستخدم — **على مسار الكلون وحدَه**. وحلقةُ التسليم هي التي تبني كلَّ وثيقةٍ
لا كلونَ لمجالها (٤١ كلوناً مقابل مجالاتٍ بلا حدّ)، فقِيست عليها وثيقةُ مكتبةٍ واقعيّة (١٢٣٠ حرفاً، ١٢ بنداً،
`isFullSpecification` **صحيح**):

| | المقيس |
|---|---|
| ما تقوله البوّابة | `requirements-verify: المحقّقُ لم يُجب (لا مزوّد أو ردٌّ غير صالح)` — لا ذكرَ لبندٍ واحدٍ من الاثني عشر |
| الأثرُ الحتميّ على الملفّات المكتوبة نفسِها | **٥/١٢** — والسبعةُ الباقية تُسمّى برقمها واسمها |
| المستخدمُ نفسُه بالوثيقة نفسِها على مسار الكلون | «31 بنداً من 44 في وثيقتك بلا أثر: …» |

فالحكمُ بلغتين حسب البانِي لا حسب الطلب. وثانيةً: وسمُ البند كان جملتَه كاملة، فوثيقةٌ تضع تفصيلَها بعد
النقطتين تعطي وسماً من **١١١ حرفاً** — ستّةٌ منها سطرٌ من ٦٧٠ حرفاً في الشات.

### ما تغيّر

- `stages/requirementsVerify.js`: `sections = isFullSpecification(context.originalGoal) ? specSections(…) : []`،
  والحكمُ الحتميُّ من `requirementsTraceOutcome(null, plan.files, '', sections)` — **الدالّةُ نفسُها** التي يستعملها مسارُ الكلون (لا قائمةَ ثانية).
  - المحقّقُ **صامتٌ** ولوثيقته بنودٌ تُتتبَّع → الحكمُ بلغة الوثيقة (كان `unverified` بلا معلومة).
  - المحقّقُ **أجاب** → حكمُه يبقى (هو يقرأ الكودَ والتتبّعُ يقرأ الحروف، فلا يُقلب اجتيازُه فشلاً بغيابٍ لفظيّ)، ويلحقه ذيلٌ إخباريّ «؛ N/M بنداً من وثيقتك له أثر».
  - لا وثيقةَ (أو قائمةٌ مرقّمةٌ قصيرة) → كما كان حرفيّاً.
- `stages/verify.js`: `requirementsTraceOutcome` تعيد `docTraced`/`docTraceable` مع الحكم — المستدعي يؤلّف ذيلَه بهما بدل انتزاعِه من نصٍّ مُنسَّق.
- `requirementsVerifier.js#sectionLabel`: رقمُ البند واسمُه — ما قبل أوّل نقطتين أو شرطةٍ محفوفةٍ بمسافتين، مقصوصاً على حدّ كلمة (٣٨). التتبّعُ يبقى على العنوان كلِّه (إشارةٌ أغنى)؛ الوسمُ للقراءة.

### الاختبار الجديد (`tests/loopDocVerdict.test.mjs`، ٥)

الوسمُ بحالاته (نقطتان، شرطةٌ محفوفة، شرطةٌ داخل الاسم تبقى، قصٌّ ≤٤٢) · الحلقةُ بلا مزوّد على وثيقة → `fail` بـ«7 بنداً من 12 في وثيقتك بلا أثر» في البوّابة والشات ·
المحقّقُ حين يُجيب (اجتيازاً ونقصاً) يبقى حكمُه ويلحقه الذيل · هدفٌ ليس وثيقة **ولا قائمةٌ مرقّمةٌ قصيرة** → كما كان · الحدود (استيرادُ الدالّة الواحدة، والأثرُ على `plan.files`).
**الطفرات ١٤/١٤ أُمسكت** (اثنتان نجتا أوّلاً — إسقاطُ عتبة الوثيقة، وقطعُ الوسم عند شرطةٍ بلا مسافات — فأُضيفت لهما دعامتان وأُعيدتا على أساسٍ أخضر).

### إعادةُ تثبيتٍ بسببٍ مكتوب

- `deliveryVerdict.test`: حالاتُ `recordGateOutcome` في المرحلة ٥ ← ٦ (الحالةُ الجديدة: صامتٌ ولوثيقته بنود). `specVerdict.test`: العددان يلحقان الحكم. `architectureMap.test`: الأعداد.

### حدودٌ مكتوبة

- الأثرُ يُقاس على `plan.files` (ما كتبته الحلقة) لا على ما وصل القرصَ بعد مراحل الجودة — وهو ما يراه المحقّقُ نفسُه، فالمصدرُ واحد.
- عتبةُ «وثيقة» هنا `isFullSpecification` (١٢٠٠ حرف/٦ بنود) كما في مسار الكلون؛ والقائمةُ المرقّمةُ القصيرة («١. أحمر ٢. أزرق») تبقى خارجها عمداً — مثبَّتٌ باختبار.
- حين يُجيب المحقّقُ ويقول «مكتمل» وللوثيقة بنودٌ بلا أثر، يبقى الحكمُ `pass` والذيلُ وحدَه يقول العدد. قرارٌ محافظ: قارئُ الكود أولى من قارئ الحروف. إن قِيس لاحقاً أنّ المحقّقَ يتساهل، يُعاد النظر.

## PM/13 — «فعلُ الطلب ليس صناعةَ المنتج» (2026-09-07)

قِياسُ الطبقة التي لم تصلها PM/5: المصمّمُ يُعلن نوعَ المشروع في كلِّ بناء. على وثيقةِ نظامِ مكتبة خرج
`النوع: construction (من كلمات الهدف)` — مقاولات. الجذر: **«بناء» كلمةٌ في قائمة صناعة المقاولات**، وهي
الفعلُ الذي يبدأ به المستخدمُ العربيُّ طلبَه.

| المقيس | |
|---|---|
| «أريد بناء X» على تسعة منتجاتٍ حقيقيّة | **٧ من ١٠** تنقلب إلى `construction`: مكتبة (`saas`)، مطعم (`restaurant`)، منصّة تعليميّة، حجز فنادق، نقاط بيع (`ecommerce`)، عقارات… حتّى «أريد بناء متجر ملابس» |
| بقيّةُ أفعال الطلب (ابنِ، صمّم، أريد تصميم، أريد إنشاء، اعمل لي) | **صفر** تغيير — «بناء» وحدَها |
| الإنجليزيّة | `building a restaurant website` → `construction` عبر الكلمة `building` |
| أثرُ النوع | لوحةُ الألوان + صورُ الستوك + بريفُ المصمّم؛ ومنه `resolveType` في مرحلة الخلفيّة → `generateDynamicSchema` يُرسل «النوع: construction» لمولّد مخطّط قاعدة البيانات؛ و`resolveStack` يقرّر المسار |

### ما تغيّر

سطرٌ واحد في `knowledgeEngine.js`: خرجت `بناء` و`building` من كلمات `construction`. الصناعةُ تبقى مسمّاةً
بأسمائها (`مقاولات`, `مقاول`, `إنشاءات`, `تشطيب`, `هندسي`, `construction`, `contractor`, `engineering`) —
ومقيسٌ أنّ طلبَ المقاولات الحقيقيّ ما زال يُصنَّف صحيحاً.

### الاختبار الجديد (`tests/requestVerbType.test.mjs`، ٤)

ستّةُ أفعالِ طلبٍ × تسعةِ منتجات لا تغيّر النوع · الإنجليزيّة · طلبُ المقاولات الحقيقيّ يبقى · حدُّ القائمة (لا فعلَ طلبٍ فيها، والأسماءُ باقية).

### إعادةُ تثبيتٍ بسببٍ مكتوب

- `productNaming.test`: وثيقةُ المكتبة كانت تُبنى بالحلقة الفانيلا لأنّ نوعَها كان `construction`؛ وبالنوع الصادق (`saas`) صارت **مشروعاً كبيراً → React/Next**. المقصودُ في ذلك الاختبار (لا كلونَ نقاط بيع، والبناءُ من وثيقته) قائمٌ كما هو.
- `requestVerbType.test` يُخلي قناةَ التقرير (`divertConsoleToStderr`) لأنّه يستورد وحدةً طابعة — عرفُ المستودع، يثبّته `reportChannel.test`.

### حدودٌ مكتوبة

- **أثرٌ جانبيٌّ مقيسٌ ومقبول**: تصحيحُ النوع يغيّر المسارَ لبعض الأهداف (مكتبة: فانيلا ← React/Next). ذلك أصحُّ بنوعه، لكنّ مسارَ React أظهر على هذه الوثيقة ضعفاً آخرَ قائماً: محتوىً عامّ (`event`، `order`، `course`) يلتقطه فحصُ صدق المجال (PM/3) فيصير الحكمُ FAILED — صادقاً. ضعفُ كاتب المحتوى في مسار React ديْنٌ مكتوبٌ لم يُلمَس هنا.
- لم تُراجَع بقيّةُ قوائم الصناعات بحثاً عن أفعالِ طلبٍ أخرى؛ المقيسُ أنّ الأفعالَ الستّة الأخرى لا تُطابق شيئاً اليوم.

## PM/14 — «لغةُ الآلة ليست لغةَ المنتج» (2026-09-07)

بوّابتا الحكم الحتميّتان — أثرُ المتطلّبات (PM/7) وصدقُ المجال (PM/3) — تقرآن مفاهيمَ المعجم من **نصّ الملفّ
خاماً**. فالصفحةُ تنطق بمفردات المنتج… ومفرداتِ المنصّة التي بُنيت بها.

### المقيس

| | |
|---|---|
| مفاهيمُ المعجم التي تنطق بها منصّةُ الويب نفسُها (وسومٌ وسماتٌ وخصائصُ CSS وعوالمُ JS، قِيست القائمةُ كلُّها مرّةً) | **٦**: `course`←`class` · `event`←`pointer-events`/`Event` · `order`←`order` · `location`←`grid-area`/`window.location` · `property`←`transition-property` · `table`←`<table>`/`table-layout` |
| صفحةٌ لا تذكر المجالَ بكلمةٍ واحدة، على بوّابة متطلّباتها | عقارات **٢/٢ «له أثر»** · مطعم ٢/٣ · فعاليات ١/٣ · تعليم ١/٣ — و«الغيابُ قاطع» هو نصُّ عقد `traceRequirements` |
| بوّابةُ التلوّث (PM/3) على الصفحة نفسِها | **صامتة**: `covered` صار غيرَ صفر بخصائص CSS، وشرطُ التلوّث `covered.length === 0` |
| حارسٌ قائمٌ كان يوثّق العطبَ في تعليقه | `strategyVerdict.test`: «Admin وحدَه بقي في **styles.css**» — متطلّبٌ «له أثر» باسم صنفِ تنسيق |

### ما تغيّر

`projectModel.js`: `productText(content, name)` — إسقاطٌ بنيويٌّ لا قائمةَ كلمات: **التنسيقُ لا يسمّي منتجاً**
(ملفُّ CSS كلُّه، و`<style>` المضمَّن، وقيمةُ السمة `style`)؛ **أسماءُ الوسوم والسمات كلماتُ المنصّة** (القيمةُ
والنصُّ المرئيُّ يبقيان — «ابحث عن طلب» في `placeholder` أثرٌ صادق)؛ و**تعبيرُ المتصفّح المعروف يُسقَط**
(`window.location.href`, `new Event(`) لا الكلمةُ وحدَها، فـ`location` حقلَ بياناتٍ في تطبيق توصيلٍ أثرٌ صادق.
ثلاثةُ مستهلكين: `traceRequirements` و`traceSections` (عبر `productCorpus`) وبوّابةُ `domainFidelity` في المحقّق
السلوكيّ. الإسقاطُ **لا يُستعمل على نصّ المستخدم** (الهدف/الوثيقة) — هناك لا آلةَ تتكلّم، و«order» فيه طلبٌ حقيقيّ.

### الاختبار الجديد (`tests/productLanguage.test.mjs`، ٥)

صفحةٌ ولا سكربتٌ بلا مجالٍ ينطقان بكيان · ملفُّ CSS يُسقَط كلُّه · لا أثرَ لمتطلّبٍ في صفحةٍ لا تذكره (الأربعةُ
المقيسة) · بوّابةُ التلوّث لا «تغطّي» ولا تجد أجنبيّاً · **وما يقوله المنتجُ فعلاً يبقى**: النصُّ المرئيُّ وقيمُ
السمات وبياناتُ السكربت. وفي `domainFidelity.test` حارسٌ من طرفٍ إلى طرف على البوّابة نفسِها.

**الطفرات ١١/١١ أُمسكت** — ومنها ثلاثٌ نجت أوّلاً فسُدَّت (قيمةُ سمة `style`، `window.location` بلا خاصّة،
وموضعُ `domainFidelity` بلا حارسٍ أصلاً). ورابعةٌ نجت فكشفت **كوداً ميّتاً**: `classList`/`className` لا تطابق
مفهوماً أصلاً (قِيس) — فحُذف السطرُ ولم يُثبَّت.

### إعادةُ تثبيتٍ بسببٍ مكتوب

- `strategyVerdict.test`: الكلونُ المعطوب من «٣ متطلّبٍ بلا أثر (١/٤ له أثر)» إلى «**٤** بلا أثر (**٠/٤**)».
  السببُ في تعليق الحارس القديم نفسِه: `Admin` كان أثراً لأنّه اسمُ صنفٍ في `styles.css`. الصفرُ هو الصادق.
  والكلونُ السليمُ باقٍ على ٤/٤ — الإسقاطُ لم يُسقط أثراً صحيحاً.
- حدّا الاستيراد في `domainFidelity.test` و`requirementsTrace.test`: `productText` من الوحدة نفسِها — الإسقاطُ
  حيث المعجم، فالمعجمُ يبقى واحداً ولا قائمةَ ثانية.

### حدودٌ مكتوبة

- الإسقاطُ **بنيويٌّ لا دلاليّ**: معرّفٌ في الكود يسمّي مجالاً يبقى أثراً (`const orders = []`) — وهذا مقصود،
  وهو ما يُبقي «أثرٌ لا تنفيذ» أثراً. لكنّه يعني أنّ معرّفاً مثل `const events = {…}` لأحداث الواجهة يُعدّ
  مفردةَ منتج. لم يُسقَط: فصلُ المعرّف الآليّ من المجاليّ يحتاج قياساً لا حَدْساً.
- المُتحقِّق على مسار React يقرأ **`index.html` وحدَها**: قِيس أنّ البناءَ يكتب ٧ صفحاتٍ ثابتة و٦ منها لا يراها
  `readProjectFiles` إطلاقاً. ديْنٌ مستقلٌّ عن هذا التغيير — الحكمُ يحكم على سُبع ما بُني.
- وثيقةُ المكتبة على React **لم تتغيّر** بـPM/14: مفاهيمُها المتتبَّعة عربيّةٌ صادقة أصلاً. حكمُها `FAILED`
  بأدوارٍ بلا واجهة — طبقةٌ أخرى (كاتبُ المحتوى) لم تُمَسّ بعد.

## PM/15 — «الحكمُ يقرأ ما بُني كلَّه» (2026-09-07)

مسارُ React يبني مشروعاً **متعدّدَ الصفحات**، والحكمُ يقرأ `index.html` وحدَها.

### المقيس (وثيقةُ مكتبةٍ، ٦ أقسام، نموذجٌ بثلاثة أدوار وأربعة كيانات)

| ما يراه المُتحقِّق | متطلّبات | بنودُ الوثيقة |
|---|---|---|
| **ضيّق** — `readProjectFiles` (الصفحةُ وما تُحمّله) | 3/4 — «شاشة مدير» بلا أثر | 10/12 |
| **واسع** — كلُّ ما وصل القرصَ (٢٩ ملفّاً) | **4/4** | **11/12** |

**سقوطٌ زائف** على صفحاتٍ بُنيت فعلاً — عكسُ اتّجاه PM/14 تماماً: تلك بوّابةٌ تُبرّئ ما لم يُبنَ، وهذه تتّهم ما بُني.

### الحدُّ الذي منع التوسيعَ في مكانه

`readProjectFiles` له **٩ مواضع نداء**، منها ثلاثةٌ في التعديل الجراحيّ وواحدٌ في جولة الإصلاح — وحداتٌ **تقرأ
لتكتب**. لو رأى المُعدِّلُ سبعَ صفحاتٍ لعدّل الخطأ منها. فشُقّ قارئٌ ثانٍ للحكم، وبقي القائمُ للتعديل على تحديده
— وهو تحديدٌ **صحيحٌ لغرضه**. وللقاعدة سابقةٌ في المستودع: `buildFromRegistry` يحكم على الملفّات التي كتبها هو.

### ما تغيّر

`projectReader.js`: `readBuiltFiles(projectPath)` — مشيٌ متكرّرٌ يعود بكلِّ مصدرٍ وصل القرصَ باسمٍ نسبيٍّ
بمجلّده؛ يستبعد `node_modules` وكلَّ مجلّدٍ يبدأ بنقطة (`.next` مخرجُ بناءٍ و`.git` سجلٌّ — لا منتجَ المستخدم)
وما ليس امتدادَ مصدر؛ `[]` عند أيّ خطأ كالقارئ القائم. مستهلكاه: حكمُ `buildReact` وحكمُ `buildFromClone`.

### الاختبار الجديد (`tests/builtEvidence.test.mjs`، ٤)

كلُّ صفحةٍ بُنيت مرئيّةٌ للحكم **وقارئُ التعديل باقٍ على صفحةٍ واحدة** (حارسٌ ضدّ توسيعه) · حكمُ المسار من طرفٍ
إلى طرف · ما ليس مصدراً لا يدخل · مسارٌ غائبٌ لا يرمي. **الطفرات ٨/٨** — واحدةٌ نجت أوّلاً لأنّ عيّنتي كانت
`.git/HEAD` بلا امتدادِ مصدرٍ فيلتقطها مرشّحُ الامتداد لا استبعادُ المخفيّ؛ استُبدلت بـ`.next/static/chunk.js`.

### حدودٌ مكتوبة

- **تغييرُ مسار الكلون بلا أثرٍ سلوكيٍّ اليوم**: قِيس على الكلونات الـ٤١ على القرص أنّ القارئ القائم **لا يخفي
  ملفّاً واحداً** منها (ملفّاتُها ثلاثة والصفحةُ تُحمّلهما). فطفرةُ إعادته تُمسَك بحارس **شكل المصدر** وحدَه
  (`specVerdict.test`)، ولا يسقط لها حارسٌ سلوكيّ (`strategyVerdict.test` يبقى أخضر). وُحِّد لأنّ قاعدةَ القاضي
  واحدة، لا لأنّه أصلح عطباً هناك — وهذا مكتوبٌ كي لا يُقرأ لاحقاً على غير وجهه.
- `verify.js` و`surgicalEdit.js` بقيا على القارئ الضيّق **قصداً**: هما يقرآن لِيكتبا.
- الحكمُ صار يقرأ ملفّاتِ التهيئة أيضاً (`package.json`, `tailwind.config.js`) — لا ضررَ مقيساً بعد PM/14
  (التنسيقُ يُسقَط، وأسماءُ الوسوم كذلك)، وأثرُها على تتبّع البنود لم يُقَس بندًا بندًا: ديْنٌ مكتوب.

## PM/16 — «النموذجُ يصل كاتبَ كلِّ صفحة» (2026-09-07)

PM/5 أوصل نموذجَ المجال إلى المصمّم والخلفيّة، وJCR/19 أوصله إلى كاتب المحتوى **الدفعيّ** (`modelAwareGoal`).
والكاتبُ **الإفراديّ** — الاحتياطُ الذي يقع على كلِّ قسمٍ تركه الدفعيُّ افتراضيّاً، أي أكثرَ الصفحات عرضةً
للمحتوى العامّ — بقي على الهدف الخام.

### المقيس (بحقن مسجّلٍ مكانَ النموذج اللغويّ — لا مفتاح ولا شبكة)

| النداء | ما وصله | مفرداتُ المجال (أمين المكتبة، كتاب، إعارة، حجز) |
|---|---|---|
| `generateContentModel` (الدفعيّ) | ١٣٦٨ حرفاً | **٤/٤** |
| `generateSectionContent` من `buildReact` | ٣٩٦ حرفاً | **٠/٤** |
| `generateSectionContent` من `addPage` | — | **٠/٤**، و«سياقُ المشروع» = `content.hero.title` |

ونصُّ ما كان يصل عند إضافة صفحة «الإعارة» لنظام مكتبة، حرفيّاً:

```
الصفحة: الإعارة — سياق المشروع: ابنِ شيئاً رائعاً
```

`ابنِ شيئاً رائعاً` عنوانُ البطل الافتراضيُّ في `defaultContent`. فيُطلب محتوىً «غيرُ عامّ» بأعمِّ سياقٍ ممكن.

### ما تغيّر

- `buildReact.js`: الكاتبُ الإفراديُّ يأخذ `modelAwareGoal` (المتغيّرُ القائمُ نفسُه) لا `goal`.
- `addPage.js`: السياقُ `buildProjectModelContext(getDomainModel(...))`. **عنوانُ البطل لم يُضَفْ إليه بل حلّ
  محلَّه** — هو نصٌّ تسويقيٌّ لا وصفٌ للمنتج حتّى حين يُخصَّص؛ ويعود احتياطاً وحدَه حين لا نموذجَ محفوظاً.
- **منفذُ حقنٍ للنموذج اللغويّ**: `opts.llm` في `buildReactProject` و`ops.llm` في `addPageNow`، كلاهما بقيمةٍ
  افتراضيّة `smartChat` فلا يتغيّر موضعُ نداءٍ قائم. بلا هذا المنفذ **لا يمكن قياسُ ما يصل الكاتبَ أصلاً**:
  الاختباراتُ بلا مزوّد، و`smartChat` يرمي فيُبتلع الخطأ في `try` — فيبقى العطبُ غيرَ مرئيّ للحرّاس.

### الاختبار الجديد (`tests/contentWriterContext.test.mjs`، ٢)

مسارُ البناء: **كلُّ** نداءٍ مسجَّل (الدفعيُّ والإفراديُّ) فيه المفرداتُ الأربع · وإضافةُ صفحةٍ لمشروعٍ
**بُني فعلاً بلا مزوّد** (فعنوانُ بطله قالبيٌّ — الحالةُ التي يقع فيها العطب) تُرسل مجالَ المشروع، ويُثبَّت
صراحةً غيابُ «ابنِ شيئاً رائعاً». **الطفرات ٦/٦ أُمسكت.**

### إعادةُ تثبيتٍ بسببٍ مكتوب

`addPageStage.test`: حدُّ «لا نداءَ على `ops` غيرَ `runMission`» صار عضوَين — `runMission` و`llm`، وكلاهما
**متعاونٌ مُحقَن** لا حالة؛ وأُضيف حدٌّ جديد: منفذُ الحقن موضعٌ واحد.

### حدودٌ مكتوبة

- ما يصل الكاتبَ صار مقيساً؛ **وما يكتبه الكاتبُ لم يُقَس** — ذلك يحتاج مزوّداً حقيقيّاً. الاختبارُ يثبّت
  المُدخل لا المُخرَج، وهذا حدُّه.
- لوحظ بالقياس ولم يُتابَع: `brand` خرج «Jaola app» رغم `projectName: 'مكتبة-المدينة'` في `generateNextScaffold`
  — يُقاس على حدة قبل أن يُمَسّ.

## PM/17 — «اسمُ المستخدم لمشروعه يُعرض كما سمّاه» (2026-09-07)

`generateNextScaffold` يشتقّ `safeName` بإسقاط كلِّ ما ليس `[a-z0-9-]`. هذا **صحيحٌ لمُعرِّف حزمة npm**
الذي يجب أن يكون ASCII — لكنّ ناتجَه كان يُعاد استعمالاً **اسمَ العلامة المعروض** في كلِّ صفحة، و`<title>`
في تبويب المتصفّح، وعنوانَ README.

### المقيس

| اسمُ المشروع | العلامةُ المعروضة (قبل) | بعد | `package.json` name |
|---|---|---|---|
| مكتبة المدينة | **Jaola app** | مكتبة المدينة | `jaola-app` |
| مطعم البحر | **Jaola app** | مطعم البحر | `jaola-app` |
| متجر-الأناقة | **Jaola app** | متجر الأناقة | `jaola-app` |
| متجر ABC | **Abc** (الكلمةُ العربيّةُ حُذفت) | متجر ABC | `abc` |
| City Library | City library (مُطبَّع) | City Library | `city-library` |

أي أنّ صاحبَ المشروع العربيَّ كان يرى **اسمَ المنصّة مكانَ اسم متجره** في كلِّ صفحةٍ من موقعه، وفي تبويب
المتصفّح، وفي أوّل سطرٍ من README.

### ما تغيّر

قيمتان لغرضَين متعارضَين لا قيمةٌ واحدة: `safeName` (مُعرِّفُ الحزمة، ASCII كما كان — بلا مساس) و`displayName`
(اسمٌ يقرؤه صاحبُ المشروع، كما كتبه؛ الشرطةُ والشرطةُ السفليّة تُعرضان مسافةً لأنّ أسماء المشاريع تصل مُشرطنة).
والاحتياطُ القديم يبقى للاسم الفارغ وحدَه.

### الاختبار الجديد (`tests/projectDisplayName.test.mjs`، ٦)

العربيُّ يبقى عربيّاً في العلامة و`<title>` وREADME · المُشرطن يُعرض بمسافة · المختلَط لا يفقد عربيّتَه ·
**مُعرِّفُ npm يبقى ASCII صالحاً في كلّ الحالات** (حارسُ الغرض الآخر) · الإنجليزيُّ بحروفه كما كتبها ·
الفارغُ لا ينهار. **الطفرات ٤/٤** — وخامسةٌ نجت فكشفت `trim()` زائداً (`displayName` يشذّب بنفسه و`safeName`
يُسقط الأطراف) فحُذف ولم يُثبَّت، كما في `classList` بـPM/14.

### حدودٌ مكتوبة

- الشرطةُ تُعرض مسافةً دائماً — فاسمٌ فيه شرطةٌ **مقصودة** («جاولا-٢») يُعرض بمسافة. مقبولٌ لأنّ الغالب
  أسماءُ مشاريعَ مُشرطنة، ومكتوبٌ كي يُقاس لو ورد خلافُه.
- لم يُقَس ما تعرضه المسارات الأخرى (الكلون، السجلّ) من اسم المشروع — هذا المسارُ وحدَه قِيس.

## قياساتٌ بعد PM/21 — ثلاثُ نتائجَ لا تستدعي تغييراً (2026-09-07)

سُجِّلت بأرقامها لتُقارَن بها جولةٌ لاحقة — فالمقياسُ المحفوظ هو ما كشف PM/21 (رقمٌ تحرّك بلا سببٍ في الكود).

### ١) حدُّ PM/18 المكتوب — **سلبيّة**

| | |
|---|---|
| كلماتُ إطارٍ إنجليزيّة يغيّرها التطبيع | **٠** — التطبيعُ عربيُّ الأثر، والإنجليزيّةُ تمرّ بالتصغير فقط |
| حرفٌ كبير (`System`, `SUPPORT`, `Must`) | **تُطابَق** — الطرفان يمرّان بالتطبيع نفسِه، وهو عينُ ما أصلحه PM/18 |
| ملاحظةٌ جانبيّة | **١٧ من ٥٤** كلمةَ إطارٍ لا تُبلَغ (مُرشِّحُ الثلاثة أحرفٍ يسبق فحصَ القائمة) — **ليس عطباً**: الكلمةُ تسقط على الحالَين |

### ٢) «٣٦ تعريف الإنجاز» — **سلبيّة**

يقع في `missing`، **بلا أثرٍ زائف** — فلا ينفخ البسط. غايتُه أنّه سطرٌ وصفيٌّ واحدٌ من ٣٦ يظهر في قائمة
الفجوات. وسنُّ قاعدةٍ ثانية على مشاهدةٍ واحدة هو عينُ ما منعه حدُّ PM/21 المكتوب.

### ٣) اسمُ المنتج دليلاً — **آليّةٌ مُبرهَنة، غيرُ مقيسةٍ على مصنوعٍ حقيقيّ: ديْنٌ مؤجَّل**

عنوانُ البند في PM/9 هو **جملتُه كاملة**، فقد تَرِد فيها كلمةُ المجال نفسُها. وكلُّ صفحةٍ في المنتج تقولها.

| | |
|---|---|
| وثيقةُ المكتبة، على صفحةٍ **اصطناعيّة** (`<h1>مكتبة المدينة</h1>`) | **١ من ١**: «٨ الصلاحيات والأدوار» أُثبت بكلمة «مكتبة» وحدَها |
| وثيقةُ نقاط البيع، على **الكلون الحقيقيّ** | **٠ من ١٠** — لا أثرَ يقوم على مفردةِ الرأس وحدَها |

**لم يُغيَّر شيء.** الإصلاحُ المغري («مفرداتُ `specHead` ليست دليلاً لبندٍ بعينه») يحمل خطراً معاكساً
مقيساً: رأسُ نقاط البيع يحوي `كاشير · بيع · pos` — وهي مفرداتُ منتجٍ صادقة؛ ورأسٌ يعدّد الميزات
(«نظام مكتبة فيه إعارة وحجز») سيُبطل أدلّةً صحيحة. يُنتظر ظهورُه على بناءٍ حقيقيّ قبل أيّ قاعدة.

### حدٌّ منهجيٌّ مكتوب

وثيقةُ المكتبة لا تصل حلقةَ التسليم بلا مزوّد: مسارُها الحرّ يطلب `coreGenerateCodePlan`/`architectReview`/
`qaVerify`. وحقنُ بدائلَ يقيس ما كُتب في البديل لا ما يبنيه النظام — فلم يُفعل، وقِيست طبقةُ الحكم وحدَها.

## PM/21 — «خطّةُ التسليم ليست مطالبَ المنتج» (2026-09-07)

اكتُشف بإعادة القياس الشامل بعد PM/13→PM/20: مواصفاتُ المستخدمين تنتهي غالباً بجدولٍ زمنيّ، ووثيقةُ نقاط
البيع تنتهي بثمانية أسطرٍ «المرحلة N: …». وهي أسطرٌ عن **ترتيب العمل** لا عن المنتج، فتُفسد حكمَ PM/9 من
طرفَيه معاً.

### المقيس (٤٤ بنداً على كلون نقاط البيع)

| | قبل | بعد |
|---|---|---|
| «له أثر» | **١٣** — منها ٣ أسطرِ مراحل بدليلِ بنودٍ عُدَّت قبلها: «٣٨ المرحلة 2»←`منتجات · كاشير` (دليلا ٢ و٤)، «٣٩ المرحلة 3»←`فواتير · دفع`، «٤٢ المرحلة 6»←`تقارير` | **١٠** |
| «بلا أثر» | **٣١** — منها ٥ أسطرِ مراحل تُعرض على صاحب المشروع فجواتٍ عليه سدُّها، وهي لا تُبنى | **٢٦** |
| المقام | ٤٤ | **٣٦** — والحكمُ يقول لمَ نقص |
| تعليمةُ الإكمال | تطلب من النموذج أن **يبني** «المرحلة 5: الإدارة» (٣١ بنداً) | ٢٦ بنداً، كلُّها مطالبُ منتج |

### ما تغيّر

`isPlanRow` في `requirementsVerifier.js` → دلوُ `untraceable` القائم (خارج البسط والمقام أصلاً). وثلاثةُ
شروطٍ مجتمعةً، كلٌّ منها يمنع إسكاتَ مطلبٍ حقيقيّ: **الصدارة** (فـ«الفواتير تُسلَّم في المرحلة 3» مطلبٌ يبقى
— وهذه الحالةُ وحدَها هي ما يحمله التثبيت)، و**الترقيم** (فـ«مرحلة الدفع في المتجر» مطلب)، و**لفظٌ لا معنى
له إلّا الجدولة**. و`stages/verify.js` يذيّل الحكمَ بعدد أسطر الخطّة المستبعدة.

### الاختبار الجديد (`tests/planRows.test.mjs`، ٦) — **الطفرات ٨/٨**

نجت اثنتان أوّلاً: التثبيتُ على الصدارة (فسُدَّ بالحالة التي يحملها وحدَه)، وصيغٌ إنجليزيّة لم تكن مقيسة.

### حدودٌ مكتوبة

- أُخرجت `stage` و«جولة» من القائمة **عمداً**: لهما معنىً منتَجيٌّ حقيقيّ (مرحلةُ خطِّ معالجة، جولةٌ سياحيّة)
  فإسكاتُهما خطرٌ لا احتياط. و`milestone`/`sprint` بقيتا بلا قياسٍ ميدانيّ — مثبَّتتان باختبارٍ لا بمشاهدة.
- «٣٦ تعريف الإنجاز (Definition of Done)» سطرٌ وصفيٌّ آخرُ ما زال يُحسَب مطلباً — لم يُعالَج: قاعدةٌ واحدة
  لِما قِيس، ولا تُوسَّع بالحدس.

## PM/20 — «المفتاحُ يُنفَّذ والتسميةُ تُقرأ» (2026-09-07)

`DEFAULT_SECTIONS` مفاتيحُ لاتينيّةٌ بالضرورة: منها يُشتقّ اسمُ المكوّن (`Features.jsx`) ومسارُ الصفحة
(`/features`). وهي تُعرض أيضاً **تسميةً**: عنوانَ الصفحة، وعنوانَ التبويب، وبندَ التنقّل، واسمَ القسم في طلب
كاتب المحتوى. قيمةٌ واحدةٌ لغرضَين — PM/17 نفسُه.

### المقيس (بناءُ «مكتبة المدينة» عربيّاً بلا مزوّد — أي بالمخطّط الاحتياطيّ بلا `keySections`)

| | قبل | بعد |
|---|---|---|
| صفحاتٌ بعنوانٍ لاتينيٍّ خالص | **٣ من ٥** (`Features`, `About`, `Contact`) | **٠ من ٥** |
| عناوينُ التبويب | «مكتبة المدينة — About» | «مكتبة المدينة — من نحن» |
| شريطُ التنقّل | «الرئيسية \| Features \| About \| Contact» | «الرئيسية \| المزايا \| من نحن \| تواصل معنا» |
| المسارات وأسماءُ المكوّنات | `/features`، `Features.jsx` | **كما هي** — لغةُ الآلة لا تتبع لغةَ العرض |

والدليلُ أنّ القصد كان غيرَ ذلك: `homeLabel` في المولّد **مترجَمٌ أصلاً** («الرئيسية») — تُرجم بندٌ واحد وبقي
جيرانُه، فخرج التنقّلُ مختلطاً.

### ما تغيّر

`planSections(sections, lang)` — الاشتقاقُ الواحد نفسُه (PM/8 وما قبله) صار يعرف اللغة، فيترجم تسميةَ ما
**حقنّاه نحن** وحدَه: الافتراضيّاتُ حين لا يسمّي الطلبُ قسماً، والهيكلُ المحقون. `DEFAULT_LABELS` تُترجم
الثلاثةَ التي تُعرض تسميتُها فعلاً (`features`/`about`/`contact`) — والشريطُ والبطلُ والتذييلُ هيكلٌ لا صفحاتٌ
فلا تسميةَ لها تُقرأ، فلم تُضَف (لا سطرَ بلا مستهلك — PM/14).

### الاختبار الجديد (`tests/defaultSectionLabels.test.mjs`، ٦)

بناءٌ عربيٌّ كامل (عنوانٌ وتبويبٌ وتنقّل) · المفتاحُ يبقى لاتينيّاً · الإنجليزيّةُ كما كانت · تسميةُ صاحب
المشروع لا تُترجَم **ولو كانت المفتاحَ نفسَه** · وبلا لغةٍ مذكورة لا ترجمة · وكاتبُ المحتوى يُخبَر بالتسمية
المعروضة (بمنفذ حقن `llm` من PM/16). **الطفرات ٨/٨** — أربعٌ نجت أوّلاً وسُدَّت بثلاثة حرّاسٍ صريحة.

### حدودٌ مكتوبة

- التسمياتُ عربيّةٌ/إنجليزيّة فقط، كما في `defaultSection` و`defaultContent` و`homeLabel` — لغةٌ ثالثة تعود
  للسلوك القديم (المفتاحُ بحرفٍ كبير).
- ما يأتي في `sections` لا يُترجَم أبداً — ولو جاء من **المخطّط** لا من صاحب المشروع: الطبقةُ لا تفرّق
  بينهما، فاختِير الأحفظ. مواصفةٌ عربيّةٌ يعود مخطّطُها بـ`keySections` إنجليزيّة تخرج بتسمياتٍ إنجليزيّة.

## PM/19 — «الذاكرةُ تسجّل ما بُني لا ما طُلب» (2026-09-07)

سطرُ «🧱 الأقسام» في تقرير التسليم يُقرأ من `projectMemory.structure.sections`، و`updateStructure` **لا يمحو
بقائمةٍ فارغة** (عمداً: تحديثٌ جزئيٌّ لا يفقد المشروعَ هيكلَه). فما لم يكتب البانيَ شيئاً، بقي هيكلُ منتجٍ سابق.
ومشروعُ الدخول واحدٌ لكلِّ مستخدم (`sandbox_app` في `server.js`) — فالمنتجُ الثاني في المشروع نفسِه هو الحالةُ
الشائعة لا النادرة.
...
| | |
|---|---|
| `buildFromRegistry` | **لا يسجّل هيكلاً قطّ** — يركّب ١٠ بلوكات ويبثّها في السجلّ («🧩 رُكّبت ١٠ أقسام: nav · hero · …») ثمّ لا يكتبها؛ فالتوريثُ **دائم** على هذا المسار |
| `buildReactProject` | يسجّل الأقسامَ **المطلوبة** لا المبنيّة؛ وبلا مزوّدٍ يعود المخطّطُ الاحتياطيُّ بـ`keySections: []` (`appBlueprint.js`)، فيمرّ `[]` ولا يُكتب شيء بينما السكافولد بنى أقسامَه الافتراضيّة فعلاً |
| `buildFromClone` | يسجّل من أدوار الكلون — **٤١/٤١ كلوناً له أدوار**، فلا ثغرةَ هنا (قياس) |
| إعادةُ الإنتاج | مطعمٌ ثمّ نظامُ مكتبةٍ في المشروع نفسِه → تقريرُ المكتبة يبثّ **«🧱 الأقسام: قائمة الطعام، الطلبات، السلة»** |
| بعد الإصلاح | السطرُ نفسُه يبثّ ما بناه السكافولد: `navbar، hero، features، about، contact، footer` |

### ما تغيّر

- `reactGenerator.js`: `meta.sections` تُعلن `secs` — الأقسامَ المبنيّةَ فعلاً (بعد حقن الشريط والبطل والتذييل).
- `stages/buildReact.js`: يسجّل الأقسامَ المسمّاةَ حين يسمّيها الطلب (تسميةُ صاحب المشروع هي المعنى)، وما بناه
  السكافولد حين لا يسمّي شيئاً — فلا قائمةَ فارغةٍ تمرّ فتُبقي الموروث.
- `stages/buildFromRegistry.js`: يسجّل البلوكاتِ التي ركّبها — وهي عينُ ما يبثّه في السجلّ.

### الاختبار الجديد (`tests/builtSections.test.mjs`، ٤)

مسارُ React بلا أقسامٍ مسمّاة (بعد منتجٍ أوّل) · ومع أقسامٍ مسمّاة تبقى بأسمائها · ومسارُ Registry يطابق
سطرَ سجلّه · وحدُّ `meta.sections` = ما بُنيت منه المكوّنات ولا يعود فارغاً. **الطفرات ٩/٩**.

### حدودٌ مكتوبة

- الأقسامُ الافتراضيّة تُسمّى بمفاتيح السكافولد الإنجليزيّة (`features`, `about`) — وهي **عينُ ما يظهر عنواناً
  على الصفحة المبنيّة** (`defaultContent` يستعمل التسميةَ نفسَها)، فالتقريرُ يوافق الموقع. لكنّ موقعاً عربيّاً
  بلا مزوّدٍ يخرج بعناوينِ أقسامٍ إنجليزيّة — عطبٌ **آخر** مُلاحَظ بالقياس ولم يُعالَج هنا.
- `updateStructure` بقي على دلالته (الفارغُ لا يمحو): كلُّ بانٍ صار يكتب قائمةً غيرَ فارغة، فأُغلق التوريثُ
  بالبناء لا بتغيير دلالة المُسنِد المشترك. مشروعٌ لم يُبنَ فيه شيءٌ بعدُ يبقى بلا سطرِ أقسامٍ أصلاً.

## PM/18 — «كلماتُ الإطار تُطبَّع كما تُطبَّع مفرداتُ البند» (2026-09-07)

`traceSections` (PM/9) يشتقّ مفرداتِ عنوان البند بـ`normalizeConceptText` — الذي يقلب `ى`→`ي` و`ة`→`ه` —
ثمّ يقارنها بقائمة إطارٍ مكتوبةٍ **خاماً**. فمفردةٌ مُطبَّعة تُقارَن بكلمةٍ غير مُطبَّعة، ولا تُطابَق أبداً.

### المقيس

| | |
|---|---|
| كلماتُ إطارٍ ميّتة (لا يمكن أن تُطابَق) | **٤ من ٣٦**: «على»→`علي` · «الى»→`الي` · «حتى»→`حتي` · («امكانية» نجت لأنّ لها توأماً مكتوباً) |
| حضورُ `علي` في الصفحات المبنيّة | **✓** — «على» حرفُ جرٍّ في أيّ نصٍّ عربيّ |
| الأثر على وثيقة مكتبةٍ حقيقيّة | بندُ **«٨ الصلاحيات والأدوار»** يُعلَن «له أثر» — وأثرُه ذلك الحرفُ وحدَه |
| بعد الإصلاح | المكتبة **٥ ← ٤** «له أثر» (سقط الزائف) · مواصفةُ نقاط البيع ١١ كما هي |

### ما تغيّر

سطرٌ واحد: `.map(normalizeConceptText)` على قائمة الإطار — تُطبَّع مرّةً عند التحميل بالتطبيع نفسِه الذي
يمرّ به العنوان. فأيُّ إملاءٍ يُكتب في القائمة يعمل، ولا حاجةَ لتذكّر صورةِ التطبيع عند إضافة كلمة.

### الاختبار الجديد (`tests/sectionStopwords.test.mjs`، ٥)

حرفُ الجرّ لا يُثبت أثراً · كلُّ كلمةِ إطارٍ تنجو من التطبيع (بالصيغ الثلاث) · **والمفردةُ الحقيقيّة ما
زالت تُتتبَّع** (لم يُوسَّع الإسكاتُ على حساب الصدق) · الحدُّ الأدنى ثلاثةُ أحرف يمنع «ما» ولو لم تكن في
القائمة · وحدُّ المصدر. **الطفرات ٣/٣** — والثالثة (حدُّ الحرفين) نجت أوّلاً فسُدَّت بحارسٍ صريح.

### قياسٌ مصاحبٌ — نتيجةٌ **سلبيّة** (تتبّعُ البنود بالمتن)

الديْنُ المكتوب كان: «٨ من ٣١ بنداً بلا أثرٍ في عنوانها لها أثرٌ في متنها — يحتاج فصلَ الصادق من الزائف».
فُصِل بالقياس على مواصفة نقاط البيع (٤٤ بنداً): توسيعُ التتبّع إلى المتن يرفع **١١ ← ٢٤**، وفحصُ الثلاثةَ
عشرَ المضافة بندًا بندًا يُظهر أنّ أدلّتها كلَّها تقريباً حروفُ جرٍّ (`علي`) أو صفاتٌ عامّة (`سريعه`،
`واضحه`) أو كلمةُ مجالٍ واحدة موجودةٌ في أيّ مكانٍ من البناء (`مخزون` لبند «الإشعارات»). **لا يُوسَّع**:
مضاعفةُ «له أثر» على أدلّةٍ كهذه تجعل الحكمَ أكذبَ لا أصدق. والمقياسُ محفوظٌ ليُعاد إن تغيّرت الأدلّة.

### حدودٌ مكتوبة

- القائمةُ عربيّةٌ وإنجليزيّة معاً، والتطبيعُ عربيُّ الأثر — فالإنجليزيّةُ تمرّ كما هي (`normalizeConceptText`
  يُصغّر الحروفَ فقط). لم يُقَس إن كانت في القائمة كلمةٌ إنجليزيّة بحرفٍ كبير تفقد المطابقة.

## PM/22 — نوعُ المشروع حقيقةٌ على القرص، لا رأيُ قارئٍ اختار غرضَه (2026-09-07)

`stages/surgicalEdit.js` كان يجيب عن «أهذا مشروعُ React؟» من قائمة `readProjectFiles` — و**هي قارئُ التعديل**:
تقرأ الصفحةَ وما تُحمّله فعلاً وحدَها، بتصميمٍ مقصودٍ وثّقه PM/15. فعلى مشروع React مبنيٍّ تعود بـ`index.html`
وحدَها (١ من ١٧ على القرص)، و`lib/content.js` موجودٌ ولا تراه — فالشرطُ **خطأٌ دائماً**.

### ما كان يقع

- عمليّاتُ الصفحات الثلاث (`renamePage`/`deletePage`/`addPage`) خلف `if (isReact)` — **لا تُستدعى قطّ**.
  يطلب صاحبُ المشروع «احذف صفحة التوصيل» فتبقى `altwsyl.html` والشريطُ كما هما، ويُقال له:
  «✅ طبّقت التعديل على: **index.html** فقط (بلا إعادة بناء الموقع) — المعاينة تحدّثت».
- إعادةُ توليد المعاينة من `lib/content.js` بعد التعديل خلف الشرط نفسِه — فجملةُ «المعاينة تحدّثت» تُقال
  بلا أن تتحدّث، ولا تحذيرَ في السجلّ.
- والتعديلُ يُوجَّه إلى الصفحة المولَّدة بدل مصدرها، فيفترق الاثنان.

### العقد

- `projectReader.js#isReactProject(projectPath)` — يسأل القرصَ عن علامتَين: `lib/content.js` **أو**
  `app/page.jsx`؛ كلتاهما تكفي وحدَها. لا يرمي، ولا يُوسّع قارئَ التعديل.
- `projectReader.js#readReactSources(projectPath)` — `lib/content.js` + كلُّ `components/*.jsx|ts|tsx`.
  `[]` عند أيّ غياب. هذه ليست صفحاتٍ يختار المُعدِّلُ خطأً من بينها (تحذيرُ PM/15)، بل مصدرُ المشروع
  الذي تُشتقّ منه كلُّ صفحاته.
- هدفُ التعديل على مشروع React = (ما يراه القارئُ بلا صفحات HTML الجذريّة) **مضموماً إليه** مصادرُ React
  بلا تكرار. ضمٌّ لا استبدال: ملفٌّ جذريٌّ غيرُ HTML تُحمّله الصفحةُ يبقى هدفاً صالحاً كما كان.
  قِيس: صفرُ ملفّاتٍ ← خمسةُ ملفّاتِ مصدرٍ حقيقيّة على مشروع React مبنيّ.
- **إعادةُ التسمية**: كلمةُ «صفحة»/«page» اختياريّةٌ في مُطابِقها وحدَه (بخلاف الحذف والإضافة)، فـ«غيّر اسم
  **المتجر** إلى زهور» تُطابقه أيضاً. القاعدة: **اللفظُ يحسم** — قالها صراحةً ⇒ عمليّةُ صفحةٍ مهما كان الاسم
  (و«⚠️ لم أجد صفحة باسم…» جوابٌ صادقٌ نافعٌ خيرٌ من إعادة بناءٍ كاملة)؛ لم يقلها ⇒ لا تُفترَض العمليّةُ
  إلّا إن سمّى صفحةً قائمةً فعلاً، وإلّا فهو تعديلٌ عاديّ.

### حدودٌ مكتوبة

- `readProjectFiles` **لم يُوسَّع** ولا سطراً — حدُّ PM/15 قائمٌ كما كُتب.
- `readReactSources` لا تمشي المجلّداتِ متكرّراً: `lib/content.js` + `components/` بمستوىً واحد. هذا ما
  يبنيه `reactGenerator` اليوم؛ مشروعٌ بمكوّناتٍ في مجلّداتٍ فرعيّة لن تصلَ مكوّناتُه المُعدِّلَ — لم يُقَس
  أنّ هذا يقع، ولا مستهلكَ لتوسيعٍ لم يُطلَب.
- `app/page.jsx` علامةُ كشفٍ صالحة، لكنّ عمليّاتِ الصفحات تحتاج `lib/content.js` لتعمل: مشروعٌ بالأولى بلا
  الثانية يُكشَف React ثمّ يُجيبه `readReactContent` بـ«⚠️ تعذّر قراءة المشروع». لم يُقَس أنّ هذه الحالةَ
  تُنتَج اليوم (المولّدُ يكتب الملفَّين معاً).

## سلسلةُ المزوّدين — العطبُ الدائمُ يُقرأ عابراً، والنصُّ الخامُ يُعرض لصاحب المشروع (2026-09-07)

قِيس من **سجلّ Render حقيقيّ** على مشروعٍ حيّ (11:02:48–11:02:53). سقط المزوّدون الأربعة بالترتيب،
واثنان منهم لسببٍ ليس الرصيد بل **اسمُ موديلٍ تقاعد عند مزوّده**:

| المزوّد | ما قاله | التصنيف الصحيح |
|---|---|---|
| Groq | `404 — The model llama-3.3-70b-versatile does not exist or you do not have access to it` | `config` |
| DeepSeek | `402 — Insufficient Balance` | `quota` |
| Gemini | `404 — This model models/gemini-2.0-flash is no longer available … use models/gemini-3.6-flash` | `config` |
| OpenAI | `429 — You have no credits remaining. Add credits … /billing` | `quota` |

وبلوغُ OpenAI ليس اختياراً: هو الحلقةُ الرابعة، ولا يُبلَغ إلّا بسقوط الثلاث قبله. وسطورُ
`[AI Failover] …` تذهب إلى سجلّ الخادم لا إلى سجلّ المشروع، فلا يرى صاحبُه إلّا خطأَ الحلقة الأخيرة.

### العطبان — وهما في الجذر واحد

- **لفظُ المزوّد لا لفظُنا**: `classifyAIError` كان يعرف `not exist`/`not found`، ولا يعرف لفظَ Google
  «no longer available» ولا لفظَ OpenAI «deprecated». فعطبُ إعدادٍ **دائم** يُقرأ عابراً، فيسقط شرطُ
  `failures.every(isPermanentAIError)`، فلا تُرفع `aiUnavailable`، فتُكمل حلقةُ النقاش دوراتِها.
  قِيس على `runDebate` الحقيقيّ: **١/٧ ← ٧/٧** — ستُّ دوراتٍ تُحرق، كلٌّ منها تطرق الأبوابَ الأربعة.
- **النصُّ الخام**: خارج حالةِ «كلُّها دائمة» كانت السلسلةُ ترمي خطأَ المزوّد كما هو، فيقرأ من يبني
  متجراً رابطَ فوترةِ حسابِ المنصّة في سجلّ مشروعه.

### العقد

- `aggregateFailure(failures, lastError)` — **موضعٌ واحد** يقرّر أمرَين: أيُوقَف التكرار (`aiUnavailable`
  إن كانت كلُّ الأعطال دائمة)، وماذا يُقال لصاحب المشروع (`AI_UNAVAILABLE_MSG` أو `AI_RETRYABLE_MSG` —
  **جملتُنا لا نصُّ المزوّد، في الحالتين**). التفصيلُ الخام يبقى في `causes` وفي `console.warn` للتشخيص.
- تقاعدُ الموديل عطبُ إعدادٍ دائم بألفاظ المزوّدين الثلاثة: `not exist` / `not found` /
  `no longer available` / `deprecated`. أُسقطت `not_found` و`retired` بعد القياس — لا رسالةَ مزوّدٍ
  حقيقيّةٍ تحتاجهما (لا سطرَ بلا مستهلك).
- أسماءُ الموديلات من البيئة: `GROQ_MODEL` / `GEMINI_MODEL` / `OPENAI_MODEL` (و`DEEPSEEK_MODEL` كما كان).
  قِيس أنّ اسماً ميّتاً واحداً كان مكتوباً في **تسعة مواضع** — فتقاعُدُ موديلٍ يُسقط السلسلةَ كلَّها
  ولا يُصلَح إلّا بنشرِ كود. صار سطراً في Render.

### حدودٌ مكتوبة

- `supported` في قائمة ألفاظ الإعداد **سابقةٌ لهذه الجولة**؛ بقيت كما كانت ولم تُقَس (لا رسالةَ مزوّدٍ
  في السجلّ تحتاجها). أُبقيت لأنّ حذفَ حارسٍ قائمٍ بلا قياسٍ أخطرُ من إبقائه.
- `utils/aiProvider.js` وحدةٌ يتيمةٌ مُقرّة (لا يصل إليها الخادم) وفيها الاسمُ الميّتُ نفسُه — لم تُمَسّ.
- الأعطالُ العابرة (حدُّ معدّلٍ حقيقيّ، 500، انقطاعُ شبكة، مهلة) تبقى عابرةً بحارسٍ مضادّ: إعادةُ
  المحاولة عليها تُجدي، فإسكاتُها خسارةٌ لا احتياط.
- الاسمُ الافتراضيُّ لـGemini بقي `gemini-2.0-flash` — **البديلُ قرارُ صاحب المنصّة**، ورسالةُ Google
  تسمّيه؛ يُضبط من البيئة بلا نشر. وكذلك Groq: لا دليلَ في السجلّ على الاسم البديل، فلم يُخترع اسم.

## مخطّطُ التطبيق — «ما يُحصَّن نصفُه لا يُحصَّن» (2026-09-07)

قِيس بجولةِ بناءٍ كاملةٍ ومزوّدٍ **ناجحٍ** مُحاكى — لا فاشلٍ هذه المرّة. أوّلُ سطرٍ في السجلّ الحيّ:
`[BLUEPRINT] ➔ [AppAnalyzer]: 🧭 undefined — تطبيق تفاعلي`.

`generateBlueprint` كان يحصّن في مسار «النموذج أجاب» أربعةَ حقول (`kind`، `category`،
`functionalComponents`، `keySections`)، بينما `buildBlueprintContext` يُقحم ثلاثةً أخرى بلا احتياط:

```
**النوع:** ${bp.appType}          **القيمة الأساسية:** ${bp.coreValue}          ${c.name} — ${c.behavior}
```

والأثرُ ليس سطرَ سجلٍّ: هذا النصُّ **يُحقن في هدف البناء**، فيُقال للمولّد حرفيّاً «النوع: undefined».
المخطّطُ الذي وُجد ليمنع «تحويلَ كلِّ شيءٍ إلى بروشور» كان يصف المنتجَ بكلمةٍ بلا معنى.

### لماذا يتّسع بتغيير المزوّد

`JSON.parse` ينجح على **أيّ** JSON صالح، ووضعُ JSON لا يفرض مخطّطاً. ولكلِّ نموذجٍ عادتُه: `app_type`
بدل `appType`، أو لفٌّ داخل `blueprint`، أو إسقاطُ حقلٍ يراه اختياريّاً. قِيست خمسُ صيغٍ صالحة، سرّبت
كلُّها. والمسارُ الاحتياطيّ — حين **يفشل** المزوّد — كان محصَّناً منذ PM/11؛ فالعطبُ لا يظهر إلّا حين
**ينجح** النموذج، وهو ما يفسّر بقاءَه مستوراً طوال جولاتٍ كان المزوّدُ فيها ميّتاً.

### العقد

- `hardenBlueprint(raw, goal)` — **موضعٌ واحد يخدم المسارَين**: ما أجاب به النموذج، وما نبنيه احتياطاً
  (الاحتياطُ صار نداءً لها بمكوّنٍ واحد). القاعدة: لا يخرج منها مخطّطٌ فيه حقلٌ **معروضٌ** بلا قيمة.
- `asText(v)` = نصٌّ غيرُ فارغٍ بعد التشذيب، وإلّا `null`. يحرس من الغائب، ومن الفارغ والأبيض،
  ومن غير النصّ (كائنٌ يُطبع `[object Object]`).
- المكوّنُ يُعرض `الاسم — السلوك`، فما نقص أحدُ نصفَيه **يُسقَط وحدَه** — ولا يُسقِط المخطّطَ كلَّه.
- `keySections` تُصفّى إلى نصوصٍ فقط قبل `join`.
- وما أجاب به النموذجُ **صحيحاً يبقى بنصّه** — التحصينُ احتياطٌ لا استبدال.

### حدودٌ مكتوبة

- لم يُكتب فحصُ نوعٍ صريحٌ للردّ نفسِه (`typeof raw === 'object'`): `{...raw}` يكفي لكلِّ ما يعيده
  `JSON.parse` (نصّ/رقم/مصفوفة/`null`) — مفاتيحُ زائدةٌ لا تُقرأ، والتحصينُ يملأ المعروضَ كلَّه.
  حُذف بعد أن أظهرت الطفراتُ أنّه بلا مستهلكٍ مقيس (لا سطرَ بلا مستهلك).
- التحصينُ **لا يخترع محتوى**: `appType` يعود إلى رأس الطلب (PM/11)، و`coreValue` إلى أوّل ٨٠ حرفاً
  منه. فهو يمنع «undefined»، ولا يدّعي فهماً لم يحصل — والفرقُ بين المسارَين يبقى ظاهراً في `_source`.
- لم يُقَس **كم مرّةً يُسقط مزوّدٌ حقيقيٌّ حقلاً** — القياسُ هنا على صيغٍ ممكنة، لا على تكرارٍ ميدانيّ.

## نسخةُ الكود العاملة — الخدمةُ تقولها، ولا تُستنتَج (2026-09-07)

سؤالٌ تكرّر في تشخيصٍ حقيقيٍّ ثلاثَ مرّاتٍ في يومٍ واحد ولم يكن لسجلّ الخادم جوابٌ عنه:
**أيُّ كوميتٍ يعمل الآن؟** فبعد دمج إصلاحٍ يبقى احتمالٌ قائمٌ أنّ النشرة أقدمُ منه — فيُقاس سلوكُ كودٍ
غيرِ الذي أُصلح، وهو أسوأُ ما يصيب قياساً: نتيجةٌ صادقةٌ عن نسخةٍ خاطئة.

المنهجُ نفسُه المتّبع في «🚦 جاهزية الإطلاق»: تقريرٌ تقوله الخدمةُ بنفسها بدل استنتاجٍ من الخارج.

### العقد

- `services/buildInfo.js#resolveBuildInfo(env, root)` → `{commit, short, branch, source}`.
  الترتيب: متغيّرُ المنصّة (`RENDER_GIT_COMMIT` / `SOURCE_COMMIT` / `GIT_COMMIT` /
  `VERCEL_GIT_COMMIT_SHA`) ثمّ `.git` **قراءةً مباشرة** — `HEAD` مرجعاً أو رأساً منفصلاً، بلا تشغيل
  `git` في الإقلاع ولا رميٍ على مستودعٍ ناقصٍ أو مشوّه.
- `buildInfoLine(info)` — سطرُ الإقلاع. يُطبع بعد `🟢 JAOLA OS Server on Port 4000` مباشرة.
- **الحدّ**: لا يُعلَن ما ليس معروفاً. `SHA` يُتحقَّق منه بـ`^[0-9a-f]{40}$` — فمتغيّرٌ موجودٌ بقيمةٍ
  ليست كوميتاً (اسمُ فرع، نصٌّ عربيّ، ٤٠ محرفاً غيرَ ستّ عشريّة، أو SHA بلاحقة) **لا يُطبع كأنّه
  كوميت**؛ يُقال «نسخةُ الكود غير معروفة (لا متغيّرَ نشرٍ ولا مستودع)».
- ومُعرِّفُ الكوميت ليس سرّاً — هو في المستودع نفسِه — فلا شيءَ في هذا السطر يكشف مفتاحاً.

### حدودٌ مكتوبة

- أسماءُ متغيّرات المنصّات الأربعة مأخوذةٌ من توثيقها، **ولم يُقَس أيُّها تضعه Render فعلاً** في هذا
  الحساب. والاحتياطُ يغطّي ذلك: إن لم يُضبط أيٌّ منها تُقرأ `.git` من صورة النشر إن وُجدت، وإلّا
  قيلت «غير معروفة» صراحةً — وهي بذاتها معلومةٌ صادقة.
- لا يُعرض تاريخُ البناء ولا رسالةُ الكوميت: المطلوبُ تمييزُ نسخةٍ من نسخة، والمختصرُ يكفيه.

## `AI_PROVIDERS` — الاستبعادُ لا يُلغي مفتاحاً (2026-09-07)

طلبٌ تشغيليّ من صاحب المنصّة: حصرُ التجارب مؤقّتاً في Groq وDeepSeek. الطريقُ الساذج حذفُ مفتاحَي
Gemini وOpenAI من بيئة Render — لكنّه **يُتلف إعداداً** ويحتاج استرجاعَ قيمتَين لا نملكهما، ويجعل
الرجوعَ عن التجربة عمليّةً لا سطراً.

### العقد

- `PROVIDER_NAMES` — `['groq','deepseek','gemini','openai']` مُجمَّدةً، بترتيب السلسلة.
- `resolveEnabledProviders(raw)` → `{ enabled: Set, asked: [], unknown: [] }`. قائمةٌ بفواصل، تُقلَّم
  مسافاتُها وتُوحَّد حالةُ أحرفها (الإعدادُ يُكتب بيد إنسان). وغيابُ المفتاح — أو قيمةٌ فارغة —
  **يعني الأربعةَ كلَّها**، فلا يغيّر السلوكَ القائم.
- `isProviderEnabled(name)` — يقرؤه `coderAgent`.
- يُقرأ **مرّةً عند الإقلاع** ويُعلَن في سطره: 🔇 لكلّ مزوّدٍ مفتاحُه موجودٌ وهو مُستبعَد، و⚠️ تُسمّي
  الأسماءَ المجهولة. إعدادٌ يُعلَن في سطر البدء ثمّ لا يتبدّل تحت المهمّة.

### الموضعان — والحدُّ الذي قِيس قبل الكتابة

الحارسُ يصل موضعَين لا موضعاً واحداً:

1. **السلسلة** — حلقاتُ `createWithFailover` الأربع، كلٌّ بشرطها.
2. **مسارُ المولّد المباشر** — `coderAgent#selectModels`. قِيس بالبحث أنّ `callDeepSeek` و`callGemini`
   ينادِيان `deepseek` و`ai` **مباشرةً خارج** `createWithFailover` (ثلاثةُ نداءات: `coderAgent.js`
   عند `chat.completions.create` مرّتين و`models.generateContent` مرّة). فحارسٌ في السلسلة وحدَها كان
   سيترك Gemini المستبعَدَ يعمل من هنا — وهو عينُ ما طُلب منعُه.

وخطٌّ بلا مزوّدٍ مُفعَّل لا يقول «فشلت جميع النماذج» ولم يُجرَّب واحد: `NO_PROVIDER_MSG` تدلّ على
الإعداد لا على عطل.

### الاسمُ المجهول لا يُصحَّح صامتاً

`AI_PROVIDERS=groq,deepsek` تُفعّل Groq وحدَه، وتُعلِن `deepsek` مجهولاً. وقائمةٌ كلُّها مجهولة →
**لا مزوّد**: «لا يوجد مزود AI مُهيأ» رسالةٌ تدلّ على الإعداد، والعودةُ الصامتة إلى «الكلّ» تُخفيه —
وهي عينُ «الاختلافِ الصامت».

### حدودٌ مكتوبة

- `selectModels` مصدَّرةٌ **لأنّ الحارسَ يُقاس سلوكاً**: أوّلُ صياغةٍ ثبّتت عددَ مواضعِ الحارس في
  النصّ، فنجا منها حذفُ الترشيح نفسِه وخطأُ وسمِ مدخلةٍ باسمِ مزوّدٍ آخر (طفرتان). وتثبيتُ شرطِ حلقةِ
  السلسلة بسطره وحدَه نجا منه حذفُ الحارس، لأنّ **سطرَ الإقلاع يحمل الشكلَ نفسَه** — فثُبّت بكتلته.
- ما لم يُقَس: أثرُ الاستبعاد على جودة الناتج. الغرضُ تشخيصيّ ومؤقّت — أيُّ المزوّدَين يُجيب فعلاً —
  لا حكمٌ على أيّهما أفضل.
- `utils/aiProvider.js` وحدةٌ يتيمةٌ مُقرّة لا يصل إليها الخادم؛ لم تُمَسّ، فلا يبلغها هذا المفتاح.

## اسمُ الموديل — «لا يوجد» تُقال، و«هذا يوجد» لا تُقال (2026-09-07)

قِيس من سجلّ إنتاجٍ حقيقيّ بعد حصر التجارب في مزوّدَين (16:12:58–16:13:00):
...
فالسقوطُ ليس في الحلقتين المُستبعَدتَين: **الأربعةُ ساقطون**، وبسببَين مختلفَين. DeepSeek رصيدٌ لا
يحلّه كود. أمّا Groq فعطبُ **إعداد**: صاحبُ المنصّة ضبط `GROQ_MODEL` في Render وبقي 404 — عبر ثلاث
نشراتٍ في يومٍ واحد، ولم يكن لأحدٍ جوابٌ عن سؤالين يُجيبهما سطر:

1. **هل وصل الضبطُ إلى الكود؟** لا سطرَ يقول أيَّ اسمٍ يُستعمل، فلا يُميَّز «ضُبط ولم يُقبَل» من
   «لم يصل الضبطُ أصلاً» — وهما علاجان مختلفان تماماً.
2. **وأيُّ اسمٍ يُقبَل؟** المزوّد يقول «لا يوجد» ولا يقول «هذا يوجد» — ونحن لا نسأله، رغم أنّ
   `models.list()` موجودةٌ في SDK الثلاثة (قِيست: `typeof … === 'function'`).

### العقد

- `modelReportLines(env, resolved, only)` — سطرٌ لكلّ مزوّدٍ في `only` باسم موديله و**مصدره**
  (`بيئة` / `افتراضي`). يُطبع عند الإقلاع لـ**المزوّد العامل وحدَه** (مفتاحُه موجودٌ وغيرُ مستبعَد).
- `discoverModels(client)` — يسأل المزوّدَ عمّا يقبله. **لا يرمي أبداً**: يُنادى داخل معالج فشل،
  ورميةٌ منه تُحوّل عطبَ إعدادٍ مفهوماً إلى انهيارٍ غامض. يُقصّ عند `MODEL_DISCOVERY_LIMIT` (٢٠) —
  سطرُ سجلٍّ يُقرأ، لا مِلفّ.
- ويُسأل المزوّدُ **مرّةً واحدةً لكلّ عمر عملية**، و**عند صنف `config` وحدَه**: الرصيدُ (402) ليس
  اسمَ موديل، وسؤالُ المزوّد عنه نداءُ شبكةٍ بلا معنى.
- واسمُ الموديل ليس سرّاً — افتراضاتُه مكتوبةٌ في المستودع — فطبعُه لا يكشف مفتاحاً. وهذا بالضبط ما
  يفصله عن كلّ ما لا يُطبع في هذه السطور؛ حارسٌ صريحٌ يمنع اقترابَ السطر من صيغة مفتاح.

### حدودٌ مكتوبة

- **عطبٌ في أوّل صياغةٍ لهذه الجولة، أوقعه حارسٌ قائم**: الأسطرُ الأربعةُ كانت تُطبع **بلا شرط**
  عند التحميل، فتكتب على المخرَج القياسيّ الذي يحمل نتائجَ مُشغّل الاختبارات — عطبُ #486 بعينه.
  أوقعه `stdoutChannel`. الإصلاحُ شرطُ «المزوّد العامل وحدَه» لا إسكاتُ الحارس؛ وهو أصدقُ سلوكاً
  أيضاً: سطرٌ عن مزوّدٍ لا يُنادى ضجيجٌ لا خبر.
- `client?.models?.list?.()` — التسلسلُ الاختياريّ **غيرُ مقيس**: الـ`catch` يعطي النتيجةَ نفسَها
  (`[]`) لعميلٍ بلا القدرة، فلا اختبارَ يفرّق بينهما (نجت طفرةٌ تحذفه). أُبقي شبكةً ثانيةً في دالّةٍ
  عقدُها «لا ترمي أبداً»، ويُقال هنا إنّه غيرُ مقيس بدل ادّعاء تغطيته.
- Gemini غيرُ موصولٍ بالاكتشاف: واجهتُه ليست متوافقةً مع `models.list` بهذا الشكل، ولا رسالةَ عطبٍ
  مقيسةٍ منه في هذه الجولة تطلبه. لا سطرَ بلا مستهلك.
- ولا يُخترع اسمُ موديلٍ بديل: القائمةُ تُعرض على صاحب المنصّة **ليختار**. لا دليلَ عندنا على
  أيّها الأنسب، واختراعُ اسمٍ هو ما جعل الاسمَ الميّتَ مكتوباً في تسعة مواضع قبل هذا.

## تشخيصُ المزوّدين في لوحة المشروع — «لا جديد» صادقةٌ ولا تدلّ على شيء (2026-09-07)

قِيس **ثلاثَ مرّاتٍ في يومٍ واحد** على مشروعٍ حيّ: صاحبُ المنصّة ينظر في لوحته فيرى السطرَ نفسَه
حرفاً بحرف مهما اختلف السبب، فيقول «لا جديد، نفس كل شيء» — وهو صادق. لكنّ السببَين اللذين قِيسا
يفترقان تماماً في العلاج:

| المزوّد | الصنف | العلاج |
|---|---|---|
| Groq | `config` | سطرٌ في إعداد المنصّة — بلا نشر |
| DeepSeek | `quota` | دفعٌ. لا سطرَ يُصلحه |

وكلاهما كان يُقال: «نفد رصيد المزوّد أو مفاتيحه غير صالحة». فيُفتح سجلُّ الخادم في كلّ مرّة —
شاشةٌ أخرى، وسؤالٌ مني، ودورةٌ ضائعة.

### العقد

- كلُّ عطبٍ يُوسَم باسم مزوّده عند التقاطه في `createWithFailover` (`e.provider = '…'`).
- `aggregateFailure` يبني `err.diagnosis = [{ provider, kind }]` — **الاسمُ والصنفُ فقط**.
- `describeAIFailure(diagnosis, lang)` — لفظُنا لكلّ صنف، مفصولاً بـ` · `.
- ويصل الموضعَين معاً: **نصَّ الخطأ** (فسطرُ السجلّ الحيّ يعرض `message` مباشرةً — وهو أظهرُ ما
  يُرى) و**رسالةَ الشات** (`buildFailureChatMessage`، وهي لا تُضمّن `message` فلا ازدواج).

### الحدُّ الأمنيّ — وهو سببُ وجود حقلٍ منفصل

`err.causes` تحمل نصَّ المزوّد الخام وتبقى **لسجلّ الخادم وحدَه**. `diagnosis` هو الوحيدُ المسموحُ
عرضُه لصاحب المشروع، ولا يحمل إلّا اسمَ مزوّدٍ وصنفاً من قائمتنا المغلقة. وهذا ما يمنع عودةَ #588:
رابطُ فوترةِ حسابِ المنصّة الذي وصل إلى سجلّ مشروع مستخدم. اختبارٌ صريحٌ يُطعم رسالةً فيها رابطٌ
ونصُّ Google الخام، ويؤكّد أنّ شيئاً منهما لا يخرج.

### حدودٌ مكتوبة

- أسماءُ المزوّدين (`groq`/`deepseek`) تظهر لصاحب المشروع. ليست أسراراً، والرسالةُ القائمة تذكر
  «المزوّد» أصلاً — فهذه دقّةٌ أكبر في الإفصاح نفسِه، لا إفصاحٌ جديد.
- الصنفُ `transient` يُذكر أيضاً، لكنّه لا يُعلن `aiUnavailable`: عطبٌ عابرٌ واحدٌ يُبقي إعادةَ
  المحاولة مجديةً — وهو حدُّ #588 محفوظاً كما كان.
- ما لم يُقَس: أثرُ السطر على مستخدمٍ ليس صاحبَ المنصّة. الصياغةُ تسمّي الصنفَ والعلاج، ولم تُختبر
  على قارئٍ غير تقنيّ.

## قيمةُ حقلٍ يملؤه إنسان لا تُطبع قبل فحصِ شكلها (2026-09-07)

قِيس في الإنتاج بعد ساعاتٍ من دمج #592: وُضع **مفتاح Groq** في `GROQ_MODEL`، فطبعه سطرُ الإقلاع
الذي أضفتُه أنا كاملاً في سجلّ Render:

```
🏷️ [AI Model]: groq = gsk_… (بيئة)
```

وهو أيضاً التفسيرُ الكاملُ لـ`404 The model does not exist` الذي دار حوله يومٌ كامل: كان يُطلب من
المزوّد موديلٌ اسمُه مفتاحُ الحساب.

### الحكمُ الذي كان خاطئاً

كُتب في #592: «اسمُ الموديل ليس سرّاً — افتراضاتُه مكتوبةٌ في المستودع — فطبعُه لا يكشف مفتاحاً».
وهو **صحيحٌ عن الاسم، خاطئٌ عن الحقل**: الحقلُ لا يحمل ما نفترضه، بل ما يكتبه إنسان. والحارسُ
الذي كُتب معه فحص **نصَّ السطر** بقيمٍ من عندي (`'g'`/`'d'`)، فلم تمرّ عليه قطُّ قيمةٌ تشبه مفتاحاً.

### العقد

- `looksLikeSecret(v)` — بادئةُ اعتمادٍ معروفة (`gsk_`/`sk-`/`AIza`/`gh?_`/`xai-`/`r8_`/`hf_`/`Bearer`)
  **أو** سلسلةٌ بطول ٣٦ فأكثر بلا `/`.
- والسطرُ عندها يصير تشخيصاً بدل أن يكون تسريباً: يُسمّى المزوّدُ والخطأ («قيمةٌ تشبه مفتاحاً لا
  اسمَ موديل») ويُدلّ على الحقل الصحيح — **بلا طبعِ القيمة**.

### حدودٌ مكتوبة

- حدُّ الطول **قياسٌ تقريبيّ**: أطولُ اسمِ موديلٍ حقيقيٍّ قِيس بلا `/` هو `llama-3.3-70b-versatile`
  (٢٣ محرفاً)، و٣٦ تترك مسافةً واسعة. واستثناءُ `/` مقصود: أسماءُ المزوّدين المركّبة تتجاوز الحدَّ
  (`meta-llama/Llama-4-Maverick-17B-128E-Instruct`) ولا تشبه اعتماداً.
- الخطرُ المتبقّي: اعتمادٌ قصيرٌ بلا بادئةٍ معروفة يمرّ. لا قياسَ عليه، ولا يُدّعى منعُه.
- وهذا حدٌّ لهذا السطر وحدَه. لم يُراجَع كلُّ ما يُطبع في المنصّة بهذا المنظار — **ديْنٌ مفتوحٌ
  مكتوب**: كلُّ حقلِ إعدادٍ يملؤه إنسانٌ وتُطبع قيمتُه يحتاج المراجعةَ نفسَها.
- `looksLikeSecret` **تعريفُ دالّةٍ لا ثابت**: سطورُ الإقلاع تناديها قبل موضعها في الملفّ، وثابتُ
  `const` في منطقة الموت الزمنيّ حينها فيرمي. أوقعه الاختبارُ قبل النشر.

## عدّادُ الرموز — «مش غالي شوي؟» سؤالٌ لم يكن له جواب (2026-09-07)

سأل صاحبُ المنصّة بعد أوّل بناءٍ ناجح. وقِس بالبحث: **صفرُ** مواضعَ في المنصّة تلتقط
`total_tokens`. فكلفةُ بناءٍ واحدٍ مجهولةٌ تماماً — لا له ولا لي — وكلُّ حكمٍ على «الأغلى/الأرخص»
رأيٌ لا قياس.

والكلفةُ ليست سعرَ الرمز وحدَه: هي **السعر × الرموز × عدد النداءات**، وحلقةُ النقاش تبلغ سبعَ
دورات. فالمقياسُ يعدّ النداءات والرموزَ معاً.

### العقد

- `noteUsage(provider, carrier)` — يسجّل نداءً واحداً؛ `carrier` أيُّ كائنٍ قد يحمل `usage`
  (ردٌّ كامل أو آخرُ قطعةِ تدفّق).
- `readAIUsage()` — **لقطةٌ منفصلة** لا مرجعٌ حيّ (حتّى `byProvider`)، فلا تتبدّل تحت قارئها.
- `usageLine(now, before)` — بلقطةِ بدايةٍ يصير الفرقُ **كلفةَ مهمّةٍ واحدة** لا كلفةَ العملية.
- الوصل: `jcr` يأخذ `context.usageAtStart` عند «🏁 بدء المهمة»، و`reportMissionSuccess` يطبع
  السطر. والردُّ الكامل يُحسب في `createWithFailover`؛ والتدفّقُ يُوسَم `__aiProvider` ويُترك
  لمستهلكه — فالمستهلكُ وحدَه يعرف متى انتهى.

### حدودٌ مكتوبة

- **لا يُرسَل معامِلٌ جديد للمزوّد** (`stream_options: {include_usage}`). مسارُ التدفّق هو مسارُ
  توليد الكود الذي بدأ يعمل بعد يومٍ كامل من التعطّل، وكسرُه بمعامِلٍ قد يرفضه مزوّدٌ ثمنٌ لا
  يُدفع لأجل عدّاد. يُقرأ ما يتطوّع به المزوّد.
- ولذلك **يُقال صراحةً كم نداءً بقي بلا أرقام**: «٣ نداءً بلا أرقامٍ من المزوّد (غيرُ محسوبة)».
  رقمٌ ناقصٌ يُعرض كاملاً أسوأُ من لا رقم.
- الرموزُ ليست مالاً: التحويلُ إلى عملةٍ يحتاج سعرَ المزوّد، وهو يتغيّر ولا يُكتب في الكود.
  الرقمُ الرسميّ في فاتورة صاحب المنصّة؛ العدّادُ يعطيه البسط لا الحاصل.
- ما لم يُقَس: أيُّ نموذجٍ **أفضل** جودةً. اختيرَ `openai/gpt-oss-120b` على أساس السعة (الأكبر
  في القائمة المتاحة) لا على قياسِ جودة. وقرارُ صاحب المنصّة: بناءُ القوالب المعقّدة به ثمّ
  التبديل — وهذا العدّادُ هو ما يجعل «ثمّ التبديل» قراراً برقم.

## خبرُ المهمّة يبقى، والمحاولةُ تُعاد فعلاً — ملاحظتان من الإنتاج (2026-09-07)

ملاحظتان قالهما صاحبُ المنصّة بعد أوّل بناءٍ ناجح، وكلتاهما **عطبُ صدقٍ لا عطبُ شبكة**.

### (١) «كل ما تركت جولا وذهبت لمتصفح آخر... يقطعها في نصفها»

قِيس: `disconnect` في `server.js` لا يُلغي شيئاً — يبثّ الحضورَ وحسب. والمهمّةُ تُكمل على
الخادم وتُنهي ملفّاتها (لقطةُ ٥٢ ملفّاً في سجلّ صاحب المنصّة نفسِه). لكنّ `Socket.IO` يبثّ
إلى **غرفة**، وغرفةٌ فارغةٌ تعني حدثاً يُرمى في الفراغ.

و`chat_history` يستعيد من `${username}::${project}` وحدَه (`server.js:767`)، ولا يكتب في
ذلك المفتاح إلّا مسارُ الشات (`recordTurn`). فتقريرُ التسليم ورسالةُ الفشل لم يكونا يُكتبان
هناك قطُّ. فما رآه ليس مهمّةً مقطوعة بل **مهمّةً تمّت ولم يصله خبرُها** — وهو أسوأ: ملفّاتُه
موجودةٌ وهو يظنّها ضاعت.

**العقد**: `rememberMissionNote(username, project, content)` في `services/conversationStore.js`.

- المفتاحُ `${username}::${project}` — **المفتاحُ الذي يُقرأ**، لا الاسمُ المجرّد. (اختبارٌ يثبت
  أنّ الاسمَ المجرّد يبقى فارغاً: كتابةٌ هناك ضياعٌ صامت.)
- **سطرُ مساعدٍ واحد**، لا `recordTurn`: تلك تدفع دورةً (مستخدم + مساعد)، فتختلق على صاحب
  المشروع كلاماً لم يقلْه ثمّ يدخل ذلك المختلَقُ نافذةَ النموذج.
- لا طيَّ ملخّصٍ معها: لا تلخيصَ بلا نموذج.
- فراغٌ لا يُخزَّن؛ وفشلُ الحفظ لا يُعطّل مسارَ المستخدم (التقريرُ بُثّ أصلاً — هذا حفظُه).
- المستهلكان: `stages/reportMissionSuccess.js` (تقريرُ التسليم) و`jcr.js` (رسالةُ الفشل)،
  وكلاهما يحفظ **النصَّ المبثوثَ حرفيّاً** — لا ملخّصاً له.

**حدٌّ مكتوب**: السجلُّ الحيّ (`log`) لا يُعاد بثُّه عند العودة — هذا حفظُ **الخبر** لا إعادةُ
تشغيلِ الشريط. إعادةُ السجلّ قطعةٌ أكبرُ منفصلة.

### (٢) «مزود الخدمة كثير الانقطاع»

قِيس: `createWithFailover` كان يجرّب كلَّ مزوّدٍ **مرّةً واحدة**؛ لا إعادةَ محاولةٍ ولا تراجع.
ورسالتُنا لصاحب المنصّة تقول «نعيد المحاولة» — وعدٌ لا يفي به الكود. ولذلك سقط سبعةُ وكلاءَ
من سبعة وانتهى البناءُ إلى المولّد الكلاسيكيّ، بينما كانت تكفي محاولةٌ ثانيةٌ بعد أقلَّ من ثانية.

**العقد**: `attemptChain` هي السلسلةُ كما كانت (نقلٌ حرفيّ)، و`createWithFailover` غلافٌ يعيدها.

- الإعادةُ على `transient` **وحدَه**. `aiUnavailable` (رصيدٌ منتهٍ، مفتاحٌ باطل، إعدادٌ خاطئ)
  لا يُعاد عليه: التكرارُ عليه هدرٌ محض — وهو الفرقُ الذي بناه #593.
- تراجعٌ تصاعديّ `RETRY_BASE_MS × 3^attempt` (٤٠٠ms، ١٢٠٠ms)، ومجموعُ الانتظار مقيسٌ
  ومحدود: مستخدمٌ ينتظر أمام شاشة.
- `AI_MAX_RETRIES` محصورٌ في [١،٣] مهما كتب في البيئة — حدٌّ يُبقي البناءَ حيّاً لا معلَّقاً.
- `AI_RETRY_BASE_MS` للاختبار (يُضبط ١ms) لا للإنتاج.
- **و«لا مزوّدَ مُهيأ» أدومُ الأعطال كلِّها**: حين لا يحاول مزوّدٌ أصلاً (لا مفاتيح، أو كلُّها
  مُستبعَدةٌ بـ`AI_PROVIDERS`) تعود `aggregateFailure` بخطأٍ عليه `aiUnavailable` — فلا إعادة.
  أوقعَ هذا العطبَ **الحزمةُ نفسُها**: كلُّ اختبارٍ يمرّ على السلسلة (وهي تعمل بلا مفاتيح) صار
  ينتظر ١٫٦ ثانية على شرطٍ لا تُصلحه محاولةٌ ثانية.

**حدودٌ مكتوبة**: الإعادةُ تلفّ **السلسلة** وحدَها. نداءاتُ `coderAgent.js` الثلاثةُ المباشرةُ إلى
`deepseek`/`ai` خارجَها لا تُعاد (لها خطُّ أنابيبها الخاصّ بمزوّدَين)، و`chatResponse.js` له
حلقتُه القديمة (محاولتان بـ٢٥٠٠ms) ولم تُمسّ. توحيدُهما قطعةٌ منفصلة لم تُقَس بعد.

## البوّابةُ تُرضى بألفاظها هي — والكلمةُ العابرةُ تختار المنتج (2026-09-07)

طلبَ صاحبُ المنصّة **منصّةَ هندسةِ برمجيّاتٍ ذاتيّةَ التشغيل** بوثيقةٍ من تسعةِ بنودٍ مرقّمة.
فبُني له **سوقٌ إلكترونيّ**، ثمّ أعلن الحكمُ `requirements-verify ✓` و`PASS` على **٩ من ٩** من بنوده.

أُعيد إنتاجُ العطب **بلا مزوّدٍ أصلاً**. سببان مستقلّان، كلاهما في الكود لا في النموذج.

### (١) الأثرُ اللفظيّ لا يُثبت تنفيذاً — و«أين» وقع الأثر هو الفرق

`traceSections` تَعُدّ البندَ «له أثر» إن ظهرت **أيُّ كلمة** من عنوانه في نصّ المنتج،
و`buildSectionFixInstruction` تُسلّم النموذجَ **عناوينَ البنود بنصّها** وتطلب تنفيذها. فالحلقةُ تُغلق
على نفسها: أرخصُ طريقٍ لإرضاء مقياسٍ يقرأ النثرَ هو كتابةُ ألفاظه. مقيسٌ: صفحةُ تسوّقٍ + تذييلٌ ميّت
يسرد العناوين = **٩/٩**؛ الصفحةُ وحدَها = **٠/٩**.

وعقدُ الدالّة كان يقول الحقيقةَ في نصّه طوال الوقت — «أثرٌ لا تنفيذ» — ثمّ يعيد `status: 'pass'`.
فالكلامُ صادقٌ والحالةُ كاذبة، والحكمُ يقرأ الحالة.

**العقد الجديد** — والعلاجُ ليس تشاؤماً شاملاً (مقياسٌ يقول الشيءَ نفسَه لكلِّ بناءٍ لا يُقرأ، وهو
درسُ «لا جديد» بعينه)، بل تمييزُ **موضع** الأثر:

- `scriptCorpus(files)` — نصُّ ما **يشغّله شيء**: ملفّاتُ `.js`/`.mjs`/`.cjs` و`<script>` المضمَّن.
- `traceSections`/`traceRequirements` تعودان بـ`decorative ⊆ traced`: ما لا يمسّه سطرُ شفرة.
- `requirementsTraceOutcome`: غيابٌ → `fail` (كما كان، والغيابُ قاطع)؛ أثرٌ **نثريٌّ** → `unverified`
  بعدده وأسمائه؛ أثرٌ يصل شفرةً تعمل → `pass` مكتوباً عليه «أثرٌ لا تنفيذ».

**حدٌّ مكتوب**: هذا يفرّق بين دليلٍ **قد** يكون تنفيذاً ودليلٍ لا يكون. لا يُثبت تنفيذاً — ذلك
للمحقّق السلوكيّ أو لمحقّق المتطلّبات بمزوّد. ودالّةٌ باسمِ المفهوم في شفرةٍ لا يناديها أحدٌ تمرّ.

### (٢) كلمةٌ عابرةٌ في جملةِ طموحٍ اختارت المنتجَ كلَّه

وردت `marketplace` مرّةً واحدة، في «…custom agents, workflows, and an agent **marketplace**» —
جملةٌ عن مستقبلٍ بعيد. وسندُ الاختيار المُسجَّل يفضح نفسَه:
`{ hits: ["marketplace"], roleCoverage: null, entityOverlap: null }` — **الفهمُ لم يساهم بشيء**،
والكلمةُ وحدَها حسمت. وPM/1 وُضع ليطابق بنموذج المنتج لا بالكلمات.

والتمييزُ كان **محسوباً ولا يُقرأ**: `explicit` لا تُرفع إلّا حين تقع الكلمةُ فيما يسمّي به
المستخدمُ منتجَه (رأسُ الوثيقة، PM/11) — وهذه وقعت بعد البنود.

**العقد**: `incidental` = إصابةٌ واحدةٌ لا غير + خارجَ ما يسمّي به المستخدمُ منتجَه + لا سندَ من
نموذج الفهم. وحينها **لا كلون** والسببُ يُسمّى (`incidental-keyword:<الكلمة>`) — فالبناءُ الحرّ
أصدقُ من قالبٍ لمنتجٍ آخر.

ومعه: **الرأسُ يُرجَّح على المتن** (`explicit` تزن ٥). قِيس حين أوقعَ الفيتوُ الجديدُ مطابقةً
صحيحة — وثيقةٌ رأسُها «أريد بناء صيدلية» وبنودُها تذكر «الفواتير» كان يحسمها ترتيبُ البُناة لا
الرأس. فالفيتوُ كشف عطباً أقدمَ منه.

**حدودٌ مكتوبة**:
- شرطُ «وثيقةٌ مرقّمة» **أُسقط لأنّه ميّت**: بغير وثيقةٍ يصير مُقارَنُ التسمية هو الهدفَ كلَّه فترتفع
  `explicit` من تلقائها. ويبقى الطلبُ القصير الذي إصابتُه الوحيدة **عبارةُ مسار** («نظام») مشمولاً
  بالفيتو — وذلك هو المقصود.
- `modelBacked` **غيرُ مقيسٍ مستقلّاً**: في كلِّ حالةٍ بُنيت وجدَ شرطُ العدد يسبقه (لأنّ إصابةَ نموذج
  الفهم تُحسب في العدد). يبقى مكتوباً ولا يُدَّعى أنّه مقيس.
- وأغلقَ الفيتوُ **آخرَ بقيّةٍ من العطب الأصل** (PM/1): «نظام إدارة وتشغيل تاكسي» بلا فهمٍ كانت
  تختار ERP بعبارةِ المسار «نظام إدارة» وحدَها — وكانت مثبَّتةً في الاختبارات «حدّاً أدنى لا مرغوباً».
  فصار الحدُّ الأدنى هو المرغوب، بلا سطرٍ كُتب لأجلها.
- وهذا **لا يجعل جولا تبني منصّةَ وكلاء**. لا قالبَ لها بين الواحدِ والأربعين، ولا مسارُ البناء من
  الصفر مُقاسٌ بمزوّدٍ حيٍّ قطُّ. غايةُ هذا أن تتوقّف عن **ادّعاء** أنّها فعلت.

## أوّلُ بناءٍ حرٍّ حيّ: النفيُ يُوجِب، والذاكرةُ ترث المهلوَس (2026-09-07)

طُلب متتبّعُ حفظِ قرآن **«يعمل بالكامل داخل المتصفّح بلا خادم ولا حساب»** على المسار الحرّ
(أوّلُ قياسٍ حيٍّ لهذا المسار في تاريخ المنصّة). خرج بـNext.js + API + Prisma، وفُهم
منصّةَ تعليمٍ مدرسيّة، وانتهى إلى `FAILED — requirements-verify ✗`.

**والحكمُ كان صادقاً** — وهذا أوّلُ اختبارٍ حيٍّ لبوّابة «الأثر اللفظيّ» المدموجة قبله بساعة.
عطبان آخران أُعيد إنتاجُهما بلا مزوّد.

### (١) الجملةُ التي تنفي هي التي تُوجِب

`needsBackend` تُطابق كلماتٍ مفردةً ولا ترى سياقاً. فالتقطت:
- **«حساب»** من «بلا **حساب**» — النفيُ نفسُه هو ما أوجب.
- **«تسجيل»** من «**تسجيل** ما حُفظ» (تسجيلُ حفظٍ لا تسجيلُ دخول).
- **«بيانات»** من «بقاء **البيانات** … يبقى في المتصفّح» — نصٌّ يقول عكسَها.

**العلاجان معاً**، لأنّ أوّلَهما وحدَه لم يكفِ (مقيس: بقي `true` بعده):

- `stripNegated(text)` — أداةُ نفيٍ (`بلا`/`بدون`/`دون`/`without`/`no`) تُلغي ما بعدها إلى
  أوّل فاصلٍ **أو رابطِ استدراك** (`لكن`/`إلّا`/`but`). فـ«متجرٌ بلا تسجيل دخول لكنّ فيه دفعاً»
  يبقى بحاجةِ خادم — مقيسٌ باختبارٍ بلا فاصلةٍ يعزل الرابطَ وحدَه.
- وأُسقطت **«تسجيل»** و**«بيانات»** المفردتان من القائمة. الصيغتان الدقيقتان
  (`تسجيل دخول`، `قاعدة بيانات`) باقيتان، و«صفحة تسجيل عضوية» يلتقطها `عضوية`.
  فما سقط هو الاتّساعُ لا الإشارة — مقيسٌ: كلُّ إثباتٍ صريحٍ ما زال يُلتقط.

**حدٌّ مكتوب**: هذا نفيٌ **معجميّ** لا نحويّ. «لا أريد أن أبني بلا حساب» تُقرأ نفياً وهي
إثبات. لم يُقَس مثالٌ حقيقيٌّ كهذا ولا يُدَّعى تغطيتُه.

### (٢) «يستفيد منه كل مشروع لاحق» — بعد أن قال القاضي FAILED

`recordModel` كان مشروطاً بـ`verdict?.ok` وحدَه — وهو حكمُ **المحقّق السلوكيّ** لا حكمُ
المنتج. فبناءٌ تعمل أزرارُه ولا يمثّل ما طُلب منه كان يُودِع فهمَه المهلوَس في ذاكرةٍ دائمة
ويُعلن ذلك لصاحب المنصّة. والتعليقُ فوق السطر يقول «نساهم فقط بما نجح تحقّقه» — **دعوى لا
يضمنها الشرط**، وهو عينُ عطب البوّابة بوجهٍ آخر.

**وهذا وحدَه يتراكم**: خطأٌ واحدٌ يُورَّث لكلِّ مشروعٍ تالٍ في فئته.

**العقد**: البوّاباتُ السابقة مُسجَّلةٌ في `context.verdicts` حين تصل هذه المرحلة
(`requirements-verify` قبلها في `DELIVERY_STAGES`). فيُقرأ ما سجّلته: `fail` أو `unverified`
يمنع الإيداع؛ و`skipped` لا يمنع (التخطّي ليس فشلاً).

---

## PM/22 — المقارنةُ المفتوحة: الفهمُ يُقاس بكلمات الطلب لا بقائمةٍ مغلقة (2026-09-08)

بعد أن قالت البوّابةُ الصدقَ في `from0` (`FAILED — requirements-verify ✗`) بقي السؤالُ الأصعب:
**لمَ فُهم متتبّعُ حفظِ القرآن منصّةَ تعليمٍ مدرسيّة أصلاً؟**

### أوّلاً: لا بوّابةَ بين الفهم والسؤال

`normalizeProjectModel` تفحص **شكلَ** النموذج (هل للاسم قيمة؟) ولا تفحص **صلتَه بالطلب** أبداً.
فما يعيده النموذجُ اللغويّ يمضي كما هو إلى المصمّم والخلفيّة وكاتبِ كلِّ صفحةٍ ومحقّقِ المتطلّبات
ومكتبةِ النماذج الدائمة.

### ثانياً: والمقياسُ القائم لا يُميّز

`domainFidelity` (PM/3) موجودةٌ منذ زمن، لكنّها لا تُطبَّق إلّا على **ما بُني**. وقِيس أنّها لو
طُبّقت على **الطلب** لما ميّزت شيئاً — المهلوَسُ والصادقُ كلاهما `covered:0`:

| النموذج على مواصفة «وِرد» | مغطّى | ملوَّث؟ |
|---|---|---|
| المهلوَس (ما حدث فعلاً) | ٠ | لا |
| نموذجٌ صادقٌ تماماً | ٠ | لا |

والسببُ أنّ **طرفَي المقارنة لا يتكلّمان لغةً واحدة**: طرفُ الفهم **مفتوح** (`conceptOf` تعود
بالاسم المطبَّع نفسِه حين يغيب المعجم)، وطرفُ النصّ **مغلق** (`conceptsInText` لا ترى إلّا المعجم).

### ثالثاً: المعجمُ أعمى خارج التجارة — والقياسُ صفرٌ في ٨/٨

المعجمُ **٧٠ مفهوماً** (٢٥ دوراً + ٤٥ كياناً) كلُّها من عالم التجارة والخدمات:

| الهدف | مفاهيم | النوع المستنتَج |
|---|---|---|
| متتبّع حفظ القرآن | **٠** | — |
| متتبّع عادات | **٠** | — |
| بطاقات مذاكرة | **٠** | — |
...
| ميزانيّة شخصيّة | **٠** | — |
| لعبة كلمات | **٠** | — |
| *(مرجع)* متجر إلكترونيّ | ٥ | `ecommerce` |
| *(مرجع)* نظام عيادة | ٥ | `clinic` |

فـ`applicable:false` و`substantive:false` و`modelProjectType ← null`: **كلُّ** بوّابات عقل المنتج
صامتةٌ في الصنف كلِّه، ولا يبقى إلّا تخمينُ النموذج اللغويّ بلا رقيب. والمفهومُ الوحيدُ الذي
رأتْه جولا في مواصفة «وِرد» كلِّها كان `account` — من عبارة «بلا **حساب**».

### العقد: `goalFidelity(model, goalText)` — جسران، والاسمُ مسنودٌ إن عبَر أحدَهما

1. **جسرُ المعجم**: الاسمُ مفهومٌ معروف والطلبُ ينطق به — يجسر اللغتين (`Product` ↔ «منتجات»).
2. **جسرُ اللفظ**: إحدى كلمات الاسم تلتقي كلمةً دالّةً في الطلب — يعمل **خارج المعجم كلِّه**.

- `goalWords(text)`: الكلماتُ المتكرّرة بعد حذف أدوات الربط وألفاظ المنصّة («تطبيق»، «صفحة»، `app`…).
- `applicable`: اسمان دالّان فأكثر (عتبةُ `domainFidelity` نفسُها — الواحدُ يُصادَف).
  والأسماءُ العامّة (`User`/`Item`) لا تُحسَب دليلاً ولا تُهمة.
- `ungrounded`: `applicable` **ولا اسمَ واحدٌ** عبَر أيَّ جسر. **انعدامُ سندٍ لا سندٌ ناقص** —
  فهمٌ أغنى من حرف الطلب («وِرد» + `Streak` + `Setting`) لا يُنذَر (مقيس).

**مقيس**: المهلوَس ٠/٦، الصادق ٤/٤ — فصلٌ تامّ.

### المستهلكان الحيّان

- `stages/understand`: «⚠️ فهمي لا يمسّ طلبَك: لا أثرَ في وصفِك لـ…» — **يسمّي** ما لا أثرَ له.
- `stages/verify`: شرطٌ ثالثٌ على الإيداع في مكتبة النماذج بجوار `verdict?.ok` و`gatesClean`.
  البوّاباتُ تحكم على **المبنيّ**، وقد يُبنى بأمانةٍ تامّة على فهمٍ لا يمسّ الطلبَ أصلاً.
  وهذا أخطرُ المواضع: ما يُودَع يُورَّث لكلِّ الفئة.

### ما لم يُبنَ، ولمَ — قرارٌ مقيس

لا **استبدالَ** للفهم بكلمات الطلب. قِيس على الأهداف الثمانية أنّ تلك الكلمات صالحةٌ **للمقارنة**
لا **للتسمية**: إشارةٌ صادقة («حفظ، آيات، عادة، بطاقة، صلاة، قبلة، لاعب») مخلوطةٌ بضوضاء
(«تزامه، بكل، ذهب، أتم» — لواصقُ الضمائر لا تُنزع، والأفعالُ تمرّ). **دَينٌ مفتوحٌ مكتوب.**

### حدٌّ مقيسٌ مكتوب

فهمٌ **صادق** بأسماءٍ إنجليزيّة في مجالٍ **خارج المعجم** على طلبٍ عربيّ لا يعبر أيَّ جسر
(`Wird/Surah/Memorizer` ← ٠/٤) — إنذارٌ كاذب. ولذلك لا يُسقِط المستهلِكُ بناءً ولا يستبدل فهماً:
يقول وحسب. والمجالُ المعروف لا يقع فيه (`Product/Order/Invoice` على طلبٍ عربيّ ← ٣/٣، مقيس).

وفي `wordsMeet` احتواءٌ لأربعةِ أحرفٍ فأكثر لا بدايةٌ وحدَها: `normalizeConceptText` تنزع «ال»
وسوابقَها الملتصقة ولا تنزع «لل» — فطلبٌ يقول «تعود **للمراجعة**» كان يُسقط اسماً صادقاً اسمُه
«مراجعة» (مقيس). أُصلح في الجسر لا في التطبيع: نزعُ «لل» هناك يمسّ كلَّ المطابقات ويلتبس
(«للعبة» = لِـ+لعبة لا لِـ+اللعبة).

### الدرس

**مقياسٌ طرفاه بلغتين مختلفتين يقول الشيءَ نفسَه للصادق والكاذب.** وهو وجهٌ ثالثٌ لدرسٍ تكرّر:
PM/9 وجدت المقياسَ يُرضى بألفاظه هو، و«الأثرُ اللفظيّ» وجدتْه يُرضى بنصٍّ لا يشغّله شيء، وهذه
تجده **صامتاً** — والصمتُ أسوأُ الثلاثة، لأنّ «لا شيءَ» تُقرأ «لا بأسَ».

---

## PM/23 — التسميةُ من عناوين صاحبِ المشروع (2026-09-08)

PM/22 جعلت جولا تعرف أنّها **لا تفهم**. هذه تجعلها تسمّي — بكلمات صاحب المشروع نفسِه.

### الدَّينُ الذي سقط بقياسٍ أفضل

كُتب في PM/22 أنّ **استبدال** الفهم يحتاج تحليلاً صرفيّاً عربيّاً، لأنّ كلماتِ المتن بالتكرار
تخلط صدقاً بضوضاء («تزامه»، «بكل»، «ذهب»، «أتم»). وكان ذلك **قياساً للإشارة الخطأ**.

الإشارةُ الصحيحة: **رأسُ كلِّ عنوانٍ مرقّم**. صاحبُ المشروع يكتب عناوينَه **أسماءً** لا أفعالاً،
مجرّدةً من لواصق الضمائر — فهي تسميتُه هو لأجزاء منتجه. المقياسُ على ستِّ مواصفات:

| القاعدة | نظيفة |
|---|---|
| **رأسُ العنوان** | **١٨/١٩ — ٩٥٪** |
| كلُّ كلمات العنوان | ٢٣/٢٨ — ٨٢٪ |
| التكرارُ في المتن | نحوُ ٥٠٪ |

الفرقُ بين الأولى والثانية هو الصفة: «الورد **اليوميّ**»، «التقرير **الشهريّ**» — وصفٌ لا كيان.

### العقد: `headingEntityNames(goalText)`

رأسُ كلِّ عنوان (أوّلُ كلمةٍ ذاتِ ثلاثة أحرفٍ فأكثر)، بشرطين:
- ليست من **ألفاظ إطار الوثيقة** (`FRAME_HEADINGS`: الغاية، الشاشات، الإعدادات، المتطلّبات…).
  وهذه ليست قائمةَ مجال — تلك مغلقةٌ بطبعها وهي علّةُ PM/22 — بل ألفاظُ **كتابةِ المواصفات**.
- ليست رقماً (لاتينيّاً **أو عربيّاً** — `٢٤٠` كانت تمرّ، والحارسُ يعرف `\d` وحدَها).

تعود **فارغةً** لطلبٍ بلا بنودٍ مرقّمة. وذلك صحيح: لا عناوينَ فلا تسمية.

### متى يُستبدَل الفهم — وحدودُه

**فقط** حين يكون `goalFidelity(...).ungrounded` (PM/22): أي قِيس أنّ الفهمَ بلا أثرٍ في الطلب
أصلاً. حينها بديلٌ ٩٥٪ نظيف خيرٌ من فهمٍ قِيس أنّه فهمُ منتجٍ آخر.

وما دون ذلك يبقى كما هو — **لا نُصلح ما لم يُقَس كسرُه**. وبلا عناوينَ لا استبدال: يُقال ولا
يُختلق بديل. والعناوينُ تسمّي **كياناتٍ** لا أدواراً (العنوانُ لا يقول من يستعمل ماذا)، فالأدوارُ
تسقط ولا تُختلق.

### والبوّابةُ نفسُها لم تكن تقع أصلاً

قِيس حيّاً أنّ مواصفة «وِرد» كلَّها تُنتج مفهوماً واحداً في المعجم — `account` — من عبارة
«بلا **حساب**». فكان يسنُد الفهمَ المهلوَس (`Grade/ForumPost/Teacher/Parent`) فينجو من الوصم.

**وهي علّةُ `needsBackend` نفسُها بوجهٍ ثانٍ**: النفيُ هو ما أوجب. ولذلك:

- `stripNegated` انتقلت إلى `textNormalizer.js` — **مصدرٌ واحد** لعلّةٍ لها مستهلكان
  (سؤالُ الخادم، وبوّابةُ الفهم). و`textNormalizer` لا يستورد شيئاً فلا دورة.
- `backendNeed.js` يعيد تصديرها، فمستهلكوها واختباراتُها لم تتغيّر.
- `goalFidelity` تطوي النفيَ قبل قراءة الطلب.

### وثالثُ موضعٍ للعلّة نفسِها — كشفه الجناحُ لا المراجعة

`fallbackModel` (الفهمُ حين لا مزوّد) تقرأ `lexiconModel(goal)` على النصّ الخام. فقِيس أنّ
**مواصفة متتبّعِ الحفظ كاملةً تُنتج كياناً واحداً اسمُه `account`** — مأخوذاً من «بلا **حساب**».
أي أنّ فهمَ جولا الاحتياطيَّ للمنتج كلِّه كان **الشيءَ الذي نفاه صاحبُه**.

وُجد هذا لأنّ اختباراً جديداً مرّ منفرداً وسقط في الجناح: الطُّعمُ كان يبذر عبر **مكتبة الفئة**
وهي مشتركةٌ بين الملفّات ويُعاد ضبطُها في غيرِه. فصار يبذر عبر **ذاكرة المشروع** المعزولة
بـ(مستخدم، مشروع) — والطُّعمُ يملك ما يقيسه. وسقوطُ الاختبار هو ما دلّ على الموضع الثالث.

### الأثرُ المقيس — بلا نموذجٍ لغويّ، على مواصفة المالك نفسِها

| | ما يصل البُناة |
|---|---|
| قبل | `Grade، ForumPost، account، Teacher، Parent` |
| بعد | **`ورد، مراجعه، شريط`** |

و«شريط» هو الواحدُ من التسعةَ عشرَ الذي ليس كياناً — الدقّةُ ٩٥٪ لا ١٠٠٪، وتُقال كما هي.

### الدرس

**مقياسٌ سيّئٌ قد يكون قياساً لإشارةٍ خطأ لا حَدّاً في الطبيعة.** كُتب بالأمس ديْنٌ يقول «هذا
يحتاج صرفاً»، والصوابُ أن يُسأل أوّلاً: أين يكتب صاحبُ المشروع أسماءَه **أصلاً**؟

---

## PM/24 — التسميةُ بأداة التعريف: الطلبُ القصير (2026-09-08)

PM/23 سمّت من عناوين البنود المرقّمة. والطلبُ القصير بلا عناوين — وهو أكثرُ ما يُكتب.

### القياس أوّلاً: أين تُكتب الأسماء في جملةٍ واحدة؟

| القاعدة | نظيفة | تغطية |
|---|---|---|
| التكرارُ في الطلب القصير | ٢١/٤٨ — **٤٤٪** | — |
| «ال» بلا فلتر | ٢٦/٣٤ — ٧٦٪ | ١٠/١٤ |
| **«ال» بإسقاط صفةِ النسبة** | **٢٥/٢٦ — ٩٦٪** | ٨/١٤ |
| الضميرُ المتّصل | ٥/٢٢ — ٢٣٪ | — |

التكرارُ **أسوأُ في القصير منه في المتن** (٤٤٪ مقابل ٥٠٪): أفعالٌ («اصنع»، «أسجّل»، «تضبط»)
ولواصقُ ضمائرَ («عاداتي»، «تدويناتي») وحروفٌ («فيه»، «بوسوم»).

والإشارةُ كانت في اللواصق نفسِها لا في إسقاطها: **«ال» لا تدخل على فعل** — فهي علامةُ اسم.

**والضميرُ المتّصل ساقط** لسببٍ بنيويّ: تطبيعُ `ة→ه` يجعل كلَّ مؤنّثٍ يبدو منتهياً بهاء
الملكيّة («مذاكرة»→«مذاكر»، «شبكة»→«شبك»)، والأفعالُ تأخذ ضمائرَ مفعولٍ («ينبّهني»).

### صفةُ النسبة

أكثرُ ما يلتبس بالاسم بعد «ال»: «اليوميّة»، «أفقيّة»، «النسخيّ»، «إلكترونيّاً». تُسقَط بلاحقتها،
وثمنُها مقيسٌ: تسقط معها أسماءٌ صحيحةٌ قليلة («السقاية»). **الدقّةُ أولى هنا** — الاسمُ الخطأ
يصير كياناً يُبنى له صفحةٌ وجدول.

### العقد

- `normalizeLetters` — شُقّ عن `normalizeConceptText` لأنّ الأخيرَ **يبتلع «ال»**، ولها الآن
  مستهلكٌ يحتاجها. مصدرٌ واحدٌ بخطوتين، لا نسخةٌ ثانية (وهي علّةُ النسختين المتكرّرة هنا).
- `articleEntityNames(goalText)` — الموسومُ بـ«ال» (ولو سبقتها أداةٌ ملتصقة: «والآيات»)،
  بلا صفةِ نسبةٍ ولا ألفاظِ إطارٍ ولا أرقام، والنفيُ مطويٌّ قبلها.
- **العناوينُ أوّلاً**: `heads.length >= 2 ? heads : articleEntityNames(goal)` — الأدقُّ
  (٩٥٪ بتغطيةٍ أوسع) يسبق الأوسعَ دقّةً بتغطيةٍ أضيق.

### التغطيةُ صريحة

٨ من ١٤ هدفاً. وطلبٌ بلا «ال» ولا عناوين يبقى بلا تسمية — **يُقال ولا يُختلق**. وستٌّ من
الأربعةَ عشرَ أُضيفت **بعد** صوغ القاعدة ونصيبُها ١٦/١٧، فلا تفصيلَ على المثال.

### حدودٌ مكتوبة

«ال» التي ليست أداةَ تعريف تمرّ («الالتزام» ← «تزام»، «الكترونيّاً» ← «كترونيا») — كأختِها
في `wordsMeet` («للعبة»)، التباسٌ لا يُحسم بلا معجمٍ صرفيّ. وصيغةُ «لل» («للقرآن») لا تُقرأ
أداةً هنا: إضافتُها تُدخل التباسَ «لِـ+لعبة»، ولم تُقَس فلم تُبنَ.

### وعلّتا عزلٍ في الاختبارات — كشفهما الجناحُ لا المراجعة

1. **ذاكرةُ المشروع تُكتب على القرص وتبقى بين الجولات.** فاختبارٌ بمستخدمٍ ثابتِ الاسم يرث
   نموذجَ الجولة السابقة. سقط اختبارٌ سليمٌ لأنّه ورث تسميةً كتبَتْها جولةٌ سابقة — فصار
   المستخدمُ فريداً لكلِّ تشغيل. وهي علّةُ مكتبة الفئة نفسُها: مصدرُ حالةٍ مشتركةٍ لا يملكه الطُّعم.
2. **«اصنع لي…» تُفعّل `isExplicitNewBuild`** فتُسقط النموذجَ السابق — فطُعمٌ يبذر فهماً
   مهلوَساً بهذه الصيغة لا يقيس شيئاً، لأنّ البذرةَ لا تصل أصلاً.

---

## صمتُ العودة: سجلُّ البناء يُستعاد، والحوارُ يُقرأ من حيث هو (2026-09-08)

من أوامرك الدائمة: «اصلح الصمت». وهذا وجهٌ ثالثٌ منه بعد صمتِ الشات وخبرِ المهمّة.

### (١) `Socket.IO` يبثّ لحظةً

من أعاد تحميلَ الصفحة أثناء البناء يعود إلى **سجلٍّ فارغ** والمهمّةُ ماضيةٌ على الخادم.
فما يراه ليس مهمّةً متوقّفة بل **مهمّةً لم يصله خبرُها** — وهو عينُ ما عولج في
`rememberMissionNote` للخبر الختاميّ، وبقي مسارُ البناء نفسُه بلا علاج.

**العقد**: شريطٌ في `RoomReporter` — وهو **بابُ البثّ الواحد**، تمرّ به `liveLog` وكلُّ
`send(room, 'log', …)`. يُقيَّد ما بُثّ، ويُعاد عند الانضمام **للمقبس العائد وحدَه**
(لا للغرفة) كي لا يتكرّر على من لم يغادر.

**حدّان مقصودان لأنّه في الذاكرة**: ٢٠٠ سطراً لكلِّ غرفة (يُقتطع الأقدم)، و٥٠ غرفةً
(تخرج الأقدم). فلا ينمو بلا حدّ على خادمٍ طويل العمر.

**حدودٌ مكتوبة**:
- يلتقط ما مرّ **بهذا الباب**. و`deployAgent`/`renderAgent`/`githubSync` تتلقّى `io`
  قيمةً وتبثّ بنفسها — وهو ما تشرحه ترويسةُ `RoomReporter` أصلاً — فأسطرُها لا تُقيَّد.
  سجلُّ البناء، وهو ما يُتابَع أثناء المهمّة، يُقيَّد كلُّه.
- شريطٌ في الذاكرة: يبقى لإعادة تحميل الصفحة، ولا يبقى لإعادة تشغيل الخادم.

### (٢) والحوارُ كان يُقرأ من مصدرٍ واحدٍ من مصدرَيه

استعادةُ `chat_history` في `server.js` كانت داخل `if (isDbConnected …)` وتسأل
`Conversation.findOne` مباشرةً. ولهذا المخزن **مصدران**: Mongo **وملفٌّ على القرص ينجو من
إعادة التشغيل**.

فبلا Mongo كان صاحبُ المشروع يفتح مشروعَه فلا يرى شيئاً — ومن ذلك **تقريرُ التسليم**
الذي جُعل باقياً أصلاً لئلّا يضيع. **أي أنّ ما حُفظ لأجل ألّا يضيع، كان يضيع في العرض.**
مقيسٌ: رسالةٌ محفوظةٌ على القرص، ومسارُ الاستعادة يعود فارغاً.

**العقد**: `loadRecent(key, limit, { fromDb })` — يُسأل Mongo أوّلاً (و`$slice` باقٍ: التعليقُ
في `server.js` يقول إنّ الوثيقة «قد تضمّ مئات الرسائل»، فجلبُ المستند كاملاً ارتدادٌ في
الأداء لا يُقبَل ثمناً لتوحيد المصدر)، فإن لم تُجب فالقرص.

**ولمَ الحقن؟** لأنّ الاختبارات تعمل بلا قاعدة، فحارسُ `online()` يمنع بلوغَ الاستعلام
ويبقى فرعُ «متّصلةٌ لكنّها فارغة» بلا اختبار — وقد **نجت طفرتُه** فعلاً. الحقنُ يقيس
القرار، والسابقةُ في الملفّ نفسِه (`recordTurn(..., summarize)`).

**نتيجةٌ سلبيّةٌ معلَنة**: جسدُ `dbRecent` نفسُه غيرُ مغطّى — تغطيتُه تحتاج قاعدةً حيّةً في
الجناح، وذلك عملٌ آخر لم يُبنَ ولا يُدَّعى. والمُغطّى هو القرارُ الذي تُغذّيه.

### الحارسُ الذي انتبه — انزياحٌ لا موضعٌ جديد

`renderConfigShape` يثبّت **مواضعَ قرارِ شكل `render.yaml` الخمسة بأسطرها**، عنوانُه
«أيُّ تغييرٍ قرارٌ لا انزلاق». وقد سقط: `server.js:2401 → false` صار `2411`.

فُحص فسقط الشكُّ: التعديلُ كلُّه فوقَه (استيرادٌ عند ١٣٨، ومُعالِجُ الانضمام عند ٧٦٠
وما بعدها، صافيه ‎+١٠‎)، والموضعُ هو هو — الحجّةُ `false`، وداخلَ حارسِ full-stack نفسِه.
**انزياحُ سطرٍ لا موضعٌ سادس**، والعددُ خمسةٌ كما قِيس. فحُدِّث الرقمُ بتعليقٍ يقول لماذا،
كما فُعل في `PM/2b` قبله. وهذا هو المقصودُ من الحارس: لا أن يمنعَ الحركة، بل أن يجعلها
**قراراً يُكتب** لا انزلاقاً يمرّ صامتاً.

## الدائرةُ: البوّاباتُ تقيس المبنيَّ بالفهم، ولا شيءَ يقيس الفهمَ بالطلب (2026-09-08)

**قياسٌ حيٌّ من الإنتاج** (`jaola.dev`، مواصفة «وِرد»: أداةٌ لمتابعة حفظ القرآن **داخل المتصفّح
بلا خادمٍ ولا حساب**). ما بُني: منصّةُ تعليمٍ كاملة — `education (knowledge-engine)`، وأدوار
`Parent/Teacher/SchoolAdmin/Student`، ونسخةُ Full-Stack بـNext.js + API + Prisma، وDocker،
ودليلُ نشرٍ على Render/Railway. أي **خادمٌ وحسابٌ لطلبٍ نفاهما نصّاً**.

وهذا الفهمُ المهلوَس هو **حرفيّاً** ما قِيس في `PM/23` (`Grade، ForumPost، account، Teacher، Parent`).

### لماذا لم تقع البوّابات

الثلاثةُ تشرب من العين نفسِها — `domainModel`:

| البوّابة | مصدرُ مرجعها | ماذا تقيس فعلاً |
|---|---|---|
| `requirements-verify` | `composeRequirements(blueprint, **domainModel**)` | المبنيُّ ضدّ الفهم |
| `domain-fidelity` | `domainFidelity(**domainModel**, نصّ المنتج)` | المبنيُّ ضدّ الفهم |
| `role-coverage` | أدوارُ `domainModel` | المبنيُّ ضدّ الفهم |

فحين يكون الفهمُ نفسُه خطأً، **يصير اجتيازُ البوّابة دليلاً على أنّ البناء نفّذ المنتجَ الخطأ
بأمانة** — لا على شيءٍ آخر. ولذلك طُبع `requirements-verify ✓`: البناءُ وافق نفسَه.
(`skipped` علامتُه `–` لا `✓`، فالاجتيازُ هنا حقيقيٌّ لا امتناع.)

**ولا بوّابةَ واحدة تقارن الفهمَ بنصّ الطلب.** وتلك بعينُها ثغرةُ `goalFidelity` (PM/22):
قِيس على هذه الحالة فأعطى `ungrounded = true` و**صفراً من عشرة** مسنودةً بالطلب، والبديلُ من
عناوين صاحب المشروع: `ورد، تسجيل، جدول، حاله، نسبه، سلسله`.

### الإصلاحُ هنا: دعوى فوق دليلها

`domainFidelity` **لا ترى نصَّ الطلب أصلاً** — `expected` مشتقٌّ من `domainModel` وحدَه. فقولُها
«ولا أثرَ لمفاهيم المنتج **المطلوب**» دعوى لا تملك دليلَها. وقد طُبعت لصاحب المشروع قائمةٌ
مهلوَسةٌ بالكامل موصوفةً بأنّها **مطلوبُه**، فيقرأ أنّ البناءَ أخفق في تنفيذ منتجِه — والخللُ
فوقُ في الفهم لا تحتُ في البناء. فصارت «المنتج **المفهوم**»: تقول ما تعرف، لا ما لا تعرف.

**والدرسُ**: قِيس في PM/22 أنّ «مقياساً يقول الشيءَ نفسَه عن كلِّ شيءٍ لا يُقرأ». وهذه صورتُه
الثانية: **مقياسٌ صادقٌ في إطاره، ومرجعُه هو نفسُه المتَّهَم.** البوّابةُ لا تكون بوّابةً إلّا
إذا كان مرجعُها خارج ما تحكم عليه.

### ما صحّ في الجولة

القاضي قال `FAILED`، والنواةُ قالت «اكتملت المهمّة — ولم يجتز المنتجُ التحقّق». فـPM/2 وPM/10
صمدتا: **لم يُعلَن نجاحٌ كاذب**. الصدقُ قائم، والفهمُ هو ما سقط.

## بنودُ المواصفة بصيغة ماركداون: «## 1.» بندٌ كما «1.» سواء (2026-09-08)

**العطبُ المقيس**، على برومتٍ حقيقيّ بخمسة عناوين مرقّمة (`## 1. Unit Tests` …):

| القياس | قبل | بعد |
|---|---|---|
| `numberedSections` | **٠** | ٥ |
| `specSections` | `[]` | خمسةُ بنودٍ بعناوينها |
| `specHead` | **الوثيقةُ كلُّها** | جملةُ التسمية وحدَها |

والسببُ حرفٌ واحد: `NUMBERED_LINE` و`NUMBERED_SECTION` يشترطان أن **يبدأ السطرُ برقم**،
و«## 1.» يبدأ بـ`#`.

**ولماذا يهمّ**: `numberedSections` تغذّي `isFullSpecification`، وهي **الحارسُ** الذي تُستدعى
خلفه `specSections`. فسقوطُها يُسقط معها كلَّ ما تحرسه دفعةً واحدة — بوّابةَ المتطلّبات
(PM/9، PM/12)، وتسميةَ الكيانات من العناوين (PM/23)، واسمَ المنتج من رأس الوثيقة (PM/11).
فمن كتب مواصفتَه بماركداون — وهو أكثرُ ما يُولَّد اليوم — **سقطت بنودُه صامتةً**، ولم يُقَل له.

**العقد**: رأسُ ماركداون (`#` إلى `######`) اختياريٌّ قبل الرقم، في **الموضعَين معاً**.

🔒 **والحدُّ المحفوظ: الرقمُ شرطٌ لا زينة.** «## الغاية» عنوانٌ لا بند — وإلّا صار كلُّ
عنوانٍ في الوثيقة مطلباً يُحاسَب عليه البناء. ولا نقطةُ قائمة (`- x`) ولا إحالةُ عطبٍ
(`#1 issue`، بلا فاصلٍ بعد الرقم) تُعدّ بنداً.

**وعتبةُ `isFullSpecification` لم تُمَسّ** (١٢٠٠ حرفاً و٦ بنود): توسيعُ الصيغة ليس إرخاءً
للحارس. والبرومتُ المقيس أعلاه ما يزال لا يعبرها — خمسةُ بنودٍ لا ستّة — وذلك **صحيحٌ
مقصود**، لا نقصٌ في الإصلاح.

**التوأمان عقدٌ لا صدفة**: `numberedSections(t) === specSections(t).length` لكلِّ نصّ —
مثبَّتٌ باختبار. لو افترقا لعدّت البوّابةُ ما لا يصل المستهلك.

**التحقّق**: ١٠ اختباراتٍ جديدة، **٥/٥ طفراتٍ مقتولة** (منها «أُصلح توأمٌ ونُسي الآخر»).
ولا انحدار: مواصفةُ «وِرد» تبقى ٩ بنودٍ بعناوينها نفسِها.

## حارسُ النطاق: أن يقول جولا «لا» بصدق (2026-09-08)

**العطبُ المقيس** (برومت «JAOLA Tester» — حزمةُ اختباراتٍ لشفرة خادم): البُناةُ الثلاثة كلُّهم
يُخرجون **موقعاً** (سطرٌ إلزاميّ في مُوجَّه `coderAgent`: «ثلاثة ملفات: index.html وstyles.css
وscript.js»)، وبوّاباتُ التحقّق كلُّها تفحص **صفحةً تعمل**. فكان يُبنى بروشورٌ عن الاختبارات
ثمّ يُحكَم `FAILED` — **حكمٌ صادقٌ لا يقول لماذا**، فيُقرأ عجزاً وهو **حدُّ نطاق**.

### الإشارة: ما سمّاه صاحبُ الطلب مُخرَجاً

لا موضوعُ الطلب ولا قائمةَ كلماتٍ محظورة. قِيس على ١١ طلباً، **فصلٌ تامّ عند ٢**:

| الطلب | مُخرَجاتٌ برمجيّة غريبة |
|---|---|
| وِرد · متجر · مطعم · لوحة مخزون | **٠** |
| «غيّر لون الأزرار في `styles.css`» | ٠ |
| موقعٌ يقرأ من `data.json` | ٠ |
| «عدّل `script.js` و`app.js`» | **٠** (ملفّاتُ جولا تُستثنى) |
| سكربتُ ترحيل · خطُّ CI | ٢ |
| مكتبةُ npm | ٣ |
| **JAOLA Tester** | **٢٣** |

**ولمَ لا قائمةُ كلمات؟** لأنّها تُصيب **موضوعاً** لا **شكلاً**: «اختبارات» ترد في موقع مدرسةٍ
بريئاً. والمقيسُ هنا شكلُ المُخرَج، وهو مفتوحٌ لا يحتاج معرفةَ المجال — **درسُ PM/22 نفسُه**.

### العقد

`foreignCodeArtifacts(goal)` و`isOutOfScopeRequest(goal)` في `textNormalizer` (لا يستورد شيئاً).
والحارسُ في `selectBuildStrategy` **قبل أيِّ بانٍ**، على شكل السابقة القائمة (`skipped: 'works'`):
يُقال نصّاً ← `COMPLETED` ← `{ skipped: 'out-of-scope', named }`. **ولا يُبنى شيء.**

**ولمَ لا عرضُ بديل** («أبني لك واجهةً لعرض نتائجها»)؟ لأنّه يُقرأ التفافاً على طلبٍ لم يُطلَب.
وهو إضافةٌ ممكنةٌ لاحقاً **فوق** هذا، لا بديلٌ عنه.

### حدودٌ مكتوبة

- **`isFreshBuild` شرط**: الحارسُ يقرأ الطلبَ لا المشروع؛ فعلى مشروعٍ قائمٍ يبقى القرارُ لغيره.
- ⚠️ **الغيابُ ليس دليلَ شيء**: «اكتب لي خوارزميّة فرز» خارجُ النطاق ولا يسمّي ملفّات، فيمرّ.
  المقياسُ **يُدين بدليلٍ ولا يُبرّئ بغيابه** — حدٌّ مكتوب لا نقصٌ مستور.
- الروابطُ تُنزع قبل القياس: `https://example.com/shop/items/` كان يُخرِج `com/shop/items/`
  مساراً، فطلبُ موقعٍ يذكر رابطَين يُدان بلا ذنب. (مقيس.)

### مقياسٌ أسقطتُه لأنّه كان يقيس الصفر

كان معه عدّادٌ ثانٍ لـ«ألفاظ الصفحات» — أعطى **صفراً لكلِّ شيء**، عربيّه وإنجليزيّه: `\b` لا يعمل
مع الحروف العربيّة في JS، **وهو تحذيرٌ مكتوبٌ في `textNormalizer.js` نفسِه**. ولولا طباعةُ
الأعمدة الخام لمرّ صامتاً واتّسق كذباً. حُذف ولم يُصلَح: العدّادُ الواحد فصَل ١١/١١ وحدَه،
فالثاني **زينةٌ لا دليل**.

**التحقّق**: ١٠ اختباراتٍ جديدة، **٩/٩ طفراتٍ مقتولة**. وإحداها نجت أوّلَ مرّة (استثناءُ ملفّات
جولا) فكشفت **اختباراً ناقصاً لا شفرةً ميتة**: «عدّل `script.js` و`app.js`» أشيعُ ما يُكتب على
مشروعٍ قائم، وبلا الاستثناء يُدان. وحدودُ `selectBuildStrategy` حُدِّثت بتعليقها (send ٢←٣،
liveLog ٩←١٠، transitionState ٢←٣).

---

## سؤالُ الوجود يفارق قارئَ المحتوى — `hasProjectSource`

### العطبُ المقيس

`readCodeContext` يفلتر بقائمةٍ **مغلقة** من ثلاثة أسماء (`index.html`/`styles.css`/`script.js`).
وأربعةٌ من مواضع ندائه لا تريد محتواه أصلاً: تسأل به **«أثمّة مشروعٌ هنا؟»** عبر `length > 100`
(وفي `selectBuildStrategy` عبر `< 80` باسم `isFreshBuild`).

فمستودعٌ بسبعةِ ملفّات PHP حقيقيّة يُقرأ **صفرَ حرف** → `isFreshBuild = true` → **وهو الشرطُ
الذي يُجيز الاستبدالَ الكامل**. مشروعٌ عامرٌ **يُدهَس صامتاً**. (قِيس على مستودعِ Laravel مصطنع:
٧ ملفّات ← `readCodeContext` صفرٌ ← يُعامَل فارغاً.) ولا يخصُّ الغريبَ وحدَه: مشروعُ React خالص،
أو مشروعٌ يسمّي ملفَّه `main.js`، يُقرأ فارغاً بالقدر نفسِه.

**سادسةُ** حالات PM/15 (بعد PM/11 قائمة، PM/14 نصّ، PM/17 اسم، PM/19 ذاكرة، PM/20 مفتاح،
PM/22 نوع): قارئٌ صحيحٌ لغرضه، أُعيد استعمالُه لغرضٍ آخر. والقارئُ **لا يُوسَّع** — تحذيرُ PM/15
قائمٌ حرفيّاً: «لو رأى المُعدِّلُ سبعَ صفحاتٍ لعدّل الخطأ منها». والسابقةُ في الملفّ نفسِه:
`isReactProject` يسأل `access` ولا يقرأ محتوى.

### العقد

`hasProjectSource(projectPath, { minBytes = 80, budget = 400 }) → boolean` في `projectReader.js`:
يمشي القرصَ بـ`stat` (لا `readFile`)، **بلا قائمةِ أسماءٍ ولا قائمةِ امتدادات**، يتجاوز
`node_modules`/`vendor`/`dist`/`build`/`__pycache__` وكلَّ ما يبدأ بنقطةٍ والثنائيّاتِ والصور،
ويخرج فور تجاوز العتبة. `false` عند أيّ خطأ — كالقرّاء القائمين.

المواضعُ الأربعة: `selectBuildStrategy` (`isFreshBuild`، عتبةُ ٨٠) و`intentHandlers` ثلاثةً
(عتبةُ ١٠٠). فلم يبقَ لـ`readCodeContext` مستهلكٌ في أيٍّ من المرحلتين.

### ما تغيّر وما لم يتغيّر — والفرقُ مقيسٌ صفّاً بصفّ

العطبُ في **القائمة المغلقة** لا في **العتبة**، فالعتبةُ تبقى وتُمرَّر من موضع النداء بقيمتِه
التي كانت له — لا رقمَ يتغيّر خلسة. والقياسُ يقول إنّ صفّاً واحداً انقلب:

| الحال | `readCodeContext` | `isFreshBuild` قبل | بعد |
|---|---|---|---|
| Laravel (٧ ملفّات) | ٠ | `true` ← **العطب** | `false` |
| ركامُ `<h1>x</h1>` | ٢٩ | `true` | `true` |
| فارغ / ضجيجٌ فقط / مفقود | ٠ | `true` | `true` |

ولولا إبقاءُ العتبة لصار ركامُ بناءٍ فاشلٍ مشروعاً قائماً **يمتنع عنه مسارُ React أبداً** —
وهو اختبارٌ سلوكيٌّ قائمٌ أمسكَ الإفراطَ قبل أن يُرفَع (`selectBuildStrategyStage` حدُّ الجدّة).

### حدٌّ مكتوب

يقول «أثمّة شيء؟» فقط. **لا يقول ما هو، ولا أنّ جولا يفهمه** — وقراءةُ محتواه سؤالٌ آخر لم
يُبنَ هنا ولا يُدَّعى. وهو الحدُّ الذي يفصل هذا البند عمّا بعده (جسرُ GitHub، ثمّ الصيانة).

**التحقّق**: ٩ اختباراتٍ جديدة (`tests/hasProjectSource.test.mjs`)، **١١/١١ طفرةً مقتولة**.
وواحدةٌ نجت أوّلَ مرّة (`minBytes` الافتراضيّةُ صفراً) فكشفت **اختباراً ناقصاً**: المواضعُ الحيّةُ
كلُّها تمرّر عتبتَها، فالافتراضُ غيرُ مُمارَسٍ إنتاجاً — ونداءٌ عارٍ لاحقٌ كان سيُعيد الإفراطَ
نفسَه من الجهة الأخرى. ثُبِّت باختبار. وحدودُ المراحل الثلاث حُدِّثت بتعليقها
(`readCodeContext` ١←٠ و٢←٠ و١←٠، و`hasProjectSource` بإزائها).

---

## جسرُ GitHub — من رابط مستودعٍ إلى مساحة عمل

### القياسُ أوّلاً: ما كان مبنيّاً وما لم يكن

الجلبُ **مبنيٌّ سلفاً**: `agents/starterFetch.js#fetchRepoFiles` تقرأ شجرةَ مستودعٍ عبر
Trees API بحدودٍ صارمة (١٢٠ ملفّاً، ١٫٥ ميغابايت، نصوصٌ فقط، بلا `node_modules`/`.git`/
الأقفال/`.env`)، ولها اختباراتُها بحاقن `fetch`. لكنّها تُعيد الملفّاتِ في **ردِّ HTTP
لمُشرِفٍ** (`POST /api/admin/starters/import`) — **ولا تكتبها إلى مساحة عملٍ قطّ**.

فالفجوةُ ليست الجلبَ بل **الإنزال**.

### السياسةُ الخامسة: `resolveRepoFile` / `landRepoFiles`

والكاتبُ القائم `writePlanFiles` لا يصلح لها — **مقيسٌ على شجرةِ مستودع Next.js واقعيّة**:

| | كُتب | رُدّ |
|---|---|---|
| `writePlanFiles` (سياسةُ المولّدات) | ١٤ | **٨** |
| `landRepoFiles` (سياسةُ المستودع) | ٢٢ | ٠ |

والثمانيةُ هي: `.eslintrc.json`، `.prettierrc`، `.nvmrc`، `.editorconfig`،
`.github/workflows/ci.yml`، `.github/dependabot.yml`، `.husky/pre-commit`،
`.vscode/settings.json`. ومنها **`.github/workflows/ci.yml` — وهو بعينه دليلُ الصيانة**:
اختباراتُ المستودع هي ما يُحكَم به على أيّ تعديلٍ فيه. فمشروعٌ يُنزَل بلا سير عمله
**يُصان بلا دليل**.

وهذه **سابعةُ** حالات PM/15، ومكتوبةٌ في وثيقة `resolveProjectFile` حرفيّاً: سياستُها
لأسماءٍ «يقترحها مولِّدٌ أو نموذج» — وملفّاتُ مستودعٍ ليست اقتراحاً، هي على GitHub قبل أن
نسأل. (والرفضُ **يُحصى لا يُبتلع** في الكاتبَين معاً؛ فالعطبُ في السياسة لا في الصمت.)

**حدّان لا استثناء لهما**: `.git/` يُمنع **بمقطعِه أينما وقع** (لا في الجذر وحدَه)، وأسرارُ
`.env` تُمنع في كلِّ مستوى (مستودعاتُ الحزم تضعها تحت كلِّ حزمة). **واستثناءٌ واحدٌ مقيس**:
`.env.example` ينزل — أوّلُ صياغةٍ منعته فرُدّ بالقياس، وهو ليس سرّاً بل قالبٌ بقيمٍ فارغة
**حسمته هذه المنصّةُ سلفاً** في `PROJECT_DOTFILES`. منعُه كان حذفَ التوثيق باسم الأمن.

### المستهلكُ الحيّ: `POST /api/project/import-repo`

خلف `verifyToken` + `validateProjectOwnership`، فالمسارُ يُشتقّ من توكن صاحبه لا من جسم
الطلب. الرابطُ يُحلَّل بـ`parseRepoUrl` قبل أيّ نداء.

🛡️ **ولا يُدهَس مشروعٌ قائم**: `hasProjectSource` يُسأل **قبل أيّ كتابة** — وإن كان ثمّة
مصدرٌ رُدَّ ٤٠٩ ولم يُكتب حرف، إلّا بـ`overwrite` صريحةٍ من صاحبه. وهذا هو سببُ كون البند
السابق شرطاً لهذا: قارئٌ بقائمةٍ مغلقة كان يقرأ مستودعاً عامراً صفراً فيُجيز الاستبدال.
والحارسُ **قبل** الكتابة لا بعدها — وذلك هو العقد، ومقيسٌ بالطفرة.

⚠️ **صدقُ الردّ**: المجلوبُ **قصٌّ بحدود**، فالردُّ يقول `bounded: true` و`skipped`
و`truncated` و`rejected` بأسمائها. لا يُقال «نزل المستودع» وهو بعضُه.

🚫 **وما لا يُدَّعى**: **لا تُشغَّل شفرةٌ ولا تُثبَّت اعتماديّة**. الإنزالُ إنزال — وتشغيلُ
شفرةٍ غريبةٍ قرارٌ **لم يُتَّخذ**، وهو ما يقف عنده البندُ التالي بانتظار قرارٍ صريح من
صاحب المشروع.

**التحقّق**: ١٠ اختباراتٍ للكاتب (**٧/٧ طفراتٍ مقتولة**) و٧ للمسار (**٩/٩ مقتولة**).
وطفرتان نجَتا أوّلَ مرّة فكشفتا **اختبارَين ناقصَين**: سرٌّ متداخلٌ في مستودع حزم
(`packages/api/.env`)، ومسارٌ مطلقٌ يقع صدفةً **داخلَ** الجذر. وثالثةٌ نجَت فكشفت **شفرةً
لا تُبلَغ** (حارسُ «المقاطعُ صفر» بعد رفضِ الفارغِ والمطلق) — فحُذفت لا زُيِّنت. وحارسُ `..`
بقي **قفلاً ثانياً معلَناً**: طفرةُ حذفِه تنجو، وذلك مكتوبٌ في موضعه لئلّا يُظنّ حاملاً.

---

## PM/25 — أدوارُك كما كتبتَها: رفعُ حدِّ PM/23 من جهته الصحيحة

PM/23 كتبت حدّاً صريحاً: «العناوينُ تسمّي **كياناتٍ** لا أدواراً — العنوانُ لا يقول من يستعمل
ماذا، فالأدوارُ تسقط ولا تُختلق». وهو صوابٌ في موضعه، لكنّه تُرك حدّاً ولم يُسأل بعده السؤالُ
الذي فتح PM/23 نفسَها: **أين يكتب صاحبُ المشروع أدوارَه أصلاً؟**

### تصحيحُ قياسٍ لي، أوّلاً

كتبتُ أوّل الأمر «`fallbackModel(spec).roles` تعود `null`» — وكان **أثرَ أداةٍ لا قياساً**:
الدالّةُ غيرُ مُصدَّرة فقرأتُ `undefined`. القياسُ الصحيح عبر `deriveProjectModel` بمزوّدٍ يرمي.

### العطبُ المقيس (مواصفةُ نقاط البيع الحقيقيّة، `tests/fixtures/pos_spec.txt`)

| | ما يصل البُناة |
|---|---|
| ما كتبه صاحبُ المواصفة | `مالك النظام، مدير الفرع، الكاشير، أمين المخزن، المحاسب` |
| ما كان يصل (`_source: lexicon`) | **`staff، customer، admin، tenant`** |

فالعطبُ ليس «لا أدوار» بل أسوأ: تصل **أربعةٌ من معجمٍ مغلقٍ إنجليزيّ**، ولا يصل من الخمسة
العربيّة ولا واحد. وبها **تقع `role-coverage` فعلاً** (٤ > ١)، فتفحص الصفحةَ بحثاً عن «tenant»
و«staff» — **ألفاظٍ لم يكتبها صاحبُ المشروع قطّ**. وهذه عائلةُ PM/22 وPM/23 نفسُها: قائمةٌ
مغلقةٌ تحلّ محلَّ كلمات صاحب المشروع.

### العقد: `labelledRoleNames(goalText)`

سطرٌ عنوانُه (ما قبل النقطتَين) لفظُ أدوار → قيمتُه قائمةٌ تُقسَّم على `،`/`,`/`/`/`؛`.
مقيسةٌ على المواصفة نفسِها: **٥/٥**، بلا نموذجٍ لغويّ ولا قائمةِ مجال. وتسبق معجمَ
`lexiconModel` في `fallbackModel`، وتُملأ في مسار الاستبدال (`understand.js`) حيث كانت `[]`.

⚠️ **فخُّ `\b` وقع ثالثةً في هذه الدفعة**: `أدوار\b` لا يطابق أبداً — الحروفُ العربيّة ليست
`\w`. والتحذيرُ مكتوبٌ في `textNormalizer.js` منذ زمن. ثُبِّت باختبارٍ صريحٍ لئلّا يُعاد.

### الأثرُ المقيس — أصدقُ لا مُعادُ التسمية فقط

| | قبل | بعد |
|---|---|---|
| ما يُقال لصاحب المشروع | «أدوار بلا واجهة: **staff، tenant**» | «أدوار بلا واجهة: **أمين المخزن**» |
| متطلّبات بلا أثر | ٣ | **٢** |
| مفاهيمُ الفهم المتتبَّعة | ٧/١٠ | **٨/١٠** |

«مالك النظام» و«مدير الفرع» و«الكاشير» **يمثّلها كلونُ نقاط البيع فعلاً**، فسقطت فجوتان
كاذبتان؛ و«أمين المخزن» فجوةٌ **حقيقيّة** سمّاها صاحبُ الوثيقة ولم يُبنَ لها شيء. فالحكمُ صار
يقيس ما طُلب، ويقوله بلغة من طلبه.

### حدودٌ مكتوبة

- **تسمّي من يُسمّى صراحةً**: لا تستنبط دوراً من فعلٍ في المتن («يسجّل الحافظ…»)، ولا تختلق
  واحداً حين لا يُسمّى — تعود فارغةً، ويُقال ولا يُخترع.
- **العنوانُ المركَّب لا يُلتقط**: «الصلاحيات والأدوار: بائع، مشرف» في سطرٍ واحد لا يُقرأ (رأسُ
  السطر «الصلاحيات»). والمواصفةُ المقيسة تفصلهما، فلم يُقَس كسرُ هذا — **ولا يُصلَح ما لم يُقَس
  كسرُه**. مثبَّتٌ باختبارٍ كي يكون توسيعُه قراراً برهانِه لا انزلاقاً.
- **سقفُ الأربعة قائمٌ ولم يُرفع**: `normalizeProjectModel` يقصّ الأدوار عند أربع، فـ«المحاسب»
  يسقط **بالسقف لا بالقاعدة**. مثبَّتٌ باختبارٍ يقول ذلك بنصّه، لا يُرفع خلسة.
- **القياسُ نقطةٌ واحدةٌ حقيقيّة** لا ستٌّ كـPM/23. تُقال كما هي ولا تُضخَّم.

### التحقّق

١١ اختباراً جديداً، **١٠/١٠ طفراتٍ مقتولة**. وواحدةٌ نجت أوّلَ مرّة (حذفُ مِرساةِ رأس السطر)
فكشفت **اختباراً ناقصاً**: بلا المِرساة يصير النفيُ إثباتاً — «لا نستعمل الأدوار: النظامُ
بمستخدمٍ واحد» تُقرأ اسمَ دور.

و**١٤ اختباراً قائماً سقط** بهذا التغيير، وكلُّها راجعتُها واحداً واحداً: لا واحدَ منها كان
يقيس شيئاً صار أسوأ — كلُّها كانت تُثبّت ألفاظَ المعجم المغلق. حُدِّثت بأسبابها المكتوبة، وأحدُها
(`lexiconFallback`) غُيِّرت **دعواه** لا رقمُه: كان يفحص «كلُّ ما في الفهم منطوقٌ في النصّ»
بعضويّةِ المعجم — وكيلاً عن الدعوى — فصار يفحص الأدوارَ **بالنصّ حرفاً بحرف**، وهو الدليلُ الأقوى.

---

## لا انتظارَ على بابٍ مغلق — ومقياسُ ما لم يُصلَح بعد

#171 جعلت «نعيد المحاولة» وعداً يُوفى: `createWithFailover` يجرّب السلسلةَ كاملةً ثمّ يُعيد
`AI_MAX_RETRIES` مرّاتٍ بتراجعٍ تصاعديّ — **على العابر وحدَه**، لأنّ تكرارَ الدائم هو عينُ ما
أُغلق في #588 («سبعُ دوراتٍ تُحرق على بابٍ مغلق»).

### العطبُ المقيس بالتشغيل

حلقةُ `chatResponse` **سبقت** تلك الإعادة، فبقيت لا تعرف `aiUnavailable` — العَلَمَ الذي يقول
«كلُّ المزوّدين فشلوا فشلاً دائماً» (رصيدٌ منتهٍ، مفتاحٌ خاطئ، اسمُ موديلٍ غير موجود). فالسلسلةُ
تحتها تمتنع عن الإعادة **بحقّ**، ثمّ تنتظر هي ٢٥٠٠ms وتُعيد الكرّة على البابِ نفسِه.

| على عطبٍ **دائم** | نداءات | انتظارُ المستخدم |
|---|---|---|
| قبل | ٢ | **٢٥٠٥ms** |
| بعد | **١** | **١ms** |

والعطبُ يُسمّى **دائماً** بسببه، ولا يُعدّ «محاولة ١/٢» — فالعدُّ وعدٌ بثانيةٍ لا تأتي.

**التحقّق**: اختبارٌ سلوكيٌّ يقيس النداءاتِ والزمنَ والسطر، **٥/٥ طفراتٍ مقتولة**.

### وما لم يُصلَح — مقياسٌ مثبَّتٌ لا دعوى

`agents/coderAgent.js` يبني سلسلتَه بنفسِه (`Groq ← DeepSeek ← Gemini`)، و**أوّلُ خطوةٍ فيها هي
السلسلةُ كلُّها**: `callGroq` ينادي `groq` وهو غلافُ `createWithFailover` لا عميلَ Groq الخام —
فيجرّب الأربعةَ ويُعيد. ثمّ تعود حلقةُ المولّد فتجرّب DeepSeek وGemini **خامَّين**: مزوّدان
استُنفدا سلفاً، وبلا إعادة.

`tests/coderChainMeasure.test.mjs` يُثبّت هذا **قياساً**، ولا يدّعي إصلاحاً: إعادةُ بناء سلسلة
المولّد تمسّ المسارَ الذي **يكتب موقعَ المستخدم**، فتُفصل ببندها ودليلها — «تدريجيٌّ ومُتحقَّقٌ
دوماً». وما يبقى قائماً في هذه الدفعة هو مرشِّح `AI_PROVIDERS` (`selectModels`)، وهو ما يمنع
مزوّداً مستبعَداً من العمل عبر النداء الخام.

---

## الإطارُ المنصوص لا يُنقَض بتصنيف — من سجلٍّ حيّ لصاحب المشروع

### تصحيحُ تشخيصٍ لي، أوّلاً

قلتُ أوّلَ قراءتي للسجلّ إنّ «`resolveStack` لا يقرأ نصَّ الطلب إطلاقاً». وكان **ناقصاً**:
الطلبُ **يصل** القرارَ عبر `resolveProjectType` → `detectProjectType`، ومقيسٌ أنّ
`detectProjectType(الطلب)` تعطي **`saas`** — و`saas` في `BIG_TYPES`. فالطلبُ كان يكفي.

### العطبُ الحقيقيّ، مقيساً

`resolveProjectType` تُعطي فئةَ المخطّط **الأولويّةَ المطلقة**:
```js
blueprint?.category && blueprint.category !== 'other' && blueprint._source !== 'fallback'
    ? blueprint.category : detectProjectType(goal)
```
وحارسُها المكتوب يحمي من **احتياطٍ** يكتب `business`، **لا** من **نموذجٍ** يُصنّف SaaS بأنّه
`business`. والسجلُّ يشهد: «✅ تم تطبيق قالب **business**» ثمّ «🧰 مسار سريع → **Vanilla**».

| المدخل | ptype | stack |
|---|---|---|
| الطلبُ وحدَه | `saas` | **react-next** |
| + مخطّطُ نموذجٍ يقول `business` | `business` | **vanilla** ← العطب |
| + المخطّطُ الاحتياطيّ `business` | `saas` | react-next (الحارسُ القائم يعمل) |

فالكلمةُ التي كتبها صاحبُ المشروع بيده («واجهة **React**») خسرت أمام تخمينِ نموذج. وهي عائلةُ
PM/22–PM/25 نفسُها. وأثرُها ليس تقنيّاً فقط: الفجوةُ التي أُبلغ بها بعدها — «١ ناقصة:
**KanbanBoard**» — **وُلدت من قرارِ مسارٍ خالفَ نصَّ طلبه**.

### العقد: `explicitStackRequest(goalText)`

`resolveStack({ projectType, scope, goal })` — **ذِكرُ الإطار في الطلب يسبق كلَّ تصنيف**.
والنفيُ يُطوى أوّلاً بـ`stripNegated` (علّةُ PM/23 بمصدرها الواحد). قِيس **٨/٨** حالات.

⚖️ **ولمَ الذكرُ الصريح وحدَه؟** لأنّ التصنيفَ **تخمينٌ من الطرفَين**: قلبُ القرار بمجرّد أنّ
الكشفَ قال «كبير» يجعل موقعَ مطعمٍ بسيطاً مشروعَ React — خسارةٌ حقيقيّة. أمّا «React» مكتوبةً
فليست تخميناً: **مطلبٌ منصوص**. والقرارُ **يُقال** في السجلّ: «🧰 طلبتَ الإطارَ باسمه».

### حدودٌ مكتوبة

- **يقرأ ذِكرَ الإطار فقط** — لا سَعةً ولا تعقيداً؛ تلك يقرؤها `projectType` و`scope` كما كانا.
- **«لا أريد Next.js» تمرّ اليوم**: `stripNegated` القائمة تطوي «بدون X» و«بلا X» و«no X» لا
  «لا أريد X». مثبَّتٌ باختبارٍ **كما هو**: توسيعُ النافي يمسّ مستهلكيه الثلاثة (سؤالُ الخادم،
  بوّابةُ الفهم، وهذا) — قرارٌ ببندِه ودليله لا رقعةٌ في الطريق.
- **وتصنيفٌ غيرُ مستقرّ، مُلاحَظٌ لا مُصلَح**: اقتطاعُ الأسطر الأخيرة من الطلب نفسِه يقلب
  `detectProjectType` من `saas` إلى `ecommerce`. كلاهما في `BIG_TYPES` فلا أثرَ هنا — لكنّه
  مُسجَّلٌ لبندِه (#186)، ولا يُخلط بهذا.

**التحقّق**: ٦ اختباراتٍ جديدة، **٨/٨ طفراتٍ مقتولة**. وسقطتان في أوّل تشغيل كانتا **خطأً في
توقّعاتي لا في الشفرة**: نصُّ الطلب في تثبيتي كان مقتطعاً، وصيغةُ النفي «لا أريد» خارج ما يطويه
النافي القائم. صُحّح الأوّل، وثُبِّت الثاني حدّاً معلَناً.

---

## 📐 قياسٌ أسقط فرضيّةً: عتبةُ بوّابة الفهم لا تُصلَح بنسبة (#186)

`goalFidelity(...).ungrounded` تقع اليومَ حين **لا اسمَ واحدٌ** من الفهم له أثرٌ في الطلب
(`supported.length === 0`). وقِيس في سجلٍّ حيٍّ لصاحب المشروع أنّها **صمتت** عن فهمٍ سبعةٌ من
ثمانيةِ أسمائه بلا أثر: طلبَ «SaaS لإدارة المشاريع» فخرج
`Customer/Company/Interaction/Desk/MeetingRoom/SalesRep` — منتجُ CRM وحجزِ مكاتب.

الفرضيّةُ المسجَّلةُ في بند #186 كانت: «فلتصر العتبةُ **نسبة** لا صفراً». والقياسُ أسقطها:

| الحالة | مسنَد/بلا سند | النسبة | الحكمُ الصحيح |
|---|---|---|---|
| SaaS الحيّ (فهمُ CRM) | ١/٧ | **١٣٪** | يجب أن تقع |
| عيادةٌ صادقة على طلبٍ قصير | ١/٥ | **١٧٪** | يجب أن تصمت |
| مدوّنةٌ صادقة على طلبٍ قصير | ٠/٦ | **٠٪** | يجب أن تصمت |

فالمهلوَسُ **محشورٌ بين صادقَين**؛ ولا عتبةَ نسبةٍ تفصل. وجُرِّب «الإصلاحُ الساذج»
(`supported.length <= 1`) على الحزمة كاملة: يُصلح الحالةَ الحيّة **ويكسر** تثبيتاً قائماً
(«سندٌ جزئيٌّ ليس انعدامَ سند») — عطبٌ مقيسٌ بعطبٍ مقيس، لا ربح.

**والعلّةُ تحت ذلك أعمقُ من العتبة**: نصفُ أسماء أيِّ نموذج **خارجَ المعجم أصلاً** —
`Company`, `Interaction`, `MeetingRoom`, `SalesRep` في المهلوَس، و`Post`, `Comment`, `Category`,
`Tag`, `Author`, `Reader` في مدوّنةٍ صادقة. فما كان خارجَه لا يُسنَد بجسر المعجم ولا يُعرَف
غريباً؛ ولا يبقى إلّا جسرُ اللفظ، فيعبر أو لا يعبر بحسب ما إذا كان صاحبُ الطلب العربيُّ كتب
اسماً إنجليزيّاً. **فالمقياسُ يقيس تغطيةَ المعجم أكثرَ ممّا يقيس صدقَ الفهم** — وهي علّةُ
«القائمةِ المغلقة» التي شخّصها PM/22 نفسُه، باقيةً في نصفِ جسرَيه.

**ومحورُ دليلٍ ثانٍ جُرِّب وسقط**: اتّفاقُ `modelProjectType(الفهم)` مع نوعِ الطلب. احتياطُ
**المطعم** المنسَّقُ بيد إنسان يُعطي `modelProjectType = 'ecommerce'` (لأنّ `Order`+`Customer`
يطابقان توقيعَ المتجر)، فلو صار الاختلافُ إدانةً لوُصم بناءٌ صادق.

**وعلّةٌ ثانيةٌ معاكسة قِيست في الطريق** (بندٌ مستقلّ #189): البوّابةُ تقع اليومَ على فهمٍ
صادق — «متجر إلكتروني لبيع الملابس» باحتياطِ الفئة المنسَّق يخرج ٠/٤ فيُقال لصاحبه «فهمي لا
يمسّ طلبَك» عن فهمٍ صحيح. ولم تُصلَح هنا: **تهدئةُ بوّابةٍ فعلٌ خطِر**، ولا يُقدَم عليه بمحورِ
دليلٍ ثبت سقوطُه.

**التحقّق**: `tests/goalFidelityMeasure.test.mjs` — ٦ اختبارات تُثبّت الأرقامَ أعلاه **قياساً
ولا تدّعي إصلاحاً**، على سنّة `tests/coderChainMeasure.test.mjs`. ولم تُغيَّر شفرةُ إنتاجٍ
واحدة في هذه الجولة: ١٩٩٦/١٩٩٦.

---

## 🔗 PM/25-ب: القوسُ الملاصقُ للفظ الدور — رفعُ حدٍّ مكتوبٍ بقياسِه (#188)

الحدُّ كُتب في جولة PM/25 نفسِها: «`labelledRoleNames` تسمّي من يُسمّى في سطرٍ **معنون**؛
و«أدوار (owner, editor, viewer)» بين قوسَين **لا تُلتقط**». ثمّ وقع حيّاً في سجلّ صاحب
المشروع: طلبَ SaaS وسمّى ثلاثةَ أدوارٍ بين قوسَين، **فلم يصل منها ولا واحد**، ووُصم البناءُ
بـ`role-coverage` لدورٍ (`SalesRep`) لم يُطلَب قطّ.

**ولم يُوسَّع بتعميم بل بقياس.** على نصوصٍ حقيقيّة (مواصفةُ نقاط البيع، طلبُ SaaS الحيّ،
«وِرد»، طلباتٌ متنوّعة): **٢٩ قوساً، ٣ فقط (١٠٪) تلي لفظَ دورٍ مباشرةً**. فأخذُ كلِّ قوسٍ كان
سيُدخل **٢٦** قوساً فيها أسماءُ شاشاتٍ ومزايا وكيانات (`POS Screen`، `Dashboard`،
`Offline Sync`، «مقاس/لون»، «سحب وإفلات»، «مستخدمين، مشاريع، مهام، تعليقات»).
فالقاعدة: **القوسُ الملاصقُ للفظ وحدَه**.

**وشرطُ «قائمةٌ لا مفردة»**: ثالثُ الثلاثة `(RBAC)` — من `pos_spec.txt:3` «الصلاحيات
والأدوار (RBAC):» — اختصارُ مفهومٍ لا اسمُ دور، ولو مرَّ لصار مطلباً تفحص `role-coverage`
الصفحةَ بحثاً عنه. فبندان فأكثر، وإلّا فلا.

| | قبل | بعد |
|---|---|---|
| أدوارُ طلب SaaS الواصلةُ للبُناة | ٠ (ثمّ تخمينُ معجم) | **`owner`، `editor`، `viewer`** |
| مواصفةُ نقاط البيع | ٥ | ٥ (بلا `RBAC`) |

**ولفظُ الأدوار مصدرٌ واحد** (`ROLE_WORD`) يقرؤه الملصقُ والقوس — لا نسختان تفترقان عند
أوّل لفظٍ يُضاف. ولا `\b` (الحرفُ العربيُّ ليس `\w` — الفخُّ الذي وقع ثلاث مرّات).

**وحُذف `continue` كتبتُه أوّلاً** («سطرٌ معنونٌ لا يُقرأ مرّتين»): نجت طفرتُه، فقيس أثرُه فوُجد
أنّه **يُنقص** لا يحمي — سطرٌ فيه الاثنان يفقد به دورَين حقيقيَّين، والتكرارَ يمنعه
`out.includes` أصلاً. حُذف بالقياس لا بالذوق.

**التحقّق**: ٦ اختباراتٍ جديدة (١٧ في الملفّ)، **٨/٨ طفراتٍ مقتولة** — ومنها طفرةٌ كشفت
حدّاً قائماً من PM/25 لم يكن مثبَّتاً قطّ (سقفُ أربع كلماتٍ للبند)، فثُبِّت. ٢٠٠٢/٢٠٠٢.

---

## ✂️ #185: مسارُ التعديل الجراحيّ كان بلا حارسٍ ولا إشارة — والحلقتان تساوتا

`coderAgent` فيه حلقتا مزوّدين: البناءُ الكامل (`coreGenerateCodePlan`) والتعديلُ الجراحيّ
(`coreEditCodePlan`). الأولى تُرشِّح بـ`selectModels` وتُشير بـ`aiUnavailable`؛ والثانية كانت
**مصفوفةَ دوالٍّ عارية**: لا مرشِّح، ولا اسمَ يُقال في السجلّ، ولا إشارةَ عطبٍ دائم.

**والأثرُ مقيسٌ بالتشغيل، لا مستنتَج.** بـ`AI_PROVIDERS=groq`:

```
🔇 [AI Core]: deepseek مُستبعَد بـAI_PROVIDERS (مفتاحُه موجود، لم يُحذف).
🔇 [AI Core]: gemini مُستبعَد بـAI_PROVIDERS (مفتاحُه موجود، لم يُحذف).
المزوّدون المُنادَون خاماً: deepseek(خام) → gemini(خام)
```

الخدمةُ **تعلن استبعادَهما ثمّ تناديهما** — فتذهب شفرةُ مشروع المستخدم إلى مزوّدٍ قال صاحبُ
المنصّة لا. وهي علّةُ #168 بعينها في مسارٍ لم يُفتَّش، وعلّةُ «الاختلاف الصامت» في أوضح صورها:
قولٌ في السجلّ وفعلٌ يخالفه.

| | قبل | بعد |
|---|---|---|
| نداءاتٌ خامّةٌ لمزوّدٍ مستبعَد | **٢** | **٠** |
| عطبٌ دائم ← `aiUnavailable` | `undefined` | **`true`** |
| لا مزوّدَ مُفعَّل | «تعذّر تطبيق التعديل» | **رسالةُ الإعداد** (`AI_PROVIDERS`) |

**وثغرةُ تغطيةٍ كشفتها الطفرة**: `failures.every(isPermanentAIError)` في **البناء الكامل** —
المسارِ الذي يكتب موقعَ المستخدم — نجت طفرتُه (`every → some`) من **٢٠٠٦ اختباراً**. أي أنّ
الفرقَ بين «كلُّها دائمة» و«إحداها» لم يكن مثبَّتاً هناك قطّ، وهو فرقٌ غيرُ تجميليّ: لو قيل
«دائم» ومعنا عطبٌ عابر لحُرم المستخدمُ إعادةً كانت ستنجح. ثُبِّت الموضعان.

**وحدٌّ باقٍ على حاله**: إعادةُ بناء سلسلة المولّد نفسِها (خطوةٌ أولى هي `createWithFailover`
كاملةً، ثمّ خطوتان على عميلَين خامَّين استُنفدا سلفاً) **ما زالت مقيسةً ولم تُصلَح** —
`tests/coderChainMeasure.test.mjs`. هذه الجولةُ سوّت الحلقتَين ولم تمسّ السلسلة.

**التحقّق**: ٤ اختباراتٍ جديدة، **٥/٥ طفراتٍ مقتولة**. وقياسُها في **عمليّةٍ ابنة**: `AI_PROVIDERS`
تُقرأ عند تحميل `llm.js`، ولو استُبدل العميلُ على نسخةٍ ذاتِ استعلامٍ لاستورد `coderAgent` نسخةً
غيرَها فلم يقع الاستبدال (وقعت هذه الغلطةُ فعلاً وكُتبت هنا). ٢٠٠٧/٢٠٠٧.

---

## 🔀 #190: رمزُ ٤٠٣ ملتبس، والرسالةُ تحسم — تشخيصان متناقضان لعطبٍ واحد
...
يُبلَّغ عنه باسم موديل، واسمُ موديلٍ مجهولٍ يقتضي مفتاحاً صالحاً قُبل به الطلب. **فأحدُهما كاذب.**

**والعلّةُ مقيسة**: `classifyAIError` كانت تقرأ `status === 403` كـ`auth` **قبل** أن تقرأ
الرسالة. والمزوّدون يردّون على علّة الموديل نفسِها بـ`404` مرّةً و`403` أخرى — ونصُّ الـ٤٠٣ هو
`The model \`x\` does not exist **or you do not have access to it**`. فيخرج التشخيصان
المتناقضان **من عطبٍ واحد**.

| المُدخَل | قبل | بعد |
|---|---|---|
| `403` + «the model … does not exist» | **`مفتاحُه غير صالح`** | **`اسمُ الموديل المضبوط غير معروف عنده`** |
| `404` + النصّ نفسه | `اسمُ الموديل…` | `اسمُ الموديل…` (كما كان) |
| `403` بلا ذِكرِ موديل | `مفتاحُه غير صالح` | `مفتاحُه غير صالح` (كما كان) |
| `401` ولو ذُكر موديل | `مفتاحُه غير صالح` | `مفتاحُه غير صالح` (كما كان) |

والضررُ الذي أُغلق ليس لفظيّاً: صاحبُ المنصّة كان يُطارد **مفتاحاً سليماً** ويترك سطرَ إعدادٍ
كان يُصلح العطبَ **بلا نشر** — وهو الفرقُ الذي بُنيت له `KIND_WORDS` في #170 أصلاً.

**والقاعدة مكتوبة**: *الرسالةُ الصريحة تغلب الرمزَ الملتبس*. و`401` تبقى `auth` بلا شرط —
«غيرُ موثَّق» لا لبسَ فيه، بخلاف «ممنوع» التي تحتمل المفتاحَ والموردَ معاً. ونمطُ «علّة
الموديل» صار **مصدراً واحداً** (`MODEL_PROBLEM`) يقرؤه الحاسمان، فلا نسختان تفترقان.

**وحدٌّ مكتوبٌ قِيس ولم يُغيَّر**: أوّلُ سطرٍ في `classifyAIError` يختصر `aiUnavailable` إلى
`quota`، فخطأٌ مجمَّعٌ سببُه إعدادٌ يُقرأ «رصيد». وقِيس أنّه **لا يبلغ صاحبَ المشروع**: العرضُ
يقرأ `diagnosis` المبنيَّ من الأخطاء **الخام** ولا يُعيد التصنيف، والمستهلكُ الوحيدُ الآخرُ
(`isPermanentAIError`) يعامل الأصنافَ الثلاثةَ سواءً. مثبَّتٌ باختبارٍ كي لا يصير عرضاً يوماً.

**التحقّق**: ٥ اختباراتٍ جديدة، **٤/٤ طفراتٍ مقتولة**. ٢٠١٢/٢٠١٢.

---

## #١٩١ — «أيُّ وكيلٍ يحرق الرموز؟» أداةُ قياسٍ قبل أيِّ توزيعِ موديلات

**الطلب**: عرض صاحبُ المشروع جدولَ توزيعٍ لموديلات Groq على الوكلاء (مخطِّط/معماريّ/مراجع →
موديلٌ كبير، مولّد → آخر، مصنّف → صغير…). والجوابُ الصادقُ أنّ الجدولَ **غيرُ قابلٍ للتطبيق
اليوم**: `GROQ_MODEL` واحدٌ للمنصّة كلِّها، وسلسلةُ الاحتياط تكتب الموديلَ فوق ما يمرّره
المنادي. ثمّ إنّه يخلط ثلاثةَ أشياء: إعادةَ توزيع، وقدراتٍ جديدة، وخفضَ كلفة. فاتُّفق:
**«لنبدأ القياس أوّلاً ثمّ نحدّد ما ينفعنا»** — وهذه الدفعةُ هي القياسُ وحدَه.

**النطاق المُلتزَم**: أداةُ قياسٍ محضة — لا تغيّر سلوكاً ولا تختار موديلاً. لا معامِلَ جديدٍ
يُرسَل لأيِّ مزوّد. ولا يُقرَّر توزيعٌ قبل أن يُشغَّل بناءٌ حقيقيّ وتُقرأ أرقامُه.

### ما كان

عدّادُ `noteUsage`/`usageLine` يسجّل لكلِّ **مزوّد**. وهو يجيب «كم» ولا يجيب «أين»:
`groq 12400` لا تقول أَمِن المولّد جاءت أم من المراجع أم من الشات. فأيُّ قرارِ توزيعٍ كان
سيُبنى على حدسٍ لا رقم.

### الأداة

وسمٌ **محيطيٌّ** بـ`AsyncLocalStorage` (`withUsageLabel`/`currentUsageLabel` + `byLabel`):
قِيس أنّ مواضعَ النداء المباشرة تسعةَ عشرَ في أربعةَ عشرَ ملفّاً، وأنّ وكلاءَ العقود كلَّهم
يمرّون بـ`runAgent` وهو يعرف `agent.id` سلفاً — فتغييرُ التواقيع كان سيمسّ كلَّ منادٍ ليخدم
عدّاداً. و`AsyncLocalStorage` تعبر `await` و`for await`.

- **٢٤ موضعَ وسمٍ** تُنتج **٢٢ وسماً** + `agent:<id>` لوكلاء العقود من نقطةٍ واحدة.
- **موضعُ اللفّ قِيس ولم يُفترَض**: في المسار المتدفّق تُحسب الرموزُ في `for await` بعد أن
  عاد `create` **بزمن** — فلفُّ `create` وحدَه كان يترك النداءَ «بلا وسم». الوسمُ يلفّ
  موضعَ الحساب: `model.call()` كلَّه في المولّد، وتصريفَ التدفّق في الشات.
- **العرض**: `usageByLabelLine` بجوار `usageLine` في تقرير التسليم — «كم» ثمّ «أين»،
  مرتَّبٌ تنازليّاً لأنّ السؤالَ عليه دائماً «مَن الأكثر؟».

### ثقبان في العدّاد ظهرا **بسبب** الوسم — قِيسا وسُدّا

الأداةُ لمّا رُكِّبت كشفت أنّ ما تقيسه لم يكن يُقاس أصلاً:

| الثقب | القياس | الأثر لو بقي |
|---|---|---|
| `callDeepSeek` غيرُ المتدفّق يخاطب عميلاً **خاماً** لا يمرّ بـ`tagged` | بـ`AI_PROVIDERS=deepseek`: مسارُ التوليد كلُّه = **نداءٌ واحدٌ للمزوّد، صفرُ رموزٍ محسوبة** | يُقرأ «المولّد رخيص» وهو **لم يُقَس** |
| حقولُ Gemini في `usageMetadata` بأسماءٍ camelCase (`promptTokenCount`…) و`noteUsage` يقرأ `prompt_tokens` وحدَها | نداءٌ بـ`{promptTokenCount:70, candidatesTokenCount:30, totalTokenCount:100}` ⇒ `counted: 0`, `total: 0` | **كلُّ** نداءِ Gemini يُعدّ «صامتاً» بصفرِ رموز — مزوّدٌ كاملٌ يسقط من الحساب |

كلاهما إصلاحُ **قياسٍ لا سلوك**: لا معامِلَ جديدٍ، ولا مسارَ نداءٍ تغيّر.

### الطفرات

قُتلت: إسقاطُ الوسم في الشات · الترتيبُ التصاعديّ · تجاهلُ لقطة البداية · إسقاطُ مرشِّح
الصفر · «الوسمُ لا يُقرأ» · «`withUsageLabel` لا يفتح نطاقاً» · إفراغُ وسم وكلاء العقود ·
إسقاطُ وسمَي المولّد · إزالةُ حساب deepseek الخام · إسقاطُ أسماء Gemini · إسقاطُ
`page:section` · إعادةُ تسمية وسم.

**ثلاثُ طفراتٍ نجت أوّلاً** (وسمُ وكلاء العقود، ووسما المولّد) — أي أنّ **صفرَ** اختباراتٍ
من ٢٠١٢ كانت تقرأ `byLabel` على تلك المسارات. فكُتبت لها اختباراتُ سلوكٍ حقيقيّة:
`agentRuntime` (وكيلان لا يختلط حسابُهما)، و`providerSelection` بعمليّةٍ ابنة (السببُ
نفسُه المكتوبُ هناك: `AI_PROVIDERS` تُقرأ عند التحميل)، و`chatResponseStage` لتصريف التدفّق.

### فجوةٌ **تُقال ولا تُخفى**

ثمانيةَ عشرَ وسماً بقيت بلا اختبارِ سلوك — لفٌّ مباشرٌ حول نداءٍ واحد، وثمنُ قتلِ طفراتها
سلوكيّاً ثمانيةَ عشرَ اختبارَ عمليّةٍ ابنة لأجل عدّادٍ لا يغيّر سلوكاً. فوُضع بدلَها **حارسُ
حدودٍ** واحد (`tests/usageLabelCoverage.test.mjs`) يثبّت مفرداتِ الوسم: يقع إن سقط وسمٌ أو
تبدّل اسمُه، **ولا يدّعي أكثر**.

وكذلك **«بلا وسم» بندٌ مُعلَنٌ في السطر لا فجوةٌ مبتلَعة**: يقع فيه ما لم يُوسَم قصداً —
أبرزُه وكلاءُ الإضافات التي يُنشئها المستخدم من اللوحة (`adminService.js` يكتب شفرتَها داخل
قوالبَ نصّيّة، فهي شفرةٌ مولَّدة لا موضعُ نداءٍ في المنصّة). فإن كبر هذا البند فالرقمُ نفسُه
هو الخبر.

### الخطوة التالية — **قرارُ المالك**

يُشغَّل بناءٌ حقيقيّ، ويُقرأ السطران في السجلّ:

```
💰 [AI Usage]: … رمزاً (دخل … / خرج …) في … نداء — groq …
🏷️ [AI Usage/وكيل]: coder:generate … (…)، agent:… … (…)، review … (…)، …
```

**وعندها وحدَها** يُقرّر التوزيعُ بدليل. ولا يُختار موديلٌ لوكيلٍ قبل ذلك.

**الاختبارات**: ٢٠٣٣/٢٠٣٣ (كانت ٢٠١٢ — ‎+٢١).


---

## #١٩٢ — التدفّقُ يُحسب قطعةً قطعة: سطرُ كلفةٍ كاذبٌ في سجلٍّ حيّ

**المصدر**: لقطةُ سجلِّ إنتاجٍ أرسلها صاحبُ المشروع، فيها:

```
💰 [AI Usage]: 312748 رمزاً (دخل 190264 / خرج 122484) في 79954 نداء
               — groq 312748 · 79913 نداءً بلا أرقامٍ من المزوّد (غيرُ محسوبة)
```

**الحسابُ وحدَه يفضح الرقم**: `الصامت = النداءات − المحسوب`، فالصامتُ لا يتجاوز النداءات
أبداً. القراءةُ المتّسقةُ الوحيدة: **نداءات ٧٩٩٥٤، محسوبٌ منها ٤١**. وثمانون ألفَ نداءٍ
لمزوّدٍ في نافذةٍ واحدة غيرُ معقولة — فالرقمُ ليس نداءات.

**العلّة** (قِيست بالتشغيل لا بالقراءة): `noteUsage` كان يُنادى **داخل `for await` لكلِّ
قطعةِ بثّ** في ثلاثة مواضع (حلقتا `coderAgent` وحلقةُ `chatResponse`). فكلُّ قطعةٍ ترفع
`calls` وتُعدّ «صامتة»:

```
نداءٌ متدفّقٌ حقيقيٌّ واحد بـ٢٠٠٠ قطعة ⇒ 💰 … في 2000 نداء · 1999 نداءً بلا أرقام
```

### الأثرُ أبعدُ من سطرٍ مغلوط

`byLabel` (#١٩١) يرث العلّةَ نفسَها. فالمولّدُ — **المتدفّقُ والأثقل** — كان سيظهر في
`🏷️ [AI Usage/وكيل]` بعشرات آلاف «النداءات» وبرموزٍ شبهِ صفر، بينما يظهر غيرُ المتدفّقين
بأرقامٍ صحيحة. أي أنّ **الأداةَ التي بُنيت لقرار توزيع الموديلات كانت ستقلب القرار**:
يُرخَّص أغلى وكيلٍ لأنّه بدا صامتاً. وهذا هو عطبُ «الصمتُ يُقرأ لا مشكلة» في الأداة التي
وُضعت لكشفه.

### الإصلاح — `drainStream` مصدراً واحداً

ثلاثةُ مواضعَ بالشكل نفسِه ⇒ **ثلاثةُ مستهلكين أحياء** تُبرّر دالّةً واحدة (لا تجريدَ بلا
مستهلك). و`drainStream(stream, onChunk)` يجمع الأرقامَ من أيِّ قطعةٍ تحملها ثمّ يحسب
**نداءً واحداً** في `finally` — فتدفّقٌ ينقطع في منتصفه نداءٌ وقع فعلاً وكلّف، يُعدّ صامتاً
ولا يُبتلَع.

| | قبل | بعد |
|---|---|---|
| ٢٠٠٠ قطعةٍ بأرقام | `calls 2000`، صامت ١٩٩٩ | `calls 1`، محسوب ١ |
| ٥٠ قطعةً بلا أرقام | `calls 50`، صامت ٥٠ | `calls 1`، صامت ١ |

**الطفرات** (قُتلت جميعاً): أيُّ قطعةٍ تُعتمد حاملةً للأرقام · الحسابُ خارج `finally` ·
نداءان بدل واحد · القطعةُ لا تصل مستهلكَها. وشاهدٌ سالبٌ (تغييرٌ بلا أثر) نجا كما يجب،
فالحزمةُ لا تُبلغ قتلاً كاذباً.

**الاختبارات**: ٢٠٣٩/٢٠٣٩ (كانت ٢٠٣٣ — ‎+٦). وحارسُ الحدود وقع فصُحّح بسببه مكتوباً.

### مقيسٌ ولم يُصلَح — في دفعةٍ مستقلّة

من السجلّ نفسِه: `Groq 400 "Failed to generate JSON. Please adjust your prompt"` يُصنَّف
`transient`، فـ`aggregateFailure` لا يرفع `aiUnavailable` (يشترط `every`)، فتُعاد السلسلةُ
كلُّها وتسقط بالنتيجتين نفسيهما (٠٩:٣٧:١١ ← ٠٩:٣٧:١٧ في السجلّ، ومعها ١٢٠٠ms انتظار).
و«أعِد صياغة موجَّهك» ليس عابراً: إعادةُ الموجَّه نفسِه تُنتج الفشلَ نفسَه حتماً. **لم يُصلَح
هنا لأنّه يغيّر سلوكاً**، وهذه الدفعةُ أداةُ قياسٍ وُعد ألّا تغيّر شيئاً.

وكذلك: `DeepSeek 402 Insufficient Balance` — رصيدٌ منتهٍ، **قرارُ دفعٍ لصاحب المشروع** لا شفرة.


---

## ‏#١٩٣ — «طلبُك غيرُ صالح» ليس «تعذّر الوصول»

**الشاهدُ الأوّل** (سجلّ Render حيّ، ٢٠٢٦-٠٩-٠٩، بناءُ منصّةِ جمعيّةٍ خيريّة) — تسعُ نوبات:

```
[AI Failover] Groq فشل (400 "Failed to generate JSON. Please adjust your prompt.") → DeepSeek
[AI Failover] DeepSeek فشل (402 Insufficient Balance) → لا بديل متبقٍ
[AI Retry] عطبٌ عابر — إعادةُ المحاولة 1/2 بعد 400ms
```

المُوجَّهُ لا يتغيّر بين المحاولات. فإعادةُ السلسلة كلِّها بحروفها **فشلٌ مضمون**: عشرون ثانيةً محروقةً في نوبةٍ واحدة (٠٩:٣٧:١٠ ← ٠٩:٣٧:٣٠).

**الشاهدُ الثاني**، من صاحب المنصّة نفسِه وبلفظٍ مغايرٍ تماماً:

```
'messages.1' : for 'role:user' … property 'at' is unsupported
```

وهذا الشاهدُ هو ما حسم **شكلَ** القاعدة. أوّلُ ما كُتب كان نمطَ ألفاظ (`failed to generate json|adjust your prompt`) — ولم يكن ليلتقط الشاهدَ الثاني، ولَعاد يُوسَّع لفظاً بلفظ كما وُسِّعت `MODEL_PROBLEM` مرّتين. فصار المقياسُ **رمزَ الحالة**: `400` في هذه الواجهة تعني `invalid_request_error` — حكمٌ على **الطلب** لا على المزوّد. و`401/402/403/429` تُحسم كلُّها قبله.

**صنفٌ ثالثٌ بين الدائم والعابر**: `prompt` دائمٌ **لهذا النداء** فلا تُعاد المحاولة، ولا يُدين المزوّدَ فمفتاحُه سليمٌ ورصيدُه قائم. وله رسالتُه: `AI_UNAVAILABLE_MSG` («رصيدُ المزوّد منتهٍ أو مفاتيحُه غير صالحة») جملةٌ **كاذبة** هنا تُطيّر صاحبَ المشروع خلف فاتورةٍ لا شأنَ لها. وخلطُ رفضِ مُوجَّهٍ بعطبٍ عابرٍ يبقى قابلاً للإعادة — فالمزوّدُ الآخرُ قد ينجح.

### والعلّةُ عولجت لا تصنيفُها وحدَه

| | |
|---|---|
| `ensureJsonWordInPrompt` | الواجهةُ المتوافقة مع OpenAI تشترط في وضع `json_object` أن تذكر الرسائلُ كلمةَ JSON. تُحقن حين تغيب فقط، **في البوّابة** لا في أربعةَ عشرَ موضعَ نداء. |
| `sanitizeMessages` | `conversationStore` يخزّن كلَّ دورٍ `{ role, content, at }`، و`chatResponse:113` ينشر التاريخَ (`...history`) في الرسائل. فحقلُ **تخزينٍ** عندنا صار حقلَ **بروتوكولٍ** عندهم. |

وأثرُ الثاني أكبرُ ممّا يبدو: المزوّدان العاملان اليومَ (Groq وDeepSeek) كلاهما على واجهة OpenAI فيرفضانه، وGemini وحدَه كان يستره لأنّه يسطّح الرسائلَ نصّاً. أي أنّ استبعادَ Gemini بـ`AI_PROVIDERS` **كشف** عطباً قائماً ولم يُحدثه — وكانت كلُّ محادثةٍ ذاتِ تاريخٍ مكسورة.

مقيسٌ بابنٍ حقيقيّ يستبدل **عميلَ المزوّد** (لا `groq` المصدَّر) فيجري المسارُ كما يجري حيّاً.

---

## ‏#١٩٤ — مكتبةُ النماذج تُورِّث فهمَ منتجٍ آخر، وتطرد فهمَ صاحبِه

من السجلّ الحيِّ نفسِه. طُلبت **منصّةُ جمعيّةٍ خيريّة** بأدوار (مدير الجمعية، مسؤول الحملات، محاسب، متطوّع). فقال المتحقّقُ السلوكيّ لصاحبها:

```
❌ أدوار بلا واجهة/تمثيل: User، SalesRep، Customer
⚠️ مفاهيمُ المنتج غير ظاهرة: customer، company، interaction، workspace، meetingroom، salesrep
```

مندوبُ مبيعاتٍ وغرفةُ اجتماعاتٍ في جمعيّةٍ خيريّة. مصدرُها فئةُ `business` في `modelLibrary` — وهي **اتحادٌ تراكميّ** لكلِّ مشروعٍ نجح فيها، فورث نظامَ عملاءَ ومساحةَ عملٍ سابقَين.

**وعطبان متمايزان، والثاني أسوأ:**

| | |
|---|---|
| **أ) الترتيبُ يخالف عقدَه المكتوب** | تعليقُ `understand.js` يقول «الأولوية: المشروع نفسه > اشتقاق هذه الجولة > **مكتبة الفئة العامة**» — والشفرةُ تنادي `mergeProjectModel(seed, derived)` فتضع المكتبةَ **أوّلاً**. ومع سقفَي `normalizeProjectModel` (٦ كيانات / ٤ أدوار) يُقصّ **الذيل**، أي فهمُ المشروع نفسِه. مقيسٌ بتشغيل: `Donor` و`Donation` و`Volunteer` و«متطوّع» **اختفت كلُّها**. والعلّةُ عينُها عولجت في السطر التالي للمرجع («كي لا تُسقطه سقوفُ التطبيع») ولم تُعالَج للبذرة. |
| **ب) البذرةُ لا تُغربَل بالطلب** | مفهومٌ موروثٌ لا أثرَ له في طلب صاحبِه كان يدخل كأنّه منه. |

**ولماذا لم يمنعه حارسُ PM/22؟** لأنّ `ungrounded` تشترط `supported.length === 0`، و«مدير الجمعية» و«محاسب» مسنودان — فيمرّ الباقي معهما، **ويُودَع الملوَّثُ في المكتبة ثانيةً**. مِسنَنةٌ تتراكم: كلُّ مشروعٍ يرث اتحادَ من قبله ويضيف إليه.

**والمقياسُ كان موجوداً**: `goalFidelity` تحسب `groundless` **لكلِّ اسمٍ على حدة**، ثمّ كان المصفوفُ يُهمَل ولا يُقرأ إلّا نعم/لا. أي إشارةٌ محسوبةٌ بدقّةٍ تُقرأ بغير دقّتها — وهذا فرقُها عن ‏#١٨٦ الذي سقطت فيه عتبةُ النسبة: هنا لا عتبةَ تُضبَط.

**المبدأ**: المكتبةُ **تُثري ما سمّاه صاحبُ الطلب، ولا تُضيف ما لم يُسمِّه.** فما نجا من `groundedSubset` يحمل حقولَه وأوصافَه المتراكمةَ عبر المشاريع — وهذه قيمةُ المكتبة الحقيقيّة — وما لا أثرَ له لا يدخل. وما يُسقَط **يُسمّى** في السجلّ (`🧹 أُسقط من فهمِ فئة «business» الموروث ما لم يمسّ طلبَك: …`)، فهو أوّلُ دليلٍ على تلوّثِ فئةٍ في المكتبة الدائمة.

### قيدٌ مقيسٌ ومثبَّتٌ باختبار

`Company` تسقط رغم أنّ صاحبَ الطلب كتب «شركتِه» — الجسرُ المعجميّ لا يربط اللفظَ العربيَّ بالاسم الإنجليزيّ (وهو ‏#١٨٦ بعينه). فالغربلةُ **متحفّظة**، ولم تُرفع بعتبةٍ متساهلة: اتّجاهُ الخطأ مقصود. الإسقاطُ الكاذب يكلّف حقولاً متراكمةً يعيد الاشتقاقُ من الطلب أكثرَها؛ والإبقاءُ الكاذب يُدخل كياناتِ منتجٍ آخرَ ثمّ **تطرد** كياناتِ صاحبِ الطلب. والقيدُ مثبَّتٌ باختبارٍ يسقط يومَ تتّسع تغطيةُ المعجم، فيُراجَع القرارُ بدليلٍ لا بالنسيان.

### الطفرةُ التي نجت

قلبُ الترتيب إلى `mergeProjectModel(seed, derived)` لم يُسقط اختباراً واحداً في الجولة الأولى — لأنّ كلَّ الطُّعوم كانت بذرتُها تسقط بالغربلة، فلا يبلغ المجموعُ السقفَ ولا يتبيّن أثرُ الترتيب. فكُتب طُعمٌ ببذرةٍ **مسنودةٍ كلِّها** (تنجو من الغربلة) + مشتقٍّ خاصٍّ بالمشروع يتجاوز مجموعُهما ستّةَ كيانات: بالبذرةِ أوّلاً يُطرد `invoice`، وبالمشتقِّ أوّلاً يبقى. وهو عينُ ما جرى لجمعيّة عطاء، مصغَّراً.

---

## ‏#١٩٥ — ما أسقط بناءَ جمعيّة عطاء: مجالٌ غائبٌ عن المعجم، وقوسٌ داخلَ اسمِ دور

سجلٌّ حيّ (٢٠٢٦-٠٩-٠٩، ١١:٠٨). طُلبت منصّةُ جمعيّةٍ خيريّة، فقال جولا لصاحبه:

```
🧩 نموذج المشروع: 3 كيان (تسجيل، ملف، لوحه) • 4 دور • 0 تدفّق
⚠️ فهمي لم يمسّ طلبَك (لا أثرَ في وصفِك لـCampaign، Donor، Donation)
   — فطرحتُه وبنيتُ على عناوينك أنت: تسجيل، ملف، لوحه.
```

**الفهمُ المطروحُ كان صحيحاً.** `Campaign/Donor/Donation` وصفٌ دقيقٌ لمنصّةِ جمعيّةٍ خيريّة، والحارسُ طرحه وبنى على `تسجيل، ملف، لوحه` — وهي أفعالٌ وألفاظٌ عامّة لا كيانات. وانتهى الحكمُ `FAILED`.

### العلّة، مقيسةً بالتشغيل

```
conceptsInText(الطلبُ كلُّه) → { accountant, admin }
```

مفهومان اثنان من طلبٍ فيه «الحملات» و«متبرع» و«تبرعاته» و«المتطوعين» — لأنّ **مجالَ العمل الخيريّ لم يكن في المعجم إطلاقاً**. ولا الجسرُ المعجميّ يعبر (لا «حملة»→`campaign`) ولا جسرُ اللفظ يلتقي (`Campaign` بحروفٍ لاتينيّة لا تلتقي «الحملات» أبداً).

وهذا هو ‏#١٨٦ بعينه مقيساً حيّاً — وقد كان مكتوباً في `CLAUDE.md` قبل وقوعه: «**العلّةُ تغطيةُ المعجم لا العتبة**».

| | قبل | بعد |
|---|---|---|
| `ungrounded` على الفهم الصحيح | **true** ← يُطرح | false |
| `supported` | — | `Campaign، Donor، Donation` |
| مفاهيمُ الطلب المرصودة | `accountant، admin` | `+ campaign، donor، donation، volunteer، charity` |

**والحارسُ لم يُسكَت** — مثبَّتٌ باختبارَين: الفهمُ المهلوَس (`Grade/ForumPost/Teacher`) ما زال مرفوضاً على طلب الجمعيّة، ومفاهيمُ الخير نفسُها مرفوضةٌ على طلبٍ لا يذكرها (متتبّعُ حفظ القرآن).

### وعطبٌ ثانٍ في السجلّ نفسِه

```
❌ أدوار بلا واجهة/تمثيل: (مدير الجمعية، مسؤول الحملات، محاسب، متطوع)
```

كتب صاحبُ المشروع `الأدوار: (مدير الجمعية، …، متطوع)`، فالتُقط أوّلُها `(مدير الجمعية` وآخرُها `متطوع)` — **بقوسَيهما**. فصار المتحقّقُ السلوكيّ يفتّش الشفرةَ المولَّدة عن دورٍ اسمُه حرفيّاً `(مدير الجمعية`، ولن يجده أبداً. أي أنّ بوّابةَ `role-coverage` كان **يستحيل** نجاحُها لمن يكتب أدوارَه بين قوسَين — وهي الصيغةُ الطبيعيّة.

أُصلح في `pushRoleItems`، وهو المصدرُ الواحدُ لمساري الالتقاط.

### قرارٌ مكتوب: هذه رقعةٌ على قائمةٍ مغلقة

إضافةُ خمسةِ مفاهيمَ تُصلح **هذا** المجال وتترك المجالَ التالي مكسوراً — وهو عينُ ما يحذّر منه `CLAUDE.md`: «القائمةُ المغلقة تقف مقامَ كلام صاحب المشروع، وهي أمُّ الأعطاب هنا». فالإصلاحُ هنا مقصودٌ محدود: **يُعيد بناءَ صاحب المشروع اليومَ**، ولا يدّعي حلَّ الشكل.

والشكلُ الصحيحُ مقيسٌ ومُعلَنٌ ولم يُبنَ، لأنّه يغيّر مُوجَّهَ الاشتقاق وهو قرارُ صاحب المشروع: **أن يُطلَب من النموذج تسميةُ الكيانات بلغة الطلب** — فتصير `حملة/متبرع/تبرع` بدل `Campaign/Donor/Donation`، فيعبر جسرُ اللفظ بلا معجمٍ أصلاً. وقِيس أنّ هذا يميّز الحالتين معاً: فهمُ الجمعيّة يُسنَد، وفهمُ المدرسة المهلوَس على متتبّعِ حفظٍ يبقى مرفوضاً — لأنّ «طالب» و«درجة» لا أثرَ لهما في طلبٍ عن الحفظ.

### شفرةٌ ميّتة حُذفت في الطريق

أُضيف `beneficiary` تخميناً، ثمّ نجت طفرتُه — لا اختبارَ يقرؤه ولا أثرَ له في الطلب المقيس. **فحُذف**: مدخلٌ في قائمةٍ مغلقة لا يقتل طفرتَه ليس تغطيةً زائدة، بل سطرٌ ميّت.

وطفرتان أخريان نجتا أوّلاً (`متبرع`، `متطوع`) — ثمّ تبيّن أنّ إحداهما **غيرُ فعّالة** (التطبيعُ يُسقط الشدّة فتبقى «متبرّع» تطابق)، وأنّ الثانية ناجيةٌ حقّاً لأنّ الطُّعمَ قاس الكياناتِ ولم يقس الدور. فأُعيدت الطفراتُ صحيحةً، وكُتب للدور اختبارُه.

---

## ‏#١٩٧ — حرفٌ لا يُنطق قلب المنتجَ كلَّه

قال صاحبُ المشروع بعد تجربةٍ ثالثة: «**المطلوب شيء والتنفيذ شيء آخر تماماً… ياخذ من كل بستان زهرة**».

والوصفُ حرفيّ. سجلٌّ حيّ (٢٠٢٦-٠٩-٠٩، ١١:٥٢): طُلبت **منصّةُ إدارةِ جمعيّةٍ خيريّة**، فبُنيت صفحةُ تسويق:

```
🧭 أبغى منصّة لجمعية خيرية. المطلوب: — موقع تعريفي
🧱 رُكّبت 10 أقسام: nav · hero · logos · features · stats
                  · pricing · testimonials · faq · cta · footer
⚖️ الحكم: PASS — guard-and-write ✓، requirements-verify –، behavior-verify ✓
```

`pricing` و`testimonials` و`logos` لمنصّةِ جمعيّة. ولا حملات، ولا متبرّعين، ولا متطوّعين، ولا لوحةَ إحصاءات، ولا تقريراً شهريّاً.

### العلّة: الشدّة

صاحبُ المشروع يكتب العربيّةَ **مشكولة**: «أبغى **منصّة**». والمعجمُ يخزّنها غُفلاً: «منصة». والشدّةُ (`ّ`) تقع **داخلَ** الكلمة فتكسر المطابقةَ الحرفيّة في `keywordMatches`.

```
❌ brochure   «أبغى منصّة لجمعية خيرية»   ← أصاب من ALL APP_HINTS: لا شيء
✅ webapp     «أبغى منصة لجمعية خيرية»    ← أصاب: منصة
```

**ومفارقةٌ تشرح لماذا لم يُكتشَف قبلاً**: التنوينُ كان يعمل («نظامٌ» ✅). لأنّه يقع **بعد** الكلمة وهو `\p{Mn}` فلا يكسر النظرةَ اللاحقة `(?![\p{L}\p{N}])`؛ والشدّةُ **بداخلها** فتكسر الحرفيّة. فالعطبُ لم يكن يظهر إلّا لمن يشكّل كتابتَه — أي لصاحب المنصّة نفسِه.

### السلسلةُ من حرفٍ واحد

| | |
|---|---|
| «منصّة» لا تُطابَق | ⇒ صفرُ ألفاظِ تطبيقٍ في الطلب |
| صفرُ ألفاظٍ | ⇒ `staticKind` = **brochure** |
| brochure | ⇒ `functionalComponents: []` في المخطّط الاحتياطيّ |
| صفرُ مكوّنات | ⇒ `composeRequirements` فارغة ⇒ بوّابةُ المتطلّبات `skipped` |
| بوّابةٌ متخطّاة | ⇒ `deliveryVerdict` تعاملها كمحايد ⇒ **PASS** |

فـ«زهرةٌ من كلِّ بستان» ليست استعارة: بما أنّه بروشور، رُكّبت عشرةُ بلوكاتٍ عامّةٍ من المكتبة.

### الإصلاح — مصدرٌ واحد

`stripDiacritics` في `textNormalizer.js` (المدى `ً-ٟ`: التنوينُ والحركاتُ والشدّةُ والسكون)، يقرؤه `normalizeArabic` و`keywordMatches` معاً. والإسقاطُ **من الطرفَين**: الطلبِ والمفتاح.

وكان في `normalizeArabic` سطرٌ ثانٍ لـ`[ّْ]` وهما داخلَ المدى أصلاً — فأُدمج.

**ونسخةٌ ثالثةٌ من القاعدة لم تُمسّ**: `reactGenerator.js` له نسخته وفيها سببٌ مكتوبٌ صريح («يُستهلَك في سياقاتٍ لا تُحمّل تلك الوحدة، والاعتمادُ هنا حرفان لا منطق») — قرارٌ مقصودٌ لا سهو، فلا يُنقض بلا قياسٍ ينقضه.

### طفرةٌ نجت، وقرارُها

إسقاطُ التشكيل من **المفتاح** وحدَه لم يقتله اختبار — كلُّ مفاتيح المعاجم اليومَ غيرُ مشكولة. ولم يُحذَف كما حُذف `beneficiary` في ‏#١٩٥، لفرقٍ مكتوب: ذاك **مدخلٌ في قائمة** لا دليلَ عليه، وهذا **تماثلٌ في مُطابِق** — ومُطابِقٌ يُطبّع طرفاً ولا يُطبّع الآخر خطأٌ كامنٌ يظهر أوّلَ ما يكتب أحدُهم مفتاحاً مشكولاً في معجمٍ يدويٍّ عربيّ. فثُبِّت الضمانُ باختبارٍ بدل أن يبقى رجاءً، وقُتلت الطفرة.

### وحارسٌ أوقعني

إضافةُ `import { keywordMatches }` إلى ملفِّ الاختبار أسقطت حارسَ قناة التقرير — وهو الفخُّ المسجَّل في `CLAUDE.md` حرفيّاً. فأُضيف `divertConsoleToStderr()`. الحارسُ يعمل.

### وما بقي مقيساً ولم يُصلَح بعد

الشقُّ الثاني من السلسلة — **`skipped` يمرّ كمحايدٍ في `deliveryVerdict`** — مسجَّلٌ ‏#١٩٦ ولم يُصلَح في هذه الدفعة. ومقيسٌ بتشغيل: حتّى `guard-and-write` وحدَها (وهي لا تقول إلّا «كُتبت ملفّاتٌ سليمةُ الصياغة») تكفي لإعلان PASS مع تخطّي البوّابتَين. و`buildFromRegistry` لا يلمس `context.plan` إطلاقاً، فمسارُ Registry بنيويّاً لا تُتحقَّق متطلّباتُه أبداً.

---

## ‏#١٩٦ — البانِي الذي يفبرك فهمَه: كيف صار PASS ممكناً على منتجٍ لا يمتّ للطلب بصلة

شكوى صاحب المشروع كانت: «حتّى الآن المطلوبُ شيءٌ والتنفيذُ شيءٌ آخر تماماً… يأخذ من كلّ بستانٍ زهرة».
وقد كانت وصفاً حرفيّاً: طلبُ «منصّة لجمعية خيرية» بأربعة أدوارٍ مسمّاة خرج **صفحةَ تسويق** من عشرة
بلوكات (`pricing` · `testimonials` · `logos`) — وحكمُ جولا عليها **PASS**.

قِيس السببُ الأوّل وأُغلق في ‏#١٩٧ (الشدّةُ على «منصّة» صنّفت الطلبَ «موقعاً تعريفيّاً»). وهذا ‏#١٩٦
هو السببُ الثاني، وهو الأخطر: **حتّى لو أخطأ التصنيفُ، كان يجب أن يقول الحكمُ FAILED.** لم يقل.

### القياس (بتشغيلٍ حقيقيّ، على الملفّات التي بناها المسارُ نفسُه)

```
▸ اليومَ  (نموذجٌ مُصطنَع)   PASS
  requirements-verify: skipped — «1 متطلّب بلا مفردةٍ تُتتبَّع»
▸ بنموذجه المفهوم        FAILED
  requirements-verify: fail — «6 متطلّب بلا أثر: شاشة مدير الجمعية، شاشة مسؤول الحملات،
  شاشة محاسب، شاشة متطوع، بيانات حملة، بيانات تبرع (0/6 له أثر)»
```

الملفّاتُ واحدة. الفرقُ كلُّه: **ماذا وصل البوّابةَ.**

### السلسلة

`buildFromRegistry` كان يكتب نموذجاً ثابتاً في شفرته:
`{ entities: [], roles: [{ name: 'Visitor' }] }` — ثمّ **يكتبه فوق** ما فهمتْه بوّابةُ الفهم قبل
سطورٍ (`_understandGoal` تسبق `_selectBuildStrategy` في `_runMissionNow`). فينتج:

١. الفهمُ يُمحى من ذاكرة المشروع.
٢. `composeRequirements` على ذلك النموذج تُخرج متطلّباً واحداً: «شاشة Visitor».
٣. اسمُه **لاتينيٌّ** فلا يُتتبَّع في صفحةٍ عربيّة → البوّابة `skipped`.
٤. و`skipped` **محايدٌ** في `deliveryVerdict` → **PASS**.

الخطوةُ الرابعة كانت هي ما سجّلتُه أوّلاً بوصفه العلّة، وهي ليست العلّة: `skipped` محايدٌ **بحقّ**
(بناءٌ بلا متطلّباتٍ أصلاً لا يُحكَم عليه بالفشل). العلّةُ أنّ البوّابةَ لم تُعطَ ما تحكم عليه.

وهي قاعدةُ «مصدرٌ واحدٌ لكلّ قاعدة» تُخالَف للمرّة التي تكرّرت في هذه الشجرة: البانيان الآخران
يقرآن النموذجَ من الذاكرة (`buildFromClone` يدمجه، `buildReact` يقرؤه) — وهذا وحدَه كان يستبدله.

### ما تغيّر

- `stages/buildFromRegistry.js`: `getDomainModel` يُقرأ؛ فإن كان فيه أدوارٌ أو كيانات فهو النموذج،
  وإلّا فنموذجُ الزائر كما كان. والنموذجُ نفسُه يصل **بوّابتَي** المتطلّبات والسلوك.
  ٩١ ← ١١٦ سطراً (الشرحُ المقيس فوق الإصلاح).

### نتيجتان سلبيّتان — قِيستا فلم يُبنَ عليهما

- **شبكةُ أمانٍ في `runRequirementsVerify`** تميّز «لا متطلّباتٍ أصلاً» (تخطٍّ صادق) من «متطلّباتٌ
  بلا ملفّاتٍ أفحصها» (`unverified`): بُحث عن مستهلكٍ حيٍّ فلم يوجد. `runDebate` يرفض خطّةً بلا
  ملفّات (سطر ٩٠)، و`guardFiles` لا تُسقط ملفّاً قطّ (تدفع الأصلَ في كلّ فرع) — فالحالةُ غيرُ
  بالغةٍ بنيةً. لا تجريدَ بلا مستهلك.
- **تمريرُ بنود وثيقة صاحب المشروع** على هذا المسار كما يفعل البانيان (PM/9، PM/12): كُتب، ثمّ
  أسقطه حارسُ `specVerdict.test` («البروشورُ ليس وثيقة»). وبالقياس كان الحارسُ مصيباً: نداءُ
  `buildFromRegistry` الوحيد داخل `if (!documentOrSystem && …)`، و`documentOrSystem` يبدأ بـ
  `isFullSpecification(goal)` — فالوثيقةُ **لا تبلغ هذا المسار بنيةً**. حُذف السطرُ وبقي الحارس.
  والحارسُ لم يُهدَّأ: هو الذي منع الرقعة.

### الاختبارات (`tests/registryStage.test.mjs`، ٤ ← ٧)

الحكمُ FAILED على طلبِ المنصّة والبوّابةُ تسمّي الستّةَ بأسمائها · البوّابةُ السلوكيّةُ تسمّي الأدوارَ بلا
واجهة · الفهمُ يبقى في الذاكرة بعد البناء · كياناتٌ بلا أدوارٍ فهمٌ يُحاكَم أيضاً · **الحدُّ المقابل**:
بلا فهمٍ سابقٍ يبقى نموذجُ الزائر ويُكتب، والصفحةُ التسويقيّةُ تمرّ PASS كما كانت.

**الطفراتُ ٥/٥ أُمسكت** — واثنتان منها نجَتا في الجولة الأولى وأُغلقتا كلٌّ بسببها: واحدةٌ **شفرةٌ
ميّتة** (حارسُ `if (!hasUnderstanding)` حول `setDomainModel`: حين يوجد فهمٌ يكون النموذجُ هو ذاتَه،
فالكتابةُ إعادةُ ما هو مكتوب — حُذف)، وواحدةٌ **اختبارٌ ناقص** (فهمٌ بكياناتٍ بلا أدوار).

### إعادةُ تثبيتٍ بسببٍ مكتوب

- `renderConfigShape.test`: موضعُ `prepareRenderDeploy` ‏٦٥ ← ٩٠. النداءُ لم يتغيّر؛ عشرون سطرَ
  شرحٍ مقيسٍ أُضيفت فوقه.
- `ARCHITECTURE_MAP.md`: `stages/buildFromRegistry.js` ‏٩١ ← ١١٦.

### وما بقي مقيساً ولم يُصلَح

بنودُ وثيقةِ صاحب المشروع لا تدخل بوّابةَ المتطلّبات إلّا إذا بلغت **١٢٠٠ حرفاً و٦ بنود**
(`SPEC_MIN_CHARS`/`SPEC_MIN_SECTIONS`). فطلبُه هو — خمسةُ بنودٍ مرقّمة في ٢٥٠ حرفاً — ليس «وثيقة»
بهذا المعيار. قِيس: `isFullSpecification` تعود `false` عليه. لم تُخفَّض العتبةُ لأنّ مسارَ النموذج
(الأدوارُ والكيانات) غطّى حالتَه كاملةً في هذه الجولة؛ فإن ظهر طلبٌ قصيرٌ مرقّمٌ لا يغطّيه النموذج
فهذه هي العتبةُ التي تُراجَع، **بقياسٍ لا بتخمين**.

---

## ‏#١٩٨ — القائمةُ المغلقة تحكم على كلام صاحب المشروع: نصفُ ما يُفهَم يسقط صامتاً

القاعدةُ السابعة في `CLAUDE.md`: «حين تجد قائمةَ أسماءٍ تحكم على نصٍّ كتبه إنسان، فتِّش عمّا
تُسقطه». فُتِّش، فوُجد أنّها تُسقط أكثرَ من النصف.

### القياس قبل الكود — ثمانيةُ مجالاتٍ عربيّة، ٤٨ اسماً يشتقّها الفهمُ من طلبٍ حقيقيّ

```
3/6  صيدليّة          يُسقط: وصفة، مخزون، تأمين
5/6  مدرسة            يُسقط: غياب
2/6  ورشة سيارات      يُسقط: قطعة غيار، أمر تشغيل، كشف، ضمان
1/6  مزرعة            يُسقط: محصول، حقل، ريّ، مبيد، حصاد
3/6  نادٍ رياضيّ       يُسقط: اشتراك، جهاز، قياس
3/6  مسجد/وقف         يُسقط: حلقة، وقف، إمام
2/6  شحن ولوجستيّات   يُسقط: مسار، مستودع، بوليصة، جمارك
3/6  محاماة           يُسقط: موكل، أتعاب، مذكرة
─────────────────────────────────────────────────────
22/48 (٤٦٪) — و٢٦ متطلّباً يخرج من البسط والمقام معاً
```

و«يخرج من البسط والمقام» ليست مجازاً: `traceRequirements` تضعه في `untraceable`، فلا يُحسب له
ولا عليه، ولا يُذكر لصاحب المشروع بحرف. مثالٌ مُشغَّل على بناءِ صيدليّةٍ يبني الوصفاتِ والمخزونَ
فعلاً في `script.js`:

```
قبل: له أثر: محاسب، دواء · بلا أثر: صيدلي، مريض · لا يُتتبَّع: وصفة، مخزون ← المقام 4/6
بعد: له أثر: محاسب، وصفة، دواء، مخزون · بلا أثر: صيدلي، مريض ← المقام 6/6
```

**الشيءُ المبنيُّ فعلاً لم يكن يُحسب لصاحبه.** والاتّجاهُ الآخر أسوأ: ما لم يُبنَ لم يكن يُحسب عليه.

### لماذا لم يُوسَّع المعجم

توسيعُه علاجُ العَرَض: كلُّ مجالٍ جديدٍ يعيد العطبَ. والشجرةُ تملك المقياسَ المفتوحَ أصلاً —
`traceSections` تحكم على بنود وثيقة صاحب المشروع **بمفرداته هو** (PM/9). فالعلاجُ إيصالُ
المتطلّبات إلى الآلة نفسِها، لا إطعامُ القائمة.

### ما تغيّر

- `composeRequirements`: كلُّ متطلّبٍ من الفهم يحمل `_term` — **كلمةَ صاحب المشروع** مجرّدةً من
  إطارِنا («شاشة»/«بيانات»).
- `traceRequirements`: المعجمُ أوّلاً **حيث ينطق** (هو وحدَه يعرف المرادف: «زبون» في النموذج
  و«عميل» في الصفحة)، ثمّ `matchTerm` حين يصمت.
- `matchTerm` جديدة: تُطبّع المصطلحَ كما يُطبَّع النصّ (نزعُ «ال» وتاءِ التأنيث) ثمّ تطابق بـ
  `keywordMatches` — **الموضعُ الوحيد** في الشجرة لقاعدة سوابق العربيّة اللاصقة (#١٩٧).
- `matchTokens` استُخرجت من `traceSections`: قاعدةُ «متى يكون للفظٍ أثر» في موضعٍ واحدٍ لمستهلكِها.
- `stages/verify.js`: البوّابةُ تقول «N لم أتتبّعه بمفرداته» بعدده وبأسمائه، وترجع `unverified`.

### الفرقُ بين «بلا أثر» و«لم أتتبّعه» — وهو قلبُ هذه الجولة

المطابقةُ بالصرف تُصيب **٦ من ٧** حالاتٍ مقيسة (أل التعريف، تاءُ التأنيث، جمعُ السلامة،
الهمزة). والسابعةُ **جمعُ التكسير**: «كتاب» ← «الكتب». لا قانونَ لواصقَ يبلغه، ولم يُبنَ له
جذّاعٌ هنا.

فماذا يُقال عن مصطلحٍ لم يُطابَق؟ **لا يُقال «بلا أثر»**: النفيُ دعوى، والدعوى تحتاج يقيناً لا
نملكه. ولا يُسكَت عنه: ذلك هو العطبُ الذي نُصلحه. فيُقال بعدده واسمِه، والحكمُ `unverified` —
**تمنع PASS ولا تتّهم بناءً صحيحاً**.

### دعوى `pass` كانت صمتاً — مثالٌ من اختباراتنا نحن

`builtEvidence.test` كان يؤكّد على بناءِ مكتبةٍ: «كلُّ متطلّبات النموذج لها أثر» ‏`pass`. وقِيس نصُّ
البناء نفسِه: فيه «كتب»، وليس فيه «كتاب» ولا «مكتبة» ولا «أمين» بحال. فالدعوى لم تكن صدقاً بل
صمتاً — متطلّبان خارجَ المعجم كانا يُسقَطان، ‏٥/٥ بدل ٥/٧. الحكمُ الآن `unverified` بعدده،
وأحدُ الاثنين **فجوةٌ حقيقيّةٌ كانت مخفيّة** («أمين المكتبة» لا أثرَ له إطلاقاً).

### حدٌّ صِين: الانفتاحُ لكلمته لا لكلماتنا

أوّلُ صياغةٍ فتحت المقياسَ على **العامّ** أيضاً، فانقلب حكمُ مسار Registry من PASS إلى FAILED:
`Visitor` دورٌ **نفبركه نحن** حين لا فهم، فحُكم على صفحةٍ عربيّةٍ بمتطلّبٍ لم يطلبه أحد. أمسكه
اختبارُ ‏#١٩٦ (الحدُّ المقابل). فبقي العامُّ (`user`/`item`) خارجَ الحكم كما قرّرت PM/7،
والانفتاحُ لكلمة صاحب المشروع وحدَها.

ولم تُوحَّد المطابقتان: تحويلُ بنود الوثيقة إلى الصرف يُرخي حارساً مقيساً (PM/9)، وذاك قياسٌ
مستقلٌّ لم يُجرَ.

### التحقّق

الحزمة **٢٠٧١/٢٠٧١** (٢٠٦٧ + ٤). **الطفرات ٦/٦ أُمسكت**: عودةُ الصمت · ابتلاعُ «لم أتتبّعه» ·
تركُ المصطلح بلا تطبيع · مطابقةٌ حرفيّةٌ بلا لواصق · إدخالُ العامّ · و«لم أتتبّعه» لا يمنع PASS.

### إعادةُ تثبيتٍ بسببٍ مكتوب

- `builtEvidence.test`: ‏`pass` ← `unverified` — والسببُ أعلاه: الدعوى القديمة كانت صمتاً.
- `requirementsTrace.test`: قائمةٌ خامسة `unmatched` في الشكل؛ وحدُّ المطابقتَين يُثبَّت بعددَيهما.
- `ARCHITECTURE_MAP.md`: `requirementsVerifier.js` ‏٢٥٦ ← ٣٢٢، `stages/verify.js` ‏١٨٧ ← ١٩٢.

### وما بقي مقيساً ولم يُصلَح

**جمعُ التكسير.** يحتاج جذّاعاً عربيّاً (أو نموذجاً)، وكلاهما قرارُ بنيةٍ لا رقعة. أثرُه اليوم
محصورٌ في `unverified` بدل `pass` — أي تشدّدٌ مرئيٌّ لا اتّهامٌ خفيّ.

---

## ‏#١٩٩ — جمعُ التكسير: قِيس اشتقاقان فسقطا، وبُني معجمٌ لا جذّاع

البندُ المفتوح من ‏#١٩٨: «كتاب» لا تُطابق «الكتب» — لا قانونَ لواصقَ يبلغ جمعَ التكسير، وأثرُه
محصورٌ في `unverified` بدل `pass`. القرارُ هنا: **قِس قبل أن تبني** (`CLAUDE.md` §1) — فقبل أيّ
جذّاعٍ، قِيست طريقتان خوارزميّتان أولاً، وسقطتا كلتاهما.

### الحلّان الخوارزميّان اللذان سقطا

- **اختزالُ هيكلٍ صامت** (تجريدُ الحركات الطويلة من الجذر): ٣٠/٤٠ إصابة، لكنّ **٢/٥ تطابقٍ زائف**
  — «كتاب» و«كاتب» يتساويان بعد الاختزال رغم اختلاف المعنى تماماً. هذا عطبٌ **بنيويّ** في صرف
  العربيّة (جذرٌ ثلاثيٌّ واحد، أنماطٌ متعدّدة) لا يُصلحه تطبيقٌ أذكى.
- **توليدُ مرشّحٍ بنمط** (فاعل → فِعال): ينطبق على ٨/٤٠ فقط (تغطيةٌ ضيّقة)، ويُصيب ٣/٨ منها فقط
  (٣٧٪ دقّة)، وفيه خطرُ التصادمِ نفسُه على جهة التوليد («كاتب» يولّد «كتاب» — كلمةٌ حقيقيّةٌ لكن
  **مختلفة**).

كلاهما عطبٌ في **الاتّجاهِ المشدِّد**: يمنح `traced`/PASS زائفاً — وهذا الاتّجاهُ غيرُ محتمَلٍ في
بوّابةٍ يُصدَّق حكمُها (‏#١٩٦: «سوقٌ إلكترونيٌّ حُكم عليه ٩/٩» بدليلٍ مصنوع). فالنتيجتان **سُجِّلتا
سقوطاً لا بُنيتا** — نتيجةٌ سلبيّةٌ مقيسة، لا تخمين.

### الحلّ: بياناتٌ لا اشتقاق

معجمُ (مفرد ← جمعُ تكسير) من **Arramooz Alwaseet** (GPLv2، Taha Zerrouki) — كلُّ زوجٍ موثَّقٌ في
مصدره، لا مخموناً. القرارُ بدمج بياناتٍ GPL في مستودعٍ تجاريٍّ **قرارُ مالك المنصّة** (٢٠٢٦-٠٩-٠٩،
مثل قرار #183 في تشغيل شفرةٍ غريبة — تراخيصُ الطرف الثالث خارج نطاق حكم الآلة). التفصيلُ والتنقية
والقياسُ في `agents/data/ARRAMOOZ_NOTICE.md`.

- **الاستخراج**: حقلا `unvocalized`/`broken_plural` من `nouns.dict.txt` (تسعةُ أصنافٍ صرفيّة).
- **التنقية**: نزعُ التشكيل؛ استبعادُ صيغٍ تحمل ضجيجَ تسريبٍ من حقولٍ مجاورة («مفرد»/«جمع»/…
  داخل النصّ — خطأُ فصلٍ في المصدر)؛ استبعادُ صيغٍ أقصرَ من ٣ أحرف أو أطولَ من ١٠.
- **القياس**: ٣٥/٤٠ (٨٧٪) تغطية على الأزواج الأربعين نفسِها التي قِيست بها الاشتقاقاتُ الساقطة —
  **بلا مخاطرِ تطابقٍ زائف**، لأنّ كلَّ زوجٍ بياناتٌ موثَّقة لا اشتقاق.
- **العدد**: ٤٥٤١ صفّاً في JSON، ٤٥٣٣ مفتاحاً بعد `normalizeLetters` (بعضُ الصفوف يتّحد مفتاحُه).

### ما تغيّر

- `agents/arabicBrokenPlurals.js` (جديد، ٤٢ سطراً): يحمّل المعجمَ عند الاستيراد، ويصدّر
  `brokenPluralsOf(term)` — مفاتيحُ الفهرس بالتطبيع نفسِه الذي تُطبَّع به الكلمةُ المستعلَمة
  (`normalizeLetters`، PM/18: مصدرٌ واحد).
- `requirementsVerifier.js#matchTerm`: كلُّ مفردةٍ تُطابَق بنفسها **وبصيغِ جمعِ تكسيرها الموثَّقة**
  معاً — `withPlurals = toks.flatMap(t => [t, ...brokenPluralsOf(t)])` — لا اختزالَ لأيّ طرف.

### الفرقُ في اختبارَي ‏#١٩٨: من «لم أتتبّعه» إلى «أثرٌ» أو يبقى فجوةً حقيقيّة

`builtEvidence.test` (بناءُ مكتبةٍ حقيقيّ): كانت الدعوى `unverified` بمتطلَّبَين خارجَ المعجم —
«كتاب/كتب» (جمعُ تكسير) و«أمين المكتبة» (فجوةٌ حقيقيّة، لا أثرَ له إطلاقاً في البناء). الآن
«كتاب» له أثر (`traced`، `decorative` لأنّه في النثر لا الشفرة)، و«أمين المكتبة» يبقى «لم
أتتبّعه» بصدق — **لا يُقال عنه «بلا أثر»**: خارجَ أيّ معجم، والنفيُ يحتاج يقيناً لا نملكه.
الحكمُ العامّ يبقى `unverified` (٦/٧ بدل ٥/٧) لا `pass` — فجوةٌ واحدة صادقة، لا اثنتان مختلطتان.

### التحقّق

الحزمة **٢٠٧٦/٢٠٧٦**. مطافراتٌ يدويّةٌ على `matchTerm`/`brokenPluralsOf/normalizeLetters`(الاستعلام
والفهرسة معاً) — كلُّها أُمسكت (تعطيلُ إضافة الجموع، تخطّي تطبيع الاستعلام، تخطّي تطبيع الفهرسة).

### إعادةُ تثبيتٍ بسببٍ مكتوب

- `requirementsTrace.test`: اختبارُ الحدّ القديم («كتاب» لا تطابق) صار اختبارَ حلٍّ («كتاب» تطابق) +
  اختبارٌ جديد لِما يبقى خارج المعجم.
- `builtEvidence.test`: ‏٥/٧ ← ٦/٧، «٢ لم أتتبّعه» ← «١ لم أتتبّعه: شاشة أمين المكتبة».
- `ARCHITECTURE_MAP.md`: `requirementsVerifier.js` ‏٣٢٢ ← ٣٢٩؛ صفٌّ جديد لـ`arabicBrokenPlurals.js`
  (٤٢)؛ عنوانُ القسم C) ‏١٣٨ ← ١٣٩ وحدة.

### وما بقي مقيساً ولم يُصلَح

المعجمُ ٨٧٪ لا ١٠٠٪ — ٥/٤٠ من عيّنة القياس خارجَه. وهذا **مقصود لا نقص يُخفى**: التوسيعُ خارج
بياناتِ Arramooz يعيد خطرَ الاشتقاق الذي سقط أعلاه. ما بقي خارج المعجم يُقال بصدق («لم أتتبّعه»)
لا يُدَّعى غيابُه.

---

## إعادةُ قياس Sprint 2e/4h — بوّابةُ الكتابة تبقى بلا مستهلكٍ حقيقيّ (لا PR)

**المدخل.** مراجعةٌ خارجيّةٌ لهذه الجلسة اقترحت «بناء Tool Runtime حقيقيّ» —
عقدٌ جديدٌ لصلاحيّات/مهلة/تدقيق يمرّ عبره كتّابُ الملفّات، مستشهدةً برقم «٤٧
وحدة وكيل تكتب مباشرة». قبل البناء، قِس: قِيست الفرضيّةُ نفسُها **مرّتين
سابقاً** في هذا المستودع (Sprint 2e أعلاه، ثمّ `ARCHITECTURE_GAP_AUDIT.md`
§٨)، وكلتاهما خلصتا صراحةً إلى عدم البناء — «لا مستهلكَ حقيقيّاً بعد».

**أعدتُ القياس اليوم بدل تصديق الرقم الخارجيّ:**
- «٤٧» لا يظهر في أيّ ملفّ بالمستودع؛ الأقربُ إليه المقيسُ الفعليّ في
  `ARCHITECTURE_GAP_AUDIT.md`: **١٩٨ موضعَ كتابةٍ في ٤٦ ملفّاً** — عدٌّ خشنٌ
  لكامل الخلفيّة لا لـ«وكلاءٍ» فقط، والأرجح أنّه مصدرُ الرقم الخارجيّ.
- الجدولُ في Sprint 2e يفصّل: `jcr.js`/`backendTeam` تأخذان اسمَ ملفٍّ من
  الـLLM **ومحروستان فعلاً** (`isInsideRoot`/`safeRelPath`)؛ `designerAgent`/
  `renderAgent`/`gitAgent`/`seoPack`/`template.agent`/`pwaAgent` تكتب بأسماءَ
  **ثابتة** (`design-brief.json`، `render.yaml`، `.gitignore`، …) فلا خطرَ
  اجتيازِ مسارٍ فيها بصرف النظر عن مصدر `projectPath`؛ و`fileManager.writeFile`
  وأخواتها **بلا مستهلكٍ أصلاً** — `jcr.js` يستورد النسخَ الاحتياطيّ فقط منها.
- تحقّقتُ اليوم من ثلاثةٍ من هذه الملفّات حرفيّاً (`backendTeam/backendTeam.js`،
  `renderAgent.js:203-256`، `gitAgent.js:61`): **لا انحرافَ عن Sprint 2e**.
  `backendTeam` لا تزال تمرّ بـ`safeRelPath`+`resolveInside`
  (`workspacePaths.js`)؛ `renderAgent`/`gitAgent` لا تزالان تكتبان أسماءً
  ثابتةً مبنيّةً داخل الدالّة نفسِها، لا مشتقّةً من مُدخَلٍ خارجيّ.

**النتيجة.** الفرضيّةُ («ثمّة كاتبٌ حيٌّ غيرُ محروسٍ يستحقّ بوّابةً جديدة»)
**سقطت بالقياس للمرّة الثالثة**. لا PR لهذه النقطة — بناءُ Tool Runtime أو حتى
توسيعُ تبنّي `writeProjectFile` القائمة على هدفٍ مثل `designerAgent` كان
سيكون تجريداً بلا حاجةٍ فعليّة، وهو ما يحذّر منه Sprint 2e بالاسم. يُبنى حين
يظهر كاتبٌ ثالثٌ يأخذ اسمَ ملفٍّ من مصدرٍ غيرِ موثوقٍ **بلا** حراسة — وهذا لم
يحدث بعد.

**وما بقي منها فرصةَ تنظيفٍ صغيرة، لا بوّابة**: `fileManager.js#writeFile/
readFile/deleteFile/renameFile` شفرةٌ ميّتةٌ مؤكَّدةٌ (بحثٌ شاملٌ اليوم عن
استيرادٍ ساكن أو ديناميكيّ: صفر) — حذفُها حين يُرغَب تنظيفُ الملفّ، لا عاجلاً.

---

## النقطة ١ — StageResult: مراحلُ الجودة تعيد ناتجاً موحَّداً وتسجّل حكمَها

**المدخل.** المراجعةُ الخارجيّة نفسُها لاحظت ثلاثةَ أنماطٍ متوازية لاستدعاء
الوكلاء (AgentBundle، AgentSpec التصريحيّ، والاستيرادُ المباشر) واقترحت عقداً
موحَّداً بالشكل `{ok, data, error}`. القياسُ (لا الاستنتاج) أكّد الأنماطَ
الثلاثة **فعلاً حيّة** لكن كشف أنّ `{ok,data,error}` الجامد يناقض فلسفةً
موثَّقةً صراحةً في `core/contracts/index.js:62-63`: «فشلُ مرحلةِ الجودة
`⚠️ تخطّي` لا إسقاطٌ للبناء» — عقدٌ يُغري بـ`if(!ok) throw` سينقضها.

**ما بُني (PR أصغر آمن — لم تُمسّ أجسادُ الوكلاء).**
- `core/contracts/index.js`: `StageResult` typedef جديد بجانب `StageFn`
  (`{attempted, issues, error?, summary}`) — تعميمُ نمط `AgentSpec`
  (`issues`/`selfReviewPassed`) لا نمطٌ رابع.
- `agents/stages/quality.js`: الدوالُّ الستّ (`runReviewStage` وأخواتها) تعيد
  الآن `StageResult` عند كلّ مخرج (نجاحٌ، فشلٌ غيرُ رامٍ، استثناءٌ مُمسَك) —
  بلا أيّ تغييرٍ فيما يُكتب للقرص أو `plan.files`.

**تصحيحٌ في أثناء التنفيذ (حارسُ حدودٍ أمسك انحرافاً معماريّاً).** الخطّةُ
الأصليّة مرّرت ناتجَ `this[stage.run](...)` من حلقة `DELIVERY_STAGES` في
`jcr.js` إلى `recordGateOutcome` مركزيّاً. أوّلُ تشغيلٍ للحزمة الكاملة كسر
`deliveryVerdict.test.mjs` (اختبارٌ ثابتٌ يقول: «الحارسُ وحده في jcr —
البوّابتان الأخريان [`requirements-verify`, `behavior-verify`] تُسجَّلان من
*داخل* وحدة كلّ مرحلة») — الحلقةُ المركزيّة رفعت عددَ نداءات `recordGateOutcome(`
في `jcr.js` من ١ إلى ٢، ناقضةً «مصدرٌ واحدٌ لكلّ قاعدة» (CLAUDE.md §٦):
النمطُ الموثَّق مسبقاً هو أن تستورد كلُّ وحدة مرحلةٍ `recordGateOutcome`
وتستدعيَها بنفسها، لا أن تفعل ذلك حلقةٌ عامّة. صُحِّح بإرجاع حلقة `jcr.js`
حرفيّاً لحالتها الأصليّة (١٠٢٢ سطراً كما كانت — **صفرَ تغييرٍ** في هذا
الملفّ)، ونقلِ الاستدعاء إلى `quality.js` نفسِها عبر دالّةٍ محليّة صغيرة
`finishStage(context, name, result)` تُستدعى من كلّ المخارج الاثني عشر
(اثنا عشر: ستُّ دوالّ × مخرجَين لكلٍّ منها).

**التحقّق.**
- `tests/qualityStages.test.mjs`: امتدادٌ لثمانية اختباراتٍ موجودة بتأكيداتِ
  `StageResult` (شكلٌ ومحتوًى) + `context.verdicts` (يثبت أنّ `recordGateOutcome`
  استُدعيت فعلاً، بحالتها الصحيحة: `pass`/`fail`/`unverified` حسب `issues`/`error`).
- اختبارُ طفرةٍ يدويّ: عطّلتُ استدعاءَ `recordGateOutcome` داخل `finishStage`
  (علّقتُه بتعليقٍ) وشغّلتُ `qualityStages.test.mjs` — **٧ من ٩ اختباراتٍ سقطت**
  فوراً (كانت ٩/٩ ناجحة). الحارسُ مغطًّى، لا زخرفة.
- الحزمة الكاملة **٢٠٧٦/٢٠٧٦** بعد التصحيح (كانت ٢٠٧٤/٢٠٧٦ بالتصميم الأصليّ —
  `architectureMap.test.mjs` أمسك عدمَ تحديث أرقام الأسطر، و`deliveryVerdict.test.mjs`
  أمسك الانحرافَ المعماريّ أعلاه؛ كلاهما أُصلح لا أُسكِت).
- `ARCHITECTURE_MAP.md`: `stages/quality.js` ١٤٨ ← ١٨٢ (+٣٤: الاستيراد +
  `finishStage` + التفاف كلّ مخرج)؛ `core/contracts/index.js` ٢٣٦ ← ٢٤٧
  (+١١: typedef)؛ `jcr.js` **بلا تغيير** (١٠٢٢) بعد التصحيح — سطرٌ جديدٌ
  يوثّق لماذا لم يُمسّ.

**وما بقي مفتوحاً.** مراحلُ الجودة الستّ كلُّها `gate: 'enhancement'` في
`DELIVERY_STAGES`، فتسجيلُها **لا يدخل في حساب PASS/FAILED** اليوم
(`deliveryVerdict` يقرأ `gate: 'gate'` فقط) — إشارةٌ للمراقبة لا حارسٌ جديد.

---

## النقطة ٣ — dependsOn في DELIVERY_STAGES: نتيجة سلبيّة مُقاسة — لا PR

**الفرضيّةُ المخطَّطة.** الخطّةُ الأصليّة اقترحت توسيمَ `DELIVERY_STAGES`
بـ`dependsOn` بيانات‌ياً فقط — بلا لمس `jcr.js` — على أساس أنّ الفرزَ
الطوبولوجيّ **يتدهور إلى ترتيب المصفوفة الحاليّ حرفيّاً** لأنّ لا مرحلةَ
اليوم تملأ `dependsOn`، فوصفت العمليّة بأنّها **«آمنةٌ بلا تغيير تنفيذ»**،
واقترحت اختباراً يثبّت أنّ `orderTasks(DELIVERY_STAGES, ...)` يطابق ترتيب
المصفوفة حرفيّاً بعد الإضافة.

**القياسُ (لا الافتراض) كشف خطأً في هذا الأساس نفسِه، من ناحيتين:**

1. **`jcr.js` يستدعي `orderTasks()` فعلاً في حلقة التنفيذ الحيّة**
   (`jcr.js:189`: `for (const stage of orderTasks(DELIVERY_STAGES, {...}))`).
   فـ«لا تغييرَ في `jcr.js`» لا تعني «لا تغييرَ في التنفيذ» — إضافةُ
   `dependsOn` وحدَها، بلا لمس سطرٍ واحدٍ في `jcr.js`، **تُغيّر ترتيبَ
   تنفيذ مراحل البناء في الإنتاج فوراً** عند أوّل مهمّةٍ تُبنى بعد الدمج.
2. **شغّلتُ `orderTasks` فعليّاً** (لا افتراضاً) بتصميم `dependsOn` المقترَح
   حرفيّاً من الخطّة (`review`→`refactor`→`testing`→`requirements-verify`
   →`seo`→`security`→`git-backup` سلسلةً، و`backend`/`advanced-modules`/
   `fullstack-scaffold`/`render-config` تعتمد `guard-and-write` فقط):
   الناتجُ **لا يطابق ترتيب المصفوفة إطلاقاً**. السببُ بنيويّ في خوارزميّة
   `TaskGraph.js#orderTasks` (فرزٌ طوبولوجيّ بالمستوى — Kahn مستقرّ): كلُّ
   عناصر الدرجة صفر في مستوًى واحد تُعالَج معاً بترتيب المصفوفة *بينها*،
   لكنّ عنصراً بلا `dependsOn` يقفز إلى المستوى الأوّل **بصرف النظر عن
   موضعه في المصفوفة**. النتيجةُ المقاسة: `backend`/`advanced-modules`/
   `fullstack-scaffold`/`render-config` تقفز لتُنفَّذ مباشرةً بعد
   `guard-and-write` (قبل `review` وأخواتها)، و`executive-memory`/
   `project-memory`/`behavior-verify` (بلا `dependsOn` في التصميم
   المقترَح) تقفز إلى **المستوى الأوّل** مع `guard-and-write` نفسِها.

**لماذا لا يوجد تصميمٌ آمن بديل.** جرّبتُ تضييقَ التوسيم (اعتماديّةٌ واحدة
فقط، مثل `git-backup` على `security`) — النتيجةُ نفسُها: أيُّ حافّة
اعتماديّةٍ حقيقيّة **واحدة** تُقفز العناصرَ المستقلّة (بلا `dependsOn`)
التي تليها في المصفوفة إلى مستوًى أبكر. المطابقةُ الحرفيّةُ لترتيب
المصفوفة ممكنةٌ **حصراً** إذا صارت كلُّ الخمس عشرة مرحلة سلسلةً خطّيّةً
واحدة (كلُّ مرحلةٍ تعتمد على التي تسبقها في المصفوفة تماماً) — وهذا
**كذبٌ معماريّ**: `advanced-modules` لا تعتمد فعلاً على `backend`،
و`behavior-verify` لا تعتمد على `render-config` تحديداً. توسيمُ اعتماديّةٍ
غيرِ حقيقيّة فقط لإرضاء اختبارٍ يناقض القاعدةَ الأولى في هذا المستند
(القياسُ لا الخُمِّن) ويصنع «بوّابةً تقع على احتياطٍ صادقٍ» من نوعٍ آخر:
حارسٌ يبدو صحيحاً لأنّه بُني ليبدو كذلك، لا لأنّه يعكس شيئاً حقيقيّاً.

**النتيجة.** فرضيّةُ «توسيمٌ آمنٌ بلا أثر تنفيذيّ» **سقطت بالقياس**. أيُّ
`dependsOn` حقيقيّ (ولو حافّةً واحدة) هو **تغييرُ سلوكٍ في الإنتاج** يستحقّ
مراجعةً وتصميماً مستقلّين — تحديداً: هل يُراد لهذه المراحل أن تُنفَّذ
بترتيبٍ مختلفٍ عن اليوم فعلاً (بصرف النظر عن التوازي)؟ هذا قرارٌ للمالك لا
قرارٌ تقنيّ صِرف. **لا PR لهذه النقطة** — لا التوسيمُ الجزئيّ (يُغيّر
الإنتاجَ فعلاً بلا مراجعة) ولا التوسيمُ الكاذبُ (سلسلةٌ خطّيّةٌ وهميّة
لإرضاء اختبار) مقبولٌ بمعايير هذا المستودع.

النقاطُ الثلاث إذن: **١ بُنيت** (PR #612)، **٢ سقطت بالقياس** (أعلاه)،
**٣ سقطت بالقياس** أيضاً — بفرضيّةٍ مختلفة عمّا افترضته الخطّةُ الأصليّة،
واكتُشفت بتشغيل الكود لا بقراءته. تنفيذُ توازٍ حقيقيّ في `DELIVERY_STAGES`
يبقى بنداً مفتوحاً يحتاج تصميم ترتيبٍ صريحاً (لا استنتاجاً من القراءة) قبل
أيّ محاولةٍ لاحقة.
