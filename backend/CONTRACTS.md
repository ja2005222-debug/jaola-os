...
## Sprint 3y — ثمانيةُ قيودِ تفرّدٍ تُعلنها النماذج ولا يُنشئها التطبيق
...
```
🔐 [قيود التفرّد]: 6/8 مضمونة · ❌ غائبة: users(username)، projects(name+owner)
...
هل تُنشأ الفهارسُ الغائبة؟ الخياران: نداءُ `createIndexes()` مرّةً عند
الإقلاع (يُنشئ الناقصَ ولا يحذف شيئاً، ويسقط إن وُجدت تكرارات فيُقال)، أو
إنشاؤها يدوياً على القاعدة خارج التطبيق. لا يُفعل شيءٌ حتى يُقال.

| قياس | قبل | بعد |
|---|---|---|
| قيودُ تفرّدٍ يُنشئها التطبيق | ٠ من ٨ | ٠ من ٨ (بلا تغيير) |
| قيودٌ يعرف المالكُ حالَها | ٠ | **٨**، في كلّ إقلاع |
