...
### وسقوطٌ في CI ليس من هذا العمل — شُخِّص، ثم زال قبل أن يُستثنى

سقط `Vercel` على الرأس الأوّل بنصٍّ صريح:

```
Resource is limited - try again in 24 hours (api-deployments-free-per-day)
```

**حصّةُ نشرٍ إدارية** نفدت على مستوى الحساب بعد ستّة PRs في اليوم — لا عطبَ
...
3. أربعَ عشرةَ من خمسَ عشرةَ خضراء.

عُرض الأمرُ على المالك فاختار الدمجَ على 8/9 مع تسجيل السبب. **ثمّ زال
المانعُ قبل أن يُستعمل الإذن**: دَفعُ هذا السجلِّ نفسِه أنشأ رأساً جديداً
(`d8ce28c`)، وعليه نجح `Vercel` — فالحصّةُ تجدّدت والمانعُ كان وقتيّاً.

📌 **والقاعدة تبقى مكتوبةً وإن لم تُستعمل هنا**: بارُ «التسعة خضراء»
يبقى، ويُستثنى منه فحصٌ **يُثبَت بالدليل** أنه لا يقيس التغيير ولا يمكن
إصلاحه برمجياً — ويُكتب الاستثناءُ ومُبرِّرُه، فلا يصير سابقةً صامتة.
وتُكتب هنا أيضاً **أنها لم تُستعمل**، لئلّا يُقرأ السجلُّ يوماً على أن
الدمج وقع ناقصاً وهو لم يقع.
