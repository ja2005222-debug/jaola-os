...
`DeepSource: Secrets` سقط ثلاث مرّات متتالية. وخمّنتُ مرّتين بلا دليل:
مرّةً أن السبب كلمات مرور اختبارية حرفية (والملف **مُعرَّفٌ اختباراً**
أصلاً في `.deepsource.toml`، فلم يكن سبباً قط)، ومرّةً أن السبب اقتباس
القيمة الافتراضية في تعليق. كلاهما خطأ. وهو حرفياً العطب الذي أطارده في
هذه المراحل: **تصرّفٌ بيقينٍ غير مملوك**.

الدليل كان متاحاً طوال الوقت: صفحة التشغيل في DeepSource تُقرأ بلا
اعتماد، وفيها الملاحظة نصّاً:

```
backend/agents/cloneTemplates/jaolaVetClinic.js
Audit: Hardcoded credential " + encodeURIComponent(sync.token) + " found in source code
SCT-A000 · MINOR
```

**سطرٌ لم ألمسه.** جرَّه إلى نافذة الفرق سطران أضفتُهما فوقه بمئتَي سطر:

```js
'…/clinicPhoto?token=' + encodeURIComponent(sync.token) + '&t=' + Date.now()
```

الماسح يقرأ `token=` ثم يأخذ ما بعده **حتى النصّ التالي** قيمةً — فيلزمه
نصٌّ يُغلقه. ولذلك لم يُبلَّغ عن أيٍّ من ٣٠+ سطراً شقيقاً في المجلد نفسه:
كلّها تضع `token` **آخر المعاملات** فلا نصّ بعده. بحثٌ عن الشكل بدقّة
أعاد سطراً واحداً في المستودع كلّه — هو المُبلَّغ عنه بالضبط.

فالإصلاح تقديمُ `t=` وتأخيرُ `token`: الرابط نفسه حرفاً بحرف (ترتيب
معاملات الاستعلام لا معنى له)، والسطر يصير على عُرف جيرانه. ولا `skipcq`
يُشحَن داخل كل موقع عميل مولَّد.
...
حارسي قال «موجود» ولم يقل «واحد» — `assert.ok(/id="stPassCur"/.test(src))`
يصدُق على واحدٍ وعلى اثنين. صار يعدّ: `assert.equal(fields, 1)`، وأُثبت
سقوطُه بإعادة التكرار (٧/٨) ثم استعادته (٨/٨). الحزمة: **880/880**.
