/**
 * 🤖 stockCommentary.js — وكيل ضيّق مهمّته الوحيدة: كتابة 2-3 جمل قصيرة
...
const SYSTEM_PROMPT = {
    ar: 'أنت محلّل بيانات فقط. مهمتك الوحيدة: كتابة جملتين أو ثلاث جمل عربية قصيرة ' +
        'تصف زخم/اتجاه سهم أو زوج عملات بلغة مبسّطة، بناءً على الأرقام المُعطاة فقط (سعر، متوسطات متحركة، ' +
    ...
    en: 'You are a data analyst only. Your sole job: write two or three short English sentences ' +
        'describing the momentum/trend of a stock or currency pair in plain language, based only on the given numbers ' +
...
export async function generateStockCommentary({ id, symbol, price, smaShort, smaLong, rsi, signal, reasonCode, timeframe, lang }, llm = smartChat) {
...
export function resetStockCommentaryCache() {
    cache.clear();
}
