...
/**
 * يبني الموقع الثابت من مصدر `lib/content.js`.
 *
 * 🔴 **كان يُميت الموقع صامتاً.** `parseContent` تحليلٌ صارم بـ`JSON.parse`
 * على ما بين أول `{` وآخر `}`، فيردّ `null` لكل ما هو JS صالحٌ وليس JSON
 * صارماً: فاصلةٌ زائدة، اقتباسٌ مفرد، تعليقٌ فيه `{`. ثم كان
 * `buildStaticSite(null)` يعيد **صفحةً واحدة كاملة الهيكل بجسمٍ فارغ**،
 * ويكتبها المستدعي **فوق `index.html`** الحقيقي.
 *
 * مُثبَتٌ بالتشغيل: محتوًى بمسارين يعطي صفحتين (`index` و`about`)؛ وبفاصلةٍ
 * زائدة وحدها يعطي **صفحةً واحدة `<main>` فارغ**. فتُدهس الصفحة الرئيسية،
 * وتبقى بقيّة الصفحات يتيمةً بمحتوى قديم، ويُقال للمستخدم «تمّ».
 *
 * فالآن يُفرَّق **الصمتُ من العطل** — وهو الدرس المتكرّر في هذا المستودع:
 *   • مصدرٌ فارغ (لا محتوى بعد) → `[]`: لا شيء يُبنى فلا شيء يُكتب.
 *   • مصدرٌ موجودٌ لا يُقرأ → **رمية**: المستدعون الثلاثة كلّهم يلتقطونها
 *     (`jcr` يسجّلها للمستخدم، والخادم يردّ 500) ولا يكتب أحدهم شيئاً.
 *     فالموقع القائم يبقى سليماً، والعطل يُقال بدل أن يُدفَن.
 */
export function buildStaticSiteFromSource(contentJsSource, lang = 'en') {
    const src = String(contentJsSource ?? '');
    if (!src.trim()) return [];
    const content = parseContent(src);
    if (!content) {
        throw new Error('تعذّر قراءة lib/content.js: المحتوى ليس كائن JSON صارماً '
            + '(فاصلة زائدة أو اقتباس مفرد أو تعليق فيه قوس). لم تُكتب أي صفحة — الموقع الحالي كما هو.');
    }
    return buildStaticSite(content, lang);
}
