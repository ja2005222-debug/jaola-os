...
test('العطب: درجةٌ قيست قبل آخر بناءٍ كانت تُعرض كأنّها عن الكود الحالي', () => {
    const [u, p] = fresh();
    // 🔴 كان هذا يسجّل **القياسَ قبل البناء** ثمّ يؤكّد أنّ القياسَ ليس قديماً.
    //    والقاعدةُ `stale = score.at < lastBuildAt`، فالتأكيدُ لا يصحّ إلا إذا
    //    وقع النداءان في **المِلّي ثانية نفسِها**. فكان الاختبارُ يمرّ منفرداً
    //    ويسقط أحياناً تحت الحزمة المتوازية — لا لعطبٍ في الكود بل لتكّةِ ساعة.
    //    (مُثبَتٌ بتجربة: تأخيرُ ٢ms بين النداءين يُسقطه دائماً.)
    //    والترتيبُ الصحيحُ هو ترتيبُ الإنتاج أيضاً: `jcr` يبني، ثمّ وكيلُ SEO يقيس.
