...
test('ميزانيةٌ مستنفَدة: نداءٌ واحد يُستهلَك (يعود false) → لا إصلاح؛ وميزانيةٌ حيّة → إصلاحٌ ثمّ مكتبةٌ حين النموذجُ غيرُ فارغ', async () => {
    ...
    // PM/3: سطرُ النجاح صار يُلحق تنبيهات «صدق المجال» بعد الملاحظة (مفاهيمُ Product/Customer غير ظاهرة
    // في صفحة الاختبار المجرّدة) — النصُّ يحتوي الملاحظةَ ولا ينتهي بها، وهذا أصدقُ لا أقلّ.
    assert.ok(L.some((m) => m.includes('(أُصلح تلقائياً)')), 'المكتبةُ بعد الإصلاح لا قبله');
    assert.ok(L.some((m) => m.includes('(أُصلح تلقائياً)') && m.includes('⚠️ مفاهيمُ المنتج غير ظاهرة')), 'التنبيهُ يُقال مع النجاح');
});
