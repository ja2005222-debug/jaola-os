...
test('لا بثَّ مباشراً على io داخلَ jcr — المُبلِّغُ هو الباب', () => {
    ...
    // (معالجا الخطّة والنمط أخذا ٧)، و30 بعد JCR/27 (الموجّهُ الموحَّد أخذ ٣)، و26 بعد JCR/28 (المصنِّفُ الأخير أخذ ٤) — الأرضيّةُ
    // تنزل إلى ٢٦: ما زالت تُمسك عودةَ البثّ إلى الصنف بلا مُبلِّغ، وتُعاد بالقياس.
    assert.ok((SRC.match(/this\.reporter\.send\(roomName, /g) || []).length >= 26, 'النقلُ الحرفيّ وقع — والبثُّ ما زال عبر المُبلِّغ');
});
