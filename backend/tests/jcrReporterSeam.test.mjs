...
test('لا بثَّ مباشراً على io داخلَ jcr — المُبلِّغُ هو الباب', () => {
    ...
    // (معالجا الخطّة والنمط أخذا ٧)، و30 بعد JCR/27 (الموجّهُ الموحَّد أخذ ٣)، و26 بعد JCR/28 (المصنِّفُ الأخير أخذ ٤)، و24 بعد JCR/29
    // (مختارُ الاستراتيجيّة أخذ ٢)، و21 بعد JCR/31 (ردُّ الشات أخذ ٣: بدءُ البثّ وقطعتُه ونهايتُه) — الأرضيّةُ تنزل إلى ٢١:
    // ما زالت تُمسك عودةَ البثّ إلى الصنف بلا مُبلِّغ، وتُعاد بالقياس لا بالتخمين.
    assert.ok((SRC.match(/this\.reporter\.send\(roomName, /g) || []).length >= 21, 'النقلُ الحرفيّ وقع — والبثُّ ما زال عبر المُبلِّغ');
});
