...
test('لا بثَّ مباشراً على io داخلَ jcr — المُبلِّغُ هو الباب', () => {
    ...
    // (مختارُ الاستراتيجيّة أخذ ٢) — الأرضيّةُ تنزل إلى ٢٤: ما زالت تُمسك عودةَ البثّ إلى الصنف بلا مُبلِّغ، وتُعاد بالقياس.
    assert.ok((SRC.match(/this\.reporter\.send\(roomName, /g) || []).length >= 24, 'النقلُ الحرفيّ وقع — والبثُّ ما زال عبر المُبلِّغ');
});
