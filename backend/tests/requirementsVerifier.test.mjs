...
test('تعليمة الإصلاح مدفوعة بالنموذج: شاشة لكل دور', () => {
    const fix = buildFixInstruction(
        [{ name: 'لوحة المطعم', fixInstruction: 'ابنِ لوحة استقبال الطلبات' }],
        { roles: [{ name: 'Customer' }, { name: 'RestaurantOwner' }], entities: [{ name: 'Order' }] }
    );
    assert.match(fix, /لكل دور شاشته/);
    assert.match(fix, /RestaurantOwner/);
    assert.match(fix, /Order/);
});
