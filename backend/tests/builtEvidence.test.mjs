...
test('حكمُ مسار React على ما بُني كلِّه: بندُ الوثيقة الذي كُتب في صفحته له أثر', async () => {
    ...
    //    ٥/٥ بدل ٥/٧.
    //
    // ✅ #١٩٩ — «كتاب» صار يُتتبَّع: معجمُ جمع التكسير (Arramooz) يعرف «كتاب ← الكتب» بياناً موثَّقاً لا
    //    اشتقاقاً. فالباقي بلا أثرٍ فجوةٌ حقيقيّةٌ وحدها: «أمين المكتبة» — لا «أمين» ولا «مكتبة» في نصّ
    //    البناء. الحكمُ الصادق يبقى `unverified` لا `fail`: النفيُ يحتاج يقيناً لا نملكه لِما هو خارج
    //    أيّ معجم، فلا يُقال عنه «بلا أثر».
    assert.equal(req.status, 'unverified', req.detail);
    assert.doesNotMatch(req.detail, /بلا أثر:/, req.detail);
    assert.match(req.detail, /6\/7 له أثر/, req.detail);
    assert.match(req.detail, /1 لم أتتبّعه بمفرداته: شاشة أمين المكتبة/, req.detail);
});
