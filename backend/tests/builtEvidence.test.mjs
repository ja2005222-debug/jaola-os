...
test('حكمُ مسار React على ما بُني كلِّه: بندُ الوثيقة الذي كُتب في صفحته له أثر', async () => {
    ...
    // «شاشة مدير» كانت بلا أثرٍ لأنّ صفحتَها لم تُقرأ — وقارئُ الحكم أصلحها (PM/15). وهي تبقى ذاتَ أثر.
    //
    // 🔻 #١٩٨ — وكانت هذه الدعوى `pass` بنصّها: «كلُّ متطلّبات النموذج لها أثر». **وقِيس أنّها لم تكن
    //    صدقاً بل صمتاً**: «أمين المكتبة» و«كتاب» خارجَ المعجم، فكانا يخرجان من البسط والمقام معاً —
    //    ٥/٥ بدل ٥/٧. ونصُّ البناء نفسِه يقول ما فيه: «كتب» موجودة، و«كتاب» و«مكتبة» و«أمين» غائباتٌ
    //    تماماً. فالحكمُ الصادق `unverified`: ٥ لها أثر، واثنان لم أتتبّعهما بمفرداتهما (أحدُهما
    //    «كتاب/كتب» جمعُ تكسيرٍ لا يبلغه قانونُ اللواصق، والآخرُ فجوةٌ حقيقيّةٌ كانت مخفيّة).
    //    ولا يُقال عنهما «بلا أثر» — النفيُ يحتاج يقيناً. `unverified` تمنع PASS ولا تتّهم.
    assert.equal(req.status, 'unverified', req.detail);
    assert.doesNotMatch(req.detail, /بلا أثر:/, req.detail);
    assert.match(req.detail, /5\/7 له أثر/, req.detail);
    assert.match(req.detail, /2 لم أتتبّعه بمفرداته/, req.detail);
});
