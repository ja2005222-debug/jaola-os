...
// 📌 حدٌّ معلوم لم يُغيَّر عمداً: اسم مشروع Vercel هويةٌ **حيّة** مرتبطة
// بنطاقات مخصّصة منشورة، وتغيير صيغته يُعيد توجيهها — قرار المالك لا
// قراري. وُحِّدت النسختان (deployAgent + customDomains) بلا مساس بالناتج.
test('اسم Vercel: نسخةٌ واحدة، والناتج كما كان بحرفه', () => {
    assert.equal(vercelProjectNameOf('ali', 'shop'), 'ali-shop');
    assert.equal(vercelProjectNameOf('ali_2005', 'My App'), 'ali-2005-my-app');
    // يبقى قابلاً لترك شرطةٍ طرفية — موثَّقٌ مفتوحاً في CONTRACTS.md
    assert.equal(vercelProjectNameOf('guest_user', 'متجري'), 'guest-user-');
});
