...
test('buildAppSections: شاشة لكل دور + كل تدفّق (تحلّ محلّ أقسام البروشور)', () => {
    const sections = buildAppSections({
        roles: [{ name: 'Customer', description: 'يطلب' }, { name: 'RestaurantOwner' }],
        flows: [{ name: 'تقديم طلب' }, { name: 'تتبّع' }],
    });
    assert.ok(sections.some(s => s.includes('Customer')), 'واجهة الزبون');
    assert.ok(sections.some(s => s.includes('RestaurantOwner')), 'واجهة المطعم');
    assert.ok(sections.includes('تقديم طلب') && sections.includes('تتبّع'), 'التدفّقات كأقسام');
    assert.equal(buildAppSections(null).length, 0, 'نموذج فارغ → لا أقسام');
});
