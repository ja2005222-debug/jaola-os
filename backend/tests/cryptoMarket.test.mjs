...
// قيمة محسوبة يدوياً (تجانس Wilder الحقيقي، لا متوسط بسيط) — تُثبّت الصيغة الدقيقة:
// نافذة تمهيد 14 تغيّراً: مجموع مكاسب 3.68 (متوسط 0.262857..)، مجموع خسائر 1.40 (متوسط 0.1).
// RS = 2.628571.. → RSI = 100 - 100/3.628571.. ≈ 72.44
test('rsi: مثال محسوب يدوياً (نافذة تمهيد فقط، 15 نقطة) → Wilder الحقيقي', () => {
    const closes = [44, 44.34, 44.09, 44.15, 43.61, 44.33, 44.83, 45.10, 45.42, 45.84, 46.08, 45.89, 46.03, 45.61, 46.28];
    const r = rsi(closes, 14);
    assert.ok(Math.abs(r - 72.44) < 0.01, 'RSI = ' + r);
});

// نفس المثال + نقطة سادسة عشرة (خسارة 0.28) — يختبر خطوة التجانس التراكمية
// بعد نافذة التمهيد (avgGain/avgLoss الجديدان = (قديم*13 + جديد) / 14):
// avgGain الجديد = (0.262857*13 + 0) / 14 ≈ 0.244082
// avgLoss الجديد = (0.1*13 + 0.28) / 14 ≈ 0.112857 → RSI ≈ 68.38
test('rsi: خطوة تجانس تراكمية بعد نافذة التمهيد → قيمة مختلفة عن نافذة التمهيد وحدها', () => {
    const closes = [44, 44.34, 44.09, 44.15, 43.61, 44.33, 44.83, 45.10, 45.42, 45.84, 46.08, 45.89, 46.03, 45.61, 46.28, 46.00];
    const r = rsi(closes, 14);
    assert.ok(Math.abs(r - 68.38) < 0.05, 'RSI = ' + r);
});
