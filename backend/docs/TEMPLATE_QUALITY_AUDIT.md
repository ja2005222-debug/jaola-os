...
## Booking clone validation checkpoint

The public booking clone remains a browser-local demo, now disclosed in its customer-facing introduction. Confirmation validates the service, offered day/slot and future time, reloads saved bookings to reject a stale occupied slot, and assigns a number beyond existing IDs. Failed storage leaves the in-memory list and confirmation step unchanged; repeated confirmation after completion does not duplicate the booking. Day keys use local calendar dates instead of UTC conversion.

Focused emitted-JavaScript tests cover stale slot rejection, invalid selection, ID allocation after gaps, repeated submission and storage failure. This is not cross-device availability or an atomic cross-tab reservation: localStorage has no compare-and-set transaction. Next work requires a server-authoritative public reservation endpoint with resource/time uniqueness, customer-scoped reads/cancellation and actual staff authentication. The existing demo staff credentials remain demo-only; no production booking readiness is claimed.

## Server-backed booking pilot

The booking clone now installs a dedicated public booking client during clone creation (when the platform public URL is configured), manual template application and the deployment endpoint. Publishing an older/incompatible booking clone fails instead of silently retaining browser-only persistence. A marked server-backed page cannot fall back to local writes if its client fails to load.

Mongo `ProjectBookings` uses one document per owner/project and revision-conditional updates. The server controls the service catalogue, price, seven-day UTC calendar and offered slots. Concurrent requests for one slot yield one active reservation; request IDs make retries idempotent. Cancellation retains history and releases availability. The public availability response contains only dates/slots; customer reads and cancellation require a random 256-bit bearer capability stored in that browser session, hashed at rest on the server. This is guest reservation ownership, not verified customer identity or a customer account. Closing/clearing the browser session loses self-service access; staff can still manage the booking.

Staff uses the existing real project-password login and project-bound, revocable one-hour session. The client keeps the admin token in memory; the server validates it for each admin operation. No default demo password is accepted on the server-backed path. Rendered booking strings are HTML-escaped, late admin responses cannot restore rows after navigation/logout, and successful confirmation waits for server acknowledgement. Database failures do not produce local success.

Boundaries: one scheduling resource, UTC explicitly displayed, four fixed services with duration at most 60 minutes and non-overlapping hourly slots; no payment or verified contact/OTP. Catalogue edits require coordinated server/template changes. The project document is capped at 2,000 historical bookings and fails explicitly when full; archival/pagination, configurable business timezone, multiple staff/resources, durable customer recovery and stronger anti-abuse controls remain follow-up work. Existing generated apps require regeneration/redeployment. Runtime integration tests cover actual HTTP admin-session revocation and emitted browser confirmation; CI additionally executes concurrent reservation against real local Mongo. No production deployment is claimed by this checkpoint.
