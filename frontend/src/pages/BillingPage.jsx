...
export default function BillingPage({ onExit }) {
  ...
  const planName = (p) => {
    const key = { free: 'planFree', pro: 'planPro', enterprise: 'planEnterprise' }[p.id];
    return key ? t(key) : (lang === 'ar' ? p.nameAr : p.nameEn);
  };
  const planFeatures = (p) => (lang === 'ar' ? p.featuresAr : (p.featuresEn || p.featuresAr)) || [];
  ...
  return (
    <div style={{ minHeight: '100dvh', background: S.bg, color: S.text, fontFamily: 'system-ui, sans-serif' }}>
      ...
      <nav style={{ height: 56, background: S.bg2, borderBottom: `1px solid ${S.border}`, display: 'flex', alignItems: 'center', padding: '0 20px', gap: 14 }}>
        ...
        <span style={{ fontWeight: 800, fontSize: 15 }}>{t('billingTitle')}</span>
        <div style={{ flex: 1 }} />
        <LanguageSwitcher />
        <button onClick={onExit} style={{ background: 'transparent', border: `1px solid ${S.border}`, borderRadius: 8, padding: '7px 14px', color: S.muted, fontSize: 13 }}>{t('billingBack')}</button>
      </nav>

      <div style={{ maxWidth: 1000, margin: '0 auto', padding: '28px 20px' }}>
        {error && <Banner color={S.red}>{error}</Banner>}
        {!stripeEnabled && <Banner color={S.amber}>{t('billingDemoMode')}</Banner>}

        {loading ? (
          <p style={{ color: S.muted, textAlign: 'center', padding: 40 }}>{t('billingLoading')}</p>
        ) : (
          <>
            {/* لوحة الاستهلاك */}
            {sub && (
              <section style={{ background: S.card, border: `1px solid ${S.border}`, borderRadius: 14, padding: 20, marginBottom: 24 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
                  <h3 style={{ fontSize: 16 }}>{t('billingCurrentPlan')}</h3>
                  <span style={{ fontSize: 12, color: S.blue, background: 'rgba(59,130,246,0.1)', border: '1px solid rgba(59,130,246,0.2)', padding: '3px 10px', borderRadius: 6, fontWeight: 700 }}>
                    {t({ free: 'planFree', pro: 'planPro', enterprise: 'planEnterprise' }[currentPlan] || 'planFree')}
                  </span>
                  {sub.status && sub.status !== 'none' && <span style={{ fontSize: 11, color: S.muted }}>({sub.status})</span>}
                </div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 24 }}>
                  <Metric label={t('billingUsedProjects')} value={sub.projects.used} />
              ...
              {plans.map(p => {
                ...
                return (
                  <div key={p.id} style={{ background: S.card, border: `1px solid ${isCurrent ? S.purple : S.border}`, borderRadius: 14, padding: 22, position: 'relative' }}>
                    {isCurrent && <span style={{ position: 'absolute', top: 14, insetInlineEnd: 14, fontSize: 10, color: S.purple, fontWeight: 800 }}>{t('billingYourPlan')}</span>}
                    <h3 style={{ fontSize: 18, marginBottom: 4 }}>{planName(p)}</h3>
                    <div style={{ fontSize: 28, fontWeight: 800, marginBottom: 14 }}>
                      {p.priceMonthly === 0 ? t('billingFree') : <>${p.priceMonthly}<span style={{ fontSize: 13, color: S.muted, fontWeight: 400 }}>{t('billingPerMonth')}</span></>}
                    </div>
                    ...
                    {isCurrent ? (
                      <button disabled style={btn(S.border, 'transparent', S.muted)}>{t('billingCurrentBtn')}</button>
                    ) : isPaid ? (
                      <button onClick={() => upgrade(p.id)} disabled={!stripeEnabled || busy === p.id} style={btn(S.purple, S.purple, '#fff')}>
                        {busy === p.id ? '…' : t('billingUpgrade')}
                      </button>
                    ) : (
                      <button disabled style={btn(S.border, 'transparent', S.muted)}>{t('billingBasePlan')}</button>
                    )}
                  </div>
                );
              })}
