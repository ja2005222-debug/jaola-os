  ...
  if (denied) return (
    <Shell onExit={onExit} tab={tab} setTab={setTab}>
      <div style={{ padding: 40, textAlign: 'center', color: S.amber }}>
        ...
        <h3 style={{ color: S.text, marginBottom: 8 }}>وصول مرفوض</h3>
        <p style={{ color: S.muted, fontSize: 14, lineHeight: 2 }}>
          حسابك ليس مشرفاً. لتفعيل الوصول، أضف اسم مستخدمك في متغير البيئة<br />
          <code style={{ color: S.blue, direction: 'ltr' }}>ADMIN_USERS=اسمك</code> على السيرفر ثم أعد التشغيل.
        </p>
      </div>
    </Shell>
  );
...
function Shell({ children, onExit, tab, setTab }) {
  const tabs = [
    ...
    { id: 'agents', icon: '🤖', label: 'الوكلاء والإضافات' },
    { id: 'files', icon: '🗂️', label: 'إدارة الملفات' },
  ];
  return (
          ...
          {tabs.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)}
              style={{
                ...
                fontSize: 14, fontWeight: tab === t.id ? 700 : 500, textAlign: 'right',
              }}>
...
function HealthTab({ api }) {
  ...
  if (loading) return <Muted>جاري فحص النظام...</Muted>;
  if (err) return <Muted>{err}</Muted>;
  if (!report) return null;

  return (
    <div>
      <Header title="🩺 صحة النظام" action={<button onClick={load} style={btnPrimary}>⟳ تحديث</button>} />
      <div style={{ ...cardStyle, marginBottom: 16, display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 12, color: S.muted }}>Uptime: {Math.floor(report.uptimeSec / 60)} دقيقة</div>
        </div>
      </div>
...
function AgentsTab({ api }) {
  ...
  const createAgent = async () => {
    if (!form.name.trim() || !form.instructions.trim()) { setMsg('الاسم والتعليمات مطلوبان.'); return; }
    setBusy(true); setMsg('');
    try {
  ...
  const del = async (file) => {
    if (!window.confirm(`حذف الإضافة "${file}" نهائياً؟`)) return;
  ...
  return (
    <div>
      <Header title="🤖 الوكلاء والإضافات" />

      {/* صناعة وكيل — أبسط طريقة */}
      <div style={{ ...cardStyle, marginBottom: 20 }}>
        <div style={{ fontWeight: 800, fontSize: 15, marginBottom: 4 }}>✨ صنع وكيل جديد</div>
        <p style={{ color: S.muted, fontSize: 12, marginBottom: 14 }}>أعطِ الوكيل اسماً وتعليماته — سيصبح عاملاً فوراً بلا برمجة.</p>
        <div style={{ display: 'grid', gap: 12 }}>
          <div>
            <span style={label}>اسم الوكيل (إنجليزي)</span>
            <input style={inputStyle} dir="ltr" value={form.name} placeholder="marketing-writer"
              onChange={e => setForm(f => ({ ...f, name: e.target.value }))} />
          </div>
          <div>
            <span style={label}>الوصف (اختياري)</span>
            <input style={inputStyle} value={form.description} placeholder="يكتب نصوصاً تسويقية جذابة"
              onChange={e => setForm(f => ({ ...f, description: e.target.value }))} />
          </div>
          <div>
            <span style={label}>التعليمات (شخصية الوكيل ومهمته)</span>
      ...
      {status?.registeredAgents?.length > 0 && (
        <div style={{ ...cardStyle, marginBottom: 20 }}>
          <div style={{ fontWeight: 800, fontSize: 14, marginBottom: 10 }}>🧪 تجربة وكيل</div>
          <textarea style={{ ...inputStyle, minHeight: 60 }} value={testInput} placeholder="اكتب مدخلاً لتجربة الوكيل..."
          ...
          {testing && <Muted>جاري التشغيل...</Muted>}
          {testResult && (
      ...
      {/* قائمة الإضافات */}
      <div style={{ fontWeight: 800, fontSize: 14, marginBottom: 10 }}>الإضافات المثبّتة ({status?.count ?? 0})</div>
      {(!status?.plugins || status.plugins.length === 0) && <Muted>لا إضافات بعد — أنشئ أول وكيل بالأعلى.</Muted>}
      <div style={{ display: 'grid', gap: 10 }}>
      ...
      {status?.errors?.length > 0 && (
        <div style={{ ...cardStyle, marginTop: 16, borderColor: 'rgba(245,158,11,0.3)' }}>
          <div style={{ color: S.amber, fontSize: 13, fontWeight: 700, marginBottom: 6 }}>⚠️ أخطاء تحميل ({status.errors.length})</div>
...
function FilesTab({ api }) {
  ...
  const save = async () => {
    try {
      ...
      setDirty(false); setMsg('💾 حُفظ');
      setTimeout(() => setMsg(''), 2000);
    } catch (e) { setMsg('❌ ' + e.message); }
  };
  const del = async (f) => {
    if (!window.confirm(`حذف "${f}"؟`)) return;
    try {
  ...
  return (
    <div>
      <Header title="🗂️ إدارة الملفات" action={msg && <span style={{ fontSize: 12, color: msg.startsWith('❌') ? S.red : S.green }}>{msg}</span>} />
            ...
            <div key={u.user} style={{ marginBottom: 10 }}>
              ...
              {u.projects.map(p => (
                <button key={p} onClick={() => openProject(u.user, p)}
                  style={{ width: '100%', textAlign: 'right', background: 'transparent', border: 'none', color: '#93c5fd', fontSize: 13, padding: '5px 8px', borderRadius: 6 }}>📁 {p}</button>
              ))}
            </div>
          ))}
          {tree.length === 0 && !sel && <Muted>لا مشاريع بعد.</Muted>}
          {sel && (
            <div>
              <button onClick={() => { setSel(null); setFiles([]); setActiveFile(null); }} style={{ background: 'transparent', border: 'none', color: S.muted, fontSize: 12, marginBottom: 8 }}>← المشاريع</button>
              <div style={{ fontSize: 11, color: S.muted, marginBottom: 6 }}>{sel.user} / {sel.project}</div>
              {files.map(f => (
                <div key={f} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                  <button onClick={() => openFile(f)}
                    style={{ flex: 1, textAlign: 'right', background: activeFile === f ? 'rgba(59,130,246,0.1)' : 'transparent', border: 'none', color: activeFile === f ? '#93c5fd' : S.text, fontSize: 12, padding: '5px 8px', borderRadius: 6, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', direction: 'ltr' }}>{f}</button>
          ...
          {activeFile ? (
            <>
              <div style={{ display: 'flex', alignItems: 'center', padding: '10px 14px', borderBottom: `1px solid ${S.border}`, gap: 10 }}>
                ...
                <button onClick={save} disabled={!dirty} style={{ ...btnPrimary, padding: '6px 14px', opacity: dirty ? 1 : 0.5 }}>💾 حفظ</button>
              </div>
            ...
            <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: S.muted, fontSize: 13 }}>اختر ملفاً لعرضه وتعديله</div>
          )}
        </div>
      </div>
    </div>
