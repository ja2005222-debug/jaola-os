...
import { 
  ...
  Target, TrendingUp, Users, Layers
} from 'lucide-react';

// ============================================================
// 1. الحالة العامة للمشروع (Digital Twin)
// ============================================================
...
// ============================================================
// 2. تكامل Groq API (محسّن)
// ============================================================
const GROQ_API_KEY = import.meta.env.VITE_GROQ_API_KEY;
const GROQ_MODEL = 'mixtral-8x7b-32768'; // أو 'llama3-70b-8192'
const GROQ_URL = 'https://api.groq.com/openai/v1/chat/completions';

// دالة استدعاء Groq مع معالجة الأخطاء
const callGroq = async (messages, temperature = 0.7) => {
  // التحقق من وجود المفتاح
  if (!GROQ_API_KEY) {
    throw new Error('مفتاح Groq API غير موجود. يرجى إضافته في ملف .env كـ VITE_GROQ_API_KEY');
  }
...
// دوال الـ Agents باستخدام Groq (مع try/catch)
...
// توليد خطة CEO (احتياطي إذا فشل Groq)
...
export default function Dashboard() {
  ...
  // ------ تنفيذ المهمة (باستخدام Groq API) ------
  const runMission = async (goal) => {
    ...
    if (ceoReply && !ceoReply.error) {
      ...
      const errorMsg = ceoReply?.error || 'فشل الاتصال بـ Groq API. تأكد من المفتاح واتصال الإنترنت.';
      setChatMessages(prev => [...prev, { agent: '👔 CEO', text: `⚠️ ${errorMsg}`, icon: '⚠️' }]);
      // استخدام الخطة الاحتياطية
      ceoPlan = generateCEOMission(goal);
    }

    // إذا لم يتم استخراج مهام، استخدم الاحتياطي
    if (ceoPlan.length === 0) {
      ceoPlan = generateCEOMission(goal);
    }

    // إضافة المهام إلى الحالة
    ceoPlan.forEach(task => {
      dispatch({ type: 'ADD_TASK', payload: task.name });
    });

    // 2. محاكاة تنفيذ المهام (مع إمكانية استدعاء Agents أخرى عبر Groq)
    const pipeline = [
    ...
    const runNextStep = async () => {
      ...
      // (اختياري) استدعاء الوكيل من Groq للحصول على تفاصيل إضافية
      // يمكن تفعيل هذا الجزء إذا أردت
      /*
      const agentReply = await agentFunctions[agentName](projectStateRef.current, step.task);
      if (agentReply && !agentReply.error) {
        setChatMessages(prev => [...prev, { agent: agentName, text: agentReply, icon: agentName.replace(/ .*/, '') }]);
      }
      */
        ...
        case 'estimate': {
          try {
            ...
            if (reply && !reply.error) {
              ...
              const est = '€2000 - €4000';
              const dur = '3 أيام';
              setChatMessages(prev => [...prev, { agent: '🧠 Planner', text: `💰 التكلفة التقديرية: ${est}\n⏱ المدة: ${dur}`, icon: '🧠' }]);
            }
          } catch (error) {
      ...
      <div className="grid grid-cols-12 grid-rows-6 gap-2 w-full h-full p-2">
        ...
        <div className="col-span-12 row-span-1 border border-slate-800 bg-slate-900/50 rounded-xl p-3 flex items-center justify-between shadow-2xl">
          <div className="flex items-center gap-4">
            ...
            <span className="text-xs bg-slate-800 px-2 py-1 rounded-full text-slate-400">v4.0</span>
          </div>
